...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ORCANIUM_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Orcanium on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ORCANIUM_TUI_TRUECOLOR=1.
