 ...
 * (`openCommand` returned null), or (c) `spawn()` threw synchroorcaniumly
...
export function openExternalUrl(rawUrl: string, dependencies: OpenDependencies = {}): boolean {
  ...
  try {
    ...
    // Async failure path: spawn returns a ChildProcess synchroorcaniumly even
    ...
    // consumer; we already returned `true` synchroorcaniumly, so the user
    ...
    // spawn can also throw synchroorcaniumly on argv-validation failures
