...
// Mirror the Python gateway's panic log (rpc/server.py::_CRASH_LOG) from
...
const logDir = join(process.env.ORCANIUM_HOME?.trim() || join(homedir(), '.Orcanium'), 'logs')
const CRASH_LOG = join(logDir, 'rpc_crash.log')

// Skipped under vitest so unit tests exercising start()/kill() can't write into
// a real ~/.Orcanium (tests must stay hermetic — see AGENTS.md).
...
// or add noticeable blocking on the synchroorcanium append. Mirrors the spirit of
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('Orcanium-tui: parent lifecycle log unavailable\n')
    }
  }
}
