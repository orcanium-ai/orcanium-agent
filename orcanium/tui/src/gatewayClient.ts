...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.ORCANIUM_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.ORCANIUM_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.ORCANIUM_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = process.env.ORCANIUM_TUI_SIDECAR_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.ORCANIUM_PYTHON?.trim() || process.env.PYTHON?.trim()
...
// `channel.start_timeout` payload, so always strip the query string and any
...
export class GatewayClient extends EventEmitter {
  ...
  private closeGatewaySocket() {
    ...
    // belonging to a discarded socket. (Test fakes emit synchroorcaniumly,
  ...
  private startReadyTimer(python: string, cwd: string) {
    this.readyTimer = setTimeout(() => {
      ...
      this.lifecycle(`[startup] timed out waiting for channel.ready (python=${python}, cwd=${cwd})`)
      this.publish({
  ...
  private handleWebSocketFrame(raw: unknown) {
    ...
    try {
      ...
      this.publish({ type: 'channel.protocol_error', payload: { preview } })
    }
  }

  private startSpawnedGateway(root: string) {
    const python = resolvePython(root)
    const cwd = process.env.ORCANIUM_CWD || root
    const env = { ...process.env }
    const pyPath = env.PYTHONPATH?.trim()

    env.PYTHONPATH = pyPath ? `${root}${delimiter}${pyPath}` : root
    this.startReadyTimer(python, cwd)
        const agentName = process.env.ORCANIUM_AGENT || ''
    const entryArgs = ['-m', 'rpc.entry']
    if (agentName) {
      entryArgs.push('--agent', agentName)
    }
    this.proc = spawn(python, entryArgs, { cwd, env, stdio: ['pipe', 'pipe', 'pipe'] })
    ...
    this.stderrRl.on('line', raw => {
      ...
      this.pushLog(line)
      this.publish({ type: 'channel.stderr', payload: { line } })
    })
    ...
    this.proc.on('error', err => {
      ...
      // `channel.start_timeout`, rejects pending RPCs, and emits or
  ...
  start() {
    const root = process.env.ORCANIUM_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
