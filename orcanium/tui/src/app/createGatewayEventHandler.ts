...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  // gate below, the auto-resume check in the `channel.ready` handler).
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    if (recoverSidRef && recoverSid) {
      ...
      // After resumeById: it synchroorcaniumly sets status to 'resuming…' on entry,
    ...
    // forging a brand-new one.  Mirrors classic CLI's `Orcanium -c` /
    // `Orcanium --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: GatewayEvent) => {
    ...
    if (ev.session_id && sid && ev.session_id !== sid && !ev.type.startsWith('channel.')) {
      return
    }
    ...
    switch (ev.type) {
      ...
      case 'notification.show': {
        // Credits/usage notice from the channel. Payload is snake_case on the
      ...
      case 'channel.start_timeout': {
        ...
        // 120-char clip used for `channel.stderr` activity entries.
