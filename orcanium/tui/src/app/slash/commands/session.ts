...
export const sessionCommands: SlashCommand[] = [
  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.channel.rpc<BackgroundStartResponse>('prompt.background', { session_id: ctx.sid, text: arg }).then(
...

  {
    ...
    name: 'image',
    run: (arg, ctx) => {
      ctx.channel.rpc<ImageAttachResponse>('image.attach', { path: arg, session_id: ctx.sid }).then(
...

  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.channel.rpc<ConfigSetResponse>('config.set', { key: 'personality', session_id: ctx.sid, value: arg }).then(
        ctx.guarded<ConfigSetResponse>(r => {
          if (r.history_reset) {
...

  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.channel.rpc<SessionBranchResponse>('session.branch', { name: arg, session_id: ctx.sid }).then(
...

  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.channel.rpc<VoiceToggleResponse>('voice.toggle', { action }).then(
...

  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.channel.rpc<ConfigSetResponse>('config.set', { key: 'indicator', value }).then(
        ctx.guarded<ConfigSetResponse>(r => {
          if (!r.value) {
            return
          }
  ...
  {
    help: 'session usage + Orcanium credits',
    name: 'usage',
    run: (_arg, ctx) => {
      ctx.channel.rpc<SessionUsageResponse>('session.usage', { session_id: ctx.sid }).then(r => {
        if (ctx.stale()) {
          return
        }
        ...
        // Orcanium credits block is agent-independent (a portal fetch), so it shows
        ...
        if (creditsLines.length) {
          ctx.transcript.panel('Orcanium credits', [{ text: creditsLines.join('\n') }])
        }
