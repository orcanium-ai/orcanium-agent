import { forceRedraw, type MouseTrackingMode } from '@Orcanium/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update Orcanium Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      ctx.transcript.sys('exiting TUI to run update...')
      // Exit code 42 signals the Python wrapper to exec `Orcanium update`.
...

  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.channel.rpc<ConfigSetResponse>('config.set', { key: 'mouse', value: next }).catch(() => {})
...

  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.channel.rpc<ConfigSetResponse>('config.set', { key: 'compact', value: next ? 'on' : 'off' }).catch(() => {})
...

  {
    ...
    run: (arg, ctx) => {
      ...
      channel.rpc<ConfigSetResponse>('config.set', { key: 'details_mode', value: next }).catch(() => {})
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try ORCANIUM_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      }
...

  {
    ...
    name: 'logs',
    run: (arg, ctx) => {
      const text = ctx.channel.gw.getLogTail(Math.min(80, Math.max(1, parseInt(arg, 10) || 20)))
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Orcanium #${i + 1}`
...

  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.channel.rpc<ConfigSetResponse>('config.set', { key: 'statusbar', value: next }).catch(() => {})
...

  {
    ...
    run: (_arg, ctx) => {
      ...
      ctx.channel.rpc<SessionUndoResponse>('session.undo', { session_id: ctx.sid }).then(
