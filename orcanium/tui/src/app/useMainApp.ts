import { type ScrollBoxHandle, useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@Orcanium/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(
    model ? composeTabTitle(marker, ui.sessionTitle, model, tabCwd ? shortCwd(tabCwd, 24) : '') : 'Orcanium'
  )
  ...
  useEffect(() => {
    ...
    const exitHandler = () => {
      ...
      // persisted session via the next channel.ready, so a single crash / OOM /
      ...
      // dead/respawning channel. recoverSidRef carries the session forward, and
  ...
  const cwd = ui.info?.cwd || process.env.ORCANIUM_CWD || process.cwd()
