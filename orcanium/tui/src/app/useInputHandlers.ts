import { forceRedraw, useInput } from '@Orcanium/ink'
...
export function useInputHandlers(ctx: InputHandlerContext): InputHandlerResult {
  ...
  useInput((ch, key) => {
    ...
    if (key.shift && key.tab && !cState.completions.length) {
      ...
      // channel.rpc swallows errors with its own sys() message and resolves to null,
      // so we only speak when it came back with a real shape. null = rpc already spoke.
      return void channel.rpc<ConfigSetResponse>('config.set', { key: 'yolo', session_id: live.sid }).then(r => {
        if (r?.value === '1') {
