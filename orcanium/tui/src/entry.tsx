...
import type { FrameEvent } from '@Orcanium/ink'
...
if (!process.stdin.isTTY) {
  console.log('Orcanium-tui: no TTY')
  process.exit(0)
}
...
// synchroorcanium code; resetTerminalModes() writes via writeSync, so it completes
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `Orcanium-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? dump?.diagPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`Orcanium-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    ...
    resetTerminalModes()
    process.stderr.write(`Orcanium-tui lifecycle: received ${signal}\n`)
  }
})
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    resetTerminalModes()
    process.stderr.write(`Orcanium-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`)
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('Orcanium-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  ...
  onWarn: snap => {
    ...
    process.stderr.write(
      `Orcanium-tui: heap climbing fast (${formatBytes(snap.heapUsed)}) — a large tool output or long session may be straining memory\n`
    )
  }
})

if (process.env.ORCANIUM_HEAPDUMP_ON_START === '1') {
