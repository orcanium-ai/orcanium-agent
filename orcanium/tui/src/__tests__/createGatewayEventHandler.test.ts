...
describe('createGatewayEventHandler', () => {
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'Orcanium-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith(
      "💾 Self-improvement review: Skill 'Orcanium-release' patched"
    )
  })
  ...
  it('annotates channel.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    const appended: Msg[] = []
    const onEvent = createGatewayEventHandler(buildCtx(appended))
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.Orcanium/config.yaml'
      },
      type: 'channel.start_timeout'
    } as any)
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mOrcanium here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `Orcanium model` to select a provider, or run `Orcanium setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('does not fetch config while constructing the gateway event handler', () => {
    ...
    ctx.channel.rpc = vi.fn(async () => null)

    createGatewayEventHandler(ctx)

    expect(ctx.channel.rpc).not.toHaveBeenCalled()
  })

  it('on channel.ready with no STARTUP_RESUME_ID and auto_resume off, forges a new session', async () => {
    const appended: Msg[] = []
    ...
    ctx.channel.rpc = vi.fn(async (method: string) => {
    ...
    createGatewayEventHandler(ctx)({ payload: {}, type: 'channel.ready' } as any)

    await vi.waitFor(() => expect(newSession).toHaveBeenCalled())
    expect(resumeById).not.toHaveBeenCalled()
  })

  it('on channel.ready after a crash, resumes the recovered session once and skips forge', async () => {
    const appended: Msg[] = []
    const newSession = vi.fn()
    const resumeById = vi.fn()
    const ctx = buildCtx(appended)
    ...
    ctx.session.newSession = newSession
    // Mimic resumeById's synchroorcanium status write so the test proves the
    ...
    onEvent({ payload: {}, type: 'channel.ready' } as any)
  ...
  it('on channel.ready with auto_resume on and a recent session, resumes it', async () => {
    const appended: Msg[] = []
    const newSession = vi.fn()
    const resumeById = vi.fn()
    const ctx = buildCtx(appended)
  ...
  it('on channel.ready with auto_resume on but no eligible session, falls back to new', async () => {
    const appended: Msg[] = []
    const newSession = vi.fn()
    const resumeById = vi.fn()
    const ctx = buildCtx(appended)
  ...
  it('on channel.ready when config.get rejects, falls back to new session', async () => {
    const appended: Msg[] = []
    const newSession = vi.fn()
    const resumeById = vi.fn()
    const ctx = buildCtx(appended)
  ...
  it('on channel.ready when session.most_recent rejects, falls back to new session', async () => {
    const appended: Msg[] = []
    const newSession = vi.fn()
    const resumeById = vi.fn()
    const ctx = buildCtx(appended)
  ...
  it('on channel.ready with STARTUP_RESUME_ID set, the env wins over config auto_resume', async () => {
    const appended: Msg[] = []
    const newSession = vi.fn()
    const resumeById = vi.fn()
    const ctx = buildCtx(appended)
  ...
  it('keeps gateway noise informational and approval out of Activity', async () => {
    ...
    onEvent({ payload: { line: 'Traceback: noisy but non-fatal' }, type: 'channel.stderr' } as any)
    onEvent({ payload: { preview: 'bad framing' }, type: 'channel.protocol_error' } as any)
    onEvent({
  ...
  it('drops stale reasoning/tool/todos events after ctrl-c until the next message starts', () => {
    ...
    try {
      const appended: Msg[] = []
      const ctx = buildCtx(appended)
      ctx.channel.gw.request = vi.fn(async () => ({ status: 'interrupted' }))
      const onEvent = createGatewayEventHandler(ctx)
  ...
  it('keepBusy interrupt holds busy until the gateway settles and suppresses the cancelled turn’s final_response', () => {
    ...
    turnController.interruptTurn(
      { appendMessage: (msg: Msg) => appended.push(msg), gw: ctx.channel.gw, sid: 'sess-1', sys: ctx.system.sys },
      { keepBusy: true }
    )
