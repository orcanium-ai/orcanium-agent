...
describe('GatewayClient websocket attach mode', () => {
  ...
  beforeEach(() => {
    originalGatewayUrl = process.env.ORCANIUM_TUI_GATEWAY_URL
    originalSidecarUrl = process.env.ORCANIUM_TUI_SIDECAR_URL
    FakeWebSocket.reset()
  ...
  afterEach(() => {
    if (originalGatewayUrl === undefined) {
      delete process.env.ORCANIUM_TUI_GATEWAY_URL
    } else {
      process.env.ORCANIUM_TUI_GATEWAY_URL = originalGatewayUrl
    }

    if (originalSidecarUrl === undefined) {
      delete process.env.ORCANIUM_TUI_SIDECAR_URL
    } else {
      process.env.ORCANIUM_TUI_SIDECAR_URL = originalSidecarUrl
    }
  ...
  it('waits for websocket open and resolves RPC requests', async () => {
    process.env.ORCANIUM_TUI_GATEWAY_URL = 'ws://channel.test/api/ws?token=abc'
  ...
  it('mirrors event frames to sidecar websocket when configured', async () => {
    process.env.ORCANIUM_TUI_GATEWAY_URL = 'ws://channel.test/api/ws?token=abc'
    process.env.ORCANIUM_TUI_SIDECAR_URL = 'ws://channel.test/api/pub?token=abc&channel=demo'
  ...
  it('reattaches when ORCANIUM_TUI_GATEWAY_URL rotates between requests', async () => {
    process.env.ORCANIUM_TUI_GATEWAY_URL = 'ws://gateway-old.test/api/ws?token=abc'
    const gw = new GatewayClient()
    ...
    process.env.ORCANIUM_TUI_GATEWAY_URL = 'ws://gateway-new.test/api/ws?token=xyz'
  ...
  it('uses the undici WebSocket fallback when global WebSocket is unavailable', () => {
    process.env.ORCANIUM_TUI_GATEWAY_URL = 'ws://channel.test/api/ws?token=hunter2&channel=secret'
    delete (globalThis as { WebSocket?: unknown }).WebSocket
    ...
    expect(FakeWebSocket.instances[0]?.url).toBe('ws://channel.test/api/ws?token=hunter2&channel=secret')

    gw.kill()
  })

  it('redacts attach URL secrets when the WebSocket constructor throws', () => {
    const secretUrl = 'ws://channel.test/api/ws?token=hunter2&channel=secret'

    process.env.ORCANIUM_TUI_GATEWAY_URL = secretUrl
    ...
    expect(tail).toContain('ws://channel.test/api/ws?***')

    gw.kill()
  })

  it('redacts sidecar URL secrets when the WebSocket constructor throws', async () => {
    const sidecarUrl = 'ws://channel.test/api/pub?token=hunter2&channel=secret'

    process.env.ORCANIUM_TUI_GATEWAY_URL = 'ws://channel.test/api/ws?token=abc'
    process.env.ORCANIUM_TUI_SIDECAR_URL = sidecarUrl
    ...
    expect(tail).toContain('ws://channel.test/api/pub?***')
  ...
  it('redacts user-info credentials even on URLs the WHATWG parser rejects', () => {
    ...
    const fixture = 'ws://alice:hunter2@channel.test:99999/api/ws?token=secret'
    expect(() => new URL(fixture)).toThrow()

    process.env.ORCANIUM_TUI_GATEWAY_URL = fixture
    ;(globalThis as { WebSocket?: unknown }).WebSocket = class ThrowingWebSocket extends FakeWebSocket {
      constructor(url: string) {
        throw new TypeError(`Invalid URL: ${url}`)
      }
