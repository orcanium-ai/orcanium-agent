...
describe('terminal mode reset', () => {
  ...
  // leaked mouse reports as keystrokes. 'exit' handlers run synchroorcaniumly only,
  // so the reset must complete via a single synchroorcanium write — verify that an
  // exit-style invocation disables every SGR mouse mode that produced the
  // reported `…;…M` garbage.
  it('disarms mouse tracking from a synchroorcanium exit-style handler', () => {
    const write = vi.fn()
