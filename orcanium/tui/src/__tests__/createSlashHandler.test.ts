...
describe('createSlashHandler', () => {
  ...
  it('opens the unified sessions overlay locally even when the current session is busy', () => {
    ...
    expect(ctx.channel.gw.request).not.toHaveBeenCalled()
  })
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ output: 'Orcanium TUI Status' }))
    ...
    expect(ctx.channel.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('Orcanium TUI Status', 'Status')
    })
  })
  ...
  it('keeps typed /model switches session-scoped by default', async () => {
    ...
    expect(ctx.channel.rpc).toHaveBeenCalledWith('config.set', {
  ...
  it('routes /skills install <name> to skills.manage without opening overlay', () => {
    ...
    expect(ctx.channel.rpc).toHaveBeenCalledWith('skills.manage', {
      action: 'install',
      query: 'foo'
    })
  })
  ...
  it('delegates non-native /skills subcommands to slash.exec', () => {
    ...
    expect(ctx.channel.rpc).not.toHaveBeenCalled()
    expect(ctx.channel.gw.request).toHaveBeenCalledWith('slash.exec', {
  ...
  it('clears a per-section override on /details <section> reset', () => {
    ...
    expect(ctx.channel.rpc).toHaveBeenLastCalledWith('config.set', {
  ...
  it('falls through to command.dispatch for skill commands and sends the message', async () => {
      ...
      gateway: {
        gw: {
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({ type: 'skill', message: skillMessage, name: 'Orcanium-agent-dev' })
            }
    ...
    const h = createSlashHandler(ctx)
    expect(h('/Orcanium-agent-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('⚡ loading skill: Orcanium-agent-dev')
    })
  ...
  it('/history pages the current TUI transcript (user + assistant)', () => {
    ...
    expect(body).toContain('[Orcanium #2]')
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/Orcanium_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/Orcanium_conversation_test.json')
    })
  })
