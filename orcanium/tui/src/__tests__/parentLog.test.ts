...
// ~/.Orcanium. To exercise the real persistence path we clear that gate, point
// ORCANIUM_HOME at a temp dir, and re-import the module fresh (path + enabled
// flag are captured at module load).
const loadFresh = async (home: string) => {
  vi.resetModules()
  vi.stubEnv('VITEST', '')
  vi.stubEnv('ORCANIUM_HOME', home)
...
describe('recordParentLifecycle', () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'Orcanium-parentlog-'))
  })
  ...
  it('appends a timestamped breadcrumb to logs/rpc_crash.log', async () => {
    ...
    const contents = readFileSync(join(home, 'logs', 'rpc_crash.log'), 'utf8')
  ...
  it('collapses embedded newlines so a value stays one breadcrumb', async () => {
    ...
    const lines = readFileSync(join(home, 'logs', 'rpc_crash.log'), 'utf8').trimEnd().split('\n')
  ...
  it('caps an oversized breadcrumb so it cannot bloat the shared crash log', async () => {
    ...
    const line = readFileSync(join(home, 'logs', 'rpc_crash.log'), 'utf8')
  ...
  it('is a no-op under VITEST so tests stay hermetic', async () => {
    ...
    expect(() => readFileSync(join(home, 'logs', 'rpc_crash.log'), 'utf8')).toThrow()
  })
})
