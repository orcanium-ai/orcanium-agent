...
React + Ink terminal UI for Orcanium. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ORCANIUM_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
Malformed stdout lines are treated as protocol noise and surfaced as `channel.protocol_error`. Stderr lines become `channel.stderr`. Neither writes directly into the terminal.
...
- theme state from channel skin data
...
- Input history is stored in `~/.orcanium/.orcanium_history` or under `ORCANIUM_HOME`.
...
| `error`                  | `{ message }`                                   |
| `channel.stderr`         | synthesized from child stderr                   |
| `channel.protocol_error` | synthesized from malformed stdout               |

## Theme model

The client starts with `DEFAULT_THEME` from `theme.ts`, then merges in gateway skin data from `channel.ready`.
...
```text
ui-tui/
  packages/orcanium-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      orcanium-ink.d.ts    type declarations for @orcanium/ink
...
rpc/
  ...
  slash_worker.py        persistent Orcanium CLI subprocess for slash commands
```
