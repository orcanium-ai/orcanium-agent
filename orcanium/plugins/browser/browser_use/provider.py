...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Orcanium tool
gateway (which Orcanium uses to bill Browser Use sessions to a Orcanium
...
Auth env vars (one of)::
    ...
    # OR a managed Orcanium gateway entry (configured via 'orcanium setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Orcanium
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Orcanium tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Orcanium gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the Orcanium auth stack which can be
        # heavy and is not needed for direct-API-key users.
        from orcanium.app.tools.managed_tool_gateway import (
            peek_orcanium_access_token,
            resolve_managed_tool_gateway,
        )
        from orcanium.app.tools.tool_backend_helpers import prefers_gateway

        # Direct API key wins unless the user has explicitly opted into the
        # managed Orcanium gateway via ``tool_gateway.browser: gateway``.
        ...
        # Keep availability scans off the synchroorcanium OAuth refresh path.
        managed = resolve_managed_tool_gateway(
            "browser-use",
            token_reader=None if refresh_token else peek_orcanium_access_token,
        )
        if managed is None:
            return None

        return {
            "api_key": managed.orcanium_user_token,
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Orcanium only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"orcanium_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
