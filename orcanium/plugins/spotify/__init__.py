...
``_check_spotify_available()`` — when the user has not run ``orcanium auth
spotify``, the tools remain registered (so they appear in ``orcanium
...
The Spotify auth flow (``orcanium auth spotify``), CLI plumbing, and docs
