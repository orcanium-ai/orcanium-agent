...
def build_pipeline_runtime_config(gateway_config: Any) -> dict[str, Any]:
    """Build pipeline config from channel platform config.
...
def build_pipeline_runtime(gateway: Any) -> TeamsMeetingPipeline:
    teams_sender = None
    teams_config = channel.config.platforms.get(Platform("teams"))
    pipeline_config = build_pipeline_runtime_config(channel.config)
    teams_delivery = dict(pipeline_config.get("teams_delivery") or {})
...
def bind_gateway_runtime(gateway: Any) -> bool:
    ...
    adapter = channel.adapters.get(Platform.MSGRAPH_WEBHOOK)
    if adapter is None:
        return False
    ...
    try:
        ...
        channel._teams_pipeline_runtime_error = error_message
        logger.warning(
    ...
    channel._teams_pipeline_runtime = runtime
    channel._teams_pipeline_runtime_error = None
    return True
