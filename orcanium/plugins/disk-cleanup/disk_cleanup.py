"""disk_cleanup — ephemeral file cleanup for Orcanium Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ORCANIUM_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ORCANIUM_HOME and /tmp/orcanium-*
Never touches: ~/.orcanium/logs/ or any system directory.
"""
...
try:
    ...
    def get_orcanium_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ORCANIUM_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".orcanium").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ORCANIUM_HOME/logs/``."""
    return get_orcanium_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ORCANIUM_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ORCANIUM_HOME or ``/tmp/orcanium-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    orcanium_home = get_orcanium_home()
    try:
        path.resolve().relative_to(orcanium_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/orcanium-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("orcanium-"):
        return True
    return False
...
# Paths under $ORCANIUM_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so ORCANIUM_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        orcanium_home = get_orcanium_home()
        for parent in ("cron", "cronjobs"):
            base = orcanium_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ORCANIUM_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ORCANIUM_HOME (but leave ORCANIUM_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "orcanium-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(orcanium_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == orcanium_home:
                continue
            try:
                rel_parts = dirpath.relative_to(orcanium_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    orcanium_home = get_orcanium_home()
    try:
        rel = path.resolve().relative_to(orcanium_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "orcanium-agent",
        }:
            return None
        ...
        # Path isn't under ORCANIUM_HOME (e.g. /tmp/orcanium-*) — fall through.
        pass
