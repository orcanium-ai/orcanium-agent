...
Auto-tracks and cleans up ephemeral files created during Orcanium Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ORCANIUM_HOME` and `/tmp/orcanium-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ORCANIUM_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ORCANIUM_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `ORCANIUM_HOME` or `/tmp/orcanium-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ORCANIUM_HOME/disk-cleanup/` is itself excluded
- `$ORCANIUM_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
