...
Krea's API is asynchroorcanium: the generate endpoint returns a ``job_id``
that you poll at ``GET /jobs/{job_id}``. This provider hides that
roundtrip behind the synchroorcanium ``generate()`` contract: submit, poll
...
# Orcanium uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "KREA_API_KEY not set. Run `orcanium tools` → Image "
        ...
        headers = {
            ...
            "User-Agent": "Orcanium-Agent/1.0 (krea-image-gen)",
        }
