...
Authentication via ``FAL_KEY`` or the managed Orcanium channel. Output is an
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (Orcanium Subscription)
# ---------------------------------------------------------------------------
...
def _resolve_managed_fal_video_gateway():
    ...
    from orcanium.app.tools.tool_backend_helpers import fal_key_is_configured, prefers_gateway
...
def _get_managed_fal_video_client(managed_gateway):
    ...
    from orcanium.app.tools.fal_common import _ManagedFalSyncClient
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    """Submit a FAL video request using direct credentials or the managed queue channel.
    ...
    try:
        ...
        from orcanium.app.tools.fal_common import _extract_http_status

        status = _extract_http_status(exc)
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"Orcanium Subscription gateway rejected endpoint '{endpoint}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Orcanium Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `orcanium tools` → Video Generation."
            ) from exc
        raise
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `orcanium tools` → Video Generation → FAL to configure) "
                    "or sign in to Orcanium (`orcanium setup`) for managed gateway access."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `orcanium tools` → Video Generation."
                    ),
