...
session-scoped state file under ``$ORCANIUM_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ORCANIUM_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_orcanium_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this orcanium install, leave it first —
    ...
    env = os.environ.copy()
    env["ORCANIUM_MEET_URL"] = url
    env["ORCANIUM_MEET_OUT_DIR"] = str(out)
    env["ORCANIUM_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ORCANIUM_MEET_HEADED"] = "1"
    if auth_state:
        env["ORCANIUM_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ORCANIUM_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ORCANIUM_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ORCANIUM_MEET_MODE"] = mode
    if realtime_model:
        env["ORCANIUM_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ORCANIUM_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ORCANIUM_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ORCANIUM_MEET_REALTIME_KEY"] = realtime_api_key
