...
Let the orcanium agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where orcanium runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `orcanium meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `orcanium meet` CLI |
...
| `cli.py` | `orcanium meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ORCANIUM_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `orcanium meet node {run,list,approve,remove,status,ping}` |
...
orcanium meet install                                      # pip + Chromium
orcanium meet setup                                        # preflight
orcanium meet auth                                         # optional
orcanium meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
orcanium meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.orcanium/.env
orcanium meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
orcanium meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
orcanium meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.orcanium/.env
orcanium meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, orcanium will **not** switch your system audio input automatically — the
...
orcanium plugins enable google_meet
orcanium meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
orcanium meet node ping my-mac
