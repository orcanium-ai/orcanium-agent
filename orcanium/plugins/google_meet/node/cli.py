"""`orcanium meet node ...` subcommand tree.

Wired into the existing ``orcanium meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``orcanium meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="orcanium-meet-node")
    ...
    app = sp.add_parser("approve", help="Register a remote node on the channel.")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``orcanium meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             orcanium meet node approve <name> ws://<host>:{args.port} {token}")
        try:
