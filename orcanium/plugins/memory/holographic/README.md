...
```bash
orcanium memory setup    # select "holographic"
```

Or manually:
```bash
orcanium config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.orcanium-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ORCANIUM_HOME/memory_store.db` | SQLite database path |
