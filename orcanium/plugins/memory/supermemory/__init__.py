...
_DEFAULT_CONTAINER_TAG = "orcanium"
...
def _load_supermemory_config(orcanium_home: str) -> dict:
    config = _default_config()
    config_path = Path(orcanium_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, orcanium_home: str) -> None:
    config_path = Path(orcanium_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "orcanium"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Orcanium writes into the "Orcanium" Space in the Supermemory
        ...
        merged = {"sm_source": "orcanium", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `orcanium memory setup`.
        # All other options are documented for $ORCANIUM_HOME/supermemory.json
    ...
    def save_config(self, values, orcanium_home):
        ...
        _save_supermemory_config(sanitized, orcanium_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from orcanium_constants import get_orcanium_home
        self._orcanium_home = kwargs.get("orcanium_home") or str(get_orcanium_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._orcanium_home)
