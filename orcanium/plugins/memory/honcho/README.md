...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/orcanium>
...
```bash
orcanium memory setup honcho   # configure Honcho directly (works on a fresh install)
orcanium memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
orcanium config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.orcanium/.env
```

> `orcanium honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `orcanium memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ORCANIUM_HOME/honcho.json` | Profile-local (isolated Orcanium instances) |
| 2 | `~/.orcanium/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Orcanium profile: `orcanium` (default) or `orcanium_<profile>`.
...
**Deployment shapes** (`orcanium memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Orcanium to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Orcanium session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`orcanium-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`orcanium`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-orcanium-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening orcanium in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Orcanium session ID (timestamp + hex). Every `orcanium` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Orcanium profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`orcanium`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Orcanium profile: `orcanium` (default) or `orcanium_<profile>` (e.g. `orcanium -p coder` -> host key `orcanium_coder`). Older `orcanium.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ORCANIUM_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `orcanium memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `orcanium honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `orcanium memory setup`) |
| `orcanium honcho status` | Show resolved config for active profile |
| `orcanium honcho enable` / `disable` | Toggle Honcho for active profile |
| `orcanium honcho mode <mode>` | Change recall or observation mode |
| `orcanium honcho peer --user <name>` | Update user peer name |
| `orcanium honcho peer --ai <name>` | Update AI peer name |
| `orcanium honcho tokens --context <N>` | Set context token budget |
| `orcanium honcho tokens --dialectic <N>` | Set dialectic max chars |
| `orcanium honcho map <name>` | Map current directory to a session name |
| `orcanium honcho sync` | Create host blocks for all Orcanium profiles |
