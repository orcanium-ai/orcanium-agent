...
Config: Uses the existing Honcho config chain:
  1. $ORCANIUM_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, orcanium_home):
        """Write config to $ORCANIUM_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(orcanium_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                ...
                # behave as they did before. New setups via `orcanium honcho setup`
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps Orcanium CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "orcanium-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # synchroorcanium setup has finished; background startup sets _manager before
        ...
        # Skip under per-session strategy: every Orcanium run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_orcanium_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "orcanium-default",
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # First fetch is asynchroorcanium: a slow Honcho backend must not block the
