...
Resolution order for config file:
  1. $ORCANIUM_HOME/honcho.json  (instance-local, enables isolated Orcanium instances)
...
from orcanium.cli.profiles import _get_default_orcanium_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Orcanium profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Orcanium profile.

    Resolution order:
      1. ORCANIUM_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``orcanium.<profile>``
      3. Fallback: ``"orcanium"`` (default profile)
    """
    explicit = os.environ.get("ORCANIUM_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ORCANIUM_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.orcanium/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_orcanium_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_orcanium_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Orcanium system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.orcanium
    ...
    def from_global_config(
        ...
        Resolution: $ORCANIUM_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Orcanium profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.orcanium block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Orcanium session title (from /title command)
          3. Gateway session key (stable per-chat identifier from channel platforms)
          4. per-session strategy — Orcanium session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Orcanium session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'orcanium honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run orcanium honcho setup" hint they used to.
        try:
            from orcanium.app.tools.lazy_deps import FeatureUnavailable, ensure as _lazy_ensure
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `orcanium honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                orcanium_cfg = load_config()
                honcho_cfg = orcanium_cfg.get("honcho", {})
