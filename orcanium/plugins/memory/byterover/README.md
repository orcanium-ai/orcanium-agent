...
```bash
orcanium memory setup    # select "byterover"
```

Or manually:
```bash
orcanium config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.orcanium/.env
```
...
Working directory: `$ORCANIUM_HOME/byterover/` (profile-scoped).
