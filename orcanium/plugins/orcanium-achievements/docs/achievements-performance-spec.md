# Orcanium Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: orcanium-achievements plugin
...
### `GET /api/plugins/orcanium-achievements/achievements`
...
### `POST /api/plugins/orcanium-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/orcanium-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/orcanium-achievements/overview`
...
- Persist plugin data under:
  - `~/.orcanium/plugins/orcanium-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.orcanium/plugins/orcanium-achievements/scan_checkpoint.json`
...
- `~/.orcanium/plugins/orcanium-achievements/`
