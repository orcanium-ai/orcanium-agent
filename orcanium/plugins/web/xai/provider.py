...
``{title, url, description, position}`` rows every other Orcanium web
provider produces.
...
prefers Orcanium-managed xAI Grok OAuth (via ``orcanium auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.orcanium/.env``, then
``os.environ``).
"""
...
from orcanium.app.tools.xai_http import (
    has_xai_credentials,
    orcanium_xai_user_agent,
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``orcanium tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `orcanium auth` to sign in with "
