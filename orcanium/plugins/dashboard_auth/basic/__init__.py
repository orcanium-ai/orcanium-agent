...
into the same ``DashboardAuthProvider`` framework as the Orcanium OAuth
...
mirroring the Orcanium provider's precedence convention:
  ...
  Environment overrides::
      ...
      ORCANIUM_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      ORCANIUM_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      ORCANIUM_DASHBOARD_BASIC_AUTH_SECRET
      ORCANIUM_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
Skip reasons:
  Like the Orcanium provider, this exposes a module-level ``LAST_SKIP_REASON``
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "ORCANIUM_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or ORCANIUM_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "ORCANIUM_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "ORCANIUM_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "ORCANIUM_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "ORCANIUM_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "ORCANIUM_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # ORCANIUM_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "ORCANIUM_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
