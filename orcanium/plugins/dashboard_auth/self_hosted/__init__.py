...
A standards-compliant OpenID Connect Relying Party for the ``orcanium dashboard``
OAuth gate. Unlike the bundled ``orcanium`` provider (which encodes Orcanium Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-orcanium-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~orcanium.cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``orcanium.cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~orcanium.cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``orcanium``
provider verifies its *access* token because Orcanium Portal mints a custom JWT
...
same precedence convention as the ``orcanium`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          issuer: https://auth.example.com/application/o/orcanium/   # required
          client_id: orcanium-dashboard                              # required
          scopes: "openid profile email"                           # optional

    # Environment overrides (Docker/Fly secret injection)
    ORCANIUM_DASHBOARD_OIDC_ISSUER
    ORCANIUM_DASHBOARD_OIDC_CLIENT_ID
    ORCANIUM_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the orcanium
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the orcanium plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Orcanium does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        operator-error case with a clear message. Mirrors the orcanium provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``ORCANIUM_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "ORCANIUM_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "ORCANIUM_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("ORCANIUM_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(ORCANIUM_DASHBOARD_OIDC_ISSUER + ORCANIUM_DASHBOARD_OIDC_CLIENT_ID) "
