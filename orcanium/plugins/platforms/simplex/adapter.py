"""SimpleX Chat platform adapter (Orcanium plugin).
...
This adapter ships as a Orcanium platform plugin under
``plugins/platforms/simplex/``. The Orcanium plugin loader scans the
...
Optional environment variables:
    ...
    ORCANIUM_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``orcanium setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Orcanium — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ORCANIUM_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``orcanium setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.orcanium/.env`` via
    ``orcanium.cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "orcanium.cli.config not available; set SIMPLEX_* vars manually in "
            "~/.orcanium/.env"
        )
        return
