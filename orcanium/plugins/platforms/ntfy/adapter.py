"""ntfy platform adapter (Orcanium plugin).
...
a Orcanium dependency.

This adapter ships as a Orcanium platform plugin under
``plugins/platforms/ntfy/``. The Orcanium plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "orcanium-in"              # subscribe topic (incoming)
          publish_topic: "orcanium-out"     # optional — defaults to topic
...
_ECHO_TAG = "orcanium-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``orcanium cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Orcanium dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `orcanium gateway status` without
