"""
LINE Messaging API platform adapter for Orcanium Agent.
...
to Orcanium Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ORCANIUM_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            orcanium_home = Path.home().joinpath(".orcanium").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``orcanium status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``orcanium status`` reflect a LINE configuration that lives entirely
...
async def _standalone_send(
    ...
    *,
    ...
    """Out-of-process push delivery for cron jobs running detached from the channel.
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``orcanium setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.orcanium/.env`` via ``orcanium.cli.config``.
    """
    print()
    ...
    try:
        from orcanium.cli.config import get_env_var, set_env_var
    except ImportError:
        print("orcanium.cli.config not available; set LINE_* vars manually in ~/.orcanium/.env")
        return
