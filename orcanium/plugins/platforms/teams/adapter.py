"""
Microsoft Teams platform adapter for Orcanium Agent.
...
class TeamsSummaryWriter:
    ...
    def _build_graph_client(self, config: dict[str, Any]) -> Any:
        ...
        from orcanium.app.tools.microsoft_graph_auth import MicrosoftGraphTokenProvider
        from orcanium.app.tools.microsoft_graph_client import MicrosoftGraphClient
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Orcanium"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        matching the cache_*_from_url helpers in channel.platforms.base.
        """
        from orcanium.app.tools.url_safety import is_safe_url
        from channel.platforms.base import _ssrf_redirect_guard
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; OrcaniumAgent/1.0)"},
            )
            response.raise_for_status()
            return response.content

    async def _on_message(self, ctx: ActivityContext[MessageActivity]) -> None:
        """Process an incoming Teams message and dispatch to the channel."""
    ...
    async def _on_card_action(
        ...
        from orcanium.app.tools.approval import resolve_gateway_approval, has_blocking_approval

        action = ctx.activity.value.action
        data = action.data or {}
        orcanium_action = data.get("orcanium_action", "")
        session_key = data.get("session_key", "")

        if not orcanium_action or not session_key:
        ...
        choice = choice_map.get(orcanium_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="orcanium_approve",
                    data={**btn_data_base, "orcanium_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="orcanium_approve",
                    data={**btn_data_base, "orcanium_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="orcanium_approve",
                    data={**btn_data_base, "orcanium_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="orcanium_approve",
                    data={**btn_data_base, "orcanium_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Orcanium\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.orcanium/.env")
