"""
IRC Platform Adapter for Orcanium Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Orcanium agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "orcanium-bot")
    ...
    async def connect(self) -> bool:
        ...
        try:
            from channel.status import acquire_scoped_lock, release_scoped_lock
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Orcanium Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Orcanium Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: orcanium_, orcanium_1, orcanium_2...
...
def check_requirements() -> bool:
    ...
    # orcanium setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `orcanium gateway setup` flow for the IRC platform.

    Lazy-imports ``orcanium.cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Orcanium to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #orcanium — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.orcanium/.env")
    print_info("Restart the gateway for changes to take effect: orcanium gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "orcanium-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "orcanium-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Orcanium Agent (cron)")
