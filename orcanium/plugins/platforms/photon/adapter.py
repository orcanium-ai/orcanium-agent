"""
Photon Spectrum (iMessage) platform adapter for Orcanium Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Orcanium dep
...
from channel.platforms.helpers import strip_markdown
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `orcanium photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `orcanium setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Orcanium defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Orcanium-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    async def _dispatch_inbound(self, event: Dict[str, Any]) -> None:
        """Normalize a sidecar inbound event and dispatch it to the channel.
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Orcanium checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `orcanium photon setup`)"
            )
    ...
    async def send_image(
        ...
        try:
            from channel.platforms.base import cache_image_from_url
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Orcanium-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Orcanium plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `orcanium photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: orcanium photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `orcanium gateway setup` alongside every other
        # channel — same unified onboarding wizard, no Photon-only detour.
        setup_fn=_cli.channel_setup,
    ...
    # Register CLI subcommands — `orcanium photon ...`
