...
Credential storage mirrors every other Orcanium channel:

    * runtime SDK creds  -> ``~/.orcanium/.env``  (``PHOTON_PROJECT_ID`` =
      spectrumProjectId, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.orcanium/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a orcanium dependency
...
# registers Orcanium as its own client_id.
...
# Default name of the project Orcanium provisions for the operator.
DEFAULT_PROJECT_NAME = "Orcanium Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of orcanium-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.orcanium/auth.json`` honouring the active Orcanium profile."""
    try:
        from orcanium_constants import get_orcanium_home
        return Path(get_orcanium_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.orcanium")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.orcanium/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.orcanium/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.orcanium/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: orcanium.cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `orcanium photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `orcanium photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `orcanium photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `orcanium photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `orcanium photon setup`)"
