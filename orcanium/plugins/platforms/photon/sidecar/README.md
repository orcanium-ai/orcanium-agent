...
Small Node helper that bridges Orcanium Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Orcanium is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Orcanium-Sidecar-Token`)
...
The Orcanium plugin's `orcanium photon setup` command runs `npm install`
here automatically.
