// Orcanium Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Orcanium): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Orcanium -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Orcanium-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `orcanium photon telemetry`)
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-orcanium-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Orcanium supplied them so a known-good
