"""
``orcanium photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Orcanium
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `orcanium photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Orcanium Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``orcanium photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Orcanium Agent" project.
    ...
    #    and persist both (runtime creds -> ~/.orcanium/.env, ids -> auth.json).
    try:
    ...
    print("  Start the gateway:  orcanium gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `orcanium photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`orcanium photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.orcanium/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `orcanium photon telemetry on` / `orcanium photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.orcanium/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  orcanium gateway restart")
    return 0
...
#
# `orcanium gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``orcanium photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def channel_setup() -> None:
    """Run Photon first-time setup from the `orcanium gateway setup` wizard."""
