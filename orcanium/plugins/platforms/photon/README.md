...
This plugin connects Orcanium Agent to iMessage (and other Spectrum
...
TypeScript-only, Orcanium runs it inside a small supervised Node sidecar and
...
- **Inbound**: the sidecar consumes the SDK's `app.messages` gRPC stream,
  ...
  a `MessageEvent` to the channel. It reconnects automatically if the stream
...
There is no separate `login` command; like every other Orcanium channel,
...
Run `orcanium photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.orcanium/.env` (the same place every other
...
Management metadata lives in `~/.orcanium/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `orcanium photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `orcanium photon status`, a DM and a group roundtrip,
