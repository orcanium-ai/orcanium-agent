...
from channel.config import Platform, PlatformConfig
import re

from channel.platforms.helpers import MessageDeduplicator, ThreadParticipationTracker
from orcanium.cli.compat import atomic_json_write
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.orcanium/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("ORCANIUM_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("ORCANIUM_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Orcanium gateway
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            from channel.platforms.base import resolve_proxy_url, proxy_kwargs_for_bot
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_orcanium_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Orcanium manages."""
                ...
                for image_url, alt_text in chunk:
                    ...
                    if image_url.startswith("file://"):
                        ...
                        try:
                            ...
                            from channel.platforms.base import resolve_proxy_url, proxy_kwargs_for_aiohttp
    ...
    async def _process_voice_input(self, guild_id: int, user_id: int, pcm_data: bytes):
        ...
        from orcanium.app.tools.voice_mode import is_whisper_hallucination
    ...
    def _is_allowed_user(
        ...
        *,
        ...
        # (see AGENTS.md pitfall #17 — same pattern as channel.run).
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Orcanium session")
        ...
        @tree.command(name="status", description="Show Orcanium session status")
        ...
        @tree.command(name="stop", description="Stop the running Orcanium agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.orcanium/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Orcanium Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Orcanium gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Orcanium session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Orcanium in the thread",
        ...
        # orcanium.cli/commands.py automatically appear as Discord slash
        ...
        try:
            from orcanium.cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from orcanium.cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Orcanium skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from orcanium.cli.commands import discord_skill_commands_by_category
    ...
    def refresh_skill_group(self) -> tuple[int, int]:
        ...
        Invoked by :meth:`channel.run.GatewayOrchestrator._handle_reload_skills_command`
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Orcanium session in the thread
    ...
    def _resolve_channel_skills(self, channel_id: str, parent_id: str | None = None) -> list[str] | None:
        ...
        """
        from channel.platforms.base import resolve_channel_skills
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Orcanium: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Orcanium"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Orcanium: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "Orcanium session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Orcanium handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Orcanium needs your input",
                description=body,
                color=discord.Color.orange(),
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``orcanium update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``orcanium update`` prompts.
...
# is not in this process (e.g. ``orcanium cron`` running standalone) and no live
    ...
    *,
    ...
    try:
        ...
        if thread_id:
            ...
            _channel_type = None
            try:
                from channel.channel_directory import lookup_channel_type
...
def interactive_setup() -> None:
    ...
    from orcanium.cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``orcanium.cli.channel.get_env_value`` at call time (not via
    ...
    """
    import orcanium.cli.channel as gateway_mod
...
def register(ctx) -> None:
    """Plugin entry point — called by the Orcanium plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'orcanium-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # orcanium.cli/setup.py::_setup_discord function.  Same shape as Teams.
        ...
        # when cron runs separately from the channel.  Mirrors Teams pattern.
