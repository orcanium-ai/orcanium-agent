...
googleapiclient is synchroorcanium. This keeps the event loop responsive.
...
# ``model_tools`` import time, so ``orcanium status``, ``orcanium chat``, etc.
...
def _load_google_modules() -> bool:
    ...
    every CLI invocation, even ones that never touch a channel.
    """
...
from channel.platforms.helpers import MessageDeduplicator
...
# ``orcanium_plugins.platforms__google_chat.adapter`` once the plugin
# loader namespaces this module, which would silently break every
# downstream log-monitor that greps for ``channel.platforms.google_chat``.
logger = logging.getLogger("channel.platforms.google_chat")
...
#     fresh "Orcanium is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a orcanium MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # attribute to ``channel.config.Platform`` — bundled platform plugins
        ...
        # Persisted to ${ORCANIUM_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from orcanium_constants import get_orcanium_home as _get_orcanium_home
            _orcanium_home = _get_orcanium_home()
        except (ModuleNotFoundError, ImportError):
            _orcanium_home = _Path.home() / ".orcanium"
        self._thread_count_store = _ThreadCountStore(
            _orcanium_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Orcanium is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ORCANIUM_HOME", str(_Path.home() / ".orcanium"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ORCANIUM_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a orcanium MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Orcanium is thinking…' marker message.
        ...
        "Orcanium is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Orcanium is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Orcanium is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Orcanium is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Orcanium is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``orcanium setup``.

    The setup wizard at ``orcanium.cli/channel.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.orcanium/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. orcanium-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.orcanium/.env")
    print_info("Restart the gateway: orcanium gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``orcanium cron`` running as a
    separate process from ``orcanium gateway``).  Without this hook,
    ...
    # Bound the synchroorcanium urllib3-backed token refresh so a hung Google
...
def register(ctx) -> None:
    """Plugin entry point — called by the Orcanium plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'orcanium-agent[google_chat]'",
        ...
        # when cron runs separately from the channel.
        ...
        # the "Orcanium is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Orcanium is thinking…' marker message "
