"""
Orcanium MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Orcanium-specific extra)

Usage:
    Orcanium mcp serve
    Orcanium mcp serve --verbose
...
logger = logging.getLogger("orcanium.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ORCANIUM_HOME."""
    try:
        from orcanium.channel.compat import get_orcanium_home as get_orcanium_home
        return get_orcanium_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ORCANIUM_HOME", Path.home() / ".Orcanium")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from orcanium.channel.compat import get_orcanium_home as get_orcanium_home
        directory_file = get_orcanium_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ORCANIUM_HOME", Path.home() / ".Orcanium")
...
class EventBridge:
    ...
    This is the Orcanium equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from orcanium.channel.compat import get_orcanium_home as get_orcanium_home
            db_file = get_orcanium_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ORCANIUM_HOME", Path.home() / ".Orcanium")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Orcanium MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "orcanium",
        instructions=(
            "Orcanium Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Orcanium MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
