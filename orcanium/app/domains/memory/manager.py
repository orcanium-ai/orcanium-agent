...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ORCANIUM_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``orcanium_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_orcanium_home()`` themselves.
        """
        if "orcanium_home" not in kwargs:
            from orcanium.orcanium_constants import get_orcanium_home
            kwargs["orcanium_home"] = str(get_orcanium_home())
        for provider in self._providers:
            try:
