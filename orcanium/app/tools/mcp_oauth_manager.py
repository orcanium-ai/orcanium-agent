...
`mcp_tool.py`, and `orcanium.cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# OrcaniumMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_orcanium_provider_class() -> Optional[type]:
    ...
    class OrcaniumMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._orcanium_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`OrcaniumTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, OrcaniumTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._orcanium_server_name, exc,
                    )
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            but runs synchroorcaniumly before the first request instead of
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._orcanium_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from orcanium.app.tools.mcp_oauth import OrcaniumTokenStorage
                        if isinstance(storage, OrcaniumTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._orcanium_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from orcanium.app.tools.mcp_oauth import OrcaniumTokenStorage
            if not isinstance(storage, OrcaniumTokenStorage):
                return
    ...
    return OrcaniumMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ORCANIUM_PROVIDER_CLS: Optional[type] = _make_orcanium_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`OrcaniumMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _ORCANIUM_PROVIDER_CLS is None:
            logger.warning(
        ...
        from orcanium.app.tools.mcp_oauth import (
            OrcaniumTokenStorage,
            _OAUTH_AVAILABLE,
        ...
        storage = OrcaniumTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``orcanium mcp remove <name>`` and (indirectly) by
        ``orcanium mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            self._entries.pop(server_name, None)

        from orcanium.app.tools.mcp_oauth import remove_oauth_tokens
    ...
    async def invalidate_if_disk_changed(self, server_name: str) -> bool:
        ...
        """
        from orcanium.app.tools.mcp_oauth import _get_token_dir, _safe_filename
