...
provider** — turn on a backend by enabling a plugin (``orcanium plugins
enable video_gen/<name>``) and selecting it in ``orcanium tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/orcanium-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `orcanium tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `orcanium tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `orcanium tools` → "
...
# `orcanium tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`orcanium tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `orcanium tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
