...
wrapper, while preserving Orcanium's persistent snapshot behavior across sessions.
"""
...
from orcanium.app.tools.environments.base import (
    BaseEnvironment,
    _ThreadedProcessHandle,
    _load_json_store,
    _save_json_store,
)
from orcanium.app.tools.environments.file_sync import (
    FileSyncManager,
    iter_sync_files,
...
_SNAPSHOT_STORE = get_orcanium_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("orcanium", create_if_missing=True)
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.Orcanium"),
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .Orcanium/ as a tar archive.

        Modal sandboxes always run as root, so /root/.Orcanium is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.Orcanium"
            )
