"""Base class for all Orcanium execution environment backends.
...
# ORCANIUM_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("ORCANIUM_DEBUG_INTERRUPT"))
...
# long-running _wait_for_process loops can report liveness to the channel.
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ORCANIUM_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_orcanium_home() / "sandboxes"
...
def _popen_bash(
    ...
    If *stdin_data* is provided, writes it asynchroorcaniumly via :func:`_pipe_stdin`.
...
def _cwd_marker(session_id: str) -> str:
    return f"__ORCANIUM_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Orcanium backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/orcanium-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/orcanium-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../orcanium-snap-*.sh: No such file or directory``
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("exit $__orcanium_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ORCANIUM_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via ORCANIUM_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __ORCANIUM_CWD_{session}__ marker from stdout output.
    ...
    def execute(
        ...
        *,
        ...
        if rewrite_compound_background:
            from orcanium.app.tools.terminal_tool import _rewrite_compound_background
    ...
    def _prepare_command(self, command: str) -> tuple[str, str | None]:
        ...
        from orcanium.app.tools.terminal_tool import _transform_sudo_command
