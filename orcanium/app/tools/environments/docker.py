...
from orcanium.app.tools.environments.base import BaseEnvironment, _popen_bash
from orcanium.app.tools.environments.local import _ORCANIUM_PROVIDER_ENV_BLOCKLIST
...
def _load_orcanium_env_vars() -> dict[str, str]:
    """Load ~/.orcanium/.env values without failing Docker command execution."""
    try:
        from orcanium.cli.compat import load_env
...
def _get_active_profile_name() -> str:
    """Return the active Orcanium profile name, or ``"default"`` on any error.
    ...
    """
    try:
        from orcanium.cli.compat import get_active_profile_name
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale orcanium-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=orcanium=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Orcanium process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=orcanium-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a Orcanium process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Orcanium
    process leaves its container behind permanently because there's no
    subsequent Orcanium process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=orcanium=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=orcanium-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ORCANIUM_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ORCANIUM_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ORCANIUM_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'orcanium'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``orcanium:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.orcanium/sandboxes/). Non-persistent
        # mode uses tmpfs (ephemeral, fast, gone on cleanup).
        from orcanium.app.tools.environments.base import get_sandbox_dir
        ...
        try:
            from orcanium.app.tools.credential_files import (
        ...
        # s6-overlay images (e.g. orcanium:latest) already use /init as PID 1
        ...
        container_name = f"orcanium-{uuid.uuid4().hex[:8]}"
        # Labels make orcanium-created containers identifiable to:
        #   * the orphan reaper (`orcanium=1` for the global sweep filter)
        #   * future cross-process reuse (`orcanium-task-id`, `orcanium-profile`)
        #   * operators running `docker ps --filter label=orcanium=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Orcanium profile is captured at
        ...
        label_args = [
            "--label", "orcanium=1",
            "--label", f"orcanium-task-id={task_label}",
            "--label", f"orcanium-profile={profile_name}",
        ]
        ...
        self._labels = {
            "orcanium": "1",
            "orcanium-task-id": task_label,
            "orcanium-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Orcanium process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        try:
            from orcanium.app.tools.env_passthrough import get_all_passthrough
            passthrough_keys = set(get_all_passthrough())
        except Exception:
            pass
        # Explicit docker_forward_env entries are an intentional opt-in and must
        # win over the generic Orcanium secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ORCANIUM_PROVIDER_ENV_BLOCKLIST)
        orcanium_env = _load_orcanium_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = orcanium_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("orcanium-task-id", "")
        profile_label = self._labels.get("orcanium-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"orcanium-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``orcanium-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=orcanium=1",
                    "--filter", f"label=orcanium-task-id={task_label}",
                    "--filter", f"label=orcanium-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Orcanium process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Orcanium exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Orcanium process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Orcanium startup. That covers the
        ...
        # outlives Orcanium processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"orcanium-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``orcanium /quit``
