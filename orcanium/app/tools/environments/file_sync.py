...
from orcanium.app.tools.environments.base import _file_mtime_key
...
_FORCE_SYNC_ENV = "ORCANIUM_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.Orcanium") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.Orcanium to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.Orcanium", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ORCANIUM_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, orcanium_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.Orcanium/`` directory as a tar archive,
        ...
        # against an uninitialized remote .Orcanium/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (orcanium_home or get_orcanium_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="orcanium-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.Orcanium/skills/a.md`` →
        ``~/.orcanium/skills/a.md``, a new remote file at
        ``/root/.Orcanium/skills/b.md`` maps to ``~/.orcanium/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
