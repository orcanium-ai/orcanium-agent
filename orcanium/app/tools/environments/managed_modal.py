"""Managed Modal environment backed by tool-channel."""
...
from orcanium.app.tools.environments.modal_utils import (
...
class ManagedModalEnvironment(BaseModalExecutionEnvironment):
    """Gateway-owned Modal sandbox with Orcanium-compatible execute/cleanup."""
    ...
    def __init__(
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway and Orcanium user token")

        self._gateway_origin = channel.gateway_origin.rstrip("/")
        self._orcanium_user_token = channel.orcanium_user_token
    ...
    def _guard_unsupported_credential_passthrough(self) -> None:
        ...
        try:
            from orcanium.app.tools.credential_files import get_credential_file_mounts
        except Exception:
            return
    ...
    def _request(self, method: str, path: str, *,
        ...
        headers = {
            "Authorization": f"Bearer {self._orcanium_user_token}",
