...
from orcanium.app.tools.environments.base import BaseEnvironment, _pipe_stdin
...
# Orcanium-internal env vars that should NOT leak into terminal subprocesses.
_ORCANIUM_PROVIDER_ENV_FORCE_PREFIX = "_ORCANIUM_FORCE_"

# Orcanium-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Orcanium inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from orcanium.cli.compat import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "ORCANIUM_DASHBOARD_SESSION_TOKEN",
...
_ORCANIUM_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_orcanium_home(env: dict) -> None:
    """Bridge the context-local Orcanium home override into subprocess env."""
    try:
        from orcanium.channel.compat import get_orcanium_home_override

        value = get_orcanium_home_override()
        if value:
            env["ORCANIUM_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Orcanium-managed secrets from a subprocess environment."""
    try:
        from orcanium.app.tools.env_passthrough import is_env_passthrough as _is_passthrough
    except Exception:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ORCANIUM_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _ORCANIUM_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_ORCANIUM_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ORCANIUM_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _ORCANIUM_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_orcanium_home(sanitized)
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ORCANIUM_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\Orcanium\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\Orcanium\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _orcanium_portable_git = os.path.join(_local_appdata, "orcanium", "git") if _local_appdata else ""
    if _orcanium_portable_git:
        for candidate in (
            os.path.join(_orcanium_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_orcanium_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Orcanium Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set ORCANIUM_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ORCANIUM_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ORCANIUM_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _ORCANIUM_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        run_env[path_key] = _append_missing_sane_path_entries(run_env.get(path_key, ""))

    _inject_context_orcanium_home(run_env)

    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,
    # npm …) into {ORCANIUM_HOME}/home/ when that directory exists.  Only the
    ...
    try:
        from channel.session_context import _UNSET, _VAR_MAP
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Orcanium trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ORCANIUM_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ORCANIUM_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_orcanium_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "orcanium_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._orcanium_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_orcanium_pgid", None)
                    if pgid is None:
                        raise
