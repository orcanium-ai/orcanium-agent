...
from orcanium.app.tools.binary_extensions import has_binary_extension
from orcanium.app.tools.file_operations import (
    ShellFileOperations,
    normalize_read_pagination,
    normalize_search_pagination,
)
from orcanium.app.tools import file_state
...
def _get_live_tracking_cwd(task_id: str = "default") -> str | None:
    ...
    try:
        from orcanium.app.tools.terminal_tool import _resolve_container_task_id
    ...
    try:
        from orcanium.app.tools.terminal_tool import _active_environments, _env_lock
...
_orcanium_config_resolved: str | None = None
_orcanium_config_resolved_loaded = False


def _get_orcanium_config_resolved() -> str | None:
    """Return the resolved absolute path of the Orcanium config file (cached)."""
    global _orcanium_config_resolved, _orcanium_config_resolved_loaded
    if _orcanium_config_resolved_loaded:
        return _orcanium_config_resolved
    _orcanium_config_resolved_loaded = True
    try:
        from orcanium.cli.compat import get_config_path
        _orcanium_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _orcanium_config_resolved = str(Path("~/.orcanium/config.yaml").expanduser().resolve())
        except Exception:
            _orcanium_config_resolved = None
    return _orcanium_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Orcanium config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    orcanium_config = _get_orcanium_config_resolved()
    if orcanium_config and (resolved == orcanium_config or normalized == orcanium_config):
        return (
            f"Refusing to write to Orcanium config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.orcanium/config.yaml directly or use 'orcanium config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Orcanium mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Orcanium
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Orcanium state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.Orcanium/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Orcanium process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.Orcanium/…`` path.

    Returns ``None`` when the write is in-scope or outside Orcanium scope.
    ...
    # in a session that cd'd into ``~/.orcanium/profiles/other/`` is
...
def _get_file_ops(task_id: str = "default") -> ShellFileOperations:
    ...
    with task_lock:
        ...
        if terminal_env is None:
            from orcanium.app.tools.terminal_tool import _task_env_overrides
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Orcanium internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under ORCANIUM_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ORCANIUM_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Orcanium-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Orcanium-profile guard for
...
def _check_file_reqs():
    ...
    from orcanium.app.tools import check_file_requirements
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Orcanium profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Orcanium profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with orcanium_tools.write_file() for very "
            "large files."
        )
