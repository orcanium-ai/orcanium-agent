"""
Cron job management tools for Orcanium Agent.
...
#      saying "cat ~/.orcanium/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `orcanium-dev` skill contains a security postmortem mentioning
#      `cat ~/.orcanium/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("ORCANIUM_SESSION_PLATFORM")
    origin_chat_id = get_session_env("ORCANIUM_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("ORCANIUM_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("ORCANIUM_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via orcanium model.
    ...
    if provider_name == "custom":
        try:
            from orcanium.cli.compat import has_named_custom_provider
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within ORCANIUM_HOME/scripts/.
    ...
    # Only relative paths within ~/.orcanium/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.orcanium/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.orcanium/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from orcanium.app.tools.path_security import validate_within_dir

    scripts_dir = get_orcanium_home() / "scripts"
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_orcanium_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("ORCANIUM_INTERACTIVE")
        or env_var_enabled("ORCANIUM_GATEWAY_SESSION")
        or env_var_enabled("ORCANIUM_EXEC_ASK")
    )
