...
Orcanium, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("ORCANIUM_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from orcanium.cli.compat import cfg_get, load_config
        cfg = load_config()
...
async def _validate_image_url_async(url: str) -> bool:
    ...
    if not _image_url_shape_ok(url):
        return False
    from orcanium.app.tools.url_safety import async_is_safe_url
...
def _resize_image_for_vision(image_path: Path, mime_type: Optional[str] = None,
    ...
    try:
        ...
        try:
            from orcanium.app.tools.lazy_deps import ensure as _ensure_dep
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * Anthropic Messages API (``anthropic`` provider, plus aggregators that
        proxy Claude — ``openrouter``, ``Orcanium``, ``vertex``, ``bedrock``):
    ...
    # them.
    _AGGREGATORS = {
        "openrouter", "orcanium", "vertex", "bedrock", "anthropic-vertex",
        "google-vertex",
    }
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_orcanium_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $ORCANIUM_HOME/cache/vision/ and cleaned up
...
def check_vision_requirements() -> bool:
    ...
    and if that fails, the auto chain (main provider → openrouter → Orcanium).
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, Orcanium, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_orcanium_dir("cache/video", "temp_video_files")
