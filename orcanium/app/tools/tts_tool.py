...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.orcanium/config.yaml``. Orcanium
...
Configuration is loaded from ~/.orcanium/config.yaml under the 'tts:' key.
The user chooses the provider and voice; the model just sends text.

Usage:
    from orcanium.app.tools.tts_tool import text_to_speech_tool, check_tts_requirements
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``orcanium.cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from orcanium.cli.compat import get_env_value as _get_env_value
    except ImportError:
        return os.getenv(name, default)
    value = _get_env_value(name)
    return default if value is None else value
from orcanium.app.tools.managed_tool_gateway import resolve_managed_tool_gateway
...
from orcanium.app.tools.xai_http import orcanium_xai_user_agent
...
def _import_elevenlabs():
    ...
    """
    try:
        from orcanium.app.tools.lazy_deps import FeatureUnavailable, ensure
...
def _import_mistral_client():
    ...
    """
    try:
        from orcanium.app.tools.lazy_deps import ensure
...
def _get_default_output_dir() -> str:
    ...
    return str(get_orcanium_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.orcanium/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.orcanium/config.yaml.
    ...
    try:
        ...
        logger.debug("orcanium_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Orcanium without any Python code
...
#           output_format: wav
#
# Orcanium writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ORCANIUM_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    from orcanium.app.tools.xai_http import resolve_xai_http_credentials

    creds = resolve_xai_http_credentials()
    api_key = str(creds.get("api_key") or "").strip()
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `orcanium model` or set XAI_API_KEY.")
    ...
    # output_format when Orcanium actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": orcanium_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Orcanium caches Piper voice models.

    Resolves to ``~/.orcanium/cache/piper-voices/`` under the active
    ORCANIUM_HOME so voice downloads follow profile boundaries.
    """
    from orcanium.channel.compat import get_orcanium_dir
    root = Path(get_orcanium_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.orcanium/config.yaml (tts: section).
    ...
    # Detect platform from channel env var to choose the best output format.
    # Telegram voice bubbles require Opus (.ogg); OpenAI and ElevenLabs can
    # produce Opus natively (no ffmpeg needed).  Edge TTS always outputs MP3
    # and needs ffmpeg for conversion.
    from channel.session_context import get_session_env
    platform = get_session_env("ORCANIUM_SESSION_PLATFORM", "").lower()
    ...
    if output_path:
        ...
        from orcanium.app.tools.path_security import has_traversal_component
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'orcanium[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run orcanium setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'orcanium setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'orcanium tools' and select Piper under TTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    if managed_gateway is None:
        ...
        if managed_orcanium_tools_enabled() or prefers_gateway("tts"):
            message += (
                ". "
                + orcanium_tool_gateway_unavailable_message(
    ...
    return managed_gateway.orcanium_user_token, urljoin(
        f"{managed_gateway.gateway_origin.rstrip('/')}/", "v1"
    )


def _has_openai_audio_backend() -> bool:
    """Return True when OpenAI audio can use direct credentials or the managed channel."""
    ...
    try:
        ...
        def _play_via_tempfile(audio_iter, stop_evt):
            ...
            try:
                ...
                from orcanium.app.tools.voice_mode import play_audio_file
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_orcanium_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
