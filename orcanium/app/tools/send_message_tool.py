...
def _handle_list():
    ...
    try:
        from channel.channel_directory import format_directory_for_display
...
def _handle_react(args, remove=False):
    ...
    runner = None
    try:
        from channel.run import _gateway_runner_ref
...
def _handle_send(args):
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.orcanium/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.orcanium/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if not home and platform_name == "weixin":
            ...
            if wx_home:
                from channel.config import HomeChannel
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: orcanium config set {home_env} <channel_id>"
            })
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                from channel.mirror import mirror_to_session
                from channel.session_context import get_session_env
                source_label = get_session_env("ORCANIUM_SESSION_PLATFORM", "cli")
                user_id = get_session_env("ORCANIUM_SESSION_USER_ID", "") or None
...
def _get_cron_auto_delivery_target():
    ...
    from channel.session_context import get_session_env
    platform = get_session_env("ORCANIUM_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("ORCANIUM_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("ORCANIUM_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    from channel.platforms.base import BasePlatformAdapter, utf16_len
    from channel.platforms.slack import SlackAdapter

    # Telegram adapter import is optional (requires python-telegram-bot)
    try:
        from channel.platforms.telegram import TelegramAdapter
    ...
    try:
        from channel.platforms.feishu import FeishuAdapter
...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False):
    ...
    try:
        ...
        try:
            from channel.platforms.base import resolve_proxy_url
        ...
        if formatted.strip():
            try:
                ...
                if _is_telegram_thread_not_found(md_error) and thread_kwargs:
                    ...
                    if not _has_html:
                        try:
                            from channel.platforms.telegram import _strip_mdv2
...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    from channel.platforms.signal_rate_limit import (
...
async def _send_matrix(token, extra, chat_id, message):
    ...
    try:
        ...
        txn_id = f"orcanium_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        from channel.platforms.matrix import MatrixAdapter
    except ImportError:
...
async def _send_weixin(pconfig, chat_id, message, media_files=None):
    ...
    try:
        from channel.platforms.weixin import check_weixin_requirements, send_weixin_direct
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        from channel.platforms.feishu import FeishuAdapter, FEISHU_AVAILABLE
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'orcanium[feishu]'"}
        from channel.platforms.feishu import FEISHU_DOMAIN, LARK_DOMAIN
    except ImportError:
        return {"error": "Feishu dependencies not installed. Run: pip install 'orcanium[feishu]'"}
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``ORCANIUM_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``ORCANIUM_HOME`` which has no ``channel.pid``, so the gateway-running check
    ...
    if os.environ.get("ORCANIUM_KANBAN_TASK"):
        return True
    from channel.session_context import get_session_env
    platform = get_session_env("ORCANIUM_SESSION_PLATFORM", "")
    if platform and platform != "local":
        return True
    try:
        from channel.status import is_gateway_running
...
async def _send_yuanbao(chat_id, message, media_files=None):
    ...
    """
    try:
        from channel.platforms.yuanbao import get_active_adapter, send_yuanbao_direct
    except ImportError:
