...
``ChannelRunner`` instance.  The CLI path (``cli.py``) uses a local
synchroorcanium variant — see ``_prompt_slash_confirm`` there.
"""
...
def resolve_sync_compat(
    ...
    """Synchroorcanium helper: schedule resolve() on a loop and wait for the result.
