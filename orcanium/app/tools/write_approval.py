...
(see ``orcanium.cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<ORCANIUM_HOME>/pending/{memory,skills}/<id>.json``
...
def write_approval_enabled(subsystem: str) -> bool:
    ...
    if subsystem not in _SUBSYSTEMS:
        return False
    try:
        from orcanium.cli.compat import load_config, cfg_get
        cfg = load_config()
...
def _pending_dir(subsystem: str) -> Path:
    return get_orcanium_home() / "pending" / subsystem
...
def current_origin() -> str:
    ...
    """
    try:
        from orcanium.app.tools.skill_provenance import get_current_write_origin
...
def _interactive_approval_available() -> bool:
    ...
    * Scripts, cron, and background threads — no user present.
    """
    try:
        from orcanium.app.tools.terminal_tool import _get_approval_callback
