...
selectable via ``orcanium tools`` → Image Generation; the active model is
...
def _load_fal_client() -> Any:
    ...
    if fal_client is not None:
        return fal_client
    from orcanium.app.tools.fal_common import import_fal_client
    fal_client = import_fal_client()
    return fal_client


from orcanium.app.tools.debug_helpers import DebugSession
from orcanium.app.tools.fal_common import (
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Managed Gateway should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Orcanium
    ...
    # existing FAL key / Managed Gateway subscription rather than register
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (Managed Gateway)
# ---------------------------------------------------------------------------
...
def _get_managed_fal_client(managed_gateway):
    ...
    client_config = (
        ...
        managed_gateway.orcanium_user_token,
    )
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.orcanium_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    """Submit a FAL request using direct credentials or the managed queue channel."""
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Managed Gateway gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Managed Gateway's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `orcanium tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _active_terminal_env(task_id: str | None):
    try:
        from orcanium.app.tools.terminal_tool import get_active_env
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.Orcanium"
    ...
    # Orcanium cache roots can be translated without side effects. SSH can still
...
def _agent_visible_cache_path(host_path: str, env: Any) -> str | None:
    ...
    try:
        from orcanium.app.tools.credential_files import map_cache_path_to_container
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Orcanium tools are enabled)
    ...
    if managed_orcanium_tools_enabled():
        ...
        gateway_message = orcanium_tool_gateway_unavailable_message(
            "managed FAL image generation",
        )
        if gateway_message:
    ...
    if managed_orcanium_tools_enabled():
        lines.append(
            "  2. Sign in to a Orcanium account that has the managed FAL "
            "gateway enabled (`orcanium setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `orcanium tools` "
        "→ Image Generation (run `orcanium plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping Orcanium with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `orcanium plugins list` to see "
