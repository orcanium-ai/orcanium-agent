...
`orcanium skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.Orcanium.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.Orcanium.blueprint`` is a mapping containing
    ...
    Orcanium = meta.get("orcanium") if isinstance(meta, dict) else None
    blueprint = Orcanium.get("blueprint") if isinstance(Orcanium, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.Orcanium.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.Orcanium.blueprint`` block) they can hand
    to ``orcanium skills publish`` to share. ``body`` is the plain-language
