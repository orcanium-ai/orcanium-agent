...
Backend is selected during ``orcanium tools`` setup (web.backend in config.yaml).
When available, Orcanium can route Firecrawl calls through a Orcanium-hosted tool-gateway
for Orcanium Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-channel.<domain> for Orcanium Subscribers)
...
from plugins.web.firecrawl.provider import (
    ...
    _get_firecrawl_client,  # noqa: F401  # re-exported for tests that `from orcanium.app.tools.web_tools import _get_firecrawl_client`
...
from orcanium.app.tools.debug_helpers import DebugSession
...
from orcanium.app.tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    build_vendor_gateway_url,
    peek_orcanium_access_token as _peek_orcanium_access_token,
    read_orcanium_access_token as _read_orcanium_access_token,
    resolve_managed_tool_gateway,
)
from orcanium.app.tools.tool_backend_helpers import (  # noqa: F401
    managed_orcanium_tools_enabled,
    orcanium_tool_gateway_unavailable_message,
    prefers_gateway,
)
from orcanium.app.tools.url_safety import async_is_safe_url, normalize_url_for_request
import sys
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Orcanium config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Orcanium's config/.env layer (``orcanium config set``, ``orcanium tools``)
    ...
    """
    try:
        from orcanium.cli.compat import get_env_value
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.orcanium/config.yaml."""
    try:
...
def _get_backend(capability: str = "search") -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``orcanium tools``).
    ...
    # pre-empted by a Orcanium OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `orcanium tools` repaint.
        try:
            from orcanium.app.tools.xai_http import has_xai_credentials
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_orcanium_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchroorcanium HTTP
    refresh against the Orcanium portal on every CLI startup (invoked at
...
def _is_orcanium_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is Orcanium Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "orcanium.com" or host.endswith(".orcanium.com")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_orcanium_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        from agent.portal_tags import orcanium_portal_tags
        extra_body = get_auxiliary_extra_body() or {"tags": orcanium_portal_tags()}
...
def _register_bundled_web_providers_directly() -> None:
    ...
    (:func:`orcanium.cli.plugins._ensure_plugins_discovered`), which auto-loads
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `orcanium tools` to set one up."
                ),
            }
        else:
            logger.info(
...
if __name__ == "__main__":
    ...
    orcanium_available = check_auxiliary_model()
    ...
    if not orcanium_available:
        print("❌ No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY, configure Orcanium Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
    ...
    print("")
    print("  # Search (synchroorcanium)")
    print("  results = web_search_tool('Python tutorials')")
    print("")
    print("  # Extract (asynchroorcanium)")
