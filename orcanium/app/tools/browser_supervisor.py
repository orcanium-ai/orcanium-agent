...
One ``CDPSupervisor`` runs per Orcanium ``task_id`` that has a reachable CDP
...
thread-safe snapshot object that tool handlers consume synchroorcaniumly.
...
DIALOG_BRIDGE_HOST = "orcanium-dialog-bridge.invalid"
...
(() => {
  if (window.__orcaniumDialogBridgeInstalled) return;
  window.__orcaniumDialogBridgeInstalled = true;
  const ENDPOINT = "http://orcanium-dialog-bridge.invalid/";
  ...
  // onbeforeunload — we can't really synchroorcaniumly prompt the user from this
...
class CDPSupervisor:
    ...
    async def _attach_initial_page(self) -> None:
        ...
        # a synchroorcanium XHR we intercept via Fetch domain. This is how we make
    ...
    async def _on_fetch_paused(
        ...
        The injected script (``_DIALOG_BRIDGE_SCRIPT``) fires a synchroorcanium
