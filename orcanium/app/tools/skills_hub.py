#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Orcanium Skills Hub.
...
Used by orcanium.cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
from orcanium.app.tools.skills_guard import (
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            orcanium_meta = metadata.get("orcanium", {})
            if isinstance(orcanium_meta, dict):
                tags = orcanium_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            orcanium_meta = metadata.get("orcanium", {})
            if isinstance(orcanium_meta, dict):
                raw_tags = orcanium_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Orcanium Research) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                orcanium_meta = meta_block.get("orcanium", {})
                if isinstance(orcanium_meta, dict):
                    tags = orcanium_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Orcanium centralized index source
# ---------------------------------------------------------------------------

ORCANIUM_INDEX_URL = "https://orcanium.orcanium.com/docs/api/skills-index.json"
ORCANIUM_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "orcanium-index.json"
ORCANIUM_INDEX_TTL = 6 * 3600  # 6 hours


def _load_orcanium_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ORCANIUM_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ORCANIUM_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ORCANIUM_INDEX_CACHE_FILE.stat().st_mtime
            if age < ORCANIUM_INDEX_TTL:
                return json.loads(ORCANIUM_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ORCANIUM_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Orcanium index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Orcanium index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ORCANIUM_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ORCANIUM_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class OrcaniumIndexSource(SkillSource):
    """Skill source backed by the centralized Orcanium Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_orcanium_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "orcanium-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "orcanium-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        OrcaniumIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "orcanium-index"
