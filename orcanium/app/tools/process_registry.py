...
from orcanium.app.tools.environments.local import _find_shell, _resolve_safe_cwd, _sanitize_subprocess_env
...
CHECKPOINT_PATH = get_orcanium_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def _terminate_host_pid(pid: int) -> None:
        ...
        existing convention in ``channel.status.terminate_pid``. We can't
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/orcanium_bg_{session.id}.log"
        pid_path = f"{temp_dir}/orcanium_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/orcanium_bg_{session.id}.exit"
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `orcanium update` restarting the gateway) is still
