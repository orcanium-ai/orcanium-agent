...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Orcanium exit
...
# ---------------------------------------------------------------------------
from orcanium.app.tools.interrupt import is_interrupted, _interrupt_event  # noqa: F401 — re-exported
# display_orcanium_home imported lazily at call site (stale-module safety during orcanium update)
...
from orcanium.app.tools.environments.singularity import _get_scratch_dir
from orcanium.app.tools.tool_backend_helpers import (
    coerce_modal_mode,
    has_direct_modal_credentials,
    managed_orcanium_tools_enabled,
    orcanium_tool_gateway_unavailable_message,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of Orcanium directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "orcanium-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("ORCANIUM_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("ORCANIUM_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ORCANIUM_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ORCANIUM_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (ORCANIUM_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Orcanium password prompt or the sudo -S password-pipe path.
...
from orcanium.app.tools.environments.local import LocalEnvironment as _LocalEnvironment
from orcanium.app.tools.environments.singularity import SingularityEnvironment as _SingularityEnvironment
from orcanium.app.tools.environments.ssh import SSHEnvironment as _SSHEnvironment
from orcanium.app.tools.environments.docker import DockerEnvironment as _DockerEnvironment
from orcanium.app.tools.environments.modal import ModalEnvironment as _ModalEnvironment
from orcanium.app.tools.environments.managed_modal import ManagedModalEnvironment as _ManagedModalEnvironment
from orcanium.app.tools.managed_tool_gateway import is_managed_tool_gateway_ready
import sys
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Orcanium can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``orcanium=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Orcanium processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Orcanium processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Orcanium processes a generous grace
    ...
    try:
        from orcanium.app.tools.environments.docker import (
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, OrcaniumSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.orcanium/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for orcanium-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Orcanium processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Orcanium Tool Gateway access is not currently available and no direct "
        ...
        from orcanium.app.tools.environments.daytona import DaytonaEnvironment as _DaytonaEnvironment
...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    for task_id, env in envs_to_stop:
        ...
        try:
            from orcanium.app.tools.file_tools import clear_file_ops_cache
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Orcanium can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            from orcanium.app.tools.approval import get_current_session_key
            from orcanium.app.tools.process_registry import process_registry
            ...
            try:
                ...
                # orcanium dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in orcanium dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    from channel.session_context import get_session_env as _gse
                    _gw_platform = _gse("ORCANIUM_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("ORCANIUM_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("ORCANIUM_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("ORCANIUM_SESSION_USER_ID", "")
                        _gw_user_name = _gse("ORCANIUM_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("ORCANIUM_SESSION_MESSAGE_ID", "")
                ...
                # asynchroorcaniumly and can spam the user long after the process ends.
            ...
            from orcanium.app.tools.tool_output_limits import get_max_bytes
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from orcanium.app.tools.environments.docker import find_docker
            docker = find_docker()
            if not docker:
