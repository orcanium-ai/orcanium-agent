...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``orcanium tools`` repaint, tool-registration scans,
    ...
    2. ``~/.orcanium/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_orcanium_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.orcanium/.env`` first, then ``os.environ``.

    Wraps :func:`orcanium.cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from orcanium.cli.compat import get_env_value as _orcanium_get_env_value

        value = _orcanium_get_env_value(name)
...
def orcanium_xai_user_agent() -> str:
    """Return a stable Orcanium-specific User-Agent for xAI HTTP calls."""
    try:
        from orcanium.cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Orcanium-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Orcanium-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``orcanium.cli.config.get_env_value`` so keys
    stored in ``~/.orcanium/.env`` (the standard Orcanium location) are honored —
