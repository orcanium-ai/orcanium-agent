...
This module provides the glue:
    - ``OrcaniumTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Orcanium Agent"
"""
...
from orcanium.channel.compat import secure_parent_dir
...
_USER_SKIPPED_ORCANIUM_SENTINEL = "__orcanium_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ORCANIUM_HOME so each profile gets its own OAuth tokens.
    Layout: ``ORCANIUM_HOME/mcp-tokens/``
    """
    try:
        from orcanium.channel.compat import get_orcanium_home
        base = Path(get_orcanium_home())
    except ImportError:
        base = Path(os.environ.get("ORCANIUM_HOME", str(Path.home() / ".Orcanium")))
...
# ---------------------------------------------------------------------------
# OrcaniumTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class OrcaniumTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ORCANIUM_HOME/mcp-tokens/<server_name>.json         -- tokens
        ORCANIUM_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ORCANIUM_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Orcanium records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Orcanium.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://orcanium.orcanium.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    if result["error"] == _USER_SKIPPED_ORCANIUM_SENTINEL:
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        if result.get("auth_code") is not None or result.get("error") is not None:
            return
        result["error"] = _USER_SKIPPED_ORCANIUM_SENTINEL
        print(
            "  OAuth skipped. Run `orcanium mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Orcanium Agent")
    scope = cfg.get("scope")
