...
from orcanium.app.tools.computer_use.backend import (
    ActionResult,
    CaptureResult,
    ComputerUseBackend,
    UIElement,
...
# regardless of approval level (e.g. logout kills the session Orcanium runs in).
_BLOCKED_KEY_COMBOS = {
...
def _get_backend() -> ComputerUseBackend:
    ...
    with _backend_lock:
        if _backend is None:
            backend_name = os.environ.get("ORCANIUM_COMPUTER_USE_BACKEND", "cua").lower()
            if backend_name in {"cua", "cua-driver", ""}:
                from orcanium.app.tools.computer_use.cua_backend import CuaDriverBackend
                _backend = CuaDriverBackend()
            elif backend_name == "noop":  # pragma: no cover
                _backend = _NoopBackend()
            else:
                raise RuntimeError(f"Unknown ORCANIUM_COMPUTER_USE_BACKEND={backend_name!r}")
            _backend.start()
        return _backend
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "Run `orcanium tools` and enable Computer Use to install cua-driver.",
        })
...
def _should_route_through_aux_vision() -> bool:
    ...
    try:
        ...
        from orcanium.app.tools.computer_use.vision_routing import (
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$ORCANIUM_HOME/cache/vision/``
    ...
    try:
        ...
        cache_dir = get_orcanium_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    if sys.platform != "darwin":
        return False
    from orcanium.app.tools.computer_use.cua_backend import cua_driver_binary_available
    return cua_driver_binary_available()


def get_computer_use_schema() -> Dict[str, Any]:
    from orcanium.app.tools.computer_use.schema import COMPUTER_USE_SCHEMA
