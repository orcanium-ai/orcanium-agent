...
from orcanium.cli.compat import env_var_enabled, is_truthy_value
...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ORCANIUM_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from orcanium.cli.compat import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    if session_key:
        return session_key
    from channel.session_context import get_session_env
    return get_session_env("ORCANIUM_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ORCANIUM_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ORCANIUM_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ORCANIUM_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ORCANIUM_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ORCANIUM_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ORCANIUM_CRON_SESSION"):
        return False
    if env_var_enabled("ORCANIUM_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ORCANIUM_HOME, or by the resolved absolute
# active profile home path such as /home/orcanium/.Orcanium/config.yaml. The
# resolved-absolute form is folded into the ~/.orcanium/ patterns at detection
...
# go stale when ORCANIUM_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ORCANIUM_ENV_PATH = (
    r'(?:~\/\.Orcanium/|'
    r'(?:\$home|\$\{home\})/\.Orcanium/|'
    r'(?:\$orcanium_home|\$\{orcanium_home\})/)'
    r'\.env\b'
)
# ~/.orcanium/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ORCANIUM_ENV_PATH; matches the ORCANIUM_HOME override form as
# well as ~/.orcanium/.
_ORCANIUM_CONFIG_PATH = (
    r'(?:~\/\.Orcanium/|'
    r'(?:\$home|\$\{home\})/\.Orcanium/|'
    r'(?:\$orcanium_home|\$\{orcanium_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_ORCANIUM_ENV_PATH}|'
    rf'{_ORCANIUM_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\borcanium\s+gateway\s+(stop|restart)\b', "stop/restart orcanium gateway (kills running agents)"),
    (r'\borcanium\s+update\b', "orcanium update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `orcanium gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart orcanium-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart orcanium-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(Orcanium|gateway|cli\.py)\b', "kill Orcanium/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill Orcanium` but not
    # `kill -9 $(pgrep -f Orcanium)` because the substitution is opaque
    ...
    # In-place edit of a Orcanium-managed security file (~/.orcanium/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ORCANIUM_CONFIG_PATH}|{_ORCANIUM_ENV_PATH})', "in-place edit of Orcanium config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ORCANIUM_CONFIG_PATH}|{_ORCANIUM_ENV_PATH})', "in-place edit of Orcanium config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ORCANIUM_CONFIG_PATH}|{_ORCANIUM_ENV_PATH})', "in-place edit of Orcanium config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.orcanium/ form so the Orcanium config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/orcanium/.Orcanium/config.yaml`) rather than
    # ~, $HOME, or $ORCANIUM_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live ORCANIUM_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_orcanium_home(command)
    return command


def _rewrite_resolved_orcanium_home(command: str) -> str:
    """Rewrite the resolved absolute Orcanium home prefix to ``~/.orcanium/``.

    Resolves the active ``ORCANIUM_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.orcanium/`` so the static ``_ORCANIUM_CONFIG_PATH`` / ``_ORCANIUM_ENV_PATH``
    ...
    try:
        ...
        home = get_orcanium_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate ORCANIUM_HOME (e.g. "/" or "") rewriting
        ...
        # /home/orcanium/.Orcanium or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.orcanium/")
    return command
...
# =========================================================================
# Blocking gateway approval (mirrors CLI's synchroorcanium input() flow)
# =========================================================================
...
def save_permanent_allowlist(patterns: set):
    ...
    try:
        from orcanium.cli.compat import load_config, save_config
        config = load_config()
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["ORCANIUM_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "ORCANIUM_SPINNER_PAUSE" in os.environ:
            del os.environ["ORCANIUM_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ORCANIUM_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ORCANIUM_INTERACTIVE or ORCANIUM_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ORCANIUM_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ORCANIUM_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ORCANIUM_EXEC_ASK")
    ...
    try:
        from orcanium.app.tools.tirith_security import check_command_security
    ...
    # responds with /approve or /deny, mirroring the CLI's synchroorcanium
