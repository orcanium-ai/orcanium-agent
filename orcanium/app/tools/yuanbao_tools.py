# Migrated from orcanium/tools/yuanbao_tools.py (legacy tree) — imports rewritten.
# This file is now part of the canonical orcanium.app.tools package.

"""
yuanbao_tools.py - 元宝平台工具集

提供以下工具函数，供 orcanium-agent 的 "orcanium-yuanbao" toolset 使用：
...
The active adapter singleton lives in ``channel.platforms.yuanbao`` and is
...
def _get_active_adapter():
    """Lazy import to avoid ImportError when channel.platforms.yuanbao is unavailable."""
    try:
        from channel.platforms.yuanbao import get_active_adapter
...
async def search_sticker(query: str = "", limit: int = 10) -> dict:
    ...
    """
    from channel.platforms.yuanbao_sticker import search_stickers
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（ORCANIUM_SESSION_CHAT_ID）。
    ...
    from channel.platforms.yuanbao_sticker import (
        get_sticker_by_id,
        get_sticker_by_name,
        get_random_sticker,
    )

    target = (chat_id or "").strip() or get_session_env("ORCANIUM_SESSION_CHAT_ID", "")
    if not target:
        return {
            "success": False,
...
from orcanium.app.tools.registry import registry, tool_result  # noqa: E402
...
def _check_yuanbao():
    ...
    try:
        from channel.session_context import get_session_env
        if get_session_env("ORCANIUM_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
...
async def _handle_yb_send_dm(args, **kw):
    ...
    if not group_code:
        try:
            from channel.session_context import get_session_env
            chat_id = get_session_env("ORCANIUM_SESSION_CHAT_ID", "")
