...
Lets the LLM write a Python script that calls Orcanium tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `orcanium_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `orcanium_tools.py` with file-based RPC stubs
...
from orcanium.app.tools.thread_context import propagate_context_to_thread

# Availability gate.  On Windows we fall back to loopback TCP for the
# sandbox RPC transport (AF_UNIX is unreliable on Windows Python) — see
# ``_use_tcp_rpc`` in ``_execute_local`` below.  That makes execute_code
# available on every platform Orcanium itself runs on.
...
# match a safe prefix, the operational ORCANIUM_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "ORCANIUM_" prefix was deliberately removed (#27303) — it leaked
# ORCANIUM_*-named config that lacks a secret substring (e.g. ORCANIUM_BASE_URL,
# ORCANIUM_KANBAN_DB, ORCANIUM_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ORCANIUM_CHILD_ALLOWED below; ORCANIUM_RPC_SOCKET /
# ORCANIUM_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ORCANIUM_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set orcanium.cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_ORCANIUM_CHILD_ALLOWED = frozenset({
    "ORCANIUM_HOME",
    "ORCANIUM_PROFILE",
    "ORCANIUM_CONFIG",
    "ORCANIUM_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ORCANIUM_* vars (_ORCANIUM_CHILD_ALLOWED) pass by exact name.
    ...
    if is_passthrough is None:
        try:
            from orcanium.app.tools.env_passthrough import is_env_passthrough as _ep
        except Exception:
    ...
    scrubbed = {}
    # Non-secret ORCANIUM_* vars dropped by the tightened allowlist (#27303). The
    # broad "ORCANIUM_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ORCANIUM_KANBAN_DB / ORCANIUM_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _ORCANIUM_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("ORCANIUM_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped ORCANIUM_* var.
            _dropped_orcanium.append(k)
    if _dropped_orcanium:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ORCANIUM_* var(s) from the "
            ...
            len(_dropped_orcanium),
            ", ".join(sorted(_dropped_orcanium)),
        )
    return scrubbed
...
# ---------------------------------------------------------------------------
# orcanium_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Orcanium-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Orcanium-profile soft guard."""',
...
def generate_orcanium_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the orcanium_tools.py stub module.
...
"""Auto-generated Orcanium tools RPC stubs."""
...
def _connect():
    ...
    ORCANIUM_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ORCANIUM_RPC_SOCKET"]
...
"""Auto-generated Orcanium tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ORCANIUM_RPC_DIR") or os.path.join(tempfile.gettempdir(), "orcanium_rpc")
_seq = 0
...
def _get_or_create_env(task_id: str):
    ...
    """
    from orcanium.app.tools.terminal_tool import (
...
def _execute_remote(
    ...
    The script and the generated orcanium_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/orcanium_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_orcanium_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/orcanium_tools.py", tools_src)
        ...
        env_prefix = (
            f"ORCANIUM_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ORCANIUM_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    from orcanium.app.tools.terminal_tool import _get_env_config
    ...
    # here before either dispatch path spawns it. Runs synchroorcaniumly in the
    # caller (tool-executor) thread, which holds the session context (#30882).
    from orcanium.app.tools.approval import check_execute_code_guard
    ...
    from orcanium.app.tools.interrupt import is_interrupted as _is_interrupted
    ...
    # --- Set up temp directory with orcanium_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="orcanium_sandbox_")
    ...
    # selector from ORCANIUM_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"orcanium_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated orcanium_tools module.
        ...
        tools_src = generate_orcanium_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "orcanium_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ORCANIUM_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ORCANIUM_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the orcanium root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from orcanium_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _orcanium_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _orcanium_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ORCANIUM_TIMEZONE is an internal Orcanium setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ORCANIUM_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ORCANIUM_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {ORCANIUM_HOME}/home/ when that directory exists.
        from orcanium.channel.compat import get_subprocess_home
        ...
        while proc.poll() is None:
            ...
            try:
                from orcanium.app.tools.environments.base import touch_activity_if_due
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.orcanium/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (orcanium's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``orcanium tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and orcanium's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.orcanium/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Orcanium tools programmatically. "
        ...
        f"Available via `from orcanium_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into orcanium_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from orcanium_tools import {import_str}` "
...
# --- Registry ---
from orcanium.app.tools.registry import registry, tool_error
