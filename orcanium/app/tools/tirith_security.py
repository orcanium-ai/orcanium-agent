...
it is automatically downloaded from GitHub releases to $ORCANIUM_HOME/bin/tirith.
...
def _get_orcanium_home() -> str:
    """Return the Orcanium home directory, respecting ORCANIUM_HOME env var."""
    return str(get_orcanium_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_orcanium_home(), ".tirith-install-failed")
...
def _orcanium_bin_dir() -> str:
    """Return $ORCANIUM_HOME/bin, creating it if needed."""
    d = os.path.join(_get_orcanium_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ORCANIUM_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_orcanium_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ORCANIUM_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ORCANIUM_HOME/bin/tirith
    ...
    orcanium_bin = os.path.join(_orcanium_bin_dir(), "tirith")
    if os.path.isfile(orcanium_bin) and os.access(orcanium_bin, os.X_OK):
        _resolved_path = orcanium_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return orcanium_bin
...
def ensure_installed(*, log_failures: bool = True):
    ...
    Quick PATH/local checks are synchroorcanium; network download runs in a
    ...
    # Explicit path: synchroorcanium check only, no download
    if explicit:
        if os.path.isfile(expanded) and os.access(expanded, os.X_OK):
            _resolved_path = expanded
            return expanded
