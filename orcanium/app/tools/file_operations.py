...
Usage:
    from orcanium.app.tools.file_operations import ShellFileOperations
    from orcanium.app.tools.terminal_tool import _active_environments
...
from orcanium.app.tools.binary_extensions import BINARY_EXTENSIONS
...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__ORCANIUM_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__ORCANIUM_FENCE_" in line or "\x1b]" in line
...
def normalize_read_pagination(offset: Any = DEFAULT_READ_OFFSET,
    ...
    """
    from orcanium.app.tools.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        """
        from orcanium.app.tools.tool_output_limits import get_max_line_length
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.orcanium-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".orcanium-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.orcanium-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.orcanium-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.orcanium-tmp``
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from orcanium.app.tools.patch_parser import parse_v4a_patch, apply_v4a_operations
