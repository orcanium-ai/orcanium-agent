...
from orcanium.orcanium_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ORCANIUM_KANBAN_TASK`` / ``ORCANIUM_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ORCANIUM_KANBAN_TASK") or os.getenv("ORCANIUM_KANBAN_BOARD"):
            result = True
        else:
            try:
                from orcanium.app.tools.kanban_tools import _profile_has_kanban_toolset
        ...
        try:
            from orcanium_constants import is_container

            result = is_container()
        except Exception:
            result = False
    elif env == "s6":
        # The Orcanium Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ORCANIUM_PLATFORM`` or
            ``ORCANIUM_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("ORCANIUM_PLATFORM")
        or get_session_env("ORCANIUM_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``orcanium`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.orcanium/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``orcanium`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ORCANIUM_HOME, not cwd
        if not p.is_absolute():
            p = (orcanium_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.orcanium/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    orcanium = metadata.get("orcanium") or {}
    if not isinstance(orcanium, dict):
        orcanium = {}
    return {
        "fallback_for_toolsets": orcanium.get("fallback_for_toolsets", []),
        "requires_toolsets": orcanium.get("requires_toolsets", []),
        "fallback_for_tools": orcanium.get("fallback_for_tools", []),
        "requires_tools": orcanium.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    orcanium = metadata.get("orcanium")
    if not isinstance(orcanium, dict):
        return []
    raw = orcanium.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Orcanium metadata, VCS, virtualenv/dependency, and cache
