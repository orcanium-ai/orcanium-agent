...
~/.orcanium/skills/. Existing skills (bundled, hub-installed, or user-created)
...
from orcanium.cli.compat import atomic_replace, is_truthy_value
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `orcanium config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.orcanium/skills/ (single source of truth)
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.orcanium/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Orcanium profiles.
    ...
    try:
        root = get_default_orcanium_root()
    except Exception:
        return matches
    ...
    # Default profile (~/.orcanium/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.orcanium/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`orcanium -p "
            ...
            base += (
                ...
                f"Switch profiles (`orcanium -p <name>`) to edit there, or "
...
def _resolve_skill_target(skill_dir: Path, file_path: str) -> Tuple[Optional[Path], Optional[str]]:
    ...
    from orcanium.app.tools.path_security import validate_within_dir
...
def _patch_skill(
    ...
    from orcanium.app.tools.fuzzy_match import fuzzy_find_and_replace
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from orcanium.app.tools.fuzzy_match import format_no_match_hint
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        try:
            from orcanium.app.tools.skill_usage import bump_patch, forget, mark_agent_created
            from orcanium.app.tools.skill_provenance import is_background_review
            if action == "create":
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_orcanium_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `orcanium curator unpin <name>`. "
