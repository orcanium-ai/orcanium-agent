...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/orcanium-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/orcanium-results/{tool_use_id}.txt on Termux)
...
from orcanium.app.tools.budget_config import (
...
STORAGE_DIR = "/tmp/orcanium-results"
HEREDOC_MARKER = "ORCANIUM_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/orcanium-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ORCANIUM_PERSIST_{uuid.uuid4().hex[:8]}"
