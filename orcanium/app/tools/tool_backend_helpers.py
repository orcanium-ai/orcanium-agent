...
def managed_orcanium_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Orcanium Tool Gateway.

    Entitlement is paid Orcanium Portal service access OR a live free tool pool
    ...
    try:
        ...
        if force_fresh:
            ...
            account_info = get_managed_gateway_identity()
...
def orcanium_tool_gateway_unavailable_message(
    capability: str = "the Orcanium Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable Orcanium Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Configure ORCANIUM_API_KEY or a local "
        "BYOK provider."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_orcanium_tools_enabled()
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.orcanium/.env`` (via
    ``orcanium.cli.config.get_env_value`` when available) so tool-side
