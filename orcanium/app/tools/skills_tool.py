...
Usage:
    from orcanium.app.tools.skills_tool import skills_list, skill_view, check_skills_requirements
...
# All skills live in ~/.orcanium/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
ORCANIUM_HOME = get_orcanium_home()
SKILLS_DIR = ORCANIUM_HOME / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ORCANIUM_HOME/.env."""
    env_path = get_orcanium_home() / ".env"
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set ORCANIUM_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ORCANIUM_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("ORCANIUM_INTERACTIVE"):
        return {
            "missing_names": missing_names,
            "setup_skipped": False,
            "channel_setup_hint": _channel_setup_hint(),
        }
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ORCANIUM_SESSION_PLATFORM"))
...
def _channel_setup_hint() -> str:
    try:
        from channel.platforms.base import GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE

        return GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE
    except Exception:
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_orcanium_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.orcanium/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    """Resolve the current platform from channel session context.

    Mirrors the platform-resolution logic in
    ``agent.skill_utils.get_disabled_skill_names`` so that
    ``_is_skill_disabled`` respects ``ORCANIUM_SESSION_PLATFORM``.
    """
    try:
        from channel.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ORCANIUM_PLATFORM`` environment variable
    3. ``ORCANIUM_SESSION_PLATFORM`` from channel session context
    """
    try:
        from orcanium.cli.compat import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ORCANIUM_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.orcanium/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``orcanium skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_orcanium_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from orcanium.cli.compat import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: Orcanium plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if ":" in name:
            try:
                ...
                pass
            from orcanium.cli.compat import discover_plugins, get_plugin_manager
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.orcanium/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `orcanium skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        if file_path and skill_dir:
            from orcanium.app.tools.path_security import validate_within_dir, has_traversal_component
        ...
        # Check metadata.Orcanium.* first (agentskills.io convention), fall back to top-level
        orcanium_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            orcanium_meta = metadata.get("orcanium", {}) or {}

        tags = _parse_tags(orcanium_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            orcanium_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
        ...
        if available_env_names:
            try:
                from orcanium.app.tools.env_passthrough import register_env_passthrough
        ...
        if required_cred_files_raw:
            try:
                from orcanium.app.tools.credential_files import register_credential_files
        ...
        if capture_result["channel_setup_hint"]:
            result["channel_setup_hint"] = capture_result["channel_setup_hint"]
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from orcanium.app.tools.skill_usage import bump_use, bump_view
