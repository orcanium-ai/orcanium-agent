...
transport, discovers their tools, and registers them into the orcanium
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.orcanium/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.orcanium/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            orcanium_home_val = os.path.expanduser(
                os.getenv(
                    "ORCANIUM_HOME", os.path.join(os.path.expanduser("~"), ".orcanium")
                )
            )
            candidates = [
                os.path.join(orcanium_home_val, "node", "bin", resolved_command),
                ...
                # image (which the Orcanium Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Orcanium MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Orcanium gateways know how to render.
    ...
    try:
        ...
        # channel.platforms.base not importable in this process (e.g. cron
        # without gateway deps). Fall back to silently dropping — callers
        # get any text blocks that did parse.
        logger.debug("MCP image caching skipped — channel.platforms.base unavailable")
        return ""
...
class MCPServerTask:
                ...
                if _MCP_NOTIFICATION_TYPES and isinstance(message, ServerNotification):
                    match message.root:
                        case ToolListChangedNotification():
                            ...
                            # request. Refreshing synchroorcaniumly inside the SDK
    ...
    async def _refresh_tools(self):
        ...
        After the initial ``await`` (list_tools), all mutations are synchroorcanium
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'orcanium[mcp]'\n"
                "or (full install):\n"
                "  pip install 'orcanium[all]'"
            )
        ...
        from orcanium.app.tools.osv_check import check_package_for_malware
    ...
    async def _run_http(self, config: dict):
        ...
        if self._auth_type == "oauth":
            try:
                from orcanium.app.tools.mcp_oauth_manager import get_manager
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Orcanium is fully
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from orcanium.app.tools.mcp_oauth import OAuthNonInteractiveError
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `orcanium mcp login {server_name}` (or delete the tokens "
            f"file under ~/.orcanium/mcp-tokens/ and restart). Do NOT retry "
...
# can be cleaned up asynchroorcaniumly by _kill_orphaned_mcp_children().
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ORCANIUM_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_orcanium_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_orcanium_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ORCANIUM_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_orcanium_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Orcanium config file.
    ...
    ``os.environ`` (which includes ``~/.orcanium/.env`` loaded at startup).
    """
    try:
        from orcanium.cli.compat import load_config
        # Safe mode (--safe-mode / ORCANIUM_SAFE_MODE=1): troubleshooting run
        # with all customizations disabled — no MCP servers connect.
        from utils import env_var_enabled as _env_enabled
        if _env_enabled("ORCANIUM_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Orcanium's MEDIA: tag convention and out to messaging
            ...
            # Orcanium's MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def _make_read_resource_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        from orcanium.app.tools.registry import tool_error
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    * MCP/Pydantic optional fields commonly arrive as
    ...
    def _strip_nullable_union(node):
        ...
        """
        from orcanium.app.tools.schema_sanitizer import strip_nullable_unions
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Orcanium'ss historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Orcanium registry schema format.
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    from orcanium.app.tools.interrupt import is_interrupted as _is_interrupted, set_interrupt as _set_interrupt
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``orcanium tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Orcanium registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple Orcanium
