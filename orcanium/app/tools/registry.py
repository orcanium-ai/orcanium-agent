"""Central registry for all orcanium tools.

# Load compat stubs BEFORE any tool module imports registry
...
Import chain (circular-import safe):
    tools/registry.py  (no imports from model_tools or tool files)
           ^
    tools/*.py  (import from orcanium.app.tools.registry at module level)
           ^
...
# timescales. Cache results for ~30 s so env-var flips via ``orcanium tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``orcanium tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``orcanium tools enable foo``)
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from orcanium.app.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
