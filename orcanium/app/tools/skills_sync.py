...
Copies bundled skills from the repo's skills/ directory into ~/.orcanium/skills/
...
The manifest lives at ~/.orcanium/skills/.bundled_manifest.
"""
...
from orcanium.channel.compat import get_bundled_skills_dir, get_orcanium_home, get_optional_skills_dir
try:
...
# Marker file written by `orcanium profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.Orcanium profile).
# When present in ORCANIUM_HOME, sync_skills() is a no-op so neither the
# installer, `orcanium update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# orcanium.cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks ORCANIUM_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.orcanium/skills/.curator_suppressed`` directly if that import
    is unavailable in a packaged/update context.
    """
    try:
        from orcanium.app.tools.skill_usage import read_suppressed_names
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.orcanium/skills/mlops/axolotl
    """
...
def _content_hash(directory: Path) -> str:
    ...
    try:
        from orcanium.app.tools.skills_guard import content_hash
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.orcanium/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.Orcanium) that wrote the
    ...
    if (ORCANIUM_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.orcanium/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `orcanium update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `orcanium skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `orcanium skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `orcanium update` runs "
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes ORCANIUM_HOME/.no-bundled-skills so the
    installer, ``orcanium update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``orcanium skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = ORCANIUM_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `orcanium update` (or `orcanium skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (ORCANIUM_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.orcanium/skills/ ...")
