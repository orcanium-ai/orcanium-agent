...
supports multiple backends — **Browser Use** (cloud, default for Orcanium
...
Usage:
    from orcanium.app.tools.browser_tool import browser_navigate, browser_snapshot, browser_click
...
from orcanium.cli.compat import env_int, is_truthy_value
from orcanium.cli.config import cfg_get

try:
    from orcanium.app.tools.website_policy import check_website_access
except Exception:
    check_website_access = lambda url: None  # noqa: E731 — fail-open if policy module unavailable

try:
    from orcanium.app.tools.url_safety import (
...
from orcanium.app.tools.tool_backend_helpers import normalize_browser_cloud_provider
# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from orcanium.app.tools.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    orcanium_node_bin = str(orcanium_home / "node" / "bin")
    orcanium_node_root = str(orcanium_home / "node")
    orcanium_nm_bin = str(orcanium_home / "node_modules" / ".bin")
    return [orcanium_node_bin, orcanium_node_root, orcanium_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_dialog_policy_config() -> Tuple[str, float]:
    ...
    from orcanium.app.tools.browser_supervisor import (
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    if not cdp_url:
        return
    try:
        from orcanium.app.tools.browser_supervisor import SUPERVISOR_REGISTRY  # type: ignore[import-not-found]
...
# plugins drop in under ``~/.orcanium/plugins/browser/<vendor>/``.
...
def _ensure_browser_plugins_loaded() -> None:
    ...
    """
    try:
        from orcanium.cli.compat import _ensure_plugins_discovered
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Orcanium gateway or
    ...
    browser plugins (``~/.orcanium/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed Orcanium gateway or
...
from orcanium.channel.compat import is_termux as _is_termux_environment
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Orcanium silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Orcanium CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-orcanium_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed Orcanium processes — this way every clean Orcanium exit sweeps
    ...
    # Sweep orphans from other crashed Orcanium processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live Orcanium processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current Orcanium PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live Orcanium process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning Orcanium process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced Orcanium PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent Orcanium instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-orcanium_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live Orcanium process.
            continue
        ...
        try:
            from orcanium.app.tools.process_registry import ProcessRegistry
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Orcanium-managed
    ...
    # Build an extended search PATH including Orcanium-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                orcanium_nm = str(get_orcanium_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=orcanium_nm)
            if not recheck:
                orcanium_node_bin = str(get_orcanium_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=orcanium_node_bin)
            if not recheck:
                orcanium_node_root = str(get_orcanium_home() / "node")
                recheck = shutil.which("agent-browser", path=orcanium_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    from orcanium.app.tools.interrupt import is_interrupted
    if is_interrupted():
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this Orcanium PID as the session owner (cross-process safe
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_type
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_console
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from orcanium.app.tools.browser_camofox import _ensure_tab, _post
    try:
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = orcanium_home / "browser_recordings"
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Orcanium
    ...
    """
    if _is_camofox_mode():
        from orcanium.app.tools.browser_camofox import camofox_vision
        return camofox_vision(question, annotate, task_id)

    import base64
    import uuid as uuid_mod
    from orcanium.channel.compat import get_orcanium_dir
    screenshots_dir = get_orcanium_dir("cache/screenshots", "browser_screenshots")
    ...
    try:
        ...
        from orcanium.app.tools.vision_tools import (
        ...
        try:
            from orcanium.cli.compat import load_config
            _cfg = load_config()
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from orcanium.app.tools.browser_camofox import camofox_close, camofox_soft_cleanup
...
def check_browser_vision_requirements() -> bool:
    ...
    if not check_browser_requirements():
        return False
    try:
        from orcanium.app.tools.vision_tools import check_vision_requirements
    except ImportError:
        return False
...
if __name__ == "__main__":
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/orcanium/orcanium:latest")
                else:
    ...
    print("\n💡 Usage:")
    print("  from orcanium.app.tools.browser_tool import browser_navigate, browser_snapshot")
