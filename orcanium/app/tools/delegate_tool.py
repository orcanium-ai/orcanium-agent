...
# Must match orcanium.cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
_RUNTIME_PROVIDER_CUSTOM = "custom"
from orcanium.app.tools import file_state
from orcanium.app.tools.terminal_tool import set_approval_callback as _set_subagent_approval_cb
...
# (orcanium-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("orcanium-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``orcanium-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Orcanium Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. orcanium-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Orcanium Portal).
    ...
    try:
        from orcanium.cli.compat import invoke_hook as _invoke_hook
        _invoke_hook(
            "subagent_start",
            parent_session_id=getattr(parent_agent, "session_id", None),
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.orcanium/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if configured_base_url:
        ...
        from orcanium.cli.compat import _detect_api_mode_for_url
    ...
    try:
        from orcanium.cli.compat import resolve_runtime_provider
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, Orcanium, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'Orcanium auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (orcanium.cli/config.py load_config()) so that
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "notify_on_complete=True) instead. delegate_task runs SYNCHROORCANIUMLY "
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Orcanium subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
