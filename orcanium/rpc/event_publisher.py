"""Best-effort WebSocket publisher transport for the PTY-side channel.

The dashboard's `/api/pty` spawns `orcanium --tui` as a child process, which
spawns its own ``rpc.entry``.  Tool/reasoning/status events fire on
