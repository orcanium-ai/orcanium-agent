...
# Orcanium modules.  orcanium.cli sets ORCANIUM_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("ORCANIUM_PYTHON_SRC_ROOT", "")
...
from orcanium.rpc import server
from orcanium.rpc.server import _CRASH_LOG, dispatch, resolve_skin, write_json
from orcanium.rpc.transport import TeeTransport
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ORCANIUM_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ORCANIUM_TUI_SIDECAR_URL")

    if not url:
        return

    from orcanium.rpc.event_publisher import WsPublisherTransport
...
# tested; override via ``ORCANIUM_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ORCANIUM_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ORCANIUM_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# with hasattr so ``python -m rpc.entry`` (spawned by
# ``orcanium --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: float = 0.75) -> None:
    ...
    slow/dead server can't freeze ``channel.ready``.  But the agent snapshots
...
def main():
    ...
    # inline before ``channel.ready``, which meant any configured-but-down
    # server stalled the whole shell on "summoning Orcanium…" for the full
    ...
    if _has_mcp_servers:
        def _discover_mcp_background() -> None:
            try:
                from orcanium.cli.compat import _agent_stub as discover_mcp_tools
                discover_mcp_tools()
            except Exception:
                logger.warning(
    ...
    if not write_json({
        "jsonrpc": "2.0",
        "method": "event",
        "params": {"type": "channel.ready", "payload": {"skin": resolve_skin()}},
    }):
