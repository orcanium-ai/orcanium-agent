"""Persistent slash-command worker — one OrcaniumCLI per TUI session.
...
import orcanium.app.cli as cli_mod
from orcanium.cli.main import main as OrcaniumCLI
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``ORCANIUM_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("ORCANIUM_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("ORCANIUM_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: OrcaniumCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ORCANIUM_SESSION_KEY"] = args.session_key
    os.environ["ORCANIUM_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) OrcaniumCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = OrcaniumCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
