...
from orcanium.channel.compat import is_truthy_value
from orcanium.rpc.transport import (
    StdioTransport,
    Transport,
    bind_transport,
    current_transport,
...
load_orcanium_dotenv(
    orcanium_home=_orcanium_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# flushes through the stderr->channel.stderr event pump. This hook
# appends every unhandled exception to ~/.Orcanium/logs/rpc_crash.log
...
_CRASH_LOG = os.path.join(_orcanium_home, "logs", "rpc_crash.log")
...
def _panic_hook(exc_type, exc_value, exc_tb):
    ...
    try:
        ...
        pass
    # Stderr goes through to the TUI as a channel.stderr Activity line —
...
from orcanium.rpc.render import make_stream_renderer, render_diff, render_message
...
try:
    _slash_timeout = float(os.environ.get("ORCANIUM_TUI_SLASH_TIMEOUT_S") or "45")
except (ValueError, TypeError):
    _slash_timeout = 45.0
_SLASH_WORKER_TIMEOUT_S = max(5.0, _slash_timeout)

# When a WebSocket client (the dashboard's embedded-chat tab / desktop app)
# disconnects, ``rpc.ws`` detaches the transport but intentionally
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ORCANIUM_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ORCANIUM_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
# so stray print() from libraries/tools becomes harmless channel.stderr instead
...
class _SlashWorker:
    """Persistent OrcaniumCLI subprocess for slash commands."""
...
def _teardown_session(session: dict | None, *, end_reason: str = "tui_close") -> None:
    ...
    try:
        from orcanium.app.tools.approval import unregister_gateway_notify
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `orcanium --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("ORCANIUM_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `orcanium --tui`, so it must NOT count as dead here (doing
...
# ``profile`` on those calls; we open that profile's db and bind its ORCANIUM_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if not name:
        return None
    try:
        from orcanium.app.cli import profiles as profiles_mod

        home = Path(profiles_mod.get_profile_dir(name))
    except Exception:
        return None
    # Already the launch profile? No override needed.
    if home.resolve() == Path(_orcanium_home).resolve():
        return None
...
def dispatch(req: dict, transport: Optional[Transport] = None) -> dict | None:
    ...
    *transport* (optional): pins every write produced by this request —
    ...
    the original behaviour for ``rpc.entry``.
    """
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `orcanium` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ORCANIUM_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_orcanium_home_override(profile_home)
                try:
                    from orcanium.channel.compat import SessionDB
            ...
            if home_token is not None:
                reset_orcanium_home_override(home_token)
...
def _register_session_cwd(session: dict | None) -> None:
    if not session:
        return
    try:
        from orcanium.app.tools.terminal_tool import register_task_env_overrides
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    ...
    try:
        from orcanium.app.tools.terminal_tool import cleanup_vm
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _orcanium_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_orcanium_home_override()
        home = override if isinstance(override, str) and override else _orcanium_home
...
def _save_cfg(cfg: dict):
    ...
    path = _orcanium_home / "config.yaml"
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["ORCANIUM_GATEWAY_SESSION"] = "1"
    os.environ["ORCANIUM_EXEC_ASK"] = "1"
    os.environ["ORCANIUM_INTERACTIVE"] = "1"
...
def _clear_pending(sid: str | None = None) -> None:
    ...
    sessions sharing the same rpc process.  When *sid* is
...
def resolve_skin() -> dict:
    try:
        from orcanium.cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("ORCANIUM_MODEL", "")
        or os.environ.get("ORCANIUM_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    from orcanium.runtime.configuration import configured_model

    return configured_model(_load_cfg(), fallback="anthropic/claude-sonnet-4")


def _config_model_target() -> tuple[str, str]:
    """(model, provider) currently selected by config (env as fallback).

    config.yaml wins over ORCANIUM_MODEL / ORCANIUM_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set ORCANIUM_INFERENCE_MODEL in the
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    from orcanium.runtime.configuration import resolve_startup_model_and_provider
    try:
        from orcanium.cli.models import detect_static_provider_for_model
    except Exception:
        detect_static_provider_for_model = None

    return resolve_startup_model_and_provider(
        _load_cfg(),
        model_override=os.environ.get("ORCANIUM_MODEL", ""),
        provider_override=os.environ.get("ORCANIUM_TUI_PROVIDER", ""),
        inference_model=os.environ.get("ORCANIUM_INFERENCE_MODEL", ""),
        inference_provider=os.environ.get("ORCANIUM_INFERENCE_PROVIDER", ""),
        fallback_model="anthropic/claude-sonnet-4",
        detect_provider=detect_static_provider_for_model,
    )
...
def _load_reasoning_config() -> dict | None:
    from orcanium.channel.compat import parse_reasoning_effort
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ORCANIUM_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("ORCANIUM_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Orcanium): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `orcanium --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ORCANIUM_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from orcanium.cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ORCANIUM_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ORCANIUM_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ORCANIUM_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ORCANIUM_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    from orcanium.cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from orcanium.cli.compat import _agent_stub as get_compatible_custom_providers, _agent_stub as load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (ORCANIUM_MODEL /
    # ORCANIUM_INFERENCE_MODEL / ORCANIUM_TUI_PROVIDER / ORCANIUM_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # ORCANIUM_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("ORCANIUM_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        from orcanium.app.cli import __version__, __release_date__
    ...
    try:
        ...
        pass
    try:
        from orcanium.cli.banner import get_available_skills

        info["skills"] = get_available_skills()
    except Exception:
        pass
    try:
        from orcanium.app.tools.mcp_tool import get_mcp_status
    ...
    try:
        ...
        pass
    try:
        from orcanium.cli.banner import get_update_result
...
# A delegated child is not a live gateway session — it runs synchroorcaniumly
...
def _wire_callbacks(sid: str):
    from orcanium.app.tools.terminal_tool import set_sudo_password_callback
    from orcanium.app.tools.skills_tool import set_secret_capture_callback
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from orcanium.cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from orcanium.cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ORCANIUM_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ORCANIUM_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``OrcaniumCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses orcanium.cli.mcp_startup; TUI stdio keeps
    # its existing rpc.entry-owned thread.
    try:
        from orcanium.cli.mcp_startup import wait_for_mcp_discovery

        wait_for_mcp_discovery()
    except Exception:
        pass
    try:
        from orcanium.rpc.entry import wait_for_mcp_discovery
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        if (
            ...
            from orcanium.cli.runtime_provider import find_custom_provider_identity
    ...
    return create_agent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ORCANIUM_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ORCANIUM_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ORCANIUM_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ORCANIUM_IGNORE_RULES")),
...
def _init_session(sid: str, key: str, agent, history: list, cols: int = 80):
    ...
    try:
        from orcanium.app.tools.approval import register_gateway_notify, load_permanent_allowlist
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind ORCANIUM_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `ORCANIUM_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_orcanium_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds ORCANIUM_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``orcanium --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``orcanium gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # Orcanium credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into Orcanium
    # or on any portal hiccup.
    try:
        # agent.account_usage is stubbed — credits not available yet
        credits = []
        if credits:
...
@method("credits.view")
def _(rid, params: dict) -> dict:
    """Structured Orcanium credit view for the TUI /credits command.

    Account-independent (a portal fetch gated on "a Orcanium account is logged in"),
    ...
    """
    try:
        # agent.account_usage is stubbed — credits view not available yet
        view = {"logged_in": False, "balance_lines": [], "topup_url": None, "identity_line": None}
        return _ok(
            rid,
            {
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Orcanium profile home
    # (~/.Orcanium/sessions/saved/) rather than the project/workspace CWD, and
    # include the system prompt so the export matches the dashboard save.
    saved_dir = get_orcanium_home() / "sessions" / "saved"
    try:
    ...
    path = saved_dir / f"orcanium_conversation_{timestamp}.json"
...
def _(rid, params: dict) -> dict:
    ...
    # prompts on unrelated sessions sharing the same rpc
    # process, silently resolving them to empty strings.
    _clear_pending(params.get("session_id", ""))
    try:
        from orcanium.app.tools.approval import resolve_gateway_approval
...
def _(rid, params: dict) -> dict:
    from orcanium.app.tools.delegate_tool import (
        is_spawn_paused,
...
def _(rid, params: dict) -> dict:
    from orcanium.app.tools.delegate_tool import set_spawn_paused
...
def _(rid, params: dict) -> dict:
    from orcanium.app.tools.delegate_tool import interrupt_subagent
...
#
# Layout:  $ORCANIUM_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_orcanium_home() / "spawn-trees"
        ...
        if truncate_user_ordinal is not None:
            ...
            if (db := _get_db()) is not None:
                try:
                    ...
                    print(f"[rpc] prompt.submit: replace_messages failed: {exc}", file=sys.stderr)
...
def _notification_poller_loop(
    ...
    """
    from orcanium.app.tools.process_registry import process_registry, format_process_notification
    ...
    while not stop_event.is_set() and not session.get("_finalized"):
        ...
        try:
            ...
            print(
                f"[rpc] notification poller dispatch failed: "
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ORCANIUM_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_orcanium_home_override(_profile_home_str)
            # The sudo password callback is thread-local (tools.terminal_tool
            # _callback_tls), so wiring it on the build thread doesn't reach this
            # turn thread — terminal sudo prompts would fall through to /dev/tty
            # and hang the headless channel. Re-wire here so the prompt routes to
            ...
            if images:
                try:
                    ...
                    from orcanium.cli.config import load_config as _tui_load_config
                    ...
                    print(
                        f"[rpc] image_routing decision failed, defaulting to text: {_img_exc}",
                        file=sys.stderr,
                    )
                    _mode = "text"
                ...
                if _mode == "native":
                    try:
                        ...
                        if _skipped:
                            print(
                                f"[rpc] native image attachment skipped {len(_skipped)} unreadable path(s)",
                                file=sys.stderr,
                            )
                        ...
                        print(
                            f"[rpc] native attach failed, falling back to text: {_img_exc}",
                            file=sys.stderr,
                        )
                        run_message = _enrich_with_attached_images(prompt, images)
                else:
                ...
                if isinstance(result.get("messages"), list):
                    with session["history_lock"]:
                        ...
                        if current_version == history_version:
                            ...
                            # not persisted.
                            print(
                                f"[rpc] prompt.submit: history_version mismatch "
            ...
            if status == "complete" and isinstance(raw, str) and raw.strip():
                try:
                    ...
                    print(
                        f"[rpc] goal continuation hook failed: "
            ...
            if (
                ...
                try:
                    from orcanium.cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: orcanium.cli.voice unavailable")
                except Exception as e:
        ...
        if goal_followup:
            ...
            try:
                ...
                print(
                    f"[rpc] goal continuation dispatch failed: "
        ...
        try:
            ...
            for _evt, synth in process_registry.drain_notifications():
                ...
                try:
                    ...
                    print(
                        f"[rpc] completion notification dispatch failed: "
            ...
            print(
                f"[rpc] completion queue drain failed: "
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from orcanium.cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _orcanium_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".Orcanium" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.Orcanium/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.Orcanium/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    Params:
      ...
      data_url (str): ``data:<mime>;base64,<b64>`` upload of the file bytes,
        required when the path isn't visible to the channel.
...
def _(rid, params: dict) -> dict:
    ...
    def run():
        ...
        try:
            ...
            result = run_agent_turn(create_agent(
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Orcanium Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Orcanium/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Orcanium Desktop. If it serves Orcanium/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    def run():
        ...
        try:
            ...
            try:
                from orcanium.app.tools.terminal_tool import clear_task_env_overrides
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if params.get("request_id"):
            from orcanium.app.core.db import CrossTalkRequest, SessionLocal
            from orcanium.app.domains.capability.cross_talk import resolve_cross_talk, queue_cross_talk
            from orcanium.app.domains.execution import LaneManager
            db = SessionLocal()
            try:
                request = db.get(CrossTalkRequest, params["request_id"])
                if request is None or request.target_session_id != params.get("session_id"):
                    return _err(rid, 4003, "Unauthorized or unknown cross-talk request")
                allowed = params.get("choice", "deny") not in ("deny", "no", "false")
                resolve_cross_talk(db, request.id, allowed)
                if allowed:
                    queue_cross_talk(db, request.id)
                    LaneManager.get_instance().enqueue("cross_talk", request.target_agent_id,
                                                       lambda: _execute_cross_talk(request.id))
                return _ok(rid, {"resolved": 1, "request_id": request.id})
            finally:
                db.close()
        from orcanium.app.tools.approval import resolve_gateway_approval
...
def _execute_cross_talk(request_id: str) -> None:
    from orcanium.app.core.db import SessionLocal
    from orcanium.app.domains.capability.cross_talk import execute_cross_talk
    db = SessionLocal()
    try:
        execute_cross_talk(db, request_id)
    finally:
        db.close()
...
def _(rid, params: dict) -> dict:
    ...
    if key == "yolo":
        ...
        try:
            ...
            if scope == "global":
                from orcanium.app.tools.approval import _normalize_approval_mode
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ORCANIUM_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["ORCANIUM_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("ORCANIUM_YOLO_MODE", None)
                    nv = "0"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from orcanium.cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_orcanium_home), "display": display_orcanium_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from orcanium.cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from orcanium.cli.runtime_provider import resolve_runtime_provider
        from orcanium.cli.auth import has_usable_secret
        from orcanium.cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Orcanium provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from orcanium.app.tools.mcp_tool import shutdown_mcp_servers, discover_mcp_tools
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.Orcanium/.env`` into the gateway process via
    ``orcanium.cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from orcanium.cli.config import reload_env
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `orcanium` is interactive — use `/orcanium chat -q …` or run `orcanium` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`orcanium setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`orcanium gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`orcanium sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`orcanium config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m orcanium.cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "orcanium.cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    try:
        from orcanium.cli.commands import resolve_command
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from orcanium.cli.plugins import (
...
def _(rid, params: dict) -> dict:
    ...
    paste_dir = _orcanium_home / "pastes"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from orcanium.cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from orcanium.cli.config import is_managed, save_env_value
        from orcanium.cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `orcanium model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.Orcanium/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from orcanium.cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from orcanium.cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ORCANIUM_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ORCANIUM_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action == "status":
        ...
        try:
            from orcanium.app.tools.voice_mode import check_voice_requirements
    ...
    if action in {"on", "off"}:
        ...
        os.environ["ORCANIUM_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from orcanium.cli.voice import stop_continuous
            ...
            os.environ["ORCANIUM_VOICE_TTS"] = "0"
    ...
    if action == "tts":
        ...
        os.environ["ORCANIUM_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from orcanium.cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from orcanium.cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from orcanium.cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from orcanium.cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        from orcanium.cli.plugins import get_plugin_manager
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("ORCANIUM_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ORCANIUM_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_orcanium_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from orcanium.app.tools.cronjob_tools import cronjob
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "search":
            from orcanium.app.tools.skills_hub import (
                GitHubAuth,
        ...
        if action == "install":
            from orcanium.cli.skills_hub import do_install
        ...
        if action == "browse":
            from orcanium.cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from orcanium.cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``orcanium plugins`` / the dashboard, so the three surfaces
...
def _(rid, params: dict) -> dict:
    ...
    if not cmd:
        return _err(rid, 4004, "empty command")
    try:
        from orcanium.app.tools.approval import detect_dangerous_command, detect_hardline_command
