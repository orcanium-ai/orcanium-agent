"""Transport abstraction for the rpc JSON-RPC server.
...
can be driven over stdio (``rpc.entry``) or WebSocket
(``rpc.ws``) without duplicating code.
...
----------------------
``rpc.server.write_json`` still works without any transport bound.
...
# will hang waiting for ``channel.ready``.  Default stays off so the
# existing flush-after-write behaviour is unchanged.
_DISABLE_FLUSH = (os.environ.get("ORCANIUM_TUI_GATEWAY_NO_FLUSH", "") or "").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
