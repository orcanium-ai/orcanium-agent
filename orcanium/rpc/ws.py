"""WebSocket transport for the rpc JSON-RPC server.

Reuses :func:`rpc.server.dispatch` verbatim so every RPC method, every
...
emits a ``channel.ready`` event immediately after connection accept, then
echoes responses/events for inbound requests. No framing differences.

Mounting
--------
    from fastapi import WebSocket
    from orcanium.rpc.ws import handle_ws
...
async def handle_ws(ws: Any) -> None:
    """Run one WebSocket session. Wire-compatible with ``rpc.entry``."""
    ...
    try:
        ...
        if transport is not None:
            ...
            # (terminate + waits) plus a synchroorcanium DB write — inline that
