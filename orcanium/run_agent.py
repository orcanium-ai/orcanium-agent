...
# on Windows.  No-op on POSIX.  See orcanium_bootstrap.py for full rationale.
try:
    import orcanium_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when orcanium_bootstrap isn't registered in the venv
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``orcanium -c`` / ``--resume`` (relaunch
...
from orcanium.cli.timeouts import (
...
_loaded_env_paths = load_orcanium_dotenv(orcanium_home=_orcanium_home, project_env=_project_env)
...
from orcanium.app.tools.interrupt import set_interrupt as _set_interrupt
from orcanium.app.tools.browser_tool import cleanup_browser
...
from orcanium.cli.compat import atomic_json_write, base_url_host_matches, base_url_hostname, is_truthy_value, model_forces_max_completion_tokens
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"OrcaniumAgent/{_ORCANIUM_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[orcanium-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        if self._session_db_created or not self._session_db:
            return
        source = self.platform or os.environ.get("ORCANIUM_SESSION_SOURCE", "cli")
        try:
        ...
        **extra_context,
        ...
        if should_start and target_session_id and hasattr(engine, "on_session_start"):
            start_context = {
                ...
                "platform": getattr(self, "platform", None) or os.environ.get("ORCANIUM_SESSION_SOURCE", "cli"),
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Orcanium' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from orcanium.cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``orcanium chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ORCANIUM_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ORCANIUM_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("ORCANIUM_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ORCANIUM_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ORCANIUM_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        orcanium-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See orcanium-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # Orcanium serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "orcanium":
            return False
        if normalized_provider == "copilot":
            try:
                from orcanium.cli.models import _should_use_copilot_responses_api
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Orcanium against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("ORCANIUM_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("orcanium.cli.plugins.has_hook", ...)`` still
        ...
        try:
            from orcanium.cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``ORCANIUM_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.orcanium/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ORCANIUM_REDACT_SECRETS via
    ...
    def steer(self, text: str) -> bool:
        ...
        Thread-safe: callable from channel/CLI/TUI threads. Multiple calls
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.orcanium/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ORCANIUM_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("ORCANIUM_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (ORCANIUM_KANBAN_TASK set),
        ...
        if os.environ.get("ORCANIUM_KANBAN_TASK"):
            try:
                from orcanium.app.tools.kanban_tools import heartbeat_current_worker_from_env
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-orcanium-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        """
        # Dev test fixture (ORCANIUM_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset ORCANIUM_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ORCANIUM_DEV_CREDITS"))
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-orcanium-credits-* headers "
                    "(miss — producer off / non-Orcanium path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # ORCANIUM_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `orcanium logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _cap_delegate_task_calls(tool_calls: list) -> list:
        ...
        """
        from orcanium.app.tools.delegate_tool import _get_max_concurrent_children
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``orcanium auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                ...
                from orcanium.cli.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_orcanium_client_credentials(
        self,
        *,
        force: bool = True,
    ) -> bool:
        if self.api_mode != "chat_completions" or self.provider != "orcanium":
            return False
        ...
        try:
            ...
            creds = resolve_orcanium_runtime_credentials(
                timeout_seconds=float(os.getenv("ORCANIUM_ORCANIUM_TIMEOUT_SECONDS", "15")),
                force_refresh=force,
            )
        except Exception as exc:
            logger.debug("Orcanium credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # Orcanium requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="orcanium_credential_refresh"):
            return False
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from orcanium.cli.copilot_auth import resolve_copilot_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from orcanium.cli.models import copilot_default_headers
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Orcanium Portal.
        """
        if base_url_host_matches(self._base_url_lower, "orcanium.com"):
            return True
        if (
            ...
            try:
                from orcanium.cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from orcanium.cli.models import lmstudio_model_reasoning_options
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        """
        from orcanium.app.tools.delegate_tool import delegate_task as _delegate_task
