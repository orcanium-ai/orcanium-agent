...
_ORCANIUM_CORE_TOOLS = [
    # Web
    "web_search", "web_extract",
    # Terminal + process management
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on ORCANIUM_DESKTOP
    ...
    "send_message",
    # Outbound email/SMS (gated on credentials via check_fn)
    "send_email", "send_sms",
    # Home Assistant smart home control (gated on HASS_TOKEN via check_fn)
    "ha_list_entities", "ha_get_state", "ha_list_services", "ha_call_service",
    # Kanban multi-agent coordination — only in schema when the agent is
    # spawned as a kanban worker (ORCANIUM_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `orcanium tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``orcanium tools`` → Video Generation."
        ),
    ...
    "messaging": {
        ...
        "tools": ["send_message", "send_email", "send_sms"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ORCANIUM_KANBAN_TASK env "
    ...
    # Coding posture (base Orcanium — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in orcanium.cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Orcanium toolsets (CLI + messaging platforms)
    #
    ...
    "orcanium-cli": {
        ...
        "tools": _ORCANIUM_CORE_TOOLS,
        "includes": []
    },

    "orcanium-cron": {
        # Mirrors orcanium-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `orcanium tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as orcanium-cli; gated by `orcanium tools`",
        "tools": _ORCANIUM_CORE_TOOLS,
        "includes": []
    },
    ...
    "orcanium-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _ORCANIUM_CORE_TOOLS,
        "includes": []
    },

    "orcanium-homeassistant": {
    ...
    "orcanium-email": {
        "description": "Email bot toolset - interact with Orcanium via email (IMAP/SMTP)",
        "tools": _ORCANIUM_CORE_TOOLS,
        "includes": []
    },
    ...
    "orcanium-sms": {
        "description": "SMS bot toolset - interact with Orcanium via SMS (Twilio)",
        "tools": _ORCANIUM_CORE_TOOLS,
        "includes": []
    },
    ...
    "orcanium-webhook": {
        ...
        "tools": _ORCANIUM_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "orcanium-gateway": {
        ...
        "tools": [],
        "includes": ["orcanium-telegram", "orcanium-discord", "orcanium-whatsapp", "orcanium-slack", "orcanium-signal", "orcanium-bluebubbles", "orcanium-homeassistant", "orcanium-email", "orcanium-sms", "orcanium-mattermost", "orcanium-matrix", "orcanium-dingtalk", "orcanium-feishu", "orcanium-wecom", "orcanium-wecom-callback", "orcanium-weixin", "orcanium-qqbot", "orcanium-webhook", "orcanium-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (orcanium-<name>).
        # Gives them _ORCANIUM_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("orcanium-"):
            platform_name = name[len("orcanium-"):]
            try:
                from channel.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ORCANIUM_CORE_TOOLS)
                    try:
                        from orcanium.app.tools.registry import registry
                        plugin_tools.update(
