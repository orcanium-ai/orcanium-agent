#!/usr/bin/env python3
"""
SWE Runner with Orcanium Trajectory Format

A runner that uses Orcanium Agent's built-in execution environments
(local, docker, modal) and outputs trajectories in the Orcanium Agent format
compatible with batch_runner.py and trajectory_compressor.py.

Features:
- Uses Orcanium Agent's Docker, Modal, or Local environments for command execution
- Outputs trajectories in Orcanium format (from/value pairs with <tool_call>/<tool_response> XML)
...
# ============================================================================
# Terminal Tool Definition (matches Orcanium Agent format)
# ============================================================================
...
def create_environment(
    ...
    **kwargs
    ...
    """
    Create an execution environment using Orcanium Agent's built-in backends.
    ...
    """
    if env_type == "local":
        from orcanium.app.tools.environments.local import LocalEnvironment
        return LocalEnvironment(cwd=cwd, timeout=timeout)
    
    elif env_type == "docker":
        from orcanium.app.tools.environments.docker import DockerEnvironment
        return DockerEnvironment(image=image, cwd=cwd, timeout=timeout, **kwargs)
    
    elif env_type == "modal":
        from orcanium.app.tools.environments.modal import ModalEnvironment
...
# ============================================================================
# Mini-SWE Runner with Orcanium Trajectory Format
# ============================================================================

class MiniSWERunner:
    """
    Agent runner that uses Orcanium Agent's built-in execution environments
    and outputs trajectories in Orcanium Agent format.
    """
    ...
    def _convert_to_orcanium_format(
        self,
        ...
        """
        Convert internal message format to Orcanium trajectory format.
...
Complete the user's task step by step."""
        ...
        # Convert to Orcanium trajectory format
        trajectory = self._convert_to_orcanium_format(messages, task, completed)
...
def main(
    ...
    """
    Run SWE tasks with Orcanium trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with Orcanium Trajectory Format")
    print("=" * 60)
