"""Centralized logging setup for Orcanium Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.orcanium/logs/`` (profile-aware via ``get_orcanium_home()``).

Log files produced:
    agent.log   — INFO+, all agent/tool/session activity (the main log)
    errors.log  — WARNING+, errors and warnings only (quick triage)
    channel.log — INFO+, gateway-only events (created when mode="gateway")
...
Component separation:
    channel.log only receives records from ``channel.*`` loggers —
    platform adapters, session management, slash commands, delivery.
    gui.log receives dashboard-side records from ``orcanium.cli.web_server``,
    ``orcanium.cli.pty_bridge``, ``rpc.*``, and ``uvicorn.*``.
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_orcanium_session_injector", False):
    ...
    _session_record_factory._orcanium_session_injector = True  # type: ignore[attr-defined]
...
class _ComponentFilter(logging.Filter):
    ...
    Used to route gateway-specific records to ``channel.log`` while
...
# Used by _ComponentFilter and exposed for ``orcanium logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "orcanium_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("orcanium.cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "orcanium.cli.web_server",
        "orcanium.cli.pty_bridge",
        "rpc",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Orcanium logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    orcanium_home
        Override for the Orcanium home directory.  Falls back to
        ``get_orcanium_home()`` (profile-aware).
    log_level
    ...
    mode
        ...
        When ``"gateway"``, an additional ``channel.log`` file is created
    ...
    # --- channel.log (INFO+, gateway component only) ------------------------
    if mode == "gateway":
        _add_rotating_handler(
            root,
            log_dir / "channel.log",
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_orcanium_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._orcanium_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the orcanium group. However, both ``_open()`` (initial creation)
    ...
    2.  ``RotatingFileHandler`` keeps an open file descriptor.  If anything
        ...
        write goes to ``channel.log.1`` instead of ``channel.log`` — silent
    ...
    def __init__(self, *args, **kwargs):
        from orcanium.cli.config import is_managed
