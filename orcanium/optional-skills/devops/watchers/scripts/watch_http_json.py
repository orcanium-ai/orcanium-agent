...
Usage (via cron with --no-agent):
    ...
    orcanium cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ORCANIUM_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Orcanium-Watcher/1.0"})
