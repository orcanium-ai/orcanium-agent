...
Usage (via cron with --no-agent):
    ...
    orcanium cron create orcanium-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ORCANIUM_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name orcanium-issues --repo Orcanium/orcanium-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Orcanium .env file
(``${ORCANIUM_HOME:-~/.orcanium}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Orcanium-Watcher/1.0",
    }
