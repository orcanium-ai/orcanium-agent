...
version: 0.1.0
author: Teknium (teknium1), Orcanium Agent
license: MIT
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.orcanium/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.orcanium/wikis/$REPO_NAME"
...
1. **User input** — [`cli.py:OrcaniumCLI.run_session`](<link>)
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "orcanium-agent code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.orcanium/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.orcanium/wikis/`. Only write into the repo when the user explicitly requests it.
