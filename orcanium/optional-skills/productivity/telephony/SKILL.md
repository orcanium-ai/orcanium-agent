---
name: telephony
description: Give Orcanium phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
author: Orcanium
license: MIT
...
This optional skill gives Orcanium practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${ORCANIUM_HOME:-~/.orcanium}/.env`
...
It does **not** turn Orcanium into a real-time inbound phone channel. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Orcanium memory
...
### 1) "I want Orcanium to own a real phone number"
Use **Twilio**.
...
- pairs well with Orcanium `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${ORCANIUM_HOME:-~/.orcanium}/.env`
...
```bash
SCRIPT="$(find ~/.orcanium/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
orcanium skills search telephony
orcanium skills install official/productivity/telephony
```
...
Then save credentials into Orcanium:
...
3. Buy it and save it into `${ORCANIUM_HOME:-~/.orcanium}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Orcanium calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Orcanium's existing `text_to_speech` support.

Use this when:
- you want the call to use Orcanium's configured TTS voice rather than Twilio `<Say>`
...
Recommended Orcanium TTS -> Twilio Play workflow:

1. Generate the audio with Orcanium `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Orcanium to create the message audio with `text_to_speech`
...
Important note:
- Orcanium TTS is great for prerecorded outbound messages
...
- Orcanium STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Orcanium memory.
...
- Do not store arbitrary third-party phone numbers in Orcanium memory.
...
3. persist that number to `${ORCANIUM_HOME:-~/.orcanium}/.env`
