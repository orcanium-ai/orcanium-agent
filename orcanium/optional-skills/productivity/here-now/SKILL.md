...
- Skill helper paths:
  - `${ORCANIUM_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ORCANIUM_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ORCANIUM_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client orcanium
```
...
```bash
PUBLISH="${ORCANIUM_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client orcanium
```
...
```bash
DRIVE="${ORCANIUM_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `orcanium`)    |
