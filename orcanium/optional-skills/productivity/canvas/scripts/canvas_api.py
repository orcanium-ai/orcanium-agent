#!/usr/bin/env python3
"""Canvas LMS API CLI for Orcanium Agent.
...
def _check_config():
    ...
    if missing:
        orcanium_env = os.path.join(
            os.environ.get("ORCANIUM_HOME", os.path.expanduser("~/.orcanium")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {orcanium_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Orcanium Agent"
    )
