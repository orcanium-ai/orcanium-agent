...
version: 0.1.0
author: Tony Simons (asimons81), Orcanium Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  orcanium:
    tags: [Coding-Agent, Antigravity, CLI, Auth, Plugins, Sandbox]
    related_skills: [grok, codex, claude-code, orcanium-agent]
---

# Antigravity CLI (`agy`)

Operator guide for the Antigravity CLI, invoked as `agy`. Run all `agy`
commands through the Orcanium `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from Orcanium itself.
