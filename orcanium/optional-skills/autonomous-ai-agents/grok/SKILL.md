...
version: 0.1.0
author: Matt Maximo (MattMaximo), Orcanium Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  orcanium:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, orcanium-agent]
---

# Grok Build CLI — Orcanium Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Orcanium terminal. Grok
...
1. Prepare stable input files first with Orcanium tools (`read_file`,
...
2. **Don't conflate Orcanium' xAI auth with the `grok` CLI's auth.** Orcanium'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Orcanium' xAI auth carries
   over.
