---
name: honcho
description: Configure and use Honcho memory with Orcanium -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
...
# Honcho Memory for Orcanium

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Orcanium profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/orcanium#running-honcho-locally-with-orcanium

### Verify

```bash
orcanium honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Orcanium profile's AI peer
...
Honcho models conversations as interactions between **peers**. Orcanium creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Orcanium instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Orcanium run |
| `global` | Single session across all directories |

Manual override: `orcanium honcho map my-project-name`
...
Each Orcanium profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
orcanium profile create coder --clone
# creates host block orcanium.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `orcanium.coder` host block in `honcho.json`
...
```bash
orcanium honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Orcanium when Honcho memory is active.
...
Config file: `$ORCANIUM_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `orcanium honcho setup`. Ensure `memory.provider: honcho` is in `~/.orcanium/config.yaml`.

### Memory not persisting across sessions
Check `orcanium honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `orcanium profile create <name> --clone`. For existing profiles: `orcanium honcho sync`.
...
| Command | Description |
|---------|-------------|
| `orcanium honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `orcanium honcho status` | Show resolved config, connection test, peer info for active profile |
| `orcanium honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `orcanium honcho disable` | Disable Honcho for the active profile |
| `orcanium honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `orcanium honcho peers` | Show peer identities across all profiles |
| `orcanium honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `orcanium honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `orcanium honcho sessions` | List known directory-to-session-name mappings |
| `orcanium honcho map <name>` | Map current working directory to a Honcho session name |
| `orcanium honcho identity` | Seed AI peer identity or show both peer representations |
| `orcanium honcho sync` | Create host blocks for all Orcanium profiles that don't have one yet |
| `orcanium honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Orcanium + Honcho |
| `orcanium memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `orcanium memory status` | Show active memory provider and config |
| `orcanium memory off` | Disable external memory provider |
