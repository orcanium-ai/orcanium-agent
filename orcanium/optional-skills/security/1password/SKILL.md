...
version: 1.0.0
author: arceus77-7, enhanced by Orcanium Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Orcanium terminal calls (desktop app flow only)
...
### Service Account (recommended for Orcanium)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ORCANIUM_HOME:-~/.orcanium}/.env` (the skill will prompt for this on first load).
...
## Orcanium Execution Pattern (desktop app flow)

Orcanium terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/orcanium-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/orcanium-op.sock"
