...
## 5. GODMODE FAST — Orcanium 4 405B (Zero Refusal)

**Model:** `orcanium/orcanium-4-405b`
**Strategy:** Orcanium models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Orcanium Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.orcanium/config.yaml`:
