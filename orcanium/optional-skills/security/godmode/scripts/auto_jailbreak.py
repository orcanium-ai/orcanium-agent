...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ORCANIUM_HOME", os.path.expanduser("~/.orcanium")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ORCANIUM_HOME", Path.home() / ".orcanium")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Orcanium config paths
# ═══════════════════════════════════════════════════════════════════

ORCANIUM_HOME = Path(os.getenv("ORCANIUM_HOME", Path.home() / ".orcanium"))
CONFIG_PATH = ORCANIUM_HOME / "config.yaml"
PREFILL_PATH = ORCANIUM_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Orcanium/Orcanium models — already uncensored, just needs clean prompt
    "orcanium": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "orcanium" in model_lower or "orcanium" in model_lower:
        return "orcanium"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Orcanium config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.orcanium/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Orcanium for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Orcanium for changes to take effect.")
