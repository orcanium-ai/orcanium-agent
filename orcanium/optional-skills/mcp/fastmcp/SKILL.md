...
- smoke-test a server with the FastMCP CLI before wiring it into Orcanium or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Orcanium. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.orcanium/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.orcanium/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Orcanium integration, either:

- configure the server in `~/.orcanium/config.yaml` using the `native-mcp` skill, or
...
### Orcanium cannot see the deployed server

The server-building part may be correct while the Orcanium config is not. Load the `native-mcp` skill and configure the server in `~/.orcanium/config.yaml`, then restart Orcanium.
