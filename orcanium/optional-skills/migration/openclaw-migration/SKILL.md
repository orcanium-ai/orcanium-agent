...
description: Migrate a user's OpenClaw customization footprint into Orcanium Agent. Imports Orcanium-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Orcanium Agent (Orcanium)
license: MIT
platforms: [linux, macos, windows]
metadata:
  orcanium:
    tags: [Migration, OpenClaw, Orcanium, Memory, Persona, Import]
    related_skills: [orcanium-agent]
---

# OpenClaw -> Orcanium Migration

Use this skill when a user wants to move their OpenClaw setup into Orcanium Agent with minimal manual cleanup.
...
```bash
orcanium claw migrate              # Full interactive migration
orcanium claw migrate --dry-run    # Preview what would be migrated
orcanium claw migrate --preset user-data   # Migrate without secrets
orcanium claw migrate --overwrite  # Overwrite existing conflicts
orcanium claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `orcanium setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_orcanium.py` to:

- import `SOUL.md` into the Orcanium home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Orcanium memory entries
- merge OpenClaw command approval patterns into Orcanium `command_allowlist`
- migrate Orcanium-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Orcanium working-directory configuration
- copy OpenClaw skills into `~/.orcanium/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Orcanium workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.orcanium/tts/`
- archive non-secret docs that do not have a direct Orcanium destination
...
- `scripts/openclaw_to_orcanium.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.orcanium/skills/migration/openclaw-migration/scripts/openclaw_to_orcanium.py`

Do not guess a shorter path like `~/.orcanium/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.orcanium/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Orcanium-compatible secrets, currently:
...
Orcanium CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Orcanium. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Orcanium workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Orcanium skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.orcanium/skills/migration/openclaw-migration/scripts/openclaw_to_orcanium.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.orcanium/skills/migration/openclaw-migration/scripts/openclaw_to_orcanium.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.orcanium/skills/migration/openclaw-migration/scripts/openclaw_to_orcanium.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.orcanium/skills/migration/openclaw-migration/scripts/openclaw_to_orcanium.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.orcanium/skills/migration/openclaw-migration/scripts/openclaw_to_orcanium.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.orcanium/skills/migration/openclaw-migration/scripts/openclaw_to_orcanium.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Orcanium unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Orcanium targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Orcanium destination. Unsupported auth blobs must still be reported as skipped.
...
- Orcanium persona state imported
- Orcanium memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.orcanium/skills/openclaw-imports/`
