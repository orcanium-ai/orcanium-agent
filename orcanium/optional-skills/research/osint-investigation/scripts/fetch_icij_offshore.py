...
$ORCANIUM_OSINT_CACHE/icij/ (default: ~/.cache/orcanium-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("ORCANIUM_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "orcanium-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "orcanium-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $ORCANIUM_OSINT_CACHE/icij or ~/.cache/orcanium-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
