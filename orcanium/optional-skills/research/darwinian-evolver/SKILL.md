...
version: 0.1.0
author: Bihruze (Asahi0x), Orcanium Agent
license: MIT
...
import upstream classes into Orcanium itself.
...
```bash
mkdir -p ~/.orcanium/cache/darwinian-evolver && cd ~/.orcanium/cache/darwinian-evolver
...
```bash
cd ~/.orcanium/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.orcanium/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.orcanium/skills/research/darwinian-evolver
DE_DIR=~/.orcanium/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Orcanium core.
   Custom driver scripts under `~/.orcanium/skills/...` are user-side and fine.
