...
the actual `setup.sh` that creates the project workspace, configures Orcanium
...
> [Orcanium/kanban-video-pipeline](https://github.com/Orcanium/kanban-video-pipeline).
...
2. **Create profiles** — `orcanium profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.orcanium/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `orcanium kanban create` assigned to the director
...
```bash
orcanium profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.orcanium/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.orcanium/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Orcanium
...
Each profile gets a `SOUL.md` at `~/.orcanium/profiles/<name>/SOUL.md` that
...
```bash
orcanium kanban create "Direct production of <video title>" \
    --assignee director \
...
the Orcanium `.env` (`${ORCANIUM_HOME:-$HOME/.orcanium}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _orcanium_env="${ORCANIUM_HOME:-$HOME/.orcanium}/.env"
    if grep -q "^${var}=" "$_orcanium_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_orcanium_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_orcanium_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY orcanium ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY orcanium OPENROUTER_API_KEY || exit 1
# ...
```
