...
Maps each role archetype to the Orcanium skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public orcanium-agent
...
## Orcanium skills relevant to video production

### Visual / rendering skills (`orcanium-agent/skills/creative/`)
...
### Design / pre-production skills (`orcanium-agent/skills/creative/`)
...
### Audio / media skills (`orcanium-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`orcanium-agent/skills/devops/`)
...
These are **not** Orcanium skills but external CLIs / APIs that profiles invoke.
...
## Built-in Orcanium tools for media review

These are native Orcanium tools — not invoked via terminal but through their own
...
key is present in `${ORCANIUM_HOME:-~/.orcanium}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Orcanium profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ORCANIUM_HOME:-~/.orcanium}/.env` → environment variable.
...
When a new Orcanium-public video skill ships:
