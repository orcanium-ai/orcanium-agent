...
orcanium kanban watch --tenant <project-slug>

# Snapshot of the board
orcanium kanban list --tenant <project-slug>
orcanium kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
orcanium kanban stats --tenant <project-slug>

# Visual dashboard (browser)
orcanium dashboard

# Inspect a specific task (includes comments + events)
orcanium kanban show <task-id>

# Follow a single task's event stream
orcanium kanban tail <task-id>
```

Verify available subcommands with `orcanium kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `orcanium kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `orcanium kanban block <id>` then `orcanium kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `orcanium kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `orcanium kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `orcanium profile list` (profile exists?), `orcanium status` (gateway/dispatcher up?), and verify tenant. |
...
orcanium kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
orcanium kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(orcanium kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
orcanium kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
orcanium kanban block <task-id>
orcanium kanban archive <task-id>

# Diagnose what happened
orcanium kanban show <task-id>      # task body, comments, recent events
orcanium kanban tail <task-id>      # follow the live event stream
orcanium kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `orcanium kanban create` for the director
...
while true; do
    clear
    orcanium kanban list --tenant <slug>
    echo "---"
    orcanium kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `orcanium kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `orcanium profile list` and start
