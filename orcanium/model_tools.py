...
from orcanium.app.tools.registry import discover_builtin_tools, registry
...
#   - cli.py, orcanium.cli/*      -> inline on startup (no event loop)
#   - rpc/server.py     -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from orcanium.cli.config import get_config_path
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("ORCANIUM_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("ORCANIUM_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by ORCANIUM_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like orcanium-cli)
    ...
    if "execute_code" in available_tool_names:
        from orcanium.app.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    for discord_tool_name in _discord_schema_fns:
        if discord_tool_name in available_tool_names:
            try:
                from orcanium.app.tools import discord_tool as _dt
    ...
    try:
        from orcanium.app.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    # window). Core Orcanium tools (toolsets._ORCANIUM_CORE_TOOLS) are NEVER
    ...
    try:
        from orcanium.app.tools.tool_search import assemble_tool_defs, load_config as _load_ts_config
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from orcanium.cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from orcanium.cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    _ts_mod = None
    try:
        from orcanium.app.tools import tool_search as _ts_mod  # noqa: F401
    except Exception:
        _ts_mod = None
    ...
    if not skip_tool_request_middleware:
        try:
            from orcanium.cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from orcanium.cli.plugins import get_pre_tool_call_block_message
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from orcanium.app.tools.file_tools import notify_other_tool_call
        ...
        try:
            ...
            from orcanium.cli.middleware import run_tool_execution_middleware
