"""Windows UTF-8 bootstrap for Orcanium entry points.
...
should be imported at the very top of every Orcanium entry point
(``orcanium``, ``orcanium-agent``, ``orcanium-acp``, ``python -m channel.run``,
...
# Apply on import — entry points just need ``import orcanium_bootstrap``
# (or ``from orcanium_bootstrap import apply_windows_utf8_bootstrap``) at
