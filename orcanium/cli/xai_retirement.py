...
Pure logic: walks a Orcanium config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `orcanium doctor` and a future `orcanium migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Orcanium config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Orcanium config and return retirement issues.
