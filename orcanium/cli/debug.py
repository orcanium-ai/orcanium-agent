"""``orcanium debug`` debug tools for Orcanium Agent.

Currently supports:
    Orcanium debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.Orcanium/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[Orcanium debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.Orcanium/pastes/pending.json``.
    ...
    ran ``orcanium debug share`` repeatedly.

    Deletion is now driven by the gateway's cron ticker
    (``gateway/run.py::_start_cron_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``orcanium debug share`` also
    runs an opportunistic sweep on entry as a fallback for CLI-only users
    who never start the channel.
    """
    return get_orcanium_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``orcanium debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    """Synchroorcaniumly DELETE any pending pastes whose ``expire_at`` has passed.
    ...
    intended to be called from every ``orcanium debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Orcanium version, provider, which API keys
    are configured — NOT the actual keys)
  • Recent log lines (agent.log, errors.log, channel.log, desktop.log — may
    contain conversation fragments and file paths)
  • Full agent.log, channel.log, and desktop.log (up to 512 KB each — likely
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `orcanium debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "orcanium-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``orcanium debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.Orcanium/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``orcanium debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "orcanium-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----OrcaniumDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from orcanium.cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_orcanium_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``orcanium debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``orcanium dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``orcanium dump`` is run
        internally.
    ...
    buf.write(f"--- channel.log (last {errors_lines} lines) ---\n")
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``orcanium debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "Orcanium debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `orcanium debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  Orcanium debug delete <url>")

    print(f"\nShare these links with the Orcanium team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: Orcanium debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'Orcanium debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``orcanium debug`` call.
    ...
    # best-effort — any failure is swallowed so ``orcanium debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: Orcanium debug <command>")
        print()
        print("Commands:")
