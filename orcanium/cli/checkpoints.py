"""`orcanium checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.Orcanium/checkpoints/``.  Actions:

    Orcanium checkpoints               # same as `status`
    Orcanium checkpoints status        # total size, project count, breakdown
    Orcanium checkpoints list          # per-project checkpoint counts + workdir
    Orcanium checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    Orcanium checkpoints clear [-f]    # nuke the entire base (asks first)
    Orcanium checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    Orcanium checkpoints
    Orcanium checkpoints prune --retention-days 3 --max-size-mb 200
    Orcanium checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    from orcanium.app.tools.checkpoint_manager import store_status
    ...
    if legacy:
        ...
        print()
        print("Clear with: Orcanium checkpoints clear-legacy")
    return 0
...
def cmd_prune(args: argparse.Namespace) -> int:
    from orcanium.app.tools.checkpoint_manager import prune_checkpoints
...
def cmd_clear(args: argparse.Namespace) -> int:
    from orcanium.app.tools.checkpoint_manager import CHECKPOINT_BASE, clear_all, store_status
...
def cmd_clear_legacy(args: argparse.Namespace) -> int:
    from orcanium.app.tools.checkpoint_manager import clear_legacy, store_status
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``orcanium checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `orcanium checkpoints` → status
