"""Windows ConPTY bridge for the `orcanium dashboard` chat tab.

Drop-in counterpart to ``orcanium.cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``orcanium.cli.web_server`` consumes: ``spawn``, ``read``,
