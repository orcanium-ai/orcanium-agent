"""``orcanium logs`` — view and filter Orcanium log files.
...
Usage examples::
    ...
    orcanium logs                    # last 50 lines of agent.log
    orcanium logs -f                 # follow agent.log in real time
    orcanium logs errors             # last 50 lines of errors.log
    orcanium logs gateway -n 100    # last 100 lines of channel.log
    orcanium logs gui -f            # follow gui.log (dashboard/pty/ws)
    orcanium logs desktop -f        # follow desktop.log (Electron app boot/backend)
    orcanium logs --level WARNING    # only WARNING+ lines
    orcanium logs --session abc123   # filter by session ID substring
    orcanium logs --component tools  # only tool-related lines
    orcanium logs --since 1h         # lines from the last hour
    orcanium logs --since 30m -f     # follow, starting 30 min ago
"""
...
# Matches: "INFO channel.run:" or "INFO [sess_abc] tools.terminal_tool:"
...
def tail_log(
    ...
    *,
    ...
    log_path = get_orcanium_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Orcanium runs — try 'orcanium chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_orcanium_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_orcanium_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_orcanium_home()}/logs/")
        return

    print(f"Log files in {display_orcanium_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'orcanium chat' to generate logs)")
