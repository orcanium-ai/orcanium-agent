"""
Backup and import commands for Orcanium CLI.

`orcanium backup` creates a zip archive of the entire ~/.Orcanium/ directory
(excluding the orcanium-agent repo and transient files).

`orcanium import` restores from a backup zip, overlaying onto the current
ORCANIUM_HOME root.
"""
...
from orcanium.channel.compat import get_default_orcanium_root, get_orcanium_home, display_orcanium_home
...
# ``orcanium-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/orcanium-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "orcanium-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to Orcanium root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``orcanium-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/orcanium-agent/`` — must be preserved.
        if part == "orcanium-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ORCANIUM_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Orcanium home directory."""
    orcanium_root = get_default_orcanium_root()

    if not orcanium_root.is_dir():
        print(f"Error: Orcanium home directory not found at {orcanium_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"orcanium-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"orcanium-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_orcanium_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(orcanium_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(orcanium_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``orcanium-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "orcanium-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(orcanium_root)
    ...
    print(f"\nRestore with: Orcanium import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Orcanium backup.
    ...
    # Look for telltale files that a Orcanium home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Orcanium backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.Orcanium/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a Orcanium dir name
        if prefix in {".Orcanium", "orcanium"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_orcanium_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (orcanium_root / "config.yaml").exists()
        has_env = (orcanium_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Orcanium configuration.")
        ...
        orcanium_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = orcanium_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(orcanium_root.resolve())
            except ValueError:
        ...
        profiles_dir = orcanium_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # orcanium.cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: Orcanium profile list  (after installing Orcanium)")

        # Guidance
        print()
        if not (orcanium_root / "orcanium-agent").is_dir():
            print("Note: The orcanium-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: orcanium update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  orcanium -p {pname} gateway install")

        print("Done. Your Orcanium configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and Orcanium backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ORCANIUM_HOME).
...
# — `orcanium update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(orcanium_home: Optional[Path] = None) -> Path:
    home = orcanium_home or get_orcanium_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    orcanium_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(orcanium_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ORCANIUM_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``orcanium update``.
    ...
    Args:
        ...
        orcanium_home: Override for the Orcanium home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(orcanium_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for Orcanium backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_orcanium_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, orcanium_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``orcanium_root`` to ``out_path``.
...
def _pre_update_backup_dir(orcanium_home: Optional[Path] = None) -> Path:
    home = orcanium_home or get_orcanium_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ORCANIUM_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ORCANIUM_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``orcanium update``) should continue even if the backup fails.
    """
    orcanium_root = orcanium_home or get_default_orcanium_root()
    if not orcanium_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(orcanium_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, orcanium_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `orcanium claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ORCANIUM_HOME under ``backups/`` before a
    ``orcanium claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``orcanium import <archive>``.  Writes to
    ``<ORCANIUM_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `orcanium import` and the
