...
import orcanium.cli.auth as auth_mod
...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "orcanium", "openai-codex", "xai-oauth", "qwen-oauth", "google-gemini-cli", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_orcanium_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:orcanium_pkce",
    ...
    if provider == "orcanium":
        # Codex-style auto-import: if a shared Orcanium credential lives at
        # <orcanium-root>/shared/orcanium_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `orcanium --profile <name>
        # auth add Orcanium --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_orcanium_state()
        if shared:
            try:
                path = auth_mod._orcanium_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing Orcanium OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_orcanium_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_orcanium_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._orcanium_device_code_login(
        ...
        # Honor `--label <name>` so Orcanium matches other providers' UX.  The
        # helper embeds this into providers.Orcanium so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("orcanium").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_orcanium_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``orcanium auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`orcanium auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Orcanium reads from
    ...
    from orcanium.cli.auth import suppress_credential_source
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `orcanium auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `orcanium auth` is called bare."""
...
def auth_command(args) -> None:
    ...
    if action == "setup":
        from orcanium.cli.main import cmd_model
        cmd_model(args)
        return
    if action == "add":
