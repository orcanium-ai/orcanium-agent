"""
Orcanium fallback — manage the fallback provider chain.
...
Subcommands:
  Orcanium fallback [list]   Show the current fallback chain (default when no subcommand)
  Orcanium fallback add      Pick provider + model via the same picker as `orcanium model`,
                           then append the selection to the chain
  Orcanium fallback remove   Pick an entry to delete from the chain
  Orcanium fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.Orcanium/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from orcanium.cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `orcanium model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  Orcanium fallback add")
        print()
        return
    ...
    print("  Docs: https://orcanium-agent.orcanium.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `orcanium model`, then append the selection to the chain."""
    from orcanium.cli.main import _require_tty, select_provider_and_model
    ...
    print("  `orcanium model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `orcanium fallback list` to view, or `orcanium fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``orcanium fallback [subcommand]``."""
