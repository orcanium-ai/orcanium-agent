"""Implementation of the ``orcanium bundles`` CLI subcommand.

Mirrors the structure of ``orcanium.cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `orcanium bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  Orcanium bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``orcanium bundles`` argparse tree.

    Called from ``orcanium.cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `orcanium bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``orcanium bundles <subcommand>`` to the right handler."""
