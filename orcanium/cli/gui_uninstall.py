"""
Orcanium Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``orcanium desktop`` / ``orcanium gui``)
     Built inside the agent checkout under ``$ORCANIUM_HOME/orcanium-agent/``:
       ...
       - ``$ORCANIUM_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/orcanium.app`` or ``~/Applications/orcanium.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Orcanium`` (NSIS per-user)
...
the app name ("Orcanium"), separate from ``$ORCANIUM_HOME``:
  - macOS:   ``~/Library/Application Support/Orcanium``
  - Windows: ``%APPDATA%\\Orcanium``
  - Linux:   ``$XDG_CONFIG_HOME/Orcanium`` (default ``~/.config/Orcanium``)
...
``orcanium uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(orcanium_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return orcanium_home / "orcanium-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Orcanium"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Orcanium"
...
def source_built_gui_artifacts(orcanium_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``orcanium desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``orcanium-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(orcanium_home)
    ...
    return [
        ...
        orcanium_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Orcanium" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/orcanium.app"),
            home / "Applications" / "orcanium.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Orcanium).
            local_base / "Programs" / "Orcanium",
            # Older / alternate layout some builds used.
            local_base / "orcanium-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Orcanium")
    else:
        ...
        paths += [
            data_base / "applications" / "orcanium.desktop",
            data_base / "applications" / "orcanium.desktop",
        ]
    return paths


def agent_is_installed(orcanium_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ORCANIUM_HOME.
    ...
    if (agent_root / "orcanium.cli").is_dir():
        return True
...
def gui_is_installed(orcanium_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(orcanium_home):
        if p.exists():
            return True
...
def gui_install_summary(orcanium_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = orcanium_home if orcanium_home is not None else get_orcanium_home()
...
def uninstall_gui(orcanium_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``orcanium-agent/orcanium.cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ORCANIUM_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove Orcanium' or "
            "'sudo dnf remove Orcanium'). AppImage builds are a single file you "
