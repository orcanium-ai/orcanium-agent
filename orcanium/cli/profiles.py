"""
Profile management for multiple isolated Orcanium instances.

Each profile is a fully independent ORCANIUM_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.Orcanium/profiles/<name>/`` by default.

The "default" profile is ``~/.Orcanium`` itself — backward compatible,
zero migration needed.

Usage::

    Orcanium profile create coder          # fresh profile + bundled skills
    Orcanium profile create coder --clone  # also copy config, .env, SOUL.md, skills
    Orcanium profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    orcanium -p coder chat                 # or via flag
    Orcanium profile use coder             # set as sticky default
    Orcanium profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See orcanium.channel.compat.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.Orcanium``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   orcanium-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `orcanium backup` archives
...
# Marker file written by `orcanium profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `Orcanium
...
# `orcanium skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Orcanium infrastructure directories that only the default profile
         (``~/.Orcanium``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_orcanium_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.Orcanium) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "orcanium-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
    ...
    "orcanium.channel.compat.db",
    "response_store.db", "response_store.db-shm", "response_store.db-wal",
    "channel.pid", "gateway_state.json", "processes.json",
...
_RESERVED_NAMES = frozenset({
    "orcanium", "default", "test", "tmp", "root", "sudo",
})

# Orcanium subcommands that cannot be used as profile names/aliases
_ORCANIUM_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the Orcanium root, NOT to the current ORCANIUM_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ORCANIUM_HOME points outside
    ``~/.Orcanium``, profiles live under ``ORCANIUM_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_orcanium_home() / "profiles"


def _get_default_orcanium_home() -> Path:
    """Return the default (pre-profile) ORCANIUM_HOME path.

    In standard deployments this is ``~/.Orcanium``.
    In Docker/custom deployments where ORCANIUM_HOME is outside ``~/.Orcanium``
    (e.g. ``/opt/data``), returns ORCANIUM_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_orcanium_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Orcanium installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ORCANIUM_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_orcanium_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, Orcanium subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ORCANIUM_SUBCOMMANDS:
        return f"'{canon}' conflicts with a Orcanium subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``orcanium -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``orcanium profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``orcanium profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    try:
        ...
        return get_running_pid(profile_dir / "channel.pid", cleanup_stale=False) is not None
    except Exception:
        return False
...
# ``config.yaml``. ``config.yaml`` is the user-facing Orcanium config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_orcanium_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``orcanium update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.Orcanium)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.Orcanium/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `orcanium -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Orcanium profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `orcanium update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`orcanium profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `orcanium update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `orcanium profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `orcanium -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ORCANIUM_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``orcanium profile create
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-c",
             "import json; from orcanium.app.tools.skills_sync import sync_skills; "
             "r = sync_skills(quiet=True); print(json.dumps(r))"],
            env={**os.environ, "ORCANIUM_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_orcanium_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.Orcanium).\n"
            "To remove everything, use: orcanium uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ORCANIUM_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from orcanium.cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ORCANIUM_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ORCANIUM_HOME")
    try:
        os.environ["ORCANIUM_HOME"] = str(profile_dir)
        from orcanium.cli.channel import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ORCANIUM_HOME"] = old_home
        elif "ORCANIUM_HOME" in os.environ:
            del os.environ["ORCANIUM_HOME"]
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    pid_file = profile_dir / "channel.pid"
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.Orcanium/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: Orcanium profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ORCANIUM_HOME.

    Returns ``"default"`` if ORCANIUM_HOME is not set or points to ``~/.Orcanium``.
    Returns the profile name if ORCANIUM_HOME points into ``~/.Orcanium/profiles/<name>``.
    Returns ``"custom"`` if ORCANIUM_HOME is set to an unrecognized path.
    """
    from orcanium.channel.compat import get_orcanium_home
    orcanium_home = get_orcanium_home()
    resolved = orcanium_home.resolve()

    default_resolved = _get_default_orcanium_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.Orcanium itself — its parent is ~/ and its
        # directory name is ".Orcanium", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: Orcanium profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.Orcanium itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.Orcanium). "
            "Specify a different name: Orcanium profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="orcanium_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"orcanium_{old_name}"
    legacy_old_host = f"orcanium.{old_name}"
    new_host = f"orcanium_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_orcanium_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("orcanium_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ORCANIUM_HOME path string.

    Called early in the CLI entry point, before any Orcanium modules
    are imported, to set the ORCANIUM_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
