"""On-demand supply-chain audit for Orcanium Agent installs.

Scans three surfaces a Orcanium user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Orcanium venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.Orcanium/plugins``
...
def _discover_plugins(orcanium_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.Orcanium/plugins``.
    ...
    """
    plugins_dir = orcanium_home / "plugins"
...
def run_audit(
    *,
    ...
    home = orcanium_home or Path(get_orcanium_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, orcanium_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(orcanium_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `orcanium security audit`."""
    home = Path(get_orcanium_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, orcanium_home=home
    )
    if total == 0:
