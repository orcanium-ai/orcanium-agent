"""Shared curses-based UI components for Orcanium CLI.

Used by `orcanium tools` and `orcanium skills` for interactive checklists.
...
def _token_score(orig: str, lower: str, token: str) -> float | None:
    ...
    A faithful port of ``fuzzyScore`` in orcanium/tui/src/lib/fuzzy.ts and
