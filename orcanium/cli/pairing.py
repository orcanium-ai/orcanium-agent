...
Usage:
    Orcanium pairing list              # Show all pending + approved users
    Orcanium pairing approve <platform> <code>  # Approve a pairing code
    Orcanium pairing revoke <platform> <user_id> # Revoke user access
    Orcanium pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle Orcanium pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: Orcanium pairing {list|approve|revoke|clear-pending}")
        print("Run 'Orcanium pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.Orcanium/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'Orcanium pairing list' to see pending codes.\n")
