...
Pure display functions with no OrcaniumCLI state dependency.
"""
...
# eagerly added ~50ms of wasted imports before ``channel.ready`` could fire.
...
from orcanium.cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

ORCANIUM_LOGO = """[#f0f7bc]
   ██████╗ ██████╗  ██████╗ █████╗ ███╗   ██╗██╗██╗   ██╗███╗   ███╗
  ██╔═══██╗██╔══██╗██╔════╝██╔══██╗████╗  ██║██║██║   ██║████╗ ████║
  ██║   ██║██████╔╝██║     ███████║██╔██╗ ██║██║██║   ██║██╔████╔██║
  ██║   ██║██╔══██╗██║     ██╔══██║██║╚██╗██║██║██║   ██║██║╚██╔╝██║
  ╚██████╔╝██║  ██║╚██████╗██║  ██║██║ ╚████║██║╚██████╔╝██║ ╚═╝ ██║
   ╚═════╝ ╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝ ╚═════╝ ╚═╝     ╚═╝
[/]
         Agentic OS with Persistent Cognitive State Engine
"""
ORCANIUM_COMPASS = """
@@@@@@@@@@@@@@@@@@@@@@@@@
@@@@@@@:         :@@@@@@@
...
# (e.g. nix-built Orcanium — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/Orcanium/orcanium-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/orcanium/orcanium-agent"
...
def _fetch_pypi_latest(package: str = "orcanium-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Orcanium update is available.

    Two paths: if ``ORCANIUM_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = orcanium_home / ".update_check"
    embedded_rev = os.environ.get("ORCANIUM_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ORCANIUM_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `orcanium update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/orcanium/update/check`
    ...
    if embedded_rev:
        ...
        # $ORCANIUM_HOME/orcanium-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = orcanium_home / "orcanium-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Orcanium git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ORCANIUM_HOME/orcanium-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``orcanium.cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from orcanium.cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/Orcanium/orcanium-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Orcanium checkout. Cached per-process. Release URL always points at the
    canonical Orcanium/orcanium-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Orcanium Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ORCANIUM_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ORCANIUM_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Orcanium Research[/]")

    if os.getenv("ORCANIUM_YOLO_MODE"):
    ...
    try:
        from orcanium.cli.codex_runtime_switch import get_current_runtime
        from orcanium.cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from orcanium.cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built Orcanium; we know an update
    ...
    # Pip-install warning — `pip install orcanium-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ORCANIUM_LOGO
        console.print(_logo)
        console.print()
