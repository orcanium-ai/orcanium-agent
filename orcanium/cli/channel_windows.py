"""Windows channel service backend (Scheduled Task + Startup-folder fallback).
...
* ``schtasks /Create /SC ONLOGON /RL LIMITED`` means the task runs at the
  ...
  ``schtasks /Run`` immediately after install so the channel starts right
  away without waiting for the next logon.
* We write two files: a shared ``channel.cmd`` wrapper script (cwd + env + the
  actual ``python -m orcanium.cli.main channel run --replace`` invocation) and
...
* Status = merge of "is the schtasks entry registered?" + "is the startup
  .cmd present?" + "is there a channel process running?" so the status
...
* Quoting is tricky: schtasks parses ``/TR`` itself and cmd.exe parses the
  generated ``channel.cmd``. Those are DIFFERENT parsers. We keep two
...
_TASK_NAME_DEFAULT = "Orcanium_Gateway"
_TASK_DESCRIPTION = "Orcanium Agent Gateway - Messaging Platform Integration"
...
def _assert_windows() -> None:
    if sys.platform != "win32":
        raise RuntimeError("channel_windows is Windows-only")
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Orcanium profile."""
    from orcanium.cli.channel import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    """Launch an elevated channel subcommand via UAC and return True on handoff.
    ...
    """
    _assert_windows()
    args = ["-m", "orcanium.cli.main", *_current_profile_cli_args(), "channel", command]
    if extra_args:
    ...
    try:
        ...
        print(f"⚠ Could not launch elevated channel {command} prompt: {exc}")
        return False
    if result <= 32:
        print(f"⚠ Elevated channel {command} prompt was not started (ShellExecuteW={result})")
        return False
    return True
...
def _launch_elevated_install(
    ...
    *,
    ...
    """Launch an elevated channel install via UAC and return True on handoff."""
    old_start_now = os.environ.get("ORCANIUM_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ORCANIUM_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ORCANIUM_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ORCANIUM_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ORCANIUM_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ORCANIUM_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ORCANIUM_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ORCANIUM_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ORCANIUM_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def _launch_elevated_uninstall() -> bool:
    """Launch an elevated channel uninstall via UAC and return True on handoff."""
...
def get_task_name() -> str:
    ...
    Default profile: ``Orcanium_Gateway``
    Named profile X: ``Orcanium_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during orcanium.cli boot.
    from orcanium.cli.channel import _profile_suffix
...
def get_task_script_path() -> Path:
    """The generated ``channel.cmd`` wrapper that the schtasks entry invokes.

    Lives under ``%LOCALAPPDATA%\\Orcanium\\channel-service\\<task_name>.cmd``
    (or ``<ORCANIUM_HOME>/channel-service/<task_name>.cmd`` so per-profile
    Orcanium installs stay self-contained).
    """
    _assert_windows()
    from orcanium.cli.config import get_orcanium_home

    script_dir = Path(get_orcanium_home()) / "channel-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Return a stable cwd for detached/startup channel runs.

    Mirror the POSIX service invariant: anchor at ``ORCANIUM_HOME`` whenever it
    ...
    source checkout only if ``ORCANIUM_HOME`` cannot be resolved yet.
    """
    from orcanium.cli.config import get_orcanium_home
...
def _build_gateway_cmd_script(
    ...
    """Build the ``channel.cmd`` wrapper content (CRLF-terminated).

    The script:
      - cd's into a stable working directory
      - exports ORCANIUM_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m orcanium.cli.main [--profile X] channel run``
        directly so the wrapper cmd.exe exits without a visible channel console
    ...
    lines.append(f'set "ORCANIUM_HOME={orcanium_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ORCANIUM_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the channel's own python detection find the venv
    # if someone imports orcanium.channel.compat-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "orcanium.cli.main"]
    if profile_arg:
        prog_args.extend(profile_arg.split())
    prog_args.extend(["channel", "run"])
    ...
    # wrapper process and made channel lifecycle/status harder to reason about.
...
def _write_task_script() -> Path:
    """Generate and write the channel.cmd wrapper. Return its absolute path."""
    _assert_windows()
    ...
    orcanium_home = str(Path(get_orcanium_home()).resolve())
    profile_arg = _profile_arg(orcanium_home)

    content = _build_gateway_cmd_script(python_path, working_dir, orcanium_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Orcanium builds and failed
    experiments may have left repeat/restart settings on the task; ``/Change``
    preserves those stale triggers and can make the channel relaunch every
...
def _derive_venv_pythonw(python_exe: str) -> str:
    ...
    shell, which is what lets the channel survive a parent-shell exit on
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    """Build (argv, working_dir, env_overlay) for the channel subprocess.

    Same logical command as what channel.cmd runs, but assembled as a
    ...
    argv = [python_exe, "-m", "orcanium.cli.main"]
    if profile_arg:
        argv.extend(profile_arg.split())
    argv.extend(["channel", "run"])

    env_overlay = {
        "ORCANIUM_HOME": orcanium_home,
        "PYTHONIOENCODING": "utf-8",
        "ORCANIUM_GATEWAY_DETACHED": "1",
...
def _spawn_detached(script_path: Path | None = None) -> int:
    """Launch the channel as a fully detached background process.

    We spawn ``pythonw.exe -m orcanium.cli.main channel run``
    ...
    # logging module writes to channel.log through a FileHandler, so the
    # real channel logs still land there — this just captures anything
    # that goes to print() or native stderr.
    from orcanium.cli.config import get_orcanium_home

    log_dir = Path(get_orcanium_home()) / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    stray_log = log_dir / "channel-stdio.log"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ORCANIUM_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ORCANIUM_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from orcanium.cli.setup import prompt_yes_no

    if start_now is None:
        start_now = prompt_yes_no("Start the channel now after install?", True)
    if start_on_login is None:
        start_on_login = prompt_yes_no(
            "Start the channel automatically on Windows login with a Scheduled Task?",
            True,
        )
    return start_now, start_on_login
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `orcanium -p <profile> channel install` must be safe.
    ...
    from orcanium.cli.channel import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"Orcanium {profile_arg} channel start" if profile_arg else "orcanium channel start"
        print("ℹ Startup fallback installed; channel not started now.")
...
def install(
    ...
    *,
    ...
    """Install the channel as a Windows Scheduled Task (with Startup fallback).
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: orcanium channel start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Orcanium channel install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the channel afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: orcanium channel status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: orcanium channel start")
        _print_next_steps()
        return
    ...
    raise RuntimeError(f"Windows channel install failed: {detail}")


def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    """Poll for a live channel process for up to ``timeout_s`` seconds.
    ...
    """
    from orcanium.cli.channel import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"⚠ Launched channel via {via}, but no process detected after 6s.")
        print("  Check the log for startup errors:")
        from orcanium.cli.config import get_orcanium_home
        print(f"    type {Path(get_orcanium_home()).resolve()}\\logs\\channel.log")
        print(f"    type {Path(get_orcanium_home()).resolve()}\\logs\\channel-stdio.log")


def _print_next_steps() -> None:
    from orcanium.cli.config import get_orcanium_home

    orcanium_home = Path(get_orcanium_home()).resolve()
    print()
    print("Next steps:")
    print("  orcanium channel status                      # Check status")
    print(f"  type {orcanium_home}\\logs\\channel.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Orcanium channel uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: orcanium channel status")
                    return
...
def _gateway_pids() -> list[int]:
    """Reuse the cross-platform PID scanner in channel.py."""
    from orcanium.cli.channel import find_gateway_pids
...
def _print_deep_probes() -> None:
    """Print PASS/FAIL per individual probe of channel liveness.
    ...
    home = Path(get_orcanium_home()).resolve()
    pid_path = home / "channel.pid"
    lock_path = home / "channel.lock"
    state_path = home / "gateway_state.json"
    diag_path = home / "logs" / "channel-exit-diag.log"
    ...
    if lock_present:
        try:
            from channel.status import is_gateway_runtime_lock_active
    ...
    try:
        from channel.status import get_running_pid
...
def status(deep: bool = False) -> None:
    """Print a status report for the Windows channel service."""
    _assert_windows()
    task_name = get_task_name()
    ...
    if pids:
        ...
        print("✗ No channel process detected")
    ...
    if not task_installed and not startup_installed and not pids:
        print()
        print("To install:")
        print("  orcanium channel install")


def start() -> None:
    """Start the channel. Prefers /Run on the scheduled task if present."""
    _assert_windows()
    running_pids = _gateway_pids()
    if running_pids:
        print(f"✓ Gateway already running (PID: {', '.join(map(str, running_pids))})")
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the channel starts on login?", True):
            print("  Run: orcanium channel install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: orcanium channel start")
            return
    ...
    # Startup fallback or failed /Run: direct spawn one foreground-detached channel.
    pid = _spawn_detached()
    _report_gateway_start(f"direct spawn (PID {pid})")


def _drain_gateway_pid(pid: int, drain_timeout: float) -> bool:
    """Write the planned-stop marker and wait for the channel PID to exit.
    ...
    the marker is the ONLY way to ask a running channel to drain
    in-flight agents and persist ``resume_pending`` before exit. The
    channel's planned-stop watcher thread (channel/run.py) polls for
    ...
    if pid <= 0:
        return False
    try:
        from channel.status import write_planned_stop_marker, _pid_exists
    except ImportError:
        return False
...
def stop() -> None:
    ...
    Writes the planned-stop marker first so the channel can drain
    in-flight agents and persist ``resume_pending`` before exit (the
    channel's marker-watcher thread picks this up — Windows asyncio
    ...
    """
    _assert_windows()
    from orcanium.cli.channel import kill_gateway_processes, _get_restart_drain_timeout
    from channel.status import get_running_pid

    # Phase 1: ask the running channel (if any) to drain itself by writing
    ...
    if killed:
        stopped_any = True
        print(f"✓ Killed {killed} channel process(es)")
    if stopped_any:
        if drained:
        ...
        print("✗ No channel was running")


def restart() -> None:
    """Stop the channel then start it again."""
    _assert_windows()
    stop()
