"""Per-provider model-selection wizard flows for ``orcanium setup`` / ``orcanium model``.

Extracted from ``orcanium.cli/main.py`` as part of the god-file decomposition
campaign (``~/.Orcanium/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from orcanium.cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``orcanium.cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from orcanium.cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``orcanium.cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from orcanium.cli.main import _prompt_api_key
    from orcanium.channel.compat import OPENROUTER_BASE_URL
    ...
    # in-flow (K/R/C) instead of having to edit ~/.Orcanium/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `orcanium setup` from scratch. OpenRouter
    ...
    from orcanium.cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_orcanium(config, current_model="", args=None):
    """Orcanium Portal provider: ensure logged in, then pick model."""
    from orcanium.cli.auth import (
        get_provider_auth_state,
        _prompt_model_selection,
        _save_model_choice,
        _update_config_for_provider,
        resolve_orcanium_runtime_credentials,
        AuthError,
        format_auth_error,
        _login_orcanium,
        PROVIDER_REGISTRY,
    ...
    from orcanium.cli.managed_capabilities import prompt_enable_tool_gateway

    state = get_provider_auth_state("orcanium")
    if not state or not state.get("access_token"):
        print("Not logged into Orcanium Portal. Starting login...")
        print()
        try:
            ...
            _login_orcanium(mock_args, PROVIDER_REGISTRY["orcanium"])
            ...
            return
        # login_orcanium already handles model selection + config update
        return
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with Orcanium Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    if not free_tier:
        try:
            refreshed_creds = resolve_orcanium_runtime_credentials(
                force_refresh=True,
            )
            if refreshed_creds:
    ...
    _orcanium_portal_url = ""
    try:
        _orcanium_state = get_provider_auth_state("orcanium")
        if _orcanium_state:
            _orcanium_portal_url = _orcanium_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        ...
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _orcanium_portal_url,
        )
    ...
    if not model_ids and not unavailable_models:
        print("No models available for Orcanium Portal after filtering.")
        return

    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from orcanium.cli.auth import DEFAULT_ORCANIUM_PORTAL_URL

            _url = (_orcanium_portal_url or DEFAULT_ORCANIUM_PORTAL_URL).rstrip("/")
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_orcanium_portal_url,
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate Orcanium as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("orcanium", inference_url)
        ...
        save_config(config)
        print(f"Default model set to: {selected} (via Orcanium Portal)")
        # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `orcanium auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "reauth":
            ...
            print()
            try:
                # Forward CLI flags from ``orcanium model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``orcanium model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``orcanium auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from orcanium.cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from orcanium.cli.auth import (
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.Orcanium/config.yaml.
    """
    from orcanium.cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from orcanium.cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from orcanium.cli.auth import _save_model_choice, deactivate_provider
    from orcanium.cli.config import get_env_value, load_config, save_config
    ...
    from orcanium.cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `orcanium model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from orcanium.cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from orcanium.cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from orcanium.cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Orcanium will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Orcanium will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from orcanium.cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from orcanium.cli.auth import _save_model_choice, deactivate_provider
    from orcanium.cli.config import load_config, save_config
    from orcanium.cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from orcanium.cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from orcanium.cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from orcanium.cli.config import save_env_value, load_config, save_config
    from orcanium.cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from orcanium.cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from orcanium.cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Orcanium turns to `copilot --acp`.")
    print("  Orcanium currently starts its own ACP subprocess for each request.")
    print("  Orcanium uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ORCANIUM_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from orcanium.cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from orcanium.cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from orcanium.cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so Orcanium knows where to find it
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Orcanium typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Orcanium, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, orcanium."
                )
                print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from orcanium.cli.main import _run_anthropic_oauth_flow
    from orcanium.cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'orcanium model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
