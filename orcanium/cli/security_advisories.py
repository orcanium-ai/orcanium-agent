"""
Security advisory checker for Orcanium Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``orcanium doctor --ack <id>``; the ack is persisted to
...
1. ``orcanium doctor`` (and ``orcanium doctor --ack <id>``)
2. CLI startup banner (one short line, then full guidance via
   ``orcanium doctor``)
3. Gateway startup (logged to channel.log; first interactive message gets
   a one-line operator banner)

This module is intentionally dependency-free beyond the stdlib so it can
run in environments where the rest of Orcanium failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including Orcanium when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.Orcanium/.env (OpenRouter, Anthropic, OpenAI, "
            "Orcanium, GitHub, AWS, Google, Mistral, etc.).",
            ...
            "After cleanup: orcanium doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Orcanium requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'orcanium doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.Orcanium/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_orcanium_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``orcanium doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `orcanium doctor` on the gateway host for details.")
