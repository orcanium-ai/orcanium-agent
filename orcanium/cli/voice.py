"""Process-wide voice recording + TTS API for the TUI channel.
...
# Modifier aliases mirrored from the TUI parser (``orcanium/tui/src/lib/platform.ts``)
...
# lookup, and orcanium-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    Mirrors the TUI parser contract (``orcanium/tui/src/lib/platform.ts``)
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and orcanium-ink rejects them; collapse
...
from orcanium.app.tools.voice_mode import (
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when ORCANIUM_VOICE_DEBUG=1.

    Goes to stderr so the TUI gateway wraps it as a channel.stderr event,
    ...
    """
    if os.environ.get("ORCANIUM_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def _play_beep(frequency: int, count: int = 1) -> None:
    ...
    if not _beeps_enabled():
        return
    try:
        from orcanium.app.tools.voice_mode import play_beep
...
def stop_continuous(force_transcribe: bool = False) -> None:
    ...
    True, the recorder stops synchroorcaniumly, then transcription/cleanup runs on a
...
def speak_text(text: str) -> None:
    ...
    try:
        ...
        os.makedirs(os.path.join(tempfile.gettempdir(), "orcanium_voice"), exist_ok=True)
