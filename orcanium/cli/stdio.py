...
on Japanese locales, etc.).  Orcanium'ss banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``orcanium.cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. orcanium is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ORCANIUM_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ORCANIUM_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Orcanium-managed Git install directories so
    ...
    # already-running shells don't see the change — which means Orcanium
    ...
    # install without Orcanium-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Orcanium'ss interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Orcanium'ss
    ``orcanium config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Orcanium (User env var in Windows Settings, or
    export in a PowerShell profile) and Orcanium picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Orcanium-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\Orcanium\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``orcanium`` from right after install) retain their old PATH.

    Any subprocess Orcanium spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Orcanium-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "orcanium", "git", "cmd"),
        os.path.join(local_appdata, "orcanium", "git", "bin"),
        os.path.join(local_appdata, "orcanium", "git", "usr", "bin"),
        # Orcanium venv Scripts directory — host of the orcanium.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes Orcanium, but harmless to include.
        os.path.join(local_appdata, "orcanium", "orcanium-agent", "venv", "Scripts"),
