"""CLI subcommand: `orcanium curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    synchroorcanium = bool(getattr(args, "synchroorcanium", False)) or not background
    if dry:
    ...
    if not synchroorcanium:
        print("llm pass running in background — check `orcanium curator status` later")
    if dry:
        if synchroorcanium:
            print(
                "dry-run: no changes applied. Read the report with "
                "`orcanium curator status` and run `orcanium curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`orcanium curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.Orcanium/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`orcanium curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.Orcanium/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from orcanium.cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_run.add_argument(
        "--sync", "--synchroorcanium", dest="synchroorcanium", action="store_true",
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.Orcanium/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.Orcanium/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by orcanium.cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="Orcanium curator")
    register_cli(parser)
