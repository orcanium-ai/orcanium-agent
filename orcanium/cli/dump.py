"""
Dump command for Orcanium CLI.

Outputs a compact, plain-text summary of the user's Orcanium setup
...
from orcanium.cli.config import get_orcanium_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.orcanium_build_sha`` by the Dockerfile's
    ``ORCANIUM_GIT_SHA`` build-arg (see ``orcanium.cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``orcanium dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from orcanium.cli.channel import get_gateway_runtime_snapshot
...
def _count_skills(orcanium_home: Path) -> int:
...
def _cron_summary(orcanium_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = orcanium_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"orcanium_home:      {display_orcanium_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `orcanium auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `orcanium auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["orcanium-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(orcanium_home)}")
    lines.append(f"  skills:             {_count_skills(orcanium_home)}")
