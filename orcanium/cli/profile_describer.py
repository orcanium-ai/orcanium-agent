...
Used by ``orcanium profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``orcanium.cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Orcanium Agent kanban board.
...
Rules:
  ...
  - Never write "Orcanium Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from orcanium.channel.compat import get_orcanium_home  # type: ignore
            profile_dir = Path(get_orcanium_home())
        else:
