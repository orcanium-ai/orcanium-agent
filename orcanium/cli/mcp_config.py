"""
MCP Server Management CLI — ``orcanium mcp`` subcommand.

Implements ``orcanium mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.Orcanium/config.yaml under the ``mcp_servers`` key.
"""
...
from orcanium.cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_orcanium_home,  # noqa: F401 — used by test mocks
)
from orcanium.cli.colors import Colors, color
from orcanium.channel.compat import display_orcanium_home
from orcanium.app.tools.mcp_tool import _ENV_VAR_PATTERN
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from orcanium.cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.Orcanium/.env`` into ``os.environ`` and recursively interpolate any
    ...
    """
    from orcanium.app.tools.mcp_tool import _interpolate_env_vars
...
def _probe_single_server(
    ...
    """
    from orcanium.app.tools.mcp_tool import (
        _ensure_mcp_loop,
        _run_on_mcp_loop,
        _connect_server,
        _stop_mcp_loop,
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``orcanium mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from orcanium.app.tools.mcp_oauth import OrcaniumTokenStorage
        return OrcaniumTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # orcanium.cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  Orcanium mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  Orcanium mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  Orcanium mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_orcanium_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: Orcanium mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_orcanium_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `orcanium mcp test` in the same session) is evicted too.
    try:
        from orcanium.app.tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  Orcanium mcp add <name> --url <endpoint>')
        _info('  Orcanium mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `orcanium mcp remove` + `orcanium mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `orcanium mcp login " + name + "`.")
            return
        if tools:
...
# ─── Orcanium mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'Orcanium mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``orcanium mcp`` subcommands."""
    ...
    if action == "picker":
        from orcanium.cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from orcanium.cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from orcanium.cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `orcanium plugin`.
        from orcanium.cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("Orcanium mcp                                    Open the catalog picker (default)")
        _info("Orcanium mcp catalog                            List Orcanium-approved MCPs")
        _info("Orcanium mcp install <name>                     Install a catalog MCP")
        _info("Orcanium mcp serve                              Run as MCP server")
        _info("Orcanium mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("Orcanium mcp add <name> --command <cmd>         Add a stdio server")
        _info("Orcanium mcp add <name> --preset <preset>       Add from a known preset")
        _info("Orcanium mcp remove <name>                      Remove a server")
        _info("Orcanium mcp list                               List configured servers")
        _info("Orcanium mcp test <name>                        Test connection")
        _info("Orcanium mcp configure <name>                   Toggle tools")
        _info("Orcanium mcp login <name>                       Re-authenticate OAuth")
        print()
