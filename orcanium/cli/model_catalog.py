...
The Orcanium docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Orcanium Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_URL = (
    "https://orcanium-agent.orcanium.com/docs/api/model-catalog.json"
)
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/Orcanium/orcanium-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_orcanium_home() / "cache" / "model_catalog.json"
...
def get_curated_orcanium_models() -> list[str] | None:
    """Return Orcanium Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("orcanium")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``orcanium update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.Orcanium/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``orcanium model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
