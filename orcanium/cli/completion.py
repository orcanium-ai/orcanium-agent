"""Shell completion script generation for Orcanium CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_orcanium_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Orcanium Agent bash completion
# Add to ~/.bashrc:
#   eval "$(Orcanium completion bash)"

_orcanium_profiles() {{
    local profiles_dir="$HOME/.Orcanium/profiles"
...
_orcanium_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_orcanium_profiles)" -- "$cur"))
        return
    fi
...
complete -F _orcanium_completion Orcanium
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef Orcanium
# Orcanium Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(Orcanium completion zsh)"

_orcanium_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.Orcanium/profiles" ]]; then
        profiles+=($HOME/.Orcanium/profiles/*(N/:t))
    fi
...
_orcanium() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_orcanium_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'Orcanium command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Orcanium Agent fish completion",
        "# Add to your config:",
        "#   Orcanium completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __orcanium_profiles",
        "    echo default",
        "    if test -d $HOME/.Orcanium/profiles",
        "        for d in $HOME/.Orcanium/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c Orcanium -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c Orcanium -f -s p -l profile"
        " -d 'Profile name' -xa '(__orcanium_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__orcanium_profiles)' -d 'Profile name'"
                )
