...
from orcanium.cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from orcanium.cli.dashboard_auth.cookies import read_session_cookies
from orcanium.cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /Orcanium``, the
    ``login_url`` is prefixed (``/orcanium/login?next=...``) so the
    ...
    """
    from orcanium.cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["orcanium.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``orcanium_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `Orcanium` provider first, which
        # tries to reach Orcanium Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        if refreshed is not None:
            ...
            from orcanium.cli.dashboard_auth.cookies import (
                detect_https,
                set_session_cookies,
            )
            from orcanium.cli.dashboard_auth.prefix import prefix_from_request
        ...
        from orcanium.cli.dashboard_auth.cookies import clear_session_cookies
        from orcanium.cli.dashboard_auth.prefix import prefix_from_request
