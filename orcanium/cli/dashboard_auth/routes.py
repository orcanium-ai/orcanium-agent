...
from orcanium.cli.dashboard_auth.base import (
    InvalidCodeError,
...
from orcanium.cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``ORCANIUM_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /Orcanium`` (Mission Control deploys) — we
    ...
    from orcanium.cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``orcanium.cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("orcanium_session_pkce", "")
...
async def api_auth_ws_ticket(request: Request):
    ...
    from orcanium.cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
