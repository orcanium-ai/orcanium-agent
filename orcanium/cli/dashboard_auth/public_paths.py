...
* ``orcanium.cli.web_server.auth_middleware`` — loopback / ``--insecure``
  mode, gates on the ephemeral ``_SESSION_TOKEN``.
* ``orcanium.cli.dashboard_auth.middleware.gated_auth_middleware`` —
...
liveness probe (``Orcanium-account-service`` ``fly-provider.ts``
