...
Profile-aware location: ``$ORCANIUM_HOME/logs/dashboard-auth.log``.
...
from ``orcanium.channel.compat`` or other orcanium.cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ORCANIUM_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``orcanium.channel.compat.get_orcanium_home`` semantics: env var wins,
    else ``~/.Orcanium``. A local copy avoids an import cycle with the
    middleware which lives below ``orcanium.cli``.
    """
    home = os.environ.get("ORCANIUM_HOME") or str(Path.home() / ".Orcanium")
