...
Visual styling mirrors the Orcanium Research design system (the
``@Orcanium-research/ui`` package the React dashboard uses): the same
...
``tests/orcanium.cli/test_dashboard_auth_401_reauth.py``.
"""
...
<title>Sign in — Orcanium Agent</title>
<style>
  /* Brand fonts shipped by @Orcanium-research/ui — same files the SPA loads. */
  @font-face {{
...
<body>
<main>
  <div class="brand">Orcanium<span class="dot"></span>Research</div>
  <div class="card">
    <h1>Sign in</h1>
    <p class="subtitle">Choose a sign-in method to continue to the Orcanium Agent dashboard.</p>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Sign-in unavailable — Orcanium Agent</title>
<style>
  @font-face {
    font-family: 'Collapse';
    font-style: normal;
...
<p>Install <code>plugins/dashboard-auth-Orcanium</code> (default) or another
