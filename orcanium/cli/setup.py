"""
Interactive setup wizard for Orcanium Agent.
...
from orcanium.app.tools.tool_backend_helpers import managed_orcanium_tools_enabled
from orcanium.cli.managed_capabilities import get_managed_capabilities
from orcanium.cli.compat import base_url_hostname
...
_DOCS_BASE = "https://agent.orcanium.com/docs"
...
# display_orcanium_home imported lazily at call sites (stale-module safety during orcanium update)
...
from orcanium.cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from orcanium.cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Orcanium Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Orcanium using environment variables or config commands:")
    print_info("  orcanium config set model.provider custom")
    print_info("  orcanium config set model.base_url http://localhost:8080/v1")
    print_info("  orcanium config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info(
        "Run 'orcanium setup' in an interactive terminal to use the full wizard."
    )
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from orcanium.cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'orcanium setup')")


def _print_setup_summary(config: dict, orcanium_home):
    ...
    tool_status = []
    subscription_features = get_managed_capabilities(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(
            ("Vision (image analysis)", False, "run 'orcanium setup' to configure")
        )
    ...
    if subscription_features.web.managed_by_orcanium:
    ...
    if subscription_features.browser.managed_by_orcanium:
        tool_status.append(("Browser Automation (Orcanium Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Orcanium), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_orcanium:
        ...
        try:
            ...
            from orcanium.cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `orcanium tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_orcanium:
        tool_status.append(
            ("Video Generation (FAL via managed Gateway)", True, None)
        )
    else:
        try:
    ...
    if subscription_features.tts.managed_by_orcanium:
        tool_status.append(
            ("Text-to-Speech (OpenAI via managed Gateway)", True, None)
        )
    ...
    if subscription_features.modal.managed_by_orcanium:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(
                ("Modal Execution", False, "run 'orcanium setup terminal'")
            )
    elif managed_orcanium_tools_enabled() and subscription_features.orcanium_auth_present:
        tool_status.append(
            ("Modal Execution (optional via managed Gateway)", True, None)
        )
    ...
    # Spotify (OAuth via Orcanium auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'orcanium setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {orcanium_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('orcanium setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('orcanium setup model', Colors.GREEN)}    Change model/provider")
    print(
        f"   {color('orcanium setup terminal', Colors.GREEN)} Change terminal backend"
    )
    print(f"   {color('orcanium setup gateway', Colors.GREEN)}  Configure messaging")
    print(
        f"   {color('orcanium setup tools', Colors.GREEN)}    Configure tool providers"
    )
    print()
    print(f"   {color('orcanium config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('orcanium config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('orcanium config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('orcanium', Colors.GREEN)}              Start chatting")
    print(f"   {color('orcanium gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('orcanium doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `orcanium tools` and `orcanium setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``orcanium model``)
    ...
    provider added to ``orcanium model`` is automatically available here.
    ...
    # Delegate to the shared orcanium model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from orcanium.cli.main import select_provider_and_model
    ...
    try:
        ...
        print_info("You can try again later with: orcanium model")
    ...
    # on demand via `orcanium auth add`, `orcanium setup` vision, and
    # `orcanium setup tts`. This keeps both quick and full setup thin.

    # Tool Gateway prompt is already shown by _model_flow_orcanium() above.
    save_config(config)
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``orcanium model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from orcanium.cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_orcanium_tools_enabled() and subscription_features.orcanium_auth_present:
        choices.append(
            "Managed Gateway (managed OpenAI TTS, key entitlement required)"
        )
        providers.append("Orcanium-openai")
    choices.extend(
        [
    ...
    selected_via_orcanium = selected == "Orcanium-openai"
    if selected == "Orcanium-openai":
        selected = "openai"
        print_info(
            "OpenAI TTS will use the managed Gateway when enabled for your API key."
        )
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.Orcanium/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_orcanium:
        ...
        # via the Orcanium auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via orcanium setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'orcanium setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Orcanium runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `orcanium setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `orcanium setup terminal`.
        ...
        from orcanium.app.tools.managed_tool_gateway import (
            is_managed_tool_gateway_ready,
        )
        from orcanium.app.tools.tool_backend_helpers import normalize_modal_mode

        managed_modal_available = bool(
            managed_orcanium_tools_enabled()
            and get_managed_capabilities(config).orcanium_auth_present
        ...
        if use_managed_modal:
            ...
            print_info(
                "Modal execution will use the managed Gateway when enabled for your API key."
            )
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ORCANIUM_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ORCANIUM_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ORCANIUM_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from orcanium.cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in orcanium_home:
            profile_name = orcanium_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Orcanium delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `orcanium update`)",
                True,
            ):
    ...
    print()
    print_info(
        "   Full guide: https://orcanium-agent.orcanium.com/docs/user-guide/messaging/slack/"
    )
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ORCANIUM_HOME, and print
    ...
    """
    try:
        from orcanium.cli.slack_cli import _build_full_manifest
        from orcanium.channel.compat import get_orcanium_home

        manifest = _build_full_manifest(
            bot_name="Orcanium",
            bot_description="Your Orcanium agent on Slack",
        )
        target = Path(get_orcanium_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `orcanium slack manifest --write` anytime to refresh after "
            "Orcanium adds new commands."
        )
...
def _setup_bluebubbles():
    """Configure BlueBubbles iMessage channel."""
    ...
    print_info("Connects Orcanium to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from orcanium.cli.channel import _setup_qqbot as _channel_setup_qqbot

    _channel_setup_qqbot()
...
def _setup_webhooks():
    ...
    print()
    print_info(
        "   Full guide: https://orcanium-agent.orcanium.com/docs/user-guide/messaging/webhooks/"
    )
    print()
    ...
    print_info(
        "   https://orcanium-agent.orcanium.com/docs/user-guide/messaging/webhooks/#configuring-routes"
    )
    print()
    print_info("   Open config in your editor:  orcanium config edit")
    print_info("   Open config in your editor:  orcanium config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Orcanium from anywhere.")
    ...
    if not selected:
        print_info(
            "No platforms selected. Run 'orcanium setup gateway' later to configure."
        )
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     orcanium config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_orcanium_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    if supports_systemd:
                        ...
                        # channel_windows.install() registers the Scheduled
                    ...
                    print_info("  You can try manually: orcanium gateway install")
            else:
                print_info("  You can install later: orcanium gateway install")
                if supports_systemd:
                    print_info(
                        "  Or as a boot-time service: sudo orcanium gateway install --system"
                    )
                print_info("  Or run in foreground:  orcanium gateway")
        else:
            ...
            if is_container():
                ...
                print_info(
                    "   orcanium gateway run          # Run as container main process"
                )
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   orcanium gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `orcanium setup tools` and `orcanium tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from orcanium.cli.config
    ...
    if section_key == "model":
        ...
        from orcanium.cli.channel import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_orcanium.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_orcanium migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_orcanium", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Orcanium.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Orcanium to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Orcanium at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Orcanium at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Orcanium at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Orcanium at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Orcanium equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(
            color(
                "  Would overwrite (conflicts with existing Orcanium config):",
                Colors.YELLOW,
            )
        )
    ...
    if warnings_shown:
        ...
        print(
            color(
                "  Note: OpenClaw config values may have different semantics in Orcanium.",
                Colors.YELLOW,
            )
        )
        print(
            color(
                "  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Orcanium'ss yolo mode.",
                Colors.YELLOW,
            )
        )
        print(
...
def _offer_openclaw_migration(orcanium_home: Path) -> bool:
    ...
    print_info("Orcanium can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: Orcanium claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=orcanium_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: Orcanium claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Orcanium configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Orcanium config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(
            f"Skipped {conflicts} item(s) that already exist in Orcanium (use Orcanium claw migrate --overwrite to force)."
        )
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      orcanium setup           — full or quick (auto-detected)
      orcanium setup model     — just model/provider
      orcanium setup tts       — just text-to-speech
      orcanium setup terminal  — just terminal backend
      orcanium setup gateway   — just messaging platforms
      orcanium setup tools     — just tool configuration
      orcanium setup agent     — just agent settings
    """
    from orcanium.cli.config import is_managed, managed_error
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Orcanium Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│  Let's configure your Orcanium Agent installation.       │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, orcanium_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Orcanium configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info(
            "Tip: jump straight to a section with 'orcanium setup model|terminal|"
        )
        print_info(
        ...
        migration_ran = _offer_openclaw_migration(orcanium_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Orcanium?",
            [
                "Quick Setup (Managed Gateway) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, orcanium_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {orcanium_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'orcanium config edit'")
    ...
    _print_setup_summary(config, orcanium_home)


def _run_first_time_quick_setup(config: dict, orcanium_home, is_existing: bool):
    """Streamlined first-time setup via Managed Gateway: OAuth, model, terminal & messaging.

    Routes straight to the Managed Gateway provider — runs the device-code OAuth
    login, picks a Orcanium model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``orcanium setup <section>``
    or switch providers with ``orcanium model``.
    """
    from orcanium.cli.config import load_config

    # Step 1: Managed Gateway — OAuth login + model selection.
    # _model_flow_orcanium() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Orcanium model picker). Provider is set to "orcanium" by the login/model save.
    print()
    print_header("Managed Gateway")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.orcanium.com/manage-subscription")
    print()
    try:
        from orcanium.cli.main import _model_flow_orcanium

        _model_flow_orcanium(config)
    except (KeyboardInterrupt, EOFError):
        print()
        print_info("Managed Gateway setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_orcanium error during quick setup: %s", exc)
        print_warning(f"Managed Gateway setup encountered an error: {exc}")
        print_info("You can try again later with: orcanium model")

    # Re-sync the wizard's config dict from disk — _model_flow_orcanium (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'orcanium setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    orcanium setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  orcanium setup gateway")
    print()

    _print_setup_summary(config, orcanium_home)


def _run_quick_setup(config: dict, orcanium_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'orcanium setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Orcanium to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'orcanium setup gateway'.")
