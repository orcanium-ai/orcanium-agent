...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Orcanium's
...
The actual runtime resolution happens in orcanium.cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "orcanium"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Orcanium's MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Orcanium for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from orcanium.cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the orcanium-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "orcanium-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Orcanium tool callback are always-on production
            ...
            if "orcanium-tools" in mig_report.migrated:
                msg_lines.append(
                    "Orcanium tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Orcanium runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Orcanium tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Orcanium runtime.")
