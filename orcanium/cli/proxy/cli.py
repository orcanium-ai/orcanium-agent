"""CLI handlers for the ``orcanium proxy`` subcommand."""
...
from orcanium.cli.proxy.adapters import ADAPTERS, get_adapter
from orcanium.cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "Orcanium proxy requires aiohttp. Install one of:\n"
        "  pip install 'orcanium-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "orcanium"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"Orcanium auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Orcanium proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Orcanium proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: Orcanium proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``orcanium proxy <subcommand>``."""
    ...
    print(
        "Orcanium proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  Orcanium proxy start [--provider Orcanium|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  Orcanium proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  Orcanium proxy providers\n"
