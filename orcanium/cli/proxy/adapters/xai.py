...
from orcanium.cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
from orcanium.cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential

logger = logging.getLogger(__name__)

_POOL_PROVIDER = "xai-oauth"

# xAI's public API is OpenAI-compatible for the endpoints Orcanium commonly
# uses. The Responses endpoint is included because Orcanium's native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Orcanium-managed OAuth credentials."""

    auth_hint = "Orcanium auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`orcanium auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`orcanium auth reset xai-oauth` or re-authenticate with "
                    "`orcanium auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `orcanium auth add xai-oauth --type oauth`."
            )
