"""
Configuration management for Orcanium Agent.

Config files are stored in ~/.Orcanium/ for easy access:
- ~/.Orcanium/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.Orcanium/.env         - API keys and secrets

This module provides:
- orcanium config          - Show current configuration
- orcanium config edit     - Open config in editor
- orcanium config set      - Set a specific value
- orcanium config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``orcanium config set`` (which rewrites
    ...
    ``config.yaml`` in place: Orcanium never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.Orcanium/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``orcanium logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  orcanium config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Orcanium makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Orcanium itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``orcanium update``, the TUI build.
...
#   on every plugin install / ``orcanium update``.
...
# * ``ORCANIUM_HOME`` / ``ORCANIUM_PROFILE`` / ``ORCANIUM_CONFIG`` /
#   ``ORCANIUM_ENV`` — Orcanium runtime location flags. Writing these into
...
#
# IMPORTANT: ``ORCANIUM_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ORCANIUM_GEMINI_CLIENT_ID,
# ORCANIUM_LANGFUSE_PUBLIC_KEY, ORCANIUM_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONORCANIUMERSITE",
    # Node
    ...
    # Orcanium runtime location — never via dashboard env writer.
    # NOT a ORCANIUM_* blanket: integration credentials (ORCANIUM_GEMINI_*,
    # ORCANIUM_LANGFUSE_*, ORCANIUM_SPOTIFY_*, ...) ARE allowed.
    "ORCANIUM_HOME", "ORCANIUM_PROFILE", "ORCANIUM_CONFIG", "ORCANIUM_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Orcanium runtime location "
            "(ORCANIUM_HOME, ORCANIUM_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.Orcanium/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ORCANIUM_TOOL_PROGRESS", "ORCANIUM_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `orcanium plugins enable
    # observability/langfuse` or `orcanium tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ORCANIUM_LANGFUSE_ENV",
    "ORCANIUM_LANGFUSE_RELEASE",
    "ORCANIUM_LANGFUSE_SAMPLE_RATE",
    "ORCANIUM_LANGFUSE_MAX_CHARS",
    "ORCANIUM_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from orcanium.cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ORCANIUM_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_orcanium_home() / ".managed"
...
def is_managed() -> bool:
    """Check if orcanium is running in package-manager-managed mode.

    Two signals: the ORCANIUM_MANAGED env var (set by the systemd service),
    or a .managed marker file in ORCANIUM_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade orcanium-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Orcanium was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.Orcanium/.install_method`` file (written by installers)
    2. ORCANIUM_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``orcanium/orcanium-agent`` image stamps ``docker``
    ...
    so ``orcanium update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_orcanium_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.Orcanium/.install_method."""
    stamp = get_orcanium_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Orcanium lives in a ``uv tool`` layout.

    ``uv tool install orcanium-agent`` places the install at
    ``.../uv/tools/orcanium-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``orcanium-agent`` happens to be uv-tool-installed on the machine while
    the *active* orcanium is a regular pip/venv install, causing
    ``orcanium update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/orcanium-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade orcanium-agent"
    if method == "docker":
        return "docker pull orcanium/orcanium-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade orcanium-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade orcanium-agent"
        return "pip install --upgrade orcanium-agent"
    return "orcanium update"
...
# Long-form text for ``orcanium update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# orcanium.cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Orcanium, it doesn't update
...
✗ ``orcanium update`` doesn't apply inside the Docker container.

Orcanium Agent runs as a published image (orcanium/orcanium-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull orcanium/orcanium-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate orcanium-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm orcanium/orcanium-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/orcanium/orcanium-agent/tags
  • Your config and session history live under ``$ORCANIUM_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``orcanium update`` inside Docker.
...
def format_managed_message(action: str = "modify this Orcanium installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ORCANIUM_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Orcanium installation is managed by NixOS "
            f"(ORCANIUM_MANAGED={env_hint}).\n"
            "Edit services.orcanium-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Orcanium installation is managed by Homebrew "
            f"(ORCANIUM_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade orcanium-agent"
        )

    return (
        f"Cannot {action}: this Orcanium installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall orcanium."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ORCANIUM_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, orcanium_bin
    or None if container mode is not active, we're already inside the
    container, or ORCANIUM_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ORCANIUM_DEV") == "1":
        return None
    ...
    container_mode_file = get_orcanium_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "orcanium-agent")
    exec_user = info.get("exec_user", "orcanium")
    orcanium_bin = info.get("orcanium_bin", "/data/current-package/bin/Orcanium")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "orcanium_bin": orcanium_bin,
    }
...
# Re-export from orcanium.channel.compat — canonical definition lives there.
from orcanium.channel.compat import get_orcanium_home  # noqa: F811,E402
from orcanium.cli.compat import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_orcanium_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_orcanium_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_orcanium_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ORCANIUM_UID / ORCANIUM_GID env vars set by Docker deployments.

    Docker containers running Orcanium commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ORCANIUM_HOME once, but
    subdirectories created at runtime by ``ensure_orcanium_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ORCANIUM_UID", "").strip()
    gid_str = os.environ.get("ORCANIUM_GID", "").strip()
    try:
...
def _chown_to_orcanium_uid(path) -> None:
    """Chown ``path`` to ``ORCANIUM_UID:ORCANIUM_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_orcanium_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_orcanium_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the Orcanium group can
    share state with the gateway service.

    The mode can be overridden via the ORCANIUM_HOME_MODE environment variable
    (e.g. ORCANIUM_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ORCANIUM_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ORCANIUM_UID``/``ORCANIUM_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ORCANIUM_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_orcanium_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Orcanium runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ORCANIUM_CONTAINER") or os.environ.get("ORCANIUM_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ORCANIUM_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ORCANIUM_HOME if the user doesn't have one yet."""
...
def ensure_orcanium_home():
    """Ensure ~/.Orcanium directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_orcanium_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_orcanium_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ORCANIUM_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Orcanium-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Orcanium
        ...
        # identity slot (SOUL.md). Empty by default. The ORCANIUM_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Orcanium adds a coding operating brief
        ...
        # blocks indefinitely (input() is synchroorcanium) and ignores this.
        "clarify_timeout": 600,
    ...
    "terminal": {
        ...
        # skipped. When empty, Orcanium auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Orcanium sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Orcanium
        ...
        #  "/home/user/.Orcanium/cache/documents:/output"]
        ...
        # bundled Orcanium image, which drops to the `orcanium` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Orcanium sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Orcanium should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``orcanium chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.Orcanium/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: Orcanium sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Orcanium truncates the
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | Orcanium | codex | custom
        ...
        # Invoked by ``orcanium kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``orcanium kanban decompose`` and the kanban
        ...
        # ``orcanium profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `orcanium model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `orcanium` (and `orcanium chat`) launches by default.
        # Interactive sessions use the TUI; this value remains for UI integrations.
        "interface": "tui",
        # When true, `orcanium --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `orcanium -c` muscle memory.  Default off so existing
        # users aren't surprised.  ORCANIUM_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `orcanium --tui` drops a one-time hint
        ...
        # Orcanium credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # replies wrapped with channel.platforms.base.EphemeralReply — agent
        ...
        # e.g. `model · 68% · ~/projects/Orcanium`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Orcanium Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``ORCANIUM_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ORCANIUM_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.orcanium.com).
        "oauth": {
        ...
        # (``ORCANIUM_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``ORCANIUM_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/Orcanium``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Orcanium appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.Orcanium/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.Orcanium/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Orcanium, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # response. If not, Orcanium feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Orcanium auto-pauses the goal and
    ...
    # always goes to ~/.Orcanium/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ORCANIUM_SKILL_DIR} and ${ORCANIUM_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `orcanium curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `orcanium update`, so pruning them only
        ...
        # skipped), snapshot ~/.Orcanium/skills/ into
        # ~/.Orcanium/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `orcanium curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for orcanium-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Orcanium Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ORCANIUM_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.Orcanium/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ORCANIUM_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `orcanium doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``orcanium.cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Orcanium to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ORCANIUM_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `orcanium -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `orcanium` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `orcanium kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     orcanium-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Orcanium tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.Orcanium/logs/.
    ...
    # curated model lists for OpenRouter and Orcanium Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a orcanium-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://orcanium-agent.orcanium.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `orcanium model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.Orcanium/.env,
        ...
        # behavior: files must live under the Orcanium cache, under
        ...
        # user. Bridged to ORCANIUM_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Orcanium
        # cache (~/.Orcanium/cache/{documents,images,audio,video,screenshots})
        ...
        # to ORCANIUM_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ORCANIUM_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ORCANIUM_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.Orcanium/state.db.
    ...
    "sessions": {
        ...
        # default of ``orcanium sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.Orcanium/sessions/session_{sid}.json`` on every turn
    ...
    # ``orcanium update`` behaviour.
    "updates": {
        # Run a full ``orcanium backup``-style zip of ORCANIUM_HOME before every
        # ``orcanium update``.  Backups land in ``<ORCANIUM_HOME>/backups/`` and
        # can be restored with ``orcanium import <path>``.  Off by default —
        # on large ORCANIUM_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `orcanium update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ORCANIUM_HOME>/lsp/bin/`` on first use.
    ...
    # `orcanium tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.Orcanium/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.Orcanium/.env (or your shell) and never in config.yaml.
            ...
            # ~/.Orcanium/bin/ on first use.  When False you must install
            ...
            # `orcanium secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
# selection step (Orcanium Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "ORCANIUM_BASE_URL": {
        "description": "Orcanium Portal base URL override",
        "prompt": "Orcanium Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'orcanium model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Orcanium Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Orcanium Subscribers only, used to derive vendor hosts, e.g. orcanium.com -> firecrawl-channel.orcanium.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Orcanium Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Orcanium Subscriber access token for tool-gateway requests (optional; otherwise read from the Orcanium auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @Orcanium:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ORCANIUM_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #Orcanium)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: orcanium-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'orcanium-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Orcanium API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via channel.proxy_url in config.yaml.",
        "prompt": "Remote Orcanium API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Orcanium API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ORCANIUM_TOOL_PROGRESS and ORCANIUM_TOOL_PROGRESS_MODE are deprecated —
    ...
    "ORCANIUM_PREFILL_MESSAGES_FILE": {
    ...
    "ORCANIUM_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.orcanium.config`` entries, then checks
    ...
    try:
        ...
        # should never break `orcanium update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Orcanium versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``orcanium.cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
      * ``channel.run._format_session_info`` (``/info`` display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'orcanium setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Orcanium won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'orcanium doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ORCANIUM_HOME", "~/.Orcanium")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ORCANIUM_TOOL_PROGRESS")
            old_mode = get_env_value("ORCANIUM_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ORCANIUM_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ORCANIUM_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ORCANIUM_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ORCANIUM_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ORCANIUM_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_orcanium_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `orcanium plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `orcanium curator status` has no
    ...
    #   3. Creates `~/.Orcanium/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_orcanium_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_orcanium_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: orcanium config set <key> <value>")
    ...
    # metadata.orcanium.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _orcanium_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.Orcanium/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.Orcanium/config.yaml.
    ...
    (which change ``ORCANIUM_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — Orcanium auth) — OpenAI Codex
#   Orcanium         (OAuth — Orcanium auth) — Orcanium Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.Orcanium/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.Orcanium/.env.
    ...
    interactive menu render (`orcanium tools`, `orcanium setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `orcanium tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Orcanium env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.Orcanium/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.Orcanium/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.Orcanium/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.Orcanium/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Orcanium — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Orcanium vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.Orcanium/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Orcanium Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from orcanium.cli.auth import get_anthropic_key
    ...
    # Warn on stale ORCANIUM_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ORCANIUM_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ORCANIUM_MAX_ITERATIONS={_env_ghost} "
                f"(run 'orcanium doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  orcanium config edit     # Edit config file", Colors.DIM))
    print(color("  orcanium config set <key> <value>", Colors.DIM))
    print(color("  orcanium setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: orcanium config set <key> <value>")
            print()
            print("Examples:")
            print("  orcanium config set model anthropic/claude-sonnet-4")
            print("  orcanium config set terminal.backend docker")
            print("  orcanium config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'orcanium config migrate' to add them")
        ...
        print("  orcanium config           Show current configuration")
        print("  orcanium config edit      Open config in editor")
        print("  orcanium config set <key> <value>   Set a config value")
        print("  orcanium config check     Check for missing/outdated config")
        print("  orcanium config migrate   Update config with new options")
        print("  orcanium config path      Show config file path")
        print("  orcanium config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``orcanium config`` UI so users
