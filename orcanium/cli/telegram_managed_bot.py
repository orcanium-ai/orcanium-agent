...
manual BotFather token copy-paste. Orcanium talks only to the Orcanium onboarding
...
# Default pairing API base URL (Orcanium-hosted Cloudflare Worker).
# Override for PoC/staging with TELEGRAM_ONBOARDING_URL.
DEFAULT_API_URL = "https://setup.orcanium-agent.orcanium.com"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"

# The Orcanium-hosted manager bot username (without @). The backend returns the
# actual deep link, so this is only used by local helpers/tests.
DEFAULT_MANAGER_BOT = "OrcaniumSetupBot"

DEFAULT_BOT_NAME = "Orcanium Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``orcanium_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``orcanium_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"orcanium_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Orcanium Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Orcanium Telegram onboarding service.")
