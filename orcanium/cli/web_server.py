"""
Orcanium Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m orcanium.cli.main web          # Start on http://127.0.0.1:9119
    python -m orcanium.cli.main web --port 8080
"""
...
from channel.status import get_running_pid, read_runtime_status
from orcanium.cli.compat import env_var_enabled
...
try:
    ...
    # running `orcanium dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ORCANIUM_WEB_DIST"]) if "ORCANIUM_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``orcanium gateway run`` — but the
    desktop app spawns a ``orcanium dashboard`` backend, not a gateway, so a cron
    ...
    ORCANIUM_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (ORCANIUM_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `Orcanium
    # dashboard` is unaffected — it relies on its own channel.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ORCANIUM_DESKTOP") == "1":
...
app = FastAPI(title="Orcanium Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# ORCANIUM_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ORCANIUM_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Orcanium-Session-Token"
...
#
# This list is defined in ``orcanium.cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from orcanium.cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Orcanium-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from orcanium.cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Orcanium (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``orcanium model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Orcanium provider slug — fine.
    ...
    1. Vendor-name → Orcanium-provider mapping: when the provider string is
       not a known Orcanium provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from orcanium.cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from orcanium.cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from orcanium.cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "ORCANIUM_DASHBOARD_FILES_ROOT"
...
def _default_orcanium_root_is_opt_data() -> bool:
    raw = os.environ.get("ORCANIUM_HOME", "").strip()
    if not raw:
        return False
    try:
        from orcanium.channel.compat import get_default_orcanium_root

        root = get_default_orcanium_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Orcanium root is actually /opt/data
    # (the container/hosted layout) or when ORCANIUM_DASHBOARD_FILES_ROOT is set.
    if _default_orcanium_root_is_opt_data():
...
async def get_status():
    ...
    try:
        from channel.config import load_gateway_config
    ...
    # and which providers are registered so ``orcanium status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Orcanium Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from orcanium.cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the Orcanium home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_orcanium_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_orcanium_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Orcanium Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from orcanium.cli.auth import get_orcanium_auth_status

        auth = get_orcanium_auth_status() or {}
    except Exception:
        auth = {}
    ...
    try:
        ...
        feats = get_managed_capabilities(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_orcanium", False):
                    state = "via Orcanium Portal"
    ...
    return {
        ...
        "subscription_url": "https://portal.orcanium.com/manage-subscription",
        "features": features,
    }
...
async def run_prompt_size():
    try:
        proc = _spawn_orcanium_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_orcanium_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_orcanium_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchroorcanium*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from orcanium.cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.Orcanium/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_orcanium_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "orcanium-update": "orcanium-update.log",
...
def _spawn_orcanium_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``orcanium <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``orcanium.cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "orcanium.cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ORCANIUM_NONINTERACTIVE": "1"},
    }
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    """Spawn ``orcanium gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``orcanium gateway restart`` children race each other on the
    ...
    return _spawn_orcanium_action(["gateway", "restart"], "gateway-restart"), False
...
async def restart_gateway():
    """Kick off a ``orcanium gateway restart`` in the background."""
    try:
...
@app.post("/api/orcanium/update")
async def orcanium():
    """Kick off ``orcanium update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("orcanium-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "orcanium-update",
    ...
    try:
        proc = _spawn_orcanium_action(["update"], "orcanium-update")
    except Exception as exc:
        _log.exception("Failed to spawn orcanium update")
...
@app.get("/api/orcanium/update/check")
async def check_orcanium_update(force: bool = False):
    """Report whether a Orcanium update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/orcanium/update`` actually runs ``orcanium update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Orcanium version string
    ...
    try:
        ...
        if force:
            try:
                (get_orcanium_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.Orcanium/config.yaml`` under ``tts.``.
    """
    ...
    try:
        from orcanium.app.tools.tts_tool import text_to_speech_tool
        loop = asyncio.get_running_loop()
...
async def get_profiles_sessions(
    ...
    from orcanium.channel.compat import SessionDB
    from orcanium.cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Orcanium surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Orcanium supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# Mirrors the model.options JSON-RPC from rpc but uses REST so the
# Models page (which has no chat PTY open) can drive it.
# ---------------------------------------------------------------------------

# Canonical auxiliary task slots. Keep in sync with DEFAULT_CONFIG["auxiliary"]
# in orcanium.cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None):
    ...
    REST equivalent of the ``model.options`` JSON-RPC on rpc, so the
    ...
    """
    try:
        from orcanium.cli.inventory import build_models_payload, load_picker_context

        # include_unconfigured + picker_hints + canonical_order mirror the
        # rpc `model.options` JSON-RPC handler exactly, so every GUI
        # surface fed by this endpoint (Settings → Model, the first-run
        # onboarding picker) sees the SAME full provider universe `orcanium model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `orcanium model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    Orcanium this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Orcanium and None otherwise. `model` may be
    ...
    if slug == "orcanium":
        try:
            from orcanium.cli.models import (
                get_curated_orcanium_model_ids,
                get_pricing_for_provider,
                check_orcanium_free_tier,
                partition_orcanium_models_by_tier,
                union_with_portal_free_recommendations,
                union_with_portal_paid_recommendations,
            )
            from orcanium.cli.auth import get_provider_auth_state

            model_ids = get_curated_orcanium_model_ids()
            pricing = get_pricing_for_provider("orcanium") or {}
            free_tier = check_orcanium_free_tier(force_fresh=True)

            portal_url = ""
            try:
                state = get_provider_auth_state("orcanium") or {}
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_orcanium_models_by_tier(
            ...
            return {"provider": "orcanium", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (Orcanium) failed")
            return {"provider": "orcanium", "model": "", "free_tier": None}

    # Non-Orcanium: first curated model for the provider, matching prior behaviour.
    try:
        from orcanium.cli.inventory import build_models_payload, load_picker_context
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.Orcanium/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    """Synchroorcanium body of POST /api/model/set.
    ...
    if scope == "main":
        ...
        # When switching the main provider to Orcanium, mirror the CLI's
        # post-model-selection behaviour (orcanium.cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_orcanium_managed_defaults):
        # auto-route any *unconfigured* tools through the Orcanium Tool Gateway.
        # This is purely additive — apply_orcanium_managed_defaults skips every
        ...
        if provider.strip().lower() == "orcanium":
            try:
                from orcanium.cli.managed_capabilities import apply_orcanium_managed_defaults
                from orcanium.cli.tools_config import _get_platform_tools

                enabled = _get_platform_tools(
                    cfg, "cli", include_default_mcp_servers=False
                )
                changed = apply_orcanium_managed_defaults(
                    cfg,
                ...
                # Portal lookup hiccups / non-subscriber / non-Orcanium gating
                # must never block saving the model assignment.
                _log.debug("apply_orcanium_managed_defaults skipped", exc_info=True)

        save_config(cfg)

        # Register a named ``custom_providers`` entry for a custom/local
        # endpoint, mirroring the ``orcanium model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from orcanium.cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. Orcanium with $0 balance) keeps paying 402s
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Orcanium from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Orcanium to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Orcanium from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Orcanium to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Orcanium in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Orcanium through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Orcanium via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Orcanium through an IMAP/SMTP mailbox.",
        "docs_url": "https://orcanium-agent.orcanium.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Orcanium to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Orcanium inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Orcanium through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Orcanium to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Orcanium to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Orcanium as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://orcanium-agent.orcanium.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://orcanium-agent.orcanium.com/docs/user-guide/messaging/webhooks/",
...
def _messaging_platform_catalog() -> tuple[dict[str, Any], ...]:
    ...
    Built-in platforms come from ``channel.config.Platform`` (LOCAL is excluded).
    Plugin platforms come from ``channel.platform_registry.plugin_entries()``,
...
def _gateway_platform_config(platform_id: str):
    from channel.config import Platform, load_gateway_config
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.orcanium-agent.orcanium.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"OrcaniumDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Orcanium Agent").strip() or "Orcanium Agent"
...
def _restart_gateway_after_telegram_onboarding() -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Orcanium appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's ORCANIUM_HOME.
...
async def test_messaging_platform(platform_id: str, profile: Optional[str] = None):
    ...
    if not payload["enabled"]:
        message = f"{entry['name']} is disabled. Enable it, then restart the channel."
    ...
    return {
        "ok": False,
        "state": payload["state"],
        "message": "Setup looks complete, but the gateway has not reported a connection yet. Restart the channel.",
    }
...
# Anthropic, device-code for Orcanium/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``orcanium auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.Orcanium/.anthropic_oauth.json`` — Orcanium-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_orcanium_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_orcanium_oauth_credentials,
            _ORCANIUM_OAUTH_FILE,
        )
    except ImportError:
        read_orcanium_oauth_credentials = None  # type: ignore
        _ORCANIUM_OAUTH_FILE = None  # type: ignore

    orcanium_creds = None
    if read_orcanium_oauth_credentials:
        try:
            orcanium_creds = read_orcanium_oauth_credentials()
        except Exception:
            orcanium_creds = None
    if orcanium_creds and orcanium_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "orcanium_pkce",
            "source_label": f"Orcanium PKCE ({_ORCANIUM_OAUTH_FILE})",
            "token_preview": _truncate_token(orcanium_creds.get("accessToken")),
            "expires_at": orcanium_creds.get("expiresAt"),
            "has_refresh_token": bool(orcanium_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Orcanium even
    when they also have a separate Orcanium-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "device_code",
        "cli_command": "Orcanium auth add Orcanium",
        "docs_url": "https://portal.orcanium.com",
        "status_fn": None,  # dispatched via auth.get_orcanium_auth_status
    },
    {
        "id": "openai-codex",
        "name": "OpenAI OAuth (ChatGPT)",
        "flow": "device_code",
        "cli_command": "Orcanium auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "Orcanium auth add qwen-oauth",
    ...
    },
    {
        ...
        # as Orcanium's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "Orcanium auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "Orcanium auth add xai-oauth",
        "docs_url": "https://orcanium-agent.orcanium.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "Orcanium auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from orcanium.cli import auth as hauth
        if provider_id == "orcanium":
            raw = hauth.get_orcanium_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "orcanium_portal",
                "source_label": raw.get("portal_base_url") or "Orcanium Portal",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("orcanium_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Orcanium-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _ORCANIUM_OAUTH_FILE
            if _ORCANIUM_OAUTH_FILE.exists():
                _ORCANIUM_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from orcanium.cli.auth import clear_provider_auth
...
#          → persists to ~/.Orcanium/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (Orcanium, OpenAI Codex):
#     1. POST /api/providers/oauth/{Orcanium|openai-codex}/start
...
# Guarded so Orcanium web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Orcanium file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``orcanium auth add anthropic``.
    """
    from agent.anthropic_adapter import _ORCANIUM_OAUTH_FILE
    payload = {
    ...
    _ORCANIUM_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _ORCANIUM_OAUTH_FILE.with_name(
        f"{_ORCANIUM_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _ORCANIUM_OAUTH_FILE)
        try:
            _ORCANIUM_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "orcanium-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    """Initiate a device-code flow (Orcanium, OpenAI Codex, or MiniMax).
    ...
    if provider_id == "orcanium":
        ...
        portal_base_url = (
            os.getenv("ORCANIUM_PORTAL_BASE_URL")
        ...
        def _do_orcanium_device_request():
            with httpx.Client(
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_orcanium_device_request
        )
        sid, sess = _new_oauth_session("orcanium", "device_code")
        ...
        threading.Thread(
            target=_orcanium_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Orcanium's device-code
...
# the tokens exactly like `orcanium auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `orcanium auth add xai-oauth`'s credential-pool insert.
...
def _orcanium_poller(session_id: str) -> None:
    """Background poller that drives a Orcanium device-code flow to completion."""
    from orcanium.cli.auth import (
        _poll_for_token,
        refresh_orcanium_oauth_from_state,
    )
    from datetime import datetime, timezone
    import httpx
    with _oauth_sessions_lock:
    ...
    try:
        ...
        # Same post-processing as _orcanium_device_code_login (validate/refresh JWT)
        ...
        full_state = refresh_orcanium_oauth_from_state(
            auth_state,
            timeout_seconds=15.0,
            force_refresh=False,
        )
        from orcanium.cli.auth import persist_orcanium_credentials
        persist_orcanium_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: Orcanium login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("Orcanium device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_orcanium_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by orcanium. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``orcanium auth add minimax-oauth``.
    """
    from orcanium.cli.auth import (
        _minimax_poll_token,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from orcanium.cli.auth import _save_codex_tokens
...
async def poll_oauth_session(provider_id: str, session_id: str):
    ...
    Shared by the device-code flows (Orcanium, OpenAI Codex, MiniMax) and the
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `orcanium sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `orcanium sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_orcanium_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from orcanium.cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_orcanium_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from orcanium_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ORCANIUM_HOME)."""
    from orcanium.cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["orcanium_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ORCANIUM_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (orcanium.cli.mcp_config), so
# servers managed here show up under `orcanium mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from orcanium.cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from orcanium.cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from orcanium.cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from orcanium.cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # ORCANIUM_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def list_mcp_catalog(profile: Optional[str] = None):
    """Browse the Orcanium-approved MCP catalog (the optional-mcps/ manifests).
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds ORCANIUM_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_orcanium_action(
    ...
    # No git step — install synchroorcaniumly via the catalog API. install_entry
...
# without shell access.  Wraps channel.pairing.PairingStore directly.
# ---------------------------------------------------------------------------
...
#
# Wraps the same JSON store the CLI uses (orcanium.cli.webhook); the webhook
...
async def list_webhooks():
    import orcanium.cli.webhook as wh
...
# spawn the real `orcanium gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_orcanium_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_orcanium_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from orcanium.cli.auth import read_credential_pool
...
# `orcanium memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `orcanium memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_orcanium_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_orcanium_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_orcanium_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `orcanium import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_orcanium_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from orcanium.cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from orcanium.cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <orcanium_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_orcanium_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_orcanium_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``orcanium`` subprocess, and
...
# Human-readable labels for each hub source id (matches `orcanium skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Orcanium)",
    "orcanium-index": "Orcanium Index",
    "skills-sh": "skills.sh",
...
def _installed_hub_identifiers(profile: Optional[str] = None) -> dict:
    ...
    """
    try:
        from orcanium.app.tools.skills_hub import HubLockFile
...
async def list_skills_hub_sources(profile: Optional[str] = None):
    ...
    def _run():
        from orcanium.app.tools.skills_hub import create_source_router
...
async def search_skills_hub(
    ...
    def _run():
        from orcanium.app.tools.skills_hub import create_source_router, parallel_search_sources
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from orcanium.cli.skills_hub import _resolve_source_meta_and_bundle
        from orcanium.app.tools.skills_hub import create_source_router
...
async def scan_skill_hub(identifier: str = ""):
    ...
    def _run():
        ...
        from orcanium.cli.skills_hub import _resolve_source_meta_and_bundle
        from orcanium.app.tools.skills_hub import create_source_router, quarantine_bundle
        from orcanium.app.tools.skills_guard import scan_skill, should_allow_install
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`orcanium -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ORCANIUM_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_orcanium_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "orcanium setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ORCANIUM_HOME override so the write lands in the target
    ...
    """
    from orcanium.channel.compat import set_orcanium_home_override, reset_orcanium_home_override

    token = set_orcanium_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
        ...
        reset_orcanium_home_override(token)
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ORCANIUM_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ORCANIUM_HOME override. Returns the
    number of skills newly disabled.
    """
    from orcanium.channel.compat import set_orcanium_home_override, reset_orcanium_home_override
    from orcanium.cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's ORCANIUM_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``orcanium profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ORCANIUM_HOME).
    """
    from orcanium.cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``orcanium profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ORCANIUM_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``orcanium profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from orcanium.cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_orcanium_home()`` at call
       time — the context-local override from ``set_orcanium_home_override``
    ...
    retargeted to the *current* ``get_orcanium_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ORCANIUM_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
    ...
    from orcanium.app.tools import skills_tool as _skills_tool
    from orcanium.app.tools import skill_manager_tool as _skill_mgr

    token = None
    if not requested or requested.lower() == "current":
        profile_dir = get_orcanium_home()
    else:
        profile_dir = _resolve_profile_dir(requested)
        token = set_orcanium_home_override(str(profile_dir))

    with _SKILLS_PROFILE_LOCK:
        old_home = _skills_tool.ORCANIUM_HOME
        old_skills_dir = _skills_tool.SKILLS_DIR
        old_mgr_home = _skill_mgr.ORCANIUM_HOME
        old_mgr_skills_dir = _skill_mgr.SKILLS_DIR
        _skills_tool.ORCANIUM_HOME = profile_dir
        _skills_tool.SKILLS_DIR = profile_dir / "skills"
        _skill_mgr.ORCANIUM_HOME = profile_dir
        _skill_mgr.SKILLS_DIR = profile_dir / "skills"
        try:
            yield profile_dir if token is not None else None
        finally:
            _skills_tool.ORCANIUM_HOME = old_home
            _skills_tool.SKILLS_DIR = old_skills_dir
            _skill_mgr.ORCANIUM_HOME = old_mgr_home
...
async def get_skills(profile: Optional[str] = None):
    from orcanium.app.tools.skills_tool import _find_all_skills
    from orcanium.cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def create_skill(body: SkillCreate):
    ...
    """
    from orcanium.app.tools.skill_manager_tool import _create_skill
...
async def update_skill_content(body: SkillContentUpdate):
    ...
    from orcanium.app.tools.skill_manager_tool import _edit_skill
...
async def get_toolsets(profile: Optional[str] = None):
    from orcanium.cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``orcanium tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``orcanium tools`` picker shows
        ...
        if cat:
            for prov in _visible_providers(cat, config, force_fresh=True):
                ...
                providers.append({
                    ...
                    "requires_orcanium_auth": bool(prov.get("requires_orcanium_auth")),
                    "is_active": is_active,
                })
    return {
        "name": name,
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``orcanium tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.Orcanium/.env`` via ``save_env_value`` —
    the same store ``orcanium tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``orcanium tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``orcanium -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ORCANIUM_HOME the rest of the
...
#
# The endpoint spawns the same ``orcanium --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from orcanium.cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from orcanium.cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from orcanium.cli.dashboard_auth.audit import AuditEvent, audit_log
        from orcanium.cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``orcanium --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ORCANIUM_TUI_RESUME`` env var —
    matching what ``orcanium.cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``orcanium/tui`` does
    not parse its argv.

    ``ORCANIUM_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``rpc`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ORCANIUM_TUI_SIDECAR_URL`` so
    the spawned ``rpc.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `profile` (when set) scopes the ENTIRE chat to that profile by pointing
    ``ORCANIUM_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``rpc.entry`` it launches) resolves
    ``get_orcanium_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``orcanium -p <name>`` performs. The in-process
    ``ORCANIUM_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from orcanium.cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    argv, cwd = _make_tui_argv(PROJECT_ROOT / "orcanium" / "tui", tui_dev=False)
    env = os.environ.copy()
    try:
    ...
    env.setdefault("ORCANIUM_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ORCANIUM_TUI_INLINE", "1")

    if profile_dir is not None:
        env["ORCANIUM_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ORCANIUM_TUI_RESUME"] = resume

    if sidecar_url:
        env["ORCANIUM_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # attach URL, gatewayClient spawns its own `rpc.entry`, which
    # inherits the profile ORCANIUM_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ORCANIUM_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from orcanium.cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Orcanium inside WSL2 to use the dashboard's /chat "
...
#
# Drives the same `rpc.dispatch` surface Ink uses over stdio, so the
...
#
# The PTY-side ``rpc.entry`` opens /api/pub at startup (driven by
# ORCANIUM_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`orcanium.cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from orcanium.cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/orcanium/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /Orcanium`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ORCANIUM_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/Orcanium``)
        ...
        ``__ORCANIUM_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ORCANIUM_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ORCANIUM_BASE_PATH__="{prefix}";'
                f"window.__ORCANIUM_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ORCANIUM_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ORCANIUM_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ORCANIUM_BASE_PATH__="{prefix}";'
                f"window.__ORCANIUM_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/Orcanium`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Orcanium backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Orcanium Teal",         "description": "Classic dark teal — the canonical Orcanium look"},
    {"name": "default-large", "label": "Orcanium Teal (Large)", "description": "Orcanium Teal with bigger fonts and roomier spacing"},
    {"name": "Orcanium-blue",     "label": "Orcanium Blue",           "description": "Light mode — vivid Orcanium-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.Orcanium/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.Orcanium/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_orcanium_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.Orcanium/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as orcanium.cli.plugins):
    1. User plugins:    ~/.Orcanium/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.Orcanium/plugins/  (only if ORCANIUM_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from orcanium.cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_orcanium_home() / "plugins", "user"),
    ...
    # ``orcanium.cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ORCANIUM_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".Orcanium" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from orcanium.cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_orcanium_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"Orcanium auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from orcanium.cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from orcanium.cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from orcanium.cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from orcanium.cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.Orcanium/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.Orcanium/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"orcanium_dashboard_plugin_{plugin['name']}"
...
from orcanium.cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from orcanium.cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing ORCANIUM_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Orcanium Agent exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from plugins.dashboard_auth import Orcanium as _orcanium_plugin

                if _orcanium_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • orcanium: {_orcanium_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            raise SystemExit(
                ...
                f"(was the dashboard_auth/Orcanium plugin removed?).\n"
    ...
    # ORCANIUM_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"ORCANIUM_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  Orcanium Web UI → http://{host}:{actual_port}")
