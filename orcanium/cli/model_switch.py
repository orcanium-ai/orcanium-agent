...
- ``orcanium.cli.providers``        -- canonical provider identity + overlays
- ``orcanium.cli.model_normalize``  -- per-provider name formatting
...
from orcanium.cli.model_normalize import (
...
_ORCANIUM_MODEL_WARNING = (
    "Orcanium Research Orcanium 3 & 4 models are NOT agentic and are not designed "
    "for use with Orcanium Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Orcanium Research Orcanium 3 / Orcanium 4 chat families.
# The previous substring check (`"orcanium" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``orcanium-brain:qwen3-14b-ctx16k`` that just
# happen to carry "orcanium" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   Orcanium/Orcanium-3-Llama-3.1-70B, orcanium-4-405b, openrouter/orcanium3:70b
# Negative examples it must NOT match:
#   orcanium-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_ORCANIUM_ORCANIUM_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])Orcanium[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_orcanium_orcanium_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Orcanium Orcanium 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_ORCANIUM_ORCANIUM_NON_AGENTIC_RE.search(model_name))


def _check_orcanium_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Orcanium Orcanium 3/4 chat model."""
    if is_orcanium_orcanium_non_agentic(model_name):
        return _ORCANIUM_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``orcanium config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from orcanium.cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "orcanium")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "orcanium")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Orcanium, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    """Core model-switching pipeline shared between CLI and channel.
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'orcanium model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'orcanium doctor' — config issues detected:"
        ...
        from orcanium.cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from orcanium.cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from orcanium.cli.runtime_provider import _auto_detect_local_model
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from orcanium.cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in orcanium.cli.runtime_provider.resolve_runtime_provider;
    ...
    orcanium_warn = _check_orcanium_model_warning(new_model)
    if orcanium_warn:
        warnings.append(orcanium_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from orcanium.cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_orcanium_tier: bool = False,
    max_models: int = 8,
    current_model: str = "",
) -> List[dict]:
    """Detect which providers have credentials and list their curated models.

    Uses the curated model lists from orcanium.cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_orcanium_tier`` bypasses the short Orcanium tier cache for explicit
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from orcanium.cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by Orcanium provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "orcanium" pulls from the remote model-catalog manifest published at
    # https://orcanium-agent.orcanium.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Orcanium release. Falls back to the in-repo
    # _PROVIDER_MODELS["orcanium"] snapshot when the manifest is unreachable.
    curated["orcanium"] = get_curated_orcanium_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from orcanium.cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from orcanium.cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Orcanium-mapped providers ---
    from orcanium.cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from orcanium.cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for orcanium_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(orcanium_id)
        if (
            _alias_target
            and _alias_target != orcanium_id
        ...
        pconfig = PROVIDER_REGISTRY.get(orcanium_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ORCANIUM_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from orcanium.cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(orcanium_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `orcanium model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(orcanium_id)
        if not model_ids:
            model_ids = curated.get(orcanium_id, [])
            if orcanium_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(orcanium_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Orcanium-only providers (Orcanium, openai-codex, copilot, opencode-go) ---
    from orcanium.cli.providers import ORCANIUM_OVERLAYS
    from orcanium.cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Orcanium provider ID.
    # ORCANIUM_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Orcanium IDs ("copilot").
    _mdev_to_orcanium = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ORCANIUM_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Orcanium slug — e.g. "github-copilot" → "copilot"
        orcanium_slug = _mdev_to_orcanium.get(pid, pid)
        if orcanium_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(orcanium_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, orcanium_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or orcanium_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(orcanium_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", orcanium_slug, exc)
        ...
        # configured.
        if not has_creds and orcanium_slug == "anthropic":
            try:
                ...
                if (orcanium_creds and orcanium_creds.get("accessToken")) or \
        ...
        if orcanium_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(orcanium_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(orcanium_slug)
                model_ids = _ids if _ids else (curated.get(orcanium_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(orcanium_slug, []) or curated.get(pid, [])
        elif orcanium_slug == "orcanium":
            # Orcanium serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `orcanium model` picker deliberately shows ONLY the curated agentic
            ...
            # order. Mirror that exactly (see _model_flow_orcanium in main.py) so
            ...
            model_ids = curated.get("orcanium", [])
            try:
                from orcanium.cli.models import (
                    get_pricing_for_provider as _orcanium_pricing,
                    check_orcanium_free_tier as _orcanium_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from orcanium.cli.auth import get_provider_auth_state as _orcanium_state

                _pricing = _orcanium_pricing("orcanium") or {}
                _portal = ""
                try:
                    _st = _orcanium_state("orcanium") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _orcanium_free(force_fresh=force_fresh_orcanium_tier):
            ...
            if not model_ids:
                model_ids = curated.get(orcanium_slug, []) or curated.get(pid, [])
                if orcanium_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(orcanium_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": orcanium_slug,
            "name": get_label(orcanium_slug),
            "is_current": orcanium_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "orcanium",
        ...
        seen_slugs.add(orcanium_slug.lower())
        _record_builtin_endpoint(orcanium_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ORCANIUM_OVERLAYS (keeps /model in sync
    # with `orcanium model`).
    try:
        from orcanium.cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Orcanium'ss canonical write key (matches
            ...
            # Orcanium writes ``models:`` as a dict keyed by model id
            # (see orcanium.cli/main.py::_save_custom_provider); older
            ...
            if api_url and api_key and discover:
                try:
                    from orcanium.cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Orcanium'ss own writer uses; a hyphen variant is accepted
            ...
            # active model. Orcanium'ss own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, orcanium.cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`orcanium.cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from orcanium.cli.models import fetch_openrouter_models
