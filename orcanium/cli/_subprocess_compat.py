...
orcanium is developed on Linux / macOS and tested natively on Windows too.
...
# Electron exits, so the gateway never gets respawned after a `orcanium
...
def windows_detach_flags_without_breakaway() -> int:
    ...
    See ``channel_windows.py::_spawn_detached`` for the canonical
...
def windows_hide_flags() -> int:
    ...
    flash but also want to collect stdout/exit code synchroorcaniumly.
