"""Unified tool configuration for Orcanium Agent.

import orcanium.cli.compat  # noqa: F401

`orcanium tools` and `orcanium setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.orcanium/config.yaml under
...
from orcanium.cli.managed_capabilities import (
    apply_orcanium_managed_defaults,
    get_managed_capabilities,
)
from orcanium.cli.managed_identity import format_managed_gateway_capability_message
from orcanium.app.tools.tool_backend_helpers import fal_key_is_configured
from orcanium.cli.compat import base_url_hostname, is_truthy_value
...
from orcanium.cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ORCANIUM_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `orcanium tools` → Video Generation, which walks
...
# `orcanium tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from orcanium.cli.auth import _read_xai_oauth_tokens
        ...
        pass
    try:
        from orcanium.app.tools.xai_http import get_env_value as _xai_get_env_value
...
# Platform-scoped toolsets: only appear in the `orcanium tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``orcanium tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from orcanium.cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``orcanium tools`` checklist actually offers
    for ``platform``.
    ...
    so ``orcanium tools`` never claims to add or remove a toolset the user was
...
from orcanium.cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "badge": "subscription",
                "tag": "Managed OpenAI TTS via Gateway entitlement",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_orcanium_auth": True,
                "managed_orcanium_feature": "tts",
    ...
    "web": {
        ...
        #   - "Managed Gateway" — managed Firecrawl billed via Orcanium
        #     subscription (requires_orcanium_auth + override_env_vars).
        ...
        "providers": [
            {
                "name": "Managed Gateway",
                "badge": "subscription",
                "tag": "Managed Firecrawl via Gateway entitlement",
                "web_backend": "firecrawl",
                "env_vars": [],
                "requires_orcanium_auth": True,
                "managed_orcanium_feature": "web",
    ...
    "image_gen": {
        ...
        #   - "Managed Gateway" — managed FAL billed via the Orcanium
        #     subscription (requires_orcanium_auth + override_env_vars).
        ...
        "providers": [
            {
                "name": "Managed Gateway",
                "badge": "subscription",
                "tag": "Managed FAL image generation via Gateway entitlement",
                "env_vars": [],
                "requires_orcanium_auth": True,
                "managed_orcanium_feature": "image_gen",
        ...
        "providers": [
            {
                "name": "Managed Gateway",
                "badge": "subscription",
                "tag": "Managed FAL video generation via Gateway entitlement",
                "env_vars": [],
                "requires_orcanium_auth": True,
                "managed_orcanium_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                ...
                # routes through the managed queue channel.
    ...
    "x_search": {
        ...
        "setup_note": (
            "orcanium routes X searches through xAI's built-in x_search "
            ...
            "(uses the quota attached to your Managed Gateway key)."
        ),
        "icon": "🐦",
        "providers": [
            {
    ...
    "browser": {
        ...
        #     billed via managed Gateway (requires_orcanium_auth +
        ...
        "providers": [
            ...
            },
            {
                ...
                "badge": "subscription",
                "tag": "Managed Browser Use via Gateway entitlement",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_orcanium_auth": True,
                "managed_orcanium_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. ORCANIUM_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ORCANIUM_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ORCANIUM_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ORCANIUM_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ORCANIUM_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ORCANIUM_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ORCANIUM_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``orcanium update`` and
      by ``orcanium computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `orcanium update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / orcanium process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                from orcanium.cli.compat import display_orcanium_home
                _print_warning(f"    npm install failed - run manually: cd {display_orcanium_home()}/orcanium && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in orcanium directory)")
            return
        ...
        try:
            ...
            from orcanium.app.tools.browser_tool import (
                _chromium_installed,
                _running_in_docker,
            )
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/orcanium/orcanium:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                import orcanium.app.tools.browser_tool as _bt
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.orcanium/config.yaml")
        ...
        # Run the full `orcanium auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.orcanium/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from orcanium.cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: orcanium auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `orcanium auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: orcanium auth spotify")
        except Exception as exc:
        ...
        try:
            from orcanium.cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: orcanium plugins enable observability/langfuse")
        _print_info("    Restart orcanium for tracing to take effect.")
        _print_info("    Verify: orcanium plugins list")
        ...
        try:
            ...
            _print_info("    Run later: orcanium auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `orcanium auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: orcanium auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: orcanium auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``orcanium tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``orcanium tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: orcanium tools post-setup <key>")
        return 2
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"orcanium-{platform}"
    ...
    # "orcanium-cli" (which include all _ORCANIUM_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[orcanium-cli, spotify]`` after enabling Spotify via ``orcanium
        ...
        # (e.g. "orcanium-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``orcanium tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `orcanium tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"orcanium-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("orcanium-")}
    ...
    # shipped as a bundled plugin but user must opt in via `orcanium tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `orcanium tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by orcanium tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (orcanium-cli, orcanium-telegram, etc.)
    ...
    # Opening `orcanium tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "browser"}:
        features = get_managed_capabilities(config, force_fresh=force_fresh)
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_orcanium):
            return True
...
def _plugin_web_search_providers() -> list[dict]:
    ...
    """
    # Built-in fallback providers (when plugin system is unavailable)
    builtin = [
        {
            "name": "SearXNG",
            "badge": "free · self-hosted",
            "tag": "Self-hosted metasearch engine",
            "env_vars": [
                {"key": "SEARXNG_BASE_URL", "prompt": "SearXNG instance URL (e.g., http://localhost:8080)"},
            ],
            "web_backend": "searxng",
            "web_search_plugin_name": "searxng",
        },
        {
            "name": "DuckDuckGo",
            "badge": "free",
            "tag": "No API key required, rate-limited",
            "env_vars": [],
            "web_backend": "ddgs",
            "web_search_plugin_name": "ddgs",
        },
    ]
...
def _visible_providers(
    ...
    *,
    ...
    Orcanium-managed Tool Gateway rows (``managed_orcanium_feature``) are always
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # Orcanium-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_orcanium_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_orcanium_auth")
            and not provider.get("managed_orcanium_feature")
            and not features.orcanium_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_orcanium_feature") == "video_gen"
...
def _hidden_orcanium_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Orcanium Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Orcanium-managed rows were filtered out for unentitled
    ...
    (``ensure_orcanium_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `orcanium tools` gets a silent no-op
...
def _toolset_needs_configuration_prompt(
    ...
    *,
    ...
    if ts_key == "web":
        ...
        from orcanium.app.tools.web_tools import check_web_api_key
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_orcanium_message = _hidden_orcanium_gateway_message(
        cat,
        config,
    ...
    if len(providers) == 1:
        ...
        if hidden_orcanium_message:
            for line in hidden_orcanium_message.splitlines():
        ...
        # When the user is logged into Orcanium, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Orcanium.
        try:
            _orcanium_logged_in = bool(
                get_managed_capabilities(
                    config,
                    force_fresh=force_fresh,
                ).orcanium_auth_present
            )
        except Exception:
            _orcanium_logged_in = False
        ...
        for p in providers:
            ...
            # Mark Orcanium-managed entries. Logged-in paid subscribers get the
            ...
            sub_marker = ""
            if p.get("managed_orcanium_feature"):
                if _orcanium_logged_in:
...
def _is_provider_active(
    ...
    *,
    ...
    if video_plugin_name and not provider.get("managed_orcanium_feature"):
        video_cfg = config.get("video_gen", {})
        return isinstance(video_cfg, dict) and video_cfg.get("provider") == video_plugin_name

    managed_feature = provider.get("managed_orcanium_feature")
    if managed_feature:
        features = get_managed_capabilities(config, force_fresh=force_fresh)
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_orcanium
        ...
        if provider.get("tts_provider"):
            return (
                feature.managed_by_orcanium
                and cfg_get(config, "tts", "provider") == provider["tts_provider"]
            )
        if "browser_provider" in provider:
            current = cfg_get(config, "browser", "cloud_provider")
            return feature.managed_by_orcanium and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_orcanium and current == provider["web_backend"]
        return feature.managed_by_orcanium
...
def _fal_model_catalog():
    ...
    from orcanium.app.tools.image_generation_tool import FAL_MODELS, DEFAULT_MODEL
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on Orcanium auth, or run
...
def _configure_provider(
    ...
    *,
    ...
    # Orcanium-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Orcanium
    ...
    # "enable all tools" prompt (that lives in `orcanium model`).
    if managed_feature:
        from orcanium.cli.managed_capabilities import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_orcanium_portal_access,
        )

        if not ensure_orcanium_portal_access(
            capability=f"{provider.get('name', 'the Orcanium Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Managed Gateway access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_orcanium_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_orcanium_auth") and not managed_feature:
        features = get_managed_capabilities(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.orcanium_auth_present or not entitled:
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool use the quota attached to your Managed Gateway key.")
    ...
    # Orcanium-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Orcanium.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_orcanium_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_orcanium_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_managed_capabilities(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.orcanium_auth_present
        except Exception:
            _show_portal_hint = False
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Managed Gateway login + entitlement gate as _configure_provider:
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `orcanium tools` and `orcanium setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Orcanium Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://orcanium.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_orcanium_managed_defaults(
                config,
    ...
    print()
    from orcanium.cli.compat import display_orcanium_home
    print(color(f"  Tool configuration saved to {display_orcanium_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'orcanium' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    try:
        from orcanium.app.tools.mcp_tool import probe_mcp_server_tools
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, orcanium mcp
