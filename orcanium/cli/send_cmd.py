"""CLI subcommand: ``orcanium send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"Orcanium send: {file_path} is not a text file. --file reads the "
                ...
                f'  Orcanium send --to telegram "MEDIA:{file_path}"\n'
                f'  Orcanium send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  Orcanium send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"Orcanium send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"Orcanium send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    """
    try:
        from channel.channel_directory import (
            format_directory_for_display,
            load_directory,
        )
    except Exception as exc:
        print(f"Orcanium send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"Orcanium send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"Orcanium send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `orcanium gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.Orcanium/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_orcanium_env() -> None:
    """Populate ``os.environ`` from ``~/.Orcanium/.env`` AND bridge top-level
    ...
    startup that ``orcanium send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.Orcanium/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.Orcanium/config.yaml`` into
       ...
       via ``orcanium config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``orcanium send``
    ...
    # channel.config.load_gateway_config() sees them. Scalars only; don't
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.Orcanium/.env and ~/.Orcanium/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "Orcanium send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  Orcanium send --to telegram \"hello\"\n"
            "  Orcanium send --to discord:#ops --file report.md\n"
            "  Orcanium send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "Orcanium send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `orcanium send --help` stays fast and does not pull in
    # the full tool registry / gateway config stack.
    from orcanium.app.tools.send_message_tool import send_message_tool
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Orcanium "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.Orcanium/.env + ~/.Orcanium/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  Orcanium send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | Orcanium send --to telegram:-1001234567890\n"
            "  Orcanium send --to discord:#ops --file /tmp/report.md\n"
            "  Orcanium send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  Orcanium send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  Orcanium send --list                  # all platforms\n"
            "  Orcanium send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `orcanium send --list telegram`.",
    )
