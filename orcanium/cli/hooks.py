"""Orcanium hooks — inspect and manage shell-script hooks.

Usage::

    Orcanium hooks list
    Orcanium hooks test <event> [--for-tool X] [--payload-file F]
    Orcanium hooks revoke <command>
    Orcanium hooks doctor

Consent records live under ``~/.Orcanium/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.Orcanium/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``orcanium hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: Orcanium hooks {list|test|revoke|doctor}")
        print("Run 'Orcanium hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.Orcanium/config.yaml.")
        print("See `orcanium hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `orcanium hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `orcanium hooks test` and `orcanium hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Orcanium wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `orcanium hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `orcanium hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `orcanium hooks doctor`.")
