"""
Cron subcommand for Orcanium CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(Orcanium\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*Orcanium)"
    r"|(systemctl\s+(restart|stop|start)\s+.*Orcanium)"
    r"|(p?kill\s+.*orcanium.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'orcanium cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: orcanium gateway install", Colors.DIM))
        print(color("                    sudo orcanium gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    orcanium gateway install    # Install as a user service")
        print("    sudo orcanium gateway install --system  # Linux servers: boot-time system service")
        print("    orcanium gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `orcanium gateway restart` from a shell outside the channel.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: orcanium cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
