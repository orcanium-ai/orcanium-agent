...
the existing module-level functions in orcanium.cli.channel and
orcanium.cli.channel_windows directly. This protocol is a thin facade
...
``orcanium gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``orcanium gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged Orcanium
    ...
    detection always fails. Since every Orcanium runtime call inside the
    container drops to Orcanium via ``s6-setuidgid``, that silent failure
...
# in ``orcanium.cli.channel`` (systemd/launchd) and ``orcanium.cli.channel_windows``
...
# active (set via the ``orcanium -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in orcanium.cli.channel.
    ...
    def start(self, name: str) -> None:
        from orcanium.cli.channel import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from orcanium.cli.channel import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from orcanium.cli.channel import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from orcanium.cli.channel import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in orcanium.cli.channel."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from orcanium.cli.channel import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from orcanium.cli.channel import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from orcanium.cli.channel import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from orcanium.cli.channel import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``orcanium.cli.channel_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        ...
        if not channel_windows.is_installed():
            return False
...
#
# Per-profile gateways are registered dynamically when `orcanium profile create`
# runs inside the container (Phase 4). Static services (main-Orcanium, dashboard)
...
# UID/GID of the in-image ``orcanium`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the Orcanium user.
    ...
    (the ``orcanium`` user — UID 10000 — running every Orcanium runtime
    ...
    is dispatched as the Orcanium user.
    ...
    with Orcanium ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           Orcanium:Orcanium, 0755 (parent must already exist)
    ``svc_dir/event/``                     Orcanium:Orcanium, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 Orcanium:Orcanium, 0755
    ``svc_dir/supervise/event/``           Orcanium:Orcanium, 03730
    ``svc_dir/supervise/control``          Orcanium:Orcanium, 0660    (FIFO)
    ...
    slot's supervise/ was orcanium-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ORCANIUM_UID, _ORCANIUM_GID)
        except PermissionError:
            # Running as the Orcanium user already — directory is orcanium-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ORCANIUM_UID, _ORCANIUM_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is orcanium-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ORCANIUM_UID, _ORCANIUM_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``orcanium -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`orcanium profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-Orcanium, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ORCANIUM_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ORCANIUM_HOME=/data/Orcanium`` is honored at run
          ...
          4. Drops to the Orcanium user and exec's
             ``orcanium -p <profile> gateway run`` (or just ``Orcanium
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``orcanium gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ORCANIUM_HOME profile" (the implicit profile that exists at
        the top of $ORCANIUM_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``orcanium.cli.channel`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ORCANIUM_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``ORCANIUM_HOME``,
        ...
        allocator in ``orcanium.cli.profiles._allocate_gateway_port``).
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/orcanium/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export ORCANIUM_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "orcanium gateway run"
        else:
            gateway_cmd = f"orcanium -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid Orcanium {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ORCANIUM_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ORCANIUM_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ORCANIUM_HOME=/data/Orcanium``
        gets its logs under /data/orcanium/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``orcanium logs`` reads and what
        ...
        return (
            ...
            f': "${{ORCANIUM_HOME:=/opt/data}}"\n'
            f'log_dir="$ORCANIUM_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R Orcanium:Orcanium "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid Orcanium s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``orcanium gateway
        ...
        if pid is not None:
            try:
                from channel.status import write_planned_stop_marker
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        """Create the s6 service directory for a profile channel.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with Orcanium ownership
            ...
            # the Orcanium user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # Give s6-svscan a moment to reap. There's no synchroorcanium
        ...
        # is orcanium-owned (see ``_seed_supervise_skeleton``), and on
