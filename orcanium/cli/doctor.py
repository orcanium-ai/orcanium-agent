"""
Doctor command for Orcanium CLI.

Diagnoses issues with Orcanium Agent setup.
"""
...
from orcanium.cli.config import get_project_root, get_orcanium_home, get_env_path
...
_DHH = display_orcanium_home()  # user-facing display path (e.g. ~/.Orcanium or ~/.Orcanium/profiles/coder)

# Load environment variables from ~/.Orcanium/.env so API key checks work
_env_path = get_env_path()
load_orcanium_dotenv(orcanium_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from orcanium.cli.colors import Colors, color
from orcanium.cli.models import _ORCANIUM_USER_AGENT
from orcanium.channel.compat import OPENROUTER_MODELS_URL
from orcanium.cli.compat import base_url_host_matches
...
from orcanium.channel.compat import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.Orcanium/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("ORCANIUM_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ORCANIUM_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from orcanium.cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from orcanium.cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches orcanium.cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``orcanium --version`` reporting a stale version while
    ...
    """
    try:
        from orcanium.cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != orcanium.cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'orcanium update', or set "
            "orcanium_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-Orcanium and dashboard static services are up
    ...
    for static in ("main-Orcanium", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `orcanium profile create <name>`")
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        ...
        check_warn("Gateway service linger", f"(could not import channel helpers: {e})")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from orcanium.cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `orcanium`.
    os.environ.setdefault("ORCANIUM_INTERACTIVE", "1")

    # Handle `orcanium doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.Orcanium/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Orcanium Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`orcanium doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and orcanium.cli/__init__.py versions
    ...
    # Check ~/.Orcanium/.env (primary location for user config)
    env_path = ORCANIUM_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'orcanium setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'orcanium setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'orcanium setup' to create one")
                issues.append("Run 'orcanium setup' to create .env")
    
    # Check ~/.Orcanium/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ORCANIUM_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from orcanium.cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from orcanium.cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'orcanium config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from orcanium.cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.Orcanium/.env or run 'orcanium setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'orcanium setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'orcanium config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from orcanium.cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from orcanium.cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'orcanium setup' to migrate config")
                else:
                    issues.append("Run 'orcanium doctor --fix' or 'orcanium setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'orcanium doctor --fix'")
        except Exception:
            pass

        # Detect stale ORCANIUM_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ORCANIUM_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from orcanium.cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("ORCANIUM_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ORCANIUM_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `orcanium setup` run)",
                )
                if should_fix:
                    if remove_env_value("ORCANIUM_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ORCANIUM_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ORCANIUM_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ORCANIUM_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ORCANIUM_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'orcanium doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        from orcanium.cli.auth import (
            get_orcanium_auth_status,
            get_codex_auth_status,
            get_gemini_oauth_auth_status,
            get_minimax_oauth_auth_status,
        )

        orcanium_status = get_orcanium_auth_status()
        if orcanium_status.get("logged_in"):
            check_ok("Orcanium Portal auth", "(logged in)")
        else:
            check_warn("Orcanium Portal auth", "(not logged in)")
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Orcanium's own device-code flow — the Codex CLI is
    ...
    # disrupt the already-printed Orcanium/Codex/Gemini/MiniMax rows above.
    try:
        from orcanium.cli.auth import get_xai_oauth_auth_status
    ...
    if orcanium_home.exists():
        ...
        orcanium_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = orcanium_home / subdir_name
    ...
    soul_path = orcanium_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Orcanium a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Orcanium Agent Persona\n\n"
                "<!-- Edit this file to customize how Orcanium communicates. -->\n\n"
                "You are Orcanium, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = orcanium_home / "memories"
    ...
    state_db_path = orcanium_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from orcanium.channel.compat import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'orcanium doctor --fix' "
                        "(or 'orcanium sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = orcanium_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'orcanium doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "orcanium"
        ...
        _cmd_link = _cmd_link_dir / "orcanium"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(Orcanium not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/Orcanium → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/Orcanium points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/Orcanium → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/Orcanium — run 'orcanium doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/Orcanium exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/Orcanium not found",
                    "(Orcanium command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/Orcanium → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/Orcanium symlink — run 'orcanium doctor --fix'")
    ...
    try:
        from orcanium.channel.compat import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `orcanium doctor` invocation.
    ...
    if _npm_bin:
        ...
        # for a routine security check. The web and orcanium/tui workspaces are
        ...
        npm_audit_targets = [
            ...
            (PROJECT_ROOT, "orcanium/tui workspace", ["--workspace", "orcanium-tui"]),
        ...
        for npm_dir, label, audit_extra in npm_audit_targets:
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        # The web/orcanium/tui workspace advisories are in build-time
                        ...
                        # tree — in that case it is an npm bug, not a Orcanium one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'orcanium config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``orcanium doctor``.
    ...
    # never the legitimate source for `orcanium doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'orcanium setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ORCANIUM_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: Orcanium skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ORCANIUM_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: Orcanium memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: Orcanium memory setup",
                    "No Honcho API key — run 'orcanium memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run Orcanium memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: Orcanium memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: Orcanium memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from orcanium.cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "orcanium -p" in content:
                            _m = _re.search(r"orcanium -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'orcanium doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
