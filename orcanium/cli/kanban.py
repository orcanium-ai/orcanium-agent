"""CLI for the Orcanium Kanban board — ``orcanium kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/orcanium-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from orcanium.cli import kanban_db as kb
from orcanium.cli import kanban_swarm as ks
from orcanium.cli.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this ORCANIUM_HOME and its
    ...
    Used by ``orcanium kanban create`` (and callers) to warn when a task
    ...
    """
    try:
        from channel.status import get_running_pid  # type: ignore
    except Exception:
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`orcanium kanban daemon --force`)."
        )
    return (
        False,
        "No gateway is running — the task will sit in 'ready' until you "
        "start it. Run:\n"
        "    orcanium gateway start\n"
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Orcanium profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://orcanium-agent.orcanium.com/docs/user-guide/features/kanban "
            "or docs/orcanium-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # ORCANIUM_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `orcanium kanban boards switch <slug>` or the "
            "ORCANIUM_KANBAN_BOARD env var). Use `orcanium kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $ORCANIUM_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $ORCANIUM_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the channel. Use `orcanium gateway start`.",
    )
    ...
    # Undocumented escape hatch for users who truly cannot run the channel.
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.Orcanium/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $ORCANIUM_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $ORCANIUM_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``orcanium kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: Orcanium kanban <action> [options]\n"
                "Run 'Orcanium kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # env-var pin for the duration of this call. Using ORCANIUM_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `orcanium kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # ORCANIUM_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("ORCANIUM_PROFILE_NAME", "ORCANIUM_PROFILE"):
...
# ---------------------------------------------------------------------------
# Boards management (Orcanium kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``orcanium kanban boards <action>``.
    ...
    DB, so a fresh ORCANIUM_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `orcanium kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `orcanium kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `orcanium kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `orcanium kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `orcanium profile
    ...
    if profiles:
        ...
        print("No profiles found under ~/.Orcanium/profiles/.")
        print("Create one with `orcanium -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  orcanium gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `orcanium -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from orcanium.cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``orcanium kanban show <id>``
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("ORCANIUM_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("ORCANIUM_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    """Deprecated — the dispatcher now runs inside the channel.
    ...
    if not getattr(args, "force", False):
        print(
            "Orcanium kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the channel. To use kanban:\n"
            "\n"
            "    orcanium gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`orcanium kanban list --status ready` / "
                    f"`orcanium kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Orcanium profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from orcanium.cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from orcanium.cli import kanban_decompose as decomp
