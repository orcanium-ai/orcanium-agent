"""
Single source of truth for provider identity in Orcanium Agent.
...
2. **Orcanium overlays** — transport type, auth patterns, aggregator flags,
...
from orcanium.cli.compat import base_url_host_matches, base_url_hostname

logger = logging.getLogger(__name__)


# -- Orcanium overlay ----------------------------------------------------------
# Orcanium-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class OrcaniumOverlay:
    """Orcanium-specific provider metadata layered on top of models.dev."""
...
ORCANIUM_OVERLAYS: Dict[str, OrcaniumOverlay] = {
    "openrouter": OrcaniumOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "orcanium": OrcaniumOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.orcanium.com/v1",
    ),
    "openai-codex": OrcaniumOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": OrcaniumOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": OrcaniumOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": OrcaniumOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ORCANIUM_QWEN_BASE_URL",
    ),
    "google-gemini-cli": OrcaniumOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": OrcaniumOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": OrcaniumOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": OrcaniumOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": OrcaniumOverlay(
    ...
    "kimi-for-coding": OrcaniumOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": OrcaniumOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": OrcaniumOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": OrcaniumOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": OrcaniumOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": OrcaniumOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": OrcaniumOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": OrcaniumOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": OrcaniumOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": OrcaniumOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": OrcaniumOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": OrcaniumOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": OrcaniumOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": OrcaniumOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": OrcaniumOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": OrcaniumOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": OrcaniumOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": OrcaniumOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": OrcaniumOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "orcanium", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Orcanium overlays (for providers not in models.dev: Orcanium, openai-codex, etc.)
      2. models.dev catalog + Orcanium overlay
    ...
    try:
        ...
        # Stub returns [] when agent.models_dev isn't available
        if not hasattr(mdev_info, "env"):
            mdev_info = None
    except Exception:
        mdev_info = None

    overlay = ORCANIUM_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + Orcanium extra
    ...
    if overlay is not None:
        # Orcanium-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ORCANIUM_OVERLAYS
    if provider == "bedrock":
...
def resolve_provider_full(
    ...
    try:
        from agent.models_dev import get_provider_info as _mdev_provider
        mdev_info = _mdev_provider(canonical)
        if mdev_info is not None and hasattr(mdev_info, "env"):
            return ProviderDef(
                id=canonical,
                name=mdev_info.name,
                transport="openai_chat",
                api_key_env_vars=mdev_info.env if hasattr(mdev_info, "env") else (),
