"""Slash command definitions and autocomplete for the orcanium CLI.
...
from orcanium.cli.compat import is_truthy_value
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of orcanium config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal orcanium works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while orcanium is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.orcanium/skills/ for newly installed or removed skills",
    ...
    CommandDef("credits", "Show orcanium credit balance and top up", "Info"),
    ...
    CommandDef("update", "Update orcanium Agent to the latest version", "Info"),
    CommandDef("version", "Show orcanium Agent version", "Info", aliases=("v",)),
...
# Set of all command names + aliases recognized by the channel.
...
def should_bypass_active_session(command_name: str | None) -> bool:
    ...
    safety net in channel.run discards any command text that reaches
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`orcanium.cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/orcanium`` subcommand mapping, and
    ...
    """
    try:
        from orcanium.cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
Telegram only displays a small BotCommand menu in practice.  The full orcanium
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        from orcanium.app.tools.skills_tool import SKILLS_DIR
        ...
        # ``orcanium skills list`` and the agent's ``/skill-name`` dispatch but
        # silently excluded from channel slash menus (#8110).
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``orcanium skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``orcanium skills list`` and
...
# unrelated command landed. These claim their slots right after /orcanium,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /orcanium <command>).
...
# native slot, the alias spelling stays reachable via /orcanium reset).
...
# ``/orcanium <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/orcanium" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /orcanium credits on Slack.
_SLACK_VIA_ORCANIUM_ONLY = frozenset({"credits"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/orcanium <verb>`` subcommand.
    ...
    can still reach them via ``/orcanium <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/orcanium`` is always reserved as the first entry so the
    legacy ``/orcanium <subcommand>`` form keeps working for anything that
    ...
    # Reserve /orcanium as the catch-all top-level command.
    entries.append(("orcanium", "Talk to orcanium or run a subcommand", "[subcommand] [args]"))
    seen.add("orcanium")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ORCANIUM_ONLY:
            # Intentionally Slack-via-/orcanium only (see _SLACK_VIA_ORCANIUM_ONLY).
            return
    ...
    # everything except /orcanium, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://orcanium-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /orcanium handler.

    Maps both canonical names and aliases so /orcanium bg do stuff works
    the same as /orcanium background do stuff.

    Plugin-registered slash commands are included so ``/orcanium <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from orcanium.cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
