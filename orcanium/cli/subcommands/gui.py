"""``orcanium gui`` subcommand parser.
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the orcanium Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any orcanium CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--orcanium-root",
        help="Override the orcanium source root used by Desktop (sets ORCANIUM_DESKTOP_ORCANIUM_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets ORCANIUM_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
