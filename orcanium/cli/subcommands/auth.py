"""``orcanium auth`` subcommand parser.
...
def build_auth_parser(subparsers, *, cmd_auth: Callable) -> None:
    ...
    auth_add.add_argument("--portal-url", help="orcanium portal base URL")
    auth_add.add_argument("--inference-url", help="orcanium inference base URL")
    ...
    # auth setup — alias for `orcanium model`
    auth_subparsers.add_parser(
        "setup",
        help="Interactively configure providers and default models",
        description="Walk through selecting inference providers, authenticating, and choosing default models. Alias for ``orcanium model``.",
    )
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate orcanium with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ORCANIUM_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
