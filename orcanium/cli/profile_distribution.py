"""Profile distributions — shareable, packaged Orcanium profiles via git.

A distribution is a Orcanium profile published as a git repository (or
...
* ``orcanium profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``orcanium skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``orcanium profile``, not a parallel tree):

    Orcanium profile install <source> [--name N] [--alias] [--force] [--yes]
    Orcanium profile update  <name>  [--force-config] [--yes]
    Orcanium profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "orcanium-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            orcanium_requires=str(data.get("orcanium_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.orcanium_requires:
            out["orcanium_requires"] = self.orcanium_requires
        if self.author:
...
def check_orcanium_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Orcanium {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Orcanium distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``orcanium profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Orcanium profile distribution."
            )
...
def plan_install(
    ...
    from orcanium.cli import __version__ as orcanium_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Orcanium distribution."
        )

    # Version check up-front so we fail fast
    check_orcanium_requires(manifest.orcanium_requires, orcanium_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.Orcanium).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="orcanium_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `orcanium profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`orcanium profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="orcanium_dist_update_") as tmp:
        plan = plan_install(
