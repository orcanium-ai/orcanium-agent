...
``$ORCANIUM_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-orcanium-setup
(the stage2 hook) has chowned the volume and seeded $ORCANIUM_HOME, but
...
_STALE_RUNTIME_FILES = ("channel.pid", "processes.json")
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ORCANIUM_HOME``,
    not under ``profiles/``). The dispatcher in ``orcanium.cli.channel``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``orcanium gateway start`` (no ``-p``) targets. Without it,
    bare ``orcanium gateway start`` inside the container would land on
    ...
    ``$ORCANIUM_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        orcanium_home: The container's ORCANIUM_HOME (typically /opt/data).
            Profiles live under ``<orcanium_home>/profiles/<name>/``;
            the default profile lives at ``<orcanium_home>`` itself.
    ...
    # ``orcanium gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(orcanium_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(orcanium_home)
    ...
    profiles_root = orcanium_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `orcanium profile create` (config.yaml
            # is not — that comes later via `orcanium setup`). Use it as the
            ...
            # via ``orcanium -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(orcanium_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = orcanium_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("ORCANIUM_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "orcanium":
        args = args[1:]
...
def _cleanup_stale_runtime_files(profile_dir: Path) -> None:
    """Remove channel.pid and processes.json — they reference PIDs in
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `orcanium -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit Orcanium
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the Orcanium user) won't hit EACCES. See
...
def _write_reconcile_log(
    orcanium_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $ORCANIUM_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    orcanium_home = Path(os.environ.get("ORCANIUM_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        orcanium_home=orcanium_home, scandir=scandir,
    )
    for a in actions:
        print(
