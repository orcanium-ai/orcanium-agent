"""
Gateway subcommand for Orcanium CLI.

Handles: orcanium channel [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from orcanium.channel.status import terminate_pid
from orcanium.channel.restart import (
...
# display_orcanium_home is imported lazily at call sites to avoid ImportError
# when orcanium.channel.compat is cached from a pre-update version during `orcanium update`.
...
from orcanium.cli.colors import Colors, color

logger = logging.getLogger(__name__)

# =============================================================================
# Process Management (for manual channel runs)
# =============================================================================
...
def _get_service_pids() -> set:
    """Return PIDs currently managed by systemd or launchd channel services.

    Used to avoid killing freshly-restarted service processes when sweeping
    for stale manual channel processes after a service restart.  Relies on the
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "orcanium CLI that invoked this scan" from "real channel process".
    """
    if pid <= 1:
        return None
    try:
...
def _request_gateway_self_restart(pid: int) -> bool:
    """Ask a running channel ancestor to restart itself asynchroorcaniumly."""
...
def _graceful_restart_via_sigusr1(pid: int, drain_timeout: float) -> bool:
    """Send SIGUSR1 to a channel PID and wait for it to exit gracefully.

    SIGUSR1 is wired in channel/run.py to ``request_restart(via_service=True)``
    ...
    Args:
        ...
        drain_timeout: Seconds to wait for the process to exit after sending
            SIGUSR1.  Should be slightly larger than the channel's
    ...
    # ``_pid_exists`` helper in channel.status which does OpenProcess +
    # WaitForSingleObject on Windows.
    from orcanium.channel.status import _pid_exists
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``orcanium channel status`` from falsely counting the ``orcanium`` CLI that
    invoked it as a running channel instance (see #13242).
    """
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    """Best-effort process-table scan for channel PIDs.

    This supplements the profile-scoped PID file so status views can still spot
    a live channel when the PID file is stale/missing, and ``--all`` sweeps can
    discover gateways outside the current profile.
    """
    # Exclude the entire ancestor chain so the CLI process that invoked this
    # scan (e.g. ``orcanium channel status``) is never mistaken for a running
    # channel.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "orcanium_cli.main channel",
        "orcanium_cli.main --profile",
        "orcanium_cli.main -p",
        "orcanium_cli/main.py channel",
        "orcanium_cli/main.py --profile",
        "orcanium_cli/main.py -p",
        "orcanium channel",
        # Windows: only match invocations that actually carry the ``channel``
        # subcommand or the channel-dedicated console-script shim. Bare
        # ``orcanium.exe --profile`` / ``orcanium.exe -p`` would also match
        # ``orcanium.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive channel PIDs (Copilot review).
        "orcanium.exe channel",
        "orcanium-channel.exe",
        "channel/run.py",
    ]
    current_home = str(get_orcanium_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"orcanium_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ORCANIUM_HOME
        ...
        # ORCANIUM_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "orcanium_home=" in command_lc
            and f"orcanium_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    # behaviour — BUT it means every channel run produces two pythonw
    # PIDs with identical command lines (one launcher stub, one actual
    # interpreter) which is confusing in ``channel status`` output.
...
def find_gateway_pids(
    ...
    """Find PIDs of running channel processes.

    Args:
        exclude_pids: PIDs to exclude from the result (e.g. service-managed
            PIDs that should not be killed during a stale-process sweep).
        all_profiles: When ``True``, return channel PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``orcanium update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Orcanium profile are returned.
    """
    _exclude = set(exclude_pids or set())
    pids: list[int] = []
    if not all_profiles:
        try:
            from orcanium.channel.status import get_running_pid
...
def find_profile_gateway_processes(
    ...
    """Return running channel PIDs mapped to Orcanium profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from orcanium.channel.status import get_running_pid
        from orcanium.cli.profiles import list_profiles
    except Exception:
        return processes
    ...
    for profile in list_profiles():
        try:
            pid = get_running_pid(profile.path / "channel.pid", cleanup_stale=False)
        except Exception:
            continue
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "orcanium_cli.main"]
    if profile != "default":
        args.extend(["--profile", profile])
    args.extend(["channel", "run", "--replace"])
    return args


def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    """Relaunch a manually-run profile channel after its current PID exits."""
    if old_pid <= 0:
        return False

    # The watcher is a tiny Python subprocess that polls the old PID and
    # respawns the channel once it's gone.  Both legs of the chain need
    ...
    # propagate and the watcher/channel survive the CLI exiting.
    #
    ...
    # when the user closes the terminal, leaving ``orcanium update`` users
    # with no running channel until they re-invoke ``orcanium channel``
    ...
    from orcanium.cli._subprocess_compat import (
    ...
    watcher = textwrap.dedent(
        ...
        # Platform-appropriate detach for the respawned channel.  On POSIX
        ...
        # without breakaway the respawned channel would die when that job
        ...
        if sys.platform == "win32":
            ...
            try:
                ...
                # fallback in channel_windows._spawn_detached.
...
def _read_systemd_unit_environment(system: bool = False) -> dict[str, str]:
    """Parse the channel unit's ``Environment=`` directives.
...
def _sync_orcanium_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ORCANIUM_HOME``.

    Under ``sudo``, ``ORCANIUM_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_orcanium_home` falls back to ``/root/.Orcanium`` — the wrong
    profile. The unit file pins ``ORCANIUM_HOME`` for the actual channel
    ...
    unit_home = env.get("ORCANIUM_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ORCANIUM_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ORCANIUM_HOME"] = unit_home
...
def _read_systemd_unit_properties(
    ...
    """Return selected ``systemctl show`` properties for the channel unit."""
    selected_system = _select_systemd_scope(system)
    try:
        result = _run_systemctl(
            [
...
def _read_gateway_runtime_status() -> dict | None:
    try:
        from orcanium.channel.status import read_runtime_status
...
def _wait_for_systemd_service_restart(
    *,
    ...
    """Wait for the channel service to become active after a restart handoff."""
    import time
        ...
        if active_state == "active":
            if new_pid and (previous_pid is None or new_pid != previous_pid):
                ...
                if gateway_state == "startup_failed":
                    ...
                    print(
                        f"⚠ {scope_label} service process restarted (PID {new_pid}), but channel startup failed: {reason}"
                    )
                    return False
                if not printed_runtime_wait:
                    print(
                        f"⏳ {scope_label} service process started (PID {new_pid}); waiting for channel runtime..."
                    )
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}orcanium channel status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}orcanium channel restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    """Return a unified view of channel liveness for the current profile."""
    ...
    from orcanium.channel.compat import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            from orcanium.cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `orcanium channel`")
    print("  can refuse to start another copy until this process stops.")


def _print_other_profiles_gateway_status() -> None:
    """Print a summary of channel status across all profiles.

    Shown at the bottom of ``orcanium channel status`` output so users with
...
def _gateway_edit() -> None:
    """Interactively edit channel channels: reassign agent, update creds, toggle, delete."""
    ...
    if not channels:
        print("  No channel channels configured.")
        print("  Create one with: orcanium channel setup")
        return

    print("  Select channel channel to edit:")
    print()
    for idx, ch in enumerate(channels, start=1):
        agent = ch.get("config", {}).get("agent_name", "—")
        status = "enabled" if ch.get("enabled") else "disabled"
        print(f"    {idx}. {ch['id']:<30} → agent: {agent:<15} ({status})")
    print()
    try:
    ...
    current_agent = config.get("agent_name", "—")

    print()
    print(f"  Channel: {channel_id}")
    print(f"  Platform: {platform}")
    print(f"  Agent: {current_agent}")
    print(f"  Status: {'enabled' if channel.get('enabled') else 'disabled'}")
    print()
    print("  What would you like to change?")
    print()
    print("    1. Change agent association")
    print("    2. Update platform credentials")
    print("    3. Toggle enable/disable")
    print("    4. Delete this channel")
    print("    5. Cancel")
    print()
    try:
        action = input("  Enter number [1]: ").strip()
    except (EOFError, KeyboardInterrupt):
        print("\n  Cancelled.")
        return
    if not action:
    ...
    if action_num == 1:
        ...
        print()
        print("  Select new agent:")
        for idx, a in enumerate(agents, start=1):
            marker = " ← current" if a.name == current_agent else ""
            print(f"    {idx}. {a.name:<20} ({a.model_provider or '—'}/{a.model_name or '—'}){marker}")
        print()
        try:
        ...
        try:
            ...
            if idx < 0 or idx >= len(agents):
                print("  Invalid choice.")
                return
            new_agent = agents[idx].name
        except ValueError:
            print("  Invalid input.")
            return

        if new_agent == current_agent:
            print("  No change — same agent.")
            return

        # Build new channel ID
        new_channel_id = f"{platform}_{new_agent}"
        ...
        try:
            ...
            with _ur.urlopen(req, timeout=5) as resp:
                print(f"  ✓ Channel '{channel_id}' reassigned to agent '{new_agent}'")
            ...
            print(f"  ✗ Failed to reassign: {e}")

    elif action_num == 2:
        # Update credentials — re-run platform setup
        print(f"  Reconfiguring platform '{platform}'...")
        from orcanium.cli.setup import prompt_choice
        platforms = _all_platforms()
        matching = [p for p in platforms if p["id"] == platform]
        if matching:
            _configure_platform(matching[0])
            # Update channel config with new env vars
            config["agent_name"] = current_agent
            params = _up.urlencode({
            ...
            try:
                ...
                with _ur.urlopen(req, timeout=5) as resp:
                    print(f"  ✓ Credentials updated for '{channel_id}'")
            except Exception:
        ...
        try:
            ...
            with _ur.urlopen(req, timeout=5):
                print(f"  ✓ Channel '{channel_id}' {'enabled' if new_enabled else 'disabled'}")
        except Exception:
            print(f"  ✗ Failed to toggle channel.")

    elif action_num == 4:
        # Delete
        try:
            answer = input(f"  Delete channel '{channel_id}'? This cannot be undone. [y/N]: ").strip().lower()
        except (EOFError, KeyboardInterrupt):
            print("\n  Cancelled.")
            return
        if answer in ("y", "yes"):
            try:
                ...
                print(f"  ✗ Failed to delete channel.")
def _gateway_list() -> None:
    """List all agents with their channel channels and running status.

    Shows a single table cross-referencing agents, their model, configured
    channel platforms (Telegram, Discord, etc.), and current status.
    """
    try:
        from orcanium.app.agent.agent_manager import AgentManager
    ...
    agent_gateways: dict[str, list[tuple[str, bool]]] = {}
    for ch in channels:
        try:
            cfg = ch.get_config() or {}
            agent_name = cfg.get("agent_name", "")
            if not agent_name:
                agent_name = ch.id.split("_", 1)[-1] if "_" in ch.id else ""
        except Exception:
            continue
        if agent_name:
            agent_gateways.setdefault(agent_name, []).append((ch.platform, ch.enabled))

    # Print table
    print()
    print(f"  {'Agent':<20} {'Model':<25} {'Gateway':<20} {'Status':<10}")
    print(f"  {'─'*20} {'─'*25} {'─'*20} {'─'*10}")

    for a in agents:
        model = a.model_name or "—"
        gw_list = agent_gateways.get(a.name, [])
        status = a.status or "stopped"
        status_dot = "●" if status == "running" else "○"

        if gw_list:
            for i, (platform, enabled) in enumerate(gw_list):
                gw_label = f"{platform} {'✓' if enabled else '✗'}"
                if i == 0:
                    print(f"  {a.name:<20} {model:<25} {gw_label:<20} {status_dot} {status}")
                else:
                    print(f"  {'':<20} {'':<25} {gw_label:<20} {'':<10}")
        else:
            print(f"  {a.name:<20} {model:<25} {'—':<20} {status_dot} {status}")

    print()
    print(f"  {len(agents)} agent(s), {len(channels)} channel channel(s)")
    print()
    print("  orcanium channel setup   — configure messaging platforms for an agent")
    print("  orcanium channel run     — start the channel")
    print("  orcanium channel status  — show running gateways")


def kill_gateway_processes(
    force: bool = False, exclude_pids: set | None = None, all_profiles: bool = False
) -> int:
    """Kill any running channel processes. Returns count killed.
...
def stop_profile_gateway() -> bool:
    """Stop only the channel for the current profile (ORCANIUM_HOME-scoped).

    Uses the PID file written by start_gateway(), so it only kills the
    channel belonging to this profile — not gateways from other profiles.
    Returns True if a process was stopped, False if none was found.
    """
    try:
        from orcanium.channel.status import get_running_pid, remove_pid_file
    except ImportError:
        return False
    ...
    try:
        from orcanium.channel.status import write_planned_stop_marker
...
from orcanium.channel.compat import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Orcanium Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    """Return True for detached Windows channel runs that should ignore Ctrl+C.

    Foreground ``orcanium channel run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ORCANIUM_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ORCANIUM_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "orcanium-channel"
SERVICE_DESCRIPTION = "Orcanium Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ORCANIUM_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ORCANIUM_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from orcanium.channel.compat import get_default_orcanium_root

    home = get_orcanium_home().resolve()
    default = get_default_orcanium_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ORCANIUM_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(orcanium_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ORCANIUM_HOME is a named profile.

    For ``~/.Orcanium/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        orcanium_home: Optional explicit ORCANIUM_HOME path. Defaults to the current
            ``get_orcanium_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from orcanium.channel.compat import get_default_orcanium_root

    home = Path(orcanium_home or str(get_orcanium_home())).resolve()
    default = get_default_orcanium_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ORCANIUM_HOME.

    Default ``~/.Orcanium`` returns ``orcanium-channel`` (backward compatible).
    Profile ``~/.Orcanium/profiles/coder`` returns ``orcanium-channel-coder``.
    Any other ORCANIUM_HOME appends a short hash for uniqueness.
    """
...
class SystemScopeRequiresRootError(RuntimeError):
    """Raised when a system-scope channel operation is attempted as non-root.
...
def _raise_user_systemd_unavailable(
    ...
    msg = (
        ...
        f"{fix_hint}\n"
        "\n"
        "  Alternative: run the channel in the foreground (stays up until\n"
        "  you exit / close the terminal):\n"
        "    orcanium channel run"
    )
...
# Legacy service names from older Orcanium installs that predate the
# orcanium-channel rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (orcanium-channel-*.service) and unrelated third-party
# "orcanium" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("orcanium.service",)

# ExecStart content markers that identify a unit as running our channel.
...
def _find_legacy_orcanium_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Orcanium channel units.

    Detects unit files installed by older Orcanium versions that used a
    different service name (e.g. ``orcanium.service`` before the rename to
    ``orcanium-channel.service``). When both a legacy unit and the current
    ``orcanium-channel.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``orcanium-channel-coder.service`` and unrelated third-party
      ``orcanium-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our channel
      entrypoint. A user-created ``orcanium.service`` running an unrelated
    ...
    for is_system, base in _legacy_unit_search_paths():
        for name in _LEGACY_SERVICE_NAMES:
            ...
            if not any(marker in text for marker in _LEGACY_UNIT_EXECSTART_MARKERS):
                # Not our channel — leave alone
                continue
            results.append((name, unit_path, is_system))
    return results


def has_legacy_orcanium_units() -> bool:
    """Return True when any legacy Orcanium channel unit files exist."""
    return bool(_find_legacy_orcanium_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Orcanium channel unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_orcanium_units()
    if not legacy:
        return
    print_warning("Legacy Orcanium channel unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current orcanium-channel service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    orcanium channel migrate-legacy")


def remove_legacy_orcanium_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Orcanium channel unit files.

    Iterates over whatever ``_find_legacy_orcanium_units()`` returns — which is
    ...
    legacy = _find_legacy_orcanium_units()
    if not legacy:
        print("No legacy Orcanium channel units found.")
        return 0, []
    ...
    print()
    print("Legacy Orcanium channel unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: orcanium channel migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo orcanium channel migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_warning(
        f"Both user and system channel services are installed ({rendered_scopes})."
    )
    print_info("  This is confusing and can make start/stop/status behavior ambiguous.")
    print_info(
        "  Default channel commands target the user service unless you pass --system."
    )
    print_info("  Keep one of these:")
    print_info("    orcanium channel uninstall")
    print_info("    sudo orcanium channel uninstall --system")
...
def _require_root_for_system_service(action: str) -> None:
    if os.geteuid() != 0:  # windows-footgun: ok — POSIX systemd helper, never invoked on Windows
        raise SystemScopeRequiresRootError(
            f"System channel {action} requires root. Re-run with sudo.",
            action,
        )
...
def _system_service_identity(run_as_user: str | None = None) -> tuple[str, str, str]:
    ...
    if not username:
        raise ValueError(
            "Could not determine which user the channel service should run as"
        )
    if username == "root" and not run_as_user:
        raise ValueError(
            "Refusing to install the channel system service as root; pass --run-as-user root to override (e.g. in LXC containers)"
        )
    if username == "root":
        print_warning("Installing channel service to run as root.")
        print_info(
...
def prompt_linux_gateway_install_scope() -> str | None:
    choice = prompt_choice(
        "  Choose how the channel should run in the background:",
        [
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Orcanium can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo orcanium channel install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo orcanium channel install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo orcanium channel start --system")
            return scope, False

        if not run_as_user:
            while True:
                run_as_user = prompt(
                    "  Run the system channel service as which user?", default=""
                )
...
def print_systemd_linger_guidance() -> None:
    ...
    if linger_enabled is True:
        ...
        print("⚠ Systemd linger is disabled (channel may stop when you log out)")
        print("  Run: sudo loginctl enable-linger $USER")
    else:
        print(f"⚠ Could not verify systemd linger ({linger_detail})")
        print("  If you want the channel user service to survive logout, run:")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Orcanium often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.Orcanium`` → ``ai.orcanium.channel.plist`` (backward compatible).
    Profile ``~/.Orcanium/profiles/coder`` → ``ai.orcanium.channel-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.orcanium.channel-{suffix}" if suffix else "ai.orcanium.channel"
...
def _build_wsl_interop_paths(path_entries: list[str]) -> list[str]:
    ...
    ``/mnt/c/WINDOWS/System32``. systemd user services do not, so channel tools
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.Orcanium/orcanium-agent  -> /home/alice/.Orcanium/orcanium-agent
      /opt/Orcanium                 -> /opt/Orcanium  (kept as-is)
...
def _orcanium_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ORCANIUM_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_orcanium_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.Orcanium                    → /home/alice/.Orcanium
      /root/.Orcanium/profiles/coder     → /home/alice/.Orcanium/profiles/coder
      /opt/custom-Orcanium               → /opt/custom-Orcanium  (kept as-is)
    """
    current_orcanium = get_orcanium_home().resolve()
    current_default = (Path.home() / ".Orcanium").resolve()
    target_default = Path(target_home_dir) / ".Orcanium"

    # Default ~/.Orcanium → remap to target user's default
    if current_orcanium == current_default:
        return str(target_default)

    # Profile or subdir of ~/.Orcanium → preserve the relative structure
    try:
        relative = current_orcanium.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.Orcanium) — keep as-is
        return str(current_orcanium)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    orcanium_home = get_orcanium_home()
    orcanium_node = orcanium_home / "node" / "bin"
    if _is_dir(orcanium_node):
        candidates.append(str(orcanium_node))
    orcanium_nm = orcanium_home / "node_modules" / ".bin"
    if _is_dir(orcanium_nm):
        candidates.append(str(orcanium_nm))
...
def _stable_service_working_dir() -> str:
    ...
    The channel does NOT need its cwd to be the source checkout — ``ExecStart``
    uses an absolute python interpreter and ``-m orcanium.cli.main``, so module
    ...
    dir, or a clone that ``orcanium update`` later relocates/removes — the path
    ...
    ``ORCANIUM_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the channel is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ORCANIUM_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    # systemd's TimeoutStopSec must exceed the channel's drain_timeout so
    ...
    if system:
        ...
        orcanium_home = _orcanium_home_for_target_user(home_dir)
        profile_arg = _profile_arg(orcanium_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ORCANIUM_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(orcanium_home) if orcanium_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m orcanium.cli.main{f" {profile_arg}" if profile_arg else ""} channel run
...
Environment="ORCANIUM_HOME={orcanium_home}"
Restart=always
RestartSec=5
...
"""
    ...
    orcanium_home = str(get_orcanium_home().resolve())
    profile_arg = _profile_arg(orcanium_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ORCANIUM_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ORCANIUM_HOME
    ...
    exported a throwaway ``ORCANIUM_HOME=/tmp/...`` — writing it to the real
    service file silently breaks the user's channel on the next (re)start:
    the channel comes back "active (running)" but pointed at an empty temp
    home ("No messaging platforms enabled"), deaf to every platform.
    Seen live 2026-06-11: an E2E guard probe ran ``orcanium channel restart``
    with ``ORCANIUM_HOME=/tmp/orcanium-e2e-<pr>`` exported; the restart path's
    unit refresh baked the temp path into the production unit and the
    post-update restart produced a zombie channel for 7+ hours.

    Matches both systemd ``Environment="ORCANIUM_HOME=..."`` lines and launchd
    ``<key>ORCANIUM_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ORCANIUM_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ORCANIUM_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ORCANIUM_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the channel {kind}: ORCANIUM_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ORCANIUM_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ORCANIUM_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ORCANIUM_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../orcanium_test`` path into
    # ``Environment="ORCANIUM_HOME=..."``. Writing that to the developer's
    # real user systemd unit file silently breaks their channel on the next
    # reboot (systemd loads the polluted env, the channel looks at an empty
    ...
    if not system and (
        ...
        or '/orcanium_test"' in new_unit
        or "/orcanium_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # ORCANIUM_HOME into the unit (manual E2E homes like /tmp/orcanium-e2e-NNN
    ...
    print(
        f"↻ Updated channel {_service_scope_label(system)} service definition to match the current Orcanium install"
    )
    return True
...
def _print_linger_enable_warning(username: str, detail: str | None = None) -> None:
    print()
    print("⚠ Linger not enabled — channel may stop when you close this terminal.")
    if detail:
    ...
    print()
    print("  Then restart the channel:")
    print(f"    systemctl --user restart {get_service_name()}.service")
    print()


def _ensure_linger_enabled() -> None:
    """Enable linger when possible so the user channel survives logout."""
    ...
    print("Enabling linger so the channel survives SSH logout...")
    try:
        result = subprocess.run(
            ["loginctl", "enable-linger", username],
            capture_output=True,
    ...
    if result.returncode == 0:
        print("✓ Linger enabled — channel will persist after logout")
        return
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo orcanium channel uninstall --system")
    print_info("         orcanium channel install")
    print_info("         orcanium channel start")


def _get_restart_drain_timeout() -> float:
    """Return the configured channel restart drain timeout in seconds."""
...
def systemd_install(
    ...
    # Offer to remove legacy units (orcanium.service from pre-rename installs)
    # before installing the new orcanium-channel.service. If both remain, they
    # flap-fight for the Telegram bot token on every channel startup.
    # Only removes units matching _LEGACY_SERVICE_NAMES + our ExecStart
    # signature — profile units are never touched.
    if has_legacy_orcanium_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_orcanium_units(interactive=False)
            print()
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}orcanium channel start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}orcanium channel status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}orcanium channel install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_orcanium_home_from_systemd_unit(system=system)
    try:
        from orcanium.channel.status import get_running_pid, write_planned_stop_marker
    ...
    try:
        ...
        print(
            ...
            "check `orcanium channel status` or logs for final shutdown state."
        )
        return
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None:
        ...
        if _graceful_restart_via_sigusr1(pid, drain_timeout + 5):
            # The channel exits with code 75 for a planned service restart.
        ...
        try:
            ...
            print(
                ...
                "check `orcanium channel status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        print("⚠ Installed channel service definition is outdated")
        print(
            f"  Run: {'sudo ' if system else ''}orcanium channel restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        print(
            f"✓ {_service_scope_label(system).capitalize()} channel service is running"
        )
    else:
        print(
            f"✗ {_service_scope_label(system).capitalize()} channel service is stopped"
        )
        print(f"  Run: {'sudo ' if system else ''}orcanium channel start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        print("  ⏳ Restart pending: systemd is waiting to relaunch the channel")
    elif _systemd_unit_is_start_limited(unit_props):
        print("  ⏳ Restart pending: systemd is temporarily rate-limiting starts")
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}orcanium channel restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}orcanium channel start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.orcanium.channel-{suffix}" if suffix else "ai.orcanium.channel"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``orcanium channel start/stop/status`` call).
_resolved_launchd_domain: str | None = None


def _launchd_domain() -> str:
    """Return the launchd domain that actually manages the channel service.
...
# supervise the channel at all, so we degrade to a detached background process
# (the documented `nohup orcanium channel run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m orcanium.cli.main [--profile X] channel run --replace` argv.

    Profile-aware: honors the active ORCANIUM_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "orcanium_cli.main"]
    profile_arg = _profile_arg()
    if profile_arg:
        cmd.extend(profile_arg.split())
    cmd.extend(["channel", "run", "--replace"])
    return cmd


def _spawn_detached_gateway() -> bool:
    """Launch the channel as a detached background process (launchd fallback).

    Used when launchctl can no longer bootstrap/kickstart the channel on
    macOS 26+ (issue #23387). Mirrors the `nohup orcanium channel run --replace`
    workaround but keeps it CLI-managed: stdout/stderr go to the profile's
    channel logs and the PID is tracked via the channel.pid file that
    ...
    log_dir = get_orcanium_home() / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    out_path = log_dir / "channel.log"
    err_path = log_dir / "channel.error.log"
    try:
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    """Start the channel detached when launchd can't manage it, with guidance.

    Returns True if the detached channel was launched. When it can't be
    ...
    """
    from orcanium.channel.compat import display_orcanium_home as _dhh

    print(f"⚠ launchd cannot manage the channel on this macOS version ({reason}).")
    if _spawn_detached_gateway():
        print("✓ Started channel as a background process instead")
        print("  It will NOT auto-start at login or auto-restart on crash.")
        print(f"  Logs: {_dhh()}/logs/channel.log")
        print("  Stop it with: orcanium channel stop")
        return True
    print_error("Failed to start the channel as a background process.")
    print(
        f"  Try manually: nohup orcanium channel run --replace "
        f"> {_dhh()}/logs/channel.log 2>&1 &"
    )
    if exit_on_failure:
        sys.exit(1)
    return False
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>orcanium.cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>ORCANIUM_HOME</key>
        <string>{orcanium_home}</string>
    </dict>
    ...
    <string>{log_dir}/channel.log</string>
    
    <key>StandardErrorPath</key>
    <string>{log_dir}/channel.error.log</string>
</dict>
</plist>
"""
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated channel launchd service definition to match the current Orcanium install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  orcanium channel status             # Check status")
    from orcanium.channel.compat import display_orcanium_home as _dhh

    print(f"  tail -f {_dhh()}/logs/channel.log  # View logs")
...
def launchd_stop():
    ...
    # `orcanium channel start` re-bootstraps when it detects the job is unloaded.
    try:
...
def _wait_for_gateway_exit(
    ...
    """Wait for the channel process (by saved PID) to exit.

    Uses the PID from the channel.pid file — not launchd labels — so this
    works correctly when multiple channel instances run under separate
    ORCANIUM_HOME directories.
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Orcanium install")
    else:
        print("⚠ Service definition is stale relative to the current Orcanium install")
        print("  Run: orcanium channel start")
    ...
    if deep:
        log_file = get_orcanium_home() / "logs" / "channel.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) == "/opt/Orcanium"
        and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
    )


def _guard_official_docker_root_gateway() -> None:
    """Refuse channel startup when the official Docker privilege drop was bypassed."""
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ORCANIUM_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Orcanium channel as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'orcanium' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/orcanium/docker/entrypoint.sh before the Orcanium command."
    )
    print(
        "  Running the channel as root can leave root-owned files in "
        "$ORCANIUM_HOME and break later non-root dashboard/channel runs."
    )
    print(
        "  Set ORCANIUM_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)


def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, agent_name: str = None):
    """Run the channel in foreground.
    ...
    # Detached Windows channel runs must ignore console-control broadcasts
    # from sibling CLI processes, but foreground `orcanium channel run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ORCANIUM_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `orcanium channel restart` which already
    ...
    # `orcanium channel start/restart` — leaving the channel vulnerable to
    # the exact failure mode the new settings were meant to prevent.
    if supports_systemd_services():
        try:
            refresh_systemd_unit_if_needed(system=False)
        except Exception:
            pass  # best-effort; don't block channel startup

    from orcanium.app.domains.channel.manager import channel_runner

    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Orcanium Gateway                            │")
    print("├─────────────────────────────────────────────────────────┤")
    print("│  Using Orcanium Gateway Manager                         │")
    print("│  Press Ctrl+C to stop                                   │")
    print("└─────────────────────────────────────────────────────────┘")
    print()

    # ── Exit-path diagnostics ────────────────────────────────────────────
    # When the channel dies silently on Windows (no shutdown log, no
    # traceback in channel.log / errors.log), we're usually blind to the
    ...
    # ORCANIUM_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ORCANIUM_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from orcanium.channel.compat import get_orcanium_home as _ghh
            ...
            with open(log_dir / "channel-exit-diag.log", "a", encoding="utf-8") as f:
                f.write(_json.dumps(line, default=str) + "\n")
        except Exception:
            pass  # never let the diagnostic itself crash the channel
        ...
        if agent_name:
            ...
            try:
                ...
                if chan:
                    ...
                    gateway_runner.reload_channel(chan.id, "telegram", config, True)
            finally:
                db.close()
        else:
            gateway_runner.start_all()
        import time
        while True:
            time.sleep(3600)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. Orcanium) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Orcanium delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Orcanium agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Orcanium will use (e.g., Orcanium@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Orcanium connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: Orcanium pairing generate bluebubbles",
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.Orcanium/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if entry is not None:
        ...
        if entry.is_connected is not None:
            try:
                from orcanium.channel.config import PlatformConfig
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_orcanium_home() / "whatsapp" / "session" / "creds.json"
...
def _runtime_health_lines() -> list[str]:
    """Summarize the latest persisted channel runtime health state."""
    try:
        from orcanium.channel.status import read_runtime_status
    except Exception:
        return []
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from orcanium.cli.telegram_managed_bot import (
    ...
    for var in platform["vars"]:
        ...
        if var.get("is_allowlist"):
            ...
            print_info("  The channel DENIES all users by default for security.")
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'orcanium pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: Orcanium pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'orcanium channel setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from orcanium.cli.main import cmd_whatsapp
    import argparse
...
def _setup_wecom():
    ...
    if method_idx == 0:
        # ── QR scan flow ──
        try:
            from orcanium.channel.platforms.wecom import qr_scan_for_bot_info
        except Exception as exc:
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: Orcanium pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'orcanium channel setup'")
...
def _is_service_installed() -> bool:
    """Check if the channel is installed as a system service."""
    if supports_systemd_services():
        return (
        ...
        from orcanium.cli import channel_windows

        return channel_windows.is_installed()
    return False


def _is_service_running() -> bool:
    """Check if the channel service is currently running."""
    if supports_systemd_services():
        ...
        if channel_windows.is_installed():
            # "installed" doesn't necessarily mean "running" on Windows. The
            # canonical check is whether a channel process actually exists.
...
def _setup_qqbot():
    ...
    if method_idx == 0:
        ...
        try:
            from orcanium.channel.platforms.qqbot import qr_register
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "OrcaniumAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``orcanium.cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from orcanium.cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.Orcanium/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.Orcanium/.env: {', '.join(required)}")
    else:
        print_info(
            f"  Configure {label} in config.yaml under channel.platforms.{platform['key']}"
        )
...
def _register_gateway_channel(platform: str, agent_name: str) -> None:
    ...
    channel_id = f"{platform}_{agent_name}"
    env_prefix = platform.upper()
    config = {"agent_name": agent_name}
    print(f"  Registering channel '{channel_id}' with agent '{agent_name}'...")
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            if resp.status == 200:
                print(f"  ✓ Channel '{channel_id}' registered via API")
                return
    except Exception:
        pass
    ...
    try:
        ...
        try:
            ...
            if existing:
                ...
                db.add(channel)
            db.commit()
            print(f"  ✓ Channel '{channel_id}' registered (direct DB)")
        finally:
            db.close()
    except Exception as e:
...
def channel_setup(agent_name: str | None = None):
    """Interactive setup for messaging platforms + channel service.

    Args:
        agent_name: Pre-selected agent to associate with this channel.
            If None, shows an interactive agent picker.
    """
    if is_managed():
        managed_error("run channel setup")
        return
    ...
    if not agent_name:
        ...
        if len(agents) == 1:
            ...
            print()
            print("  Select an agent to configure the channel for:")
            print()
            for idx, a in enumerate(agents, start=1):
                print(f"    {idx}. {a.name:<20} ({a.model_provider or '—'}/{a.model_name or '—'})")
            print()
            try:
                choice = input("  Enter number [1]: ").strip()
            except (EOFError, KeyboardInterrupt):
            ...
            try:
                idx = int(choice) - 1
                if idx < 0 or idx >= len(agents):
                    print(f"  Invalid choice. Select 1-{len(agents)}.")
                    return
    ...
    print(
        color(
            f"│             ⚕ Gateway Setup — {agent_name:<28}│", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Configure messaging platforms and the channel service. │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    while True:
        ...
        _register_gateway_channel(platforms[choice]["key"], agent_name)

    # ── Post-setup: offer to install/restart channel ──
    ...
    if any_configured:
        ...
        if service_running:
            ...
            elif prompt_yes_no("  Restart the channel to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        systemd_restart()
                    elif is_macos():
                        ...
                        channel_windows.restart()
                    else:
                        stop_profile_gateway()
                        print_info("Start manually: orcanium channel")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                _print_system_scope_remediation("start")
            elif prompt_yes_no("  Start the channel service?", True):
                try:
                    if supports_systemd_services():
                        systemd_start()
                    elif is_macos():
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                start_now = prompt_yes_no("  Start the channel now?", True)
                start_on_login = prompt_yes_no(
                    f"  Start the channel automatically on login/boot as a {platform_name} service?{wsl_note}",
                    True,
                )
                if start_now or start_on_login:
                    try:
                        ...
                        if supports_systemd_services():
                            ...
                            channel_windows.install(force=False)
                            did_install = True
                        print()
                        ...
                        print_info("  You can try manually: orcanium channel install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: orcanium channel install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo orcanium channel install --system"
                        )
                    print_info("  Or run in foreground:  orcanium channel run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: orcanium channel run")
                print_info(
                    "  For persistence:   tmux new -s Orcanium 'orcanium channel run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: orcanium channel run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup orcanium channel run >{_dhh()}/logs/channel.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'orcanium channel setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    """If we're in a container with s6, dispatch channel lifecycle via s6.
    ...
    :mod:`orcanium.cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from orcanium.cli.service_manager import (
    ...
    if profile is None:
        ...
        # ORCANIUM_HOME=<root>/profiles/<name>, "" for the default root,
        # or a hash for unrelated paths. Map "" → "default" so the
        # default-profile channel is reachable as channel-default.
        profile = _profile_suffix() or "default"
    mgr = get_service_manager()
    service_name = f"channel-{profile}"
    try:
        if action == "start":
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    registered profile channel.

    Returns True iff dispatched (caller should ``return``); False
    otherwise — caller continues with the host-side code path.

    Without this, ``orcanium channel stop --all`` and ``... restart --all``
    fall through to ``kill_gateway_processes(all_profiles=True)``, which
    just ``pkill``s every channel process. s6-supervise observes the
    crash and restarts each one ~1s later — so ``--all`` ends up
    *kicking* every channel instead of *stopping* it. By iterating
    ...
    if succeeded:
        print(f"✓ {verb.capitalize()} {succeeded} profile channel(s) under s6")
    for profile, exc in errors:
        print(f"✗ Could not {action} channel-{profile}: {exc}")
    return True
...
def channel_command(args):
    ...
    try:
        return _channel_command_inner(args)
    except UserSystemdUnavailableError as e:
        ...
        # The direct ``orcanium channel install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    """Inside an s6 container, redirect bare ``channel run`` to the
    supervised path.

    Background. Before the s6 image landed, ``docker run <image> channel
    run`` was the standard way to start a containerized channel: the
    channel was the container's main process, tini reaped zombies, and
    container exit code == channel exit code. With s6-overlay as PID 1,
    we'd much rather have the channel run as a supervised s6 longrun
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``orcanium channel run`` are unaffected.
      2. ``ORCANIUM_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``orcanium channel
         run --replace`` as a longrun, this guard short-circuits the
         redirect so the supervised channel actually runs in
         foreground (otherwise we'd recurse: run → start → run → start
         → ...).
      3. ``--no-supervise`` (or ``ORCANIUM_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ORCANIUM_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("ORCANIUM_S6_SUPERVISED_CHILD"):
        # We ARE the supervised child s6-supervise is running. Fall
        # through to the foreground code path so the channel actually
        # starts.
        return False
    ...
    # supervised channel's own logs are routed by s6-log to both
    # `docker logs` and ${ORCANIUM_HOME}/logs/gateways/<profile>/current,
    # so the user sees a clear sequence: this banner first, then the
    # channel's own stdout/stderr from the supervisor.
    print(
        "→ channel is now running under s6 supervision (auto-restart on crash,\n"
        "  dashboard supervised alongside if ORCANIUM_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  channel will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ORCANIUM_GATEWAY_NO_SUPERVISE=1) to opt out\n"
        "  and get the pre-s6 foreground behavior instead.",
        file=sys.stderr,
        flush=True,
    )
    # Keep the CMD process alive as a no-op heartbeat. The supervised
    # channel's lifetime is independent of this process — s6-supervise
    # restarts it on crash, and we don't want the container to exit when
    # the channel flaps. The CMD process keeps /init alive until
    # `docker stop` sends SIGTERM, at which point /init runs stage 3
    # shutdown (which tears down the supervised channel cleanly).
    #
    # Prefer `sleep infinity` (matches the static main-Orcanium service's
    # pattern in docker/s6-rc.d/main-Orcanium/run, and frees the Python
...
def _pid_is_alive(pid: int) -> bool:
    """Check if a PID is currently alive via kill(0)."""
    import os
    try:
        os.kill(pid, 0)
        return True
...
def _start_gateway_background(agent_name: str, replace: bool = False) -> None:
    """Launch the channel as a detached background process for an agent.

    Writes a PID file so ``orcanium channel status`` can find it,
    then returns immediately.
    """
    import subprocess
    from pathlib import Path

    # Resolve agent model/provider for the status file
    agent_provider = "—"
    agent_model = "—"
    try:
        from orcanium.app.agent.agent_manager import AgentManager
        ...
        try:
            ...
            if agent:
                agent_provider = agent.model_provider or "—"
    ...
    # Check for existing channel process
    ethalon_path = get_orcanium_home()
    pid_file = ethalon_path / "channel.pid"
    if pid_file.exists():
        try:
            ...
            if _pid_is_alive(old_pid):
                if replace:
                    ...
                    print(f"  Gateway already running (PID {old_pid}). Use --replace to restart.")
                    return
        except (ValueError, OSError):
            pass

    # Build argv: re-invoke ourselves with --agent and --foreground so the
    # background child runs in foreground mode (it becomes the persistent
    # process while this CLI invocation returns).
    python = sys.executable
    script = Path(__file__).resolve().parent.parent / "cli" / "__main__.py"

    cmd = [python, str(script), "channel", "run", "--agent", agent_name, "--foreground"]

    # Platform-appropriate detach
    if sys.platform == "win32":
        try:
            from orcanium.cli.__subprocess_compat import windows_detach_popen_kwargs
    ...
    # Write agent metadata alongside PID for status display
    meta_file = ethalon_path / "channel.meta.json"
    import json
    ...
    print(f"  ✓ Gateway started for agent '{agent_name}' (PID {proc.pid})")
    print(f"    {agent_provider}/{agent_model}")
    print()
    print("  Run 'orcanium channel status' to see running gateways.")
    print("  Run 'orcanium channel stop' to stop the channel.")




def _stop_gateway_for_agent(agent_name: str) -> None:
    """Stop the channel for a specific agent by finding its channel PID."""
    import os
    import signal as _signal
        ...
        try:
            ...
            for ch in channels:
                ...
                if cfg.get("agent_name") == agent_name:
                    pid_file = ch.id.replace("/", "_") + ".pid"
                    # Try profile path first
                    from orcanium.cli.config import get_orcanium_home
                    pfile = get_orcanium_home() / f"gateway_{pid_file}"
                    if pfile.exists():
                        pid = int(pfile.read_text().strip())
                        os.kill(pid, _signal.SIGTERM)
                        print(f"  ✓ Stopped channel for agent '{agent_name}' (PID {pid})")
                        return
        finally:
            db.close()
    except Exception:
    ...
    # Fallback: try stopping by profile
    from orcanium.cli.channel import stop_profile_gateway
    stop_profile_gateway()
def _channel_command_inner(args):
    subcmd = getattr(args, "channel_command", None)
    ...
    if subcmd is None or subcmd == "run":
        ...
        if not agent_name:
            try:
                ...
                try:
                    ...
                    for ch in channels:
                        cfg = ch.get_config() or {}
                        aid = cfg.get("agent_name", "").strip()
                        if aid:
                            agents_from_db.add(aid)
                    if agents_from_db:
                        agent_list = sorted(agents_from_db)
                        if len(agent_list) == 1:
                            agent_name = agent_list[0]
                            print(f"  Using agent from channel config: {agent_name}")
                        else:
                            print()
                            print("  Multiple agents have channel channels configured.")
                            print("  Select which agent's channels to run:")
                            print()
                            for idx, a in enumerate(agent_list, start=1):
                                print(f"    {idx}. {a}")
                            print()
                            try:
                                choice = input("  Enter number [1]: ").strip()
        ...
        # Fallback: interactive agent selection when no channel channels exist yet# Start channel — background by default, foreground with --foreground
        if foreground:
            run_gateway(verbose, quiet=quiet, replace=replace, agent_name=agent_name)
        else:
            _start_gateway_background(agent_name, replace=replace)
        return

    if subcmd == "setup":
        agent_name = getattr(args, "agent", None)
        channel_setup(agent_name=agent_name)
        return

    # Service management commands
    if subcmd == "install":
        if is_managed():
            managed_error("install channel service (managed by NixOS)")
            return
        ...
        if is_termux():
            ...
            print("Run manually: orcanium channel")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: orcanium channel run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s Orcanium 'orcanium channel run'"
                )
                print()
            start_now = prompt_yes_no("Start the channel now after installing the service?", True)
            start_on_login = prompt_yes_no("Start the channel automatically on login/boot with systemd?", True)
            systemd_install(
                force=force,
                system=system,
            ...
            print("or run the channel in foreground mode:")
            print()
            print(
                "  orcanium channel run                              # direct foreground"
            )
            print(
                "  tmux new -s Orcanium 'orcanium channel run'         # persistent via tmux"
            )
            print(
                "  nohup orcanium channel run > ~/.Orcanium/logs/channel.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            # Phase 4: inside a container with s6 the channel service is
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  Orcanium profile create <name>     # creates the s6 service slot")
                print("  orcanium -p <name> channel start   # bring it up via s6")
                print("  orcanium status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the channel: orcanium channel run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: orcanium channel run")
            sys.exit(1)

    elif subcmd == "uninstall":
        if is_managed():
            managed_error("uninstall channel service (managed by NixOS)")
            return
        system = getattr(args, "system", False)
        if is_termux():
            print(
            ...
            print("Stop manual runs with: orcanium channel stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            channel_windows.uninstall()
        elif is_container():
            from orcanium.cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-unregistered when you delete the profile.")
                print()
                print("  Orcanium profile delete <name>     # tears down the s6 service slot")
                print("  orcanium -p <name> channel stop    # stop without deleting the profile")
                return
            print("Service uninstall is not applicable inside a Docker container.")
            print("To stop the channel, stop or remove the container:")
            print()
        ...
        # slot — start them individually via `orcanium -p <name> channel
        # start`), so just bring up the current profile's slot.
        if not start_all and _dispatch_via_service_manager_if_s6("start"):
            return

        if start_all:
            # Kill all stale channel processes across all profiles before starting
            killed = kill_gateway_processes(all_profiles=True)
            if killed:
                print(
                    f"✓ Killed {killed} stale channel process(es) across all profiles"
                )
                _wait_for_gateway_exit(timeout=10.0, force_after=5.0)
        ...
        if supports_systemd_services():
            ...
            print("Run the channel in foreground mode instead:")
            print()
            print(
                "  orcanium channel run                              # direct foreground"
            )
        ...
        stop_agent = getattr(args, "agent", None)
        if stop_agent:
            _stop_gateway_for_agent(stop_agent)
            return

        # Defense: refuse self-targeting channel stop from inside the channel.
        # Prevents agent-initiated kill loops when combined with supervisor KeepAlive.
        if os.getenv("_ORCANIUM_GATEWAY") == "1":
            print_error(
                "Refusing to stop the channel from inside the channel process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `orcanium channel stop` from a shell outside the running channel."
            )
            sys.exit(1)
        ...
        # Phase 4: inside a container with s6, dispatch via the service
        # manager. ``--all`` iterates every registered profile channel
        ...
        if stop_all:
            # --all: kill every channel process on the machine
            ...
            if total:
                print(f"✓ Stopped {total} channel process(es) across all profiles")
            else:
                print("✗ No channel processes found")
        else:
            # Default: stop only the current profile's channel
            service_available = False
            if supports_systemd_services() and (
                get_systemd_unit_path(system=False).exists()
                or get_systemd_unit_path(system=True).exists()
            ...
            if not service_available:
                ...
                if stop_profile_gateway():
                    print("✓ Stopped channel for this profile")
                else:
                    print("✗ No channel running for this profile")
            else:
                print(f"✓ Stopped {get_service_name()} service")

    elif subcmd == "restart":
        # Defense: refuse self-targeting channel restart from inside the channel.
        # Prevents agent-initiated kill loops when combined with supervisor KeepAlive.
        if os.getenv("_ORCANIUM_GATEWAY") == "1":
            print_error(
                "Refusing to restart the channel from inside the channel process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `orcanium channel restart` from a shell outside the running channel."
            )
            sys.exit(1)
        ...
        # iterates every registered profile channel through s6; without
        ...
        if restart_all:
            # --all: stop every channel process across all profiles, then start fresh
            ...
            if supports_systemd_services() and (
                ...
                # entry, channel_windows.start() uses the safe detached
                # pythonw.exe launcher.  Do not fall back to run_gateway() here:
                # when invoked from a channel-hosted agent/tool call, foreground
                # run_gateway() is tied to the very channel process we just
        ...
        if supports_systemd_services() and (
            ...
            # path that can be reaped with the old channel process.  If the
            # Windows backend raises, intentionally preserve the existing
            # generic failure fallback below.
            service_configured = channel_windows.is_installed()
            try:
                channel_windows.restart()
                return
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print(
                        "⚠ Cannot restart channel as a service — linger is not enabled."
                    )
                    print(
                        "  The channel user service requires linger to function on headless servers."
                    )
                    print()
                    print(f"  Run:  sudo loginctl enable-linger {_username}")
                    print()
                    print("  Then restart the channel:")
                    print("    orcanium channel restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: orcanium channel start")
                sys.exit(1)

            # Manual restart: stop only this profile's channel
            if stop_profile_gateway():
                print("✓ Stopped channel for this profile")
        ...
        # Stop, disable, and remove legacy Orcanium channel unit files from
        # pre-rename installs (e.g. orcanium.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_orcanium_units(interactive=not yes, dry_run=dry_run)
