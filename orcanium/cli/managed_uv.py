...
Orcanium owns its own uv binary at ``$ORCANIUM_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$ORCANIUM_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Orcanium keeps *its* uv binary.

    ``$ORCANIUM_HOME/bin/uv`` on POSIX, ``$ORCANIUM_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``orcanium update`` runs
    the call site from the *old*, already-imported ``orcanium.cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``orcanium update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$ORCANIUM_HOME/bin/`` instead of ``~/.local/bin/``.
    """
