"""
Orcanium Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with orcanium-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.Orcanium/plugins/<name>/``
3. **Project plugins** – ``./.Orcanium/plugins/<name>/`` (opt-in via
   ``ORCANIUM_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``orcanium_agent.plugins``
   entry-point group.
...
from orcanium.cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``ORCANIUM_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ORCANIUM_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``ORCANIUM_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.Orcanium/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ORCANIUM_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ORCANIUM_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Orcanium loggers.
    """
    ...
    logger.debug(
        "ORCANIUM_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "orcanium_agent.plugins"

_NS_PARENT = "orcanium_plugins"
...
class PluginManifest:
    ...
    #              in ~/.Orcanium/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``orcanium plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``orcanium honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``orcanium <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`orcanium.cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from orcanium.cli.dashboard_auth import (
    ...
    def register_platform(
        ...
        **entry_kwargs: Any,
        ...
        """
        from channel.platform_registry import platform_registry, PlatformEntry
    ...
    def register_slack_action_handler(
        ...
        Orcanium's Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``orcanium model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: orcanium.cli.main imports plugins indirectly
        from orcanium.cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.Orcanium/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("ORCANIUM_SAFE_MODE"):
            logger.info("ORCANIUM_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to orcanium.cli/. Two layouts are
        ...
        # 2. User plugins (~/.Orcanium/plugins/)
        user_dir = get_orcanium_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.Orcanium/plugins/)
        if _env_enabled("ORCANIUM_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".Orcanium" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ORCANIUM_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with Orcanium and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Orcanium ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `orcanium plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `orcanium plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``orcanium_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``orcanium_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    own loop so the caller still gets a concrete result synchroorcaniumly. The
    ...
    thread = threading.Thread(
        target=_runner,
        name="orcanium-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``orcanium tools`` TUI so plugin-provided toolsets appear
