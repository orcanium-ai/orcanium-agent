...
Add, remove, or reorder entries here — both `orcanium setup` and
`orcanium` provider-selection will pick up the change automatically.
"""
...
from orcanium.cli import __version__ as _ORCANIUM_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_ORCANIUM_USER_AGENT = f"orcanium-cli/{_ORCANIUM_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `orcanium model`
    flow without maintaining a separate static list.
    """
    from orcanium.cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ORCANIUM_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ORCANIUM_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Orcanium refreshes the cache from
...
# ---------------------------------------------------------------------------
# Orcanium Portal free-model helper
# ---------------------------------------------------------------------------
# The Orcanium Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Orcanium Portal account tier detection
# ---------------------------------------------------------------------------
def is_orcanium_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_orcanium_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Orcanium models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/Orcanium/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["orcanium"]`` list happens to contain or whether the
    ...
    older Orcanium that doesn't ship that model in its hardcoded curated
    ...
    * ``pricing`` gets a synthetic ``{"prompt": "0", "completion": "0"}``
      ...
      map, so :func:`partition_orcanium_models_by_tier` keeps it.

    Failures (network, parse, missing field) are silent and degrade to
    returning the inputs unchanged.
    """
    try:
        payload = fetch_orcanium_recommended_models(
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/Orcanium/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["orcanium"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Orcanium that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_orcanium_models_by_tier` keeps free models selectable;
...
def check_orcanium_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Orcanium Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_managed_gateway_identity(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# Orcanium Portal recommended models
#
...
ORCANIUM_RECOMMENDED_MODELS_PATH = "/api/Orcanium/recommended-models"
_ORCANIUM_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_orcanium_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def _orcanium_recommended_disk_path() -> "Path":
    """Disk path for the persisted recommended-models cache."""
    from orcanium.channel.compat import get_orcanium_home
    return get_orcanium_home() / "cache" / "orcanium_recommended_cache.json"


def _read_orcanium_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_orcanium_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_orcanium_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _orcanium_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "Orcanium recommended-models disk cache write failed: %s", exc
        )


def fetch_orcanium_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Orcanium Portal's curated recommended-models payload.

    Hits ``<portal>/api/Orcanium/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_ORCANIUM_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$ORCANIUM_HOME/cache/orcanium_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.orcanium.com").rstrip("/")
    now = time.monotonic()
    cached = _orcanium_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _ORCANIUM_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{ORCANIUM_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _orcanium_recommended_cache[base] = (data, now)
        _write_orcanium_recommended_disk(base, data)
        return data
    ...
    disk = _read_orcanium_recommended_disk(base)
    if disk:
        _orcanium_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_orcanium_portal_url() -> str:
    """Best-effort lookup of the Portal base URL the user is authed against."""
    try:
        from orcanium.cli.auth import (
            DEFAULT_ORCANIUM_PORTAL_URL,
        ...
        return str(DEFAULT_ORCANIUM_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.orcanium.com"
...
def get_orcanium_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_orcanium_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_orcanium_portal_url()
    payload = fetch_orcanium_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_orcanium_free_tier()
        except Exception:
...
# this list:  orcanium model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `orcanium model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `orcanium model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("orcanium",           "Orcanium Portal",              "Orcanium Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Orcanium provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `orcanium model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``orcanium model``, the
...
# sensible default, but Orcanium Portal is a per-token *metered aggregator* whose
...
# non-interactive fallback when a profile sets ``provider: Orcanium`` with no model
...
# path. The *interactive* default (GUI onboarding / ``orcanium model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in orcanium.cli/web_server.py and ``partition_orcanium_models_by_tier`` — which can
# hit the Portal; this fallback must stay cheap and network-free.
_PROVIDER_SILENT_DEFAULT_OVERRIDES: dict[str, str] = {
    "orcanium": "deepseek/deepseek-v4-flash",
}
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``orcanium auth add openai-codex`` without
    ``orcanium model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``orcanium model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    orcanium-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Orcanium Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from orcanium.cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — orcanium-agent
...
def get_curated_orcanium_model_ids() -> list[str]:
    """Return the curated Orcanium Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["orcanium"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from orcanium.cli.model_catalog import get_curated_orcanium_models
        remote = get_curated_orcanium_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("orcanium", []))
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Orcanium Portal).
    """
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": _ORCANIUM_USER_AGENT,
    }
    if api_key:
...
_DEFAULT_ORCANIUM_INFERENCE_BASE = "https://inference-api.orcanium.com"


def _resolve_orcanium_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Orcanium Portal pricing.

    The Orcanium inference ``/v1/models`` endpoint exposes pricing without
    ...
    """
    try:
        from orcanium.cli.auth import resolve_orcanium_runtime_credentials
        creds = resolve_orcanium_runtime_credentials()
        if creds:
            return (creds.get("api_key", ""), creds.get("base_url", ""))
    except Exception:
        pass
    return ("", _DEFAULT_ORCANIUM_INFERENCE_BASE)


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, Orcanium, novita)."""
    ...
    if normalized == "orcanium":
        api_key, base_url = _resolve_orcanium_pricing_credentials()
        if base_url:
            # Orcanium base_url typically looks like https://inference-api.orcanium.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``orcanium model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from orcanium.cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        orcanium:orcanium-3                           →  ("orcanium", "orcanium-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Orcanium, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"orcanium", "openrouter", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from orcanium.cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model Orcanium` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Orcanium's canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``orcanium.cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Orcanium should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``orcanium auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.Orcanium/.env``.
    ...
    """
    try:
        from orcanium.cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from orcanium.cli.copilot_auth import (
...
#   - "orcanium": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Orcanium),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Orcanium release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
        return model_ids(force_refresh=force_refresh)
    if normalized == "openai-codex":
        from orcanium.cli.codex_models import get_codex_model_ids
        ...
        # a Orcanium release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from orcanium.cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "orcanium":
        # Try live Orcanium Portal /models endpoint
        try:
            from orcanium.cli.auth import fetch_orcanium_models, resolve_orcanium_runtime_credentials
            creds = resolve_orcanium_runtime_credentials()
            if creds:
                live = fetch_orcanium_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["orcanium"] snapshot — so newly
        # added Portal models still surface without a Orcanium release.
        manifest_ids = get_curated_orcanium_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``orcanium model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $ORCANIUM_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from orcanium.channel.compat import get_orcanium_home
    return get_orcanium_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, Orcanium portal), the
    relevant tokens live in ``$ORCANIUM_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from orcanium.cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "OrcaniumAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ORCANIUM_USER_AGENT}
...
def _lmstudio_fetch_raw_models(
    ...
    try:
        ...
        if exc.code in {401, 403}:
            from orcanium.cli.auth import AuthError
            raise AuthError(
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Orcanium's default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ORCANIUM_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from orcanium.channel.compat import get_orcanium_home
    return get_orcanium_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Orcanium will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Orcanium cannot verify the model name."
