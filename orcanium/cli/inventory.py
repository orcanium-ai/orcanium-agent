...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Orcanium /models returns ~400
...
def load_picker_context() -> ConfigContext:
    ...
    ``rpc/server.py`` (×2 sites) used to do.
    """
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Orcanium, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``orcanium model`` CLI picker. Adds network calls
      (pricing fetch + Orcanium tier check); only set for interactive pickers.
    ...
    - ``force_fresh_orcanium_tier``: bypass the short Orcanium free-tier cache when
      selecting Portal-recommended Orcanium models and applying tier gating. Keep
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_orcanium_tier=force_fresh_orcanium_tier,
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_orcanium_tier=force_fresh_orcanium_tier)
    if capabilities:
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from orcanium.cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `orcanium model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from orcanium.cli.models import CANONICAL_PROVIDERS
...
def _apply_pricing(
    ...
    *,
    ...
    """Enrich each provider row with per-model pricing + Orcanium tier gating.

    Mutates ``rows`` in-place. For every row whose provider supports live
    pricing (openrouter / Orcanium / novita) adds::

        row["pricing"] = {model_id: {"input": "$3.00", "output": "$15.00",
                                     "cache": "$0.30" | None, "free": bool}}

    For Orcanium additionally adds::
    ...
    # Resolve Orcanium free-tier once (cached in models.py for the TTL window).
    orcanium_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        if slug == "orcanium":
            try:
                if orcanium_free_tier is None:
                    orcanium_free_tier = check_orcanium_free_tier(
                        force_fresh=force_fresh_orcanium_tier
                    )
                row["free_tier"] = bool(orcanium_free_tier)
                if orcanium_free_tier:
                    _selectable, unavailable = partition_orcanium_models_by_tier(
