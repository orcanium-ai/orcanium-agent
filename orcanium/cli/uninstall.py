"""
Orcanium Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.Orcanium/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Orcanium PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing orcanium-agent or Orcanium PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Orcanium Agent" comment and following line
                if '# Orcanium Agent' in line or '# orcanium-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('orcanium' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing Orcanium
                if 'orcanium' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the Orcanium wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "orcanium",
        Path("/usr/local/bin/Orcanium"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains orcanium.cli reference)
                content = wrapper.read_text()
                if 'orcanium.cli' in content or 'orcanium-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(orcanium_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``orcanium`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Orcanium home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (orcanium_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - Windows: Scheduled Task + Startup-folder fallback, via ``channel_windows``
    - All platforms: standalone ``orcanium gateway run`` processes
    ...
    try:
        from orcanium.cli.channel import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from orcanium.cli.channel import get_launchd_plist_path
    ...
    #    channel_windows module already knows how to locate and remove both
    ...
    #    uninstall logic stays in exactly one place.
        ...
        try:
            ...
            if channel_windows.is_installed() or channel_windows.is_task_registered() \
                    or channel_windows.is_startup_entry_installed():
                try:
...
#
#   1. Sets User-scope env vars ``ORCANIUM_HOME`` and ``ORCANIUM_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\Orcanium\git\cmd``, ``%LOCALAPPDATA%\Orcanium\git\bin``,
#      ``%LOCALAPPDATA%\Orcanium\git\usr\bin``, ``%LOCALAPPDATA%\Orcanium\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\Orcanium\git\`` and Node to
#      ``%LOCALAPPDATA%\Orcanium\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``orcanium dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\Orcanium\gateway-service\`` and sometimes
#      ``%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\`` — the
#      latter is handled by ``channel_windows.uninstall()`` already.
#
...
def _orcanium_path_markers(orcanium_home: Path) -> list[str]:
    """Path-entry substrings that identify Orcanium-owned User-PATH entries."""
    root = str(orcanium_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare orcanium-agent install dir.
    markers = [root + "\\orcanium-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ORCANIUM_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "orcanium".  We don't want to catch
    # unrelated entries like "corcanium-foo" or "ephermeral", so we look for
    # backslash-Orcanium as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(orcanium_home: Path) -> list[str]:
    """Strip Orcanium-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _orcanium_path_markers(orcanium_home)
            kept: list[str] = []
...
def remove_orcanium_env_vars_windows() -> list[str]:
    """Delete ORCANIUM_HOME and ORCANIUM_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ORCANIUM_HOME", "ORCANIUM_GIT_BASH_PATH"):
                try:
...
def _is_default_orcanium_home(orcanium_home: Path) -> bool:
    """Return True when ``orcanium_home`` points at the default (non-profile) root."""
    try:
        from orcanium.channel.compat import get_default_orcanium_root
        return orcanium_home.resolve() == get_default_orcanium_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ORCANIUM_HOME directory.

    We shell out to ``orcanium -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ORCANIUM_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m orcanium.cli.main` so we don't depend on a `orcanium`
    #    wrapper that may be half-removed mid-uninstall.
    orcanium_invocation = [_sys.executable, "-m", "orcanium.cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                orcanium_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ORCANIUM_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``orcanium uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ORCANIUM_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from orcanium.cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(orcanium_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Orcanium Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Orcanium Chat GUI installation was found.")
        print(f"  Checked: {orcanium_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Orcanium agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(orcanium_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Orcanium agent at {orcanium_home / 'orcanium-agent'}")
        print(f"  • Your config, sessions, and secrets under {orcanium_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Orcanium agent is still installed. Run 'orcanium' to use the CLI,")
    print("or 'orcanium uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.Orcanium/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.Orcanium/ for future reinstall
    """
    project_root = get_project_root()
    orcanium_home = get_orcanium_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ORCANIUM_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_orcanium_home(orcanium_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.Orcanium data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Orcanium Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {orcanium_home / 'config.yaml'}")
    print(f"  Secrets: {orcanium_home / '.env'}")
    print(f"  Data:    {orcanium_home / 'cron/'}, {orcanium_home / 'sessions/'}, {orcanium_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ORCANIUM_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Orcanium data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Orcanium code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``orcanium`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ORCANIUM_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    log_info("Checking for running channel...")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in orcanium_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\Orcanium\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(orcanium_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Orcanium-owned PATH entries in User environment")

        log_info("Removing ORCANIUM_HOME / ORCANIUM_GIT_BASH_PATH User env vars...")
        removed_env = remove_orcanium_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Orcanium-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing Orcanium command...")
    ...
    #     (only when they still point into this Orcanium home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Orcanium-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(orcanium_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Orcanium-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(orcanium_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ORCANIUM_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from orcanium.cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(orcanium_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.Orcanium/, just remove the orcanium-agent subdir
            if orcanium_home in project_root.parents or project_root.parent == orcanium_home:
    ...
    #     under ORCANIUM_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(orcanium_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(orcanium_home)
    ...
    # 5. Optionally remove ~/.Orcanium/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ORCANIUM_HOME dirs live under
        ...
        try:
            if orcanium_home.exists():
                shutil.rmtree(orcanium_home)
                log_success(f"Removed {orcanium_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {orcanium_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {orcanium_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {orcanium_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://orcanium-agent.orcanium.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://orcanium-agent.orcanium.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Orcanium Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m orcanium.cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import orcanium.app.cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``orcanium.channel.compat`` + ``orcanium.cli.colors``
    (and lazily ``orcanium.cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m orcanium.cli.uninstall")
    parser.add_argument(
        "--mode",
