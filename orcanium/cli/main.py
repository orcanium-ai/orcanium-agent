#!/usr/bin/env python3
"""
orcanium CLI - Main entry point.

Usage:
    orcanium                     # Interactive chat (default)
    orcanium chat                # Interactive chat
    orcanium channel run         # Run messaging channels in foreground
    orcanium channel start       # Start channel service
    orcanium channel stop        # Stop channel service
    orcanium channel status      # Show channel status
    orcanium channel install     # Install channel service
    orcanium channel uninstall   # Uninstall channel service
    orcanium setup               # Interactive setup wizard
    orcanium logout              # Clear stored authentication
    orcanium status              # Show status of all components
    orcanium cron                # Manage cron jobs
    orcanium cron list           # List cron jobs
    orcanium cron status         # Check if cron scheduler is running
    orcanium doctor              # Check configuration and dependencies
    orcanium honcho setup                    # Configure Honcho AI memory integration
    orcanium honcho status                   # Show Honcho config and connection status
    orcanium honcho sessions                 # List directory → session name mappings
    orcanium honcho map <name>               # Map current directory to a session name
    orcanium honcho peer                     # Show peer names and dialectic settings
    orcanium honcho peer --user NAME         # Set user peer name
    orcanium honcho peer --ai NAME           # Set AI peer name
    orcanium honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    orcanium honcho mode                     # Show current memory mode
    orcanium honcho mode [hybrid|honcho|local]  # Set memory mode
    orcanium honcho tokens                   # Show token budget settings
    orcanium honcho tokens --context N       # Set session.context() token cap
    orcanium honcho tokens --dialectic N     # Set dialectic result char cap
    orcanium honcho identity                 # Show AI peer identity representation
    orcanium honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    orcanium honcho migrate                  # Step-by-step migration guide: OpenClaw native → orcanium + Honcho
    orcanium version             Show version
    orcanium update              Update to latest version
    orcanium uninstall           Uninstall orcanium Agent
    orcanium acp                 Run as an ACP server for editor integration
    orcanium sessions browse     Interactive session picker with search

    orcanium claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: orcanium_cli_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``orcanium_cli_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``orcanium update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``orcanium_cli_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, orcanium crashes on import and the user can't run
# ``orcanium update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import orcanium_cli_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'orcanium' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``orcanium tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``orcanium.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("orcanium")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"orcanium", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"orcanium")
        # Windows: the .exe name is already ``orcanium.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* orcanium.cli.config is importable. Mirrors the explicit
# The TUI is the sole interactive surface. The config read remains for launch
# diagnostics and compatibility with existing display settings.
_EARLY_INTERFACE_CACHE: "list | None" = None


def _config_default_interface_early() -> str:
    """Read the configured interface, defaulting to the terminal UI."""
    global _EARLY_INTERFACE_CACHE
    if _EARLY_INTERFACE_CACHE is not None:
        return _EARLY_INTERFACE_CACHE[0]
    value = "tui"
    try:
        home = os.environ.get("ORCANIUM_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".orcanium", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    The interactive surface is always the TUI. ``--tui`` and the environment
    variable remain accepted as explicit launch hints.
    """
    if argv is None:
        argv = sys.argv[1:]
    if os.environ.get("ORCANIUM_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ORCANIUM_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ORCANIUM_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`orcanium --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from orcanium.cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"orcanium Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``orcanium --version`` before config/logging imports on Termux."""
    if os.environ.get("ORCANIUM_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from orcanium.cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from orcanium.cli.subcommands.cron import build_cron_parser
from orcanium.cli.subcommands.channel import build_channel_parser
from orcanium.cli.subcommands.profile import build_profile_parser
from orcanium.cli.subcommands.model import build_model_parser
from orcanium.cli.subcommands.setup import build_setup_parser
from orcanium.cli.subcommands.postinstall import build_postinstall_parser
from orcanium.cli.subcommands.whatsapp import build_whatsapp_parser
from orcanium.cli.subcommands.slack import build_slack_parser
from orcanium.cli.subcommands.login import build_login_parser
from orcanium.cli.subcommands.logout import build_logout_parser
from orcanium.cli.subcommands.auth import build_auth_parser
from orcanium.cli.subcommands.status import build_status_parser
from orcanium.cli.subcommands.webhook import build_webhook_parser
from orcanium.cli.subcommands.hooks import build_hooks_parser
from orcanium.cli.subcommands.doctor import build_doctor_parser
from orcanium.cli.subcommands.agent import build_agent_parser, cmd_agent
from orcanium.cli.subcommands.tui import build_tui_parser, cmd_tui
from orcanium.cli.subcommands.security import build_security_parser
from orcanium.cli.subcommands.dump import build_dump_parser
from orcanium.cli.subcommands.debug import build_debug_parser
from orcanium.cli.subcommands.backup import build_backup_parser
from orcanium.cli.subcommands.import_cmd import build_import_cmd_parser
from orcanium.cli.subcommands.config import build_config_parser
from orcanium.cli.subcommands.version import build_version_parser
from orcanium.cli.subcommands.update import build_update_parser
from orcanium.cli.subcommands.run import build_run_parser
from orcanium.cli.subcommands.admin import build_admin_parser
from orcanium.cli.subcommands.uninstall import build_uninstall_parser
from orcanium.cli.subcommands.dashboard import build_dashboard_parser
from orcanium.cli.subcommands.gui import build_gui_parser
from orcanium.cli.subcommands.logs import build_logs_parser
from orcanium.cli.subcommands.prompt_size import build_prompt_size_parser
from orcanium.cli.subcommands.memory import build_memory_parser
from orcanium.cli.subcommands.acp import build_acp_parser
from orcanium.cli.subcommands.tools import build_tools_parser
from orcanium.cli.subcommands.insights import build_insights_parser
from orcanium.cli.subcommands.knowledge import build_knowledge_parser
from orcanium.cli.subcommands.skills import build_skills_parser
from orcanium.cli.subcommands.pairing import build_pairing_parser
from orcanium.cli.subcommands.plugins import build_plugins_parser
from orcanium.cli.subcommands.mcp import build_mcp_parser
from orcanium.cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (orcanium tools, orcanium setup, orcanium model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'orcanium {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any orcanium module import.
#
# Many modules cache ORCANIUM_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ORCANIUM_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.orcanium/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ORCANIUM_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `orcanium mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to orcanium's own profile selector.
        """
        try:
    ...
    # after the subcommand (`orcanium chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors orcanium.cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ORCANIUM_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.orcanium/profiles/coder or
    # /opt/data/profiles/coder).  If ORCANIUM_HOME points to the orcanium root
    # instead (e.g. systemd hardcodes ORCANIUM_HOME=/root/.orcanium), we must
    # still read active_profile — the user may have switched profiles via
    # `orcanium profile use` and the gateway should honour that choice.
    # See issue #22502.
    orcanium_home_env = os.environ.get("ORCANIUM_HOME", "")
    if profile_name is None and orcanium_home_env:
        if Path(orcanium_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the orcanium root
    if profile_name is None:
        try:
            from orcanium.channel.compat import get_default_orcanium_root

            active_path = get_default_orcanium_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ORCANIUM_HOME
    if profile_name is not None:
        try:
            from orcanium.cli.profiles import resolve_profile_env

            orcanium_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent orcanium from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ORCANIUM_HOME"] = orcanium_home
...
# Load .env from ~/.orcanium/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from orcanium.cli.config import get_orcanium_home
from orcanium.cli.env_loader import load_orcanium_dotenv

load_orcanium_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ORCANIUM_REDACT_SECRETS env
# var BEFORE orcanium_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_orcanium_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "ORCANIUM_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ORCANIUM_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `orcanium` subcommands
...
try:
    from orcanium_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from orcanium.cli.compat import _agent_stub as apply_ipv4_preference
        _apply_ipv4 = apply_ipv4_preference
        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if orcanium.channel.compat not importable yet
...
from orcanium.cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to orcanium.cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (orcanium.cli.main._model_flow_*) keep resolving unchanged.
from orcanium.cli.model_setup_flows import (
    _prompt_auth_credentials_choice,
    _model_flow_openrouter,
    _model_flow_orcanium,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_orcanium_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ORCANIUM_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ORCANIUM_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from orcanium.cli.config import get_env_path, get_orcanium_home, load_config
    from orcanium.cli.auth import get_auth_status

    # Determine whether orcanium itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from orcanium.cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_orcanium_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from orcanium.cli.auth import PROVIDER_REGISTRY
    ...
    # Check for orcanium Portal OAuth credentials
    auth_file = get_orcanium_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if orcanium has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants orcanium to use their tokens.
    if _has_orcanium_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from orcanium.channel.compat import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, orcanium_bin
        cli_args: the original CLI arguments (everything after 'orcanium')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    orcanium_bin = container_info["orcanium_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo orcanium {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo orcanium {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  orcanium --tui --resume {target}")
    if title:
        print(f'  orcanium --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @orcanium/ink is missing or node_modules is behind package-lock.json.
    ...
    ``node_modules/`` live at the workspace root (the parent of the
    ```orcanium/tui/`` directory).  The lockfile / ink / marker checks use that
    workspace root; only the prebuilt-bundle sentinel stays relative to
    *root* (``orcanium/tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@orcanium" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/orcanium-ink/package.json",
    "packages/orcanium-ink/index.js",
    "packages/orcanium-ink/text-input.js",
)
...
def _iter_tui_build_inputs(root: Path):
    """Yield source/config files that affect ``orcanium/tui/dist/entry.js``."""
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ORCANIUM_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ORCANIUM_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ORCANIUM_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ORCANIUM_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    orcanium_home = os.environ.get("ORCANIUM_HOME") or str(Path.home() / ".orcanium")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ORCANIUM_HOME": orcanium_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(orcanium_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(orcanium_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if orcanium_cli_dir is None:
        orcanium_cli_dir = Path(__file__).parent
    bundled = orcanium_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ORCANIUM_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ORCANIUM_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from orcanium.cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ORCANIUM_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ORCANIUM_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ORCANIUM_TUI_DIR (e.g. `unset ORCANIUM_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    #    scopes the install to orcanium/tui so launch does not pull desktop/web
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("ORCANIUM_QUIET"):
            print("Installing TUI dependencies…")
        npm_cwd = _workspace_root(tui_dir)
        # --workspace orcanium-tui avoids resolving apps/desktop (Electron + node-pty).
        # See #38772.
        # When `orcanium/tui/ has its own package-lock.json (e.g. curl install),
        # _workspace_root() returns tui_dir itself.  Passing --workspace in
        # that case fails because npm cannot find a workspace named "orcanium-tui"
        # inside `orcanium/tui/.  See #42973.
        npm_workspace_args: tuple[str, ...] = () if npm_cwd == tui_dir else ("--workspace", "orcanium-tui")
        if termux_startup:
    ...
    if tui_dev:
        # Keep the local @orcanium/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @orcanium/ink resolves through
        # packages/orcanium-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "orcanium-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
    ...
    if should_build:
        npm = _node_bin("npm")
        # Ensure @orcanium/ink is built first (needed in git clones)
        ink_dir = tui_dir / "packages" / "orcanium-ink"
        ink_dist = ink_dir / "dist" / "entry-exports.js"
        if not ink_dist.exists():
            ink_result = subprocess.run(
                [npm, "run", "build"],
                cwd=str(ink_dir),
                capture_output=True, text=True, encoding="utf-8", errors="replace",
            )
            if ink_result.returncode != 0:
                combined = f"{ink_result.stdout or ''}{ink_result.stderr or ''}".strip()
                preview = "\n".join(combined.splitlines()[-30:])
                print("TUI ink prebuild failed.")
                if preview:
                    print(preview)
                sys.exit(1)
        result = subprocess.run(
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from orcanium.cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    tui_dir = PROJECT_ROOT / "orcanium" / "tui"

    import tempfile

    env = os.environ.copy()
    try:
        from orcanium.cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="orcanium-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ORCANIUM_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ORCANIUM_PYTHON_SRC_ROOT"] = os.environ.get(
        "ORCANIUM_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ORCANIUM_PYTHON", sys.executable)
    env.setdefault("ORCANIUM_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ORCANIUM_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ORCANIUM_MODEL"] = model
        env["ORCANIUM_INFERENCE_MODEL"] = model
    if provider:
        env["ORCANIUM_TUI_PROVIDER"] = provider
        env["ORCANIUM_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ORCANIUM_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ORCANIUM_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ORCANIUM_TUI_SKILLS"] = value
    if query:
        env["ORCANIUM_TUI_QUERY"] = query
    if image:
        env["ORCANIUM_TUI_IMAGE"] = image
    if checkpoints:
        env["ORCANIUM_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ORCANIUM_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ORCANIUM_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ORCANIUM_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ORCANIUM_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ORCANIUM_ACCEPT_HOOKS"] = "1"
    ...
    # ORCANIUM_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `orcanium --tui` try to
    ...
    # resolved for this invocation; direct `node orcanium/tui/dist/entry.js` users can
    # still set ORCANIUM_TUI_RESUME themselves.
    env.pop("ORCANIUM_TUI_RESUME", None)
    if resume_session_id:
        env["ORCANIUM_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `orcanium update` so
    ...
    if code == 42:
        from orcanium.cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ORCANIUM_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``orcanium kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``orcanium kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ORCANIUM_KANBAN_BOARD"):
        return
    try:
        from orcanium.cli.kanban_db import get_current_board

        os.environ["ORCANIUM_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.orcanium/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with orcanium — chat, dashboard (the desktop GUI's backend),
    and channel. The skills_sync module is manifest-based and idempotent:
    ...
    dependency. orcanium still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Interactive chat always uses the TUI. The explicit flag and environment
    variable are retained as harmless launch hints for integrations.
    """
    return True


def _resolve_agent_name(args) -> str:
    """Resolve agent name from args or prompt."""
    from orcanium.app.core.db import SessionLocal, AgentState
    from orcanium.cli.subcommands.agent import _cmd_agent_list
    
    agent_name = getattr(args, "agent", None) or getattr(args, "agent_name", "")
    if agent_name:
        return agent_name
    ...
    if not agents:
        ...
        try:
            reply = input("  Create a default 'PersonalAgent'? [Y/n]: ").strip().lower()
        ...
        if reply in ("", "y", "yes"):
            ...
            try:
                AgentManager.sync_all_agents(db)
                AgentManager.create_agent(db=db, name="PersonalAgent", config=cfg)
                print("  ✓ Agent 'PersonalAgent' created.")
                ...
                db.close()
        print("  Create one with: orcanium agent create <name>")
        sys.exit(1)
    ...
    # Multiple agents — prompt
    print("  Select an agent:")
    for i, a in enumerate(agents, 1):
        print(f"  {i}. {a.name} ({a.model_provider or '?'}/{a.model_name or '?'})")
    try:
        choice = int(input("  Choice [1]: ").strip() or "1")
        idx = max(0, min(choice - 1, len(agents) - 1))
...
def cmd_chat(args):
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        print(
            "Error: 'orcanium chat' requires an interactive terminal.\n"
            "For scripts, pipes, CI, or scheduled jobs, use:\n"
            "  orcanium run \"your task\"",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'orcanium sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from orcanium.cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from orcanium.cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'orcanium doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like orcanium isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  orcanium setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'orcanium setup' at any time to configure.")
        sys.exit(1)

    # Resolve the selected agent before launching any interface. The TUI is a
    # child process, so the selection must be exported before it starts.
    agent_name = _resolve_agent_name(args)
    os.environ["ORCANIUM_AGENT"] = agent_name
    ...
    if _termux_should_prefetch_update_check():
        try:
            from orcanium.cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ORCANIUM_YOLO_MODE"] = "1"
    ...
    # setup (config, rules files, plugins, MCP servers) or from orcanium itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (ORCANIUM_SAFE_MODE is checked by
    # orcanium.cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["ORCANIUM_SAFE_MODE"] = "1"
        os.environ["ORCANIUM_IGNORE_USER_CONFIG"] = "1"
        os.environ["ORCANIUM_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.orcanium/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["ORCANIUM_SESSION_SOURCE"] = args.source
    ...
    raise RuntimeError("TUI launcher returned unexpectedly")


def cmd_channel_runtime(args):
    """Local messaging-channel management commands."""
    _sync_bundled_skills_quietly()

    from orcanium.cli.channel import channel_command

    channel_command(args)


def cmd_run(args):
    """Run one task in the automation-safe execution mode."""
    from orcanium.cli.oneshot import run_oneshot
    from orcanium.runtime.agent_registry import (
        AgentSelectionError,
        resolve_agent_name,
    )

    try:
        selection = resolve_agent_name(
            getattr(args, "agent", None),
            require_explicit_when_multiple=True,
        )
    except AgentSelectionError as exc:
        print(f"orcanium run: {exc}", file=sys.stderr)
        sys.exit(2)

    if selection.name:
        os.environ["ORCANIUM_AGENT"] = selection.name
...
def cmd_channel(args):
    """Dispatch the public channel command to the existing runtime."""
def cmd_admin(args):
    """Launch the optional local administration UI."""
    args.isolated = False
    args.stop = False
    args.status = False
    args.open_profile = ""
...
def cmd_proxy(args):
    ...
    # for users who don't run the proxy or the messaging channel.
    from orcanium.cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from orcanium.cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with orcanium?")
        print()
    ...
    # has no creds.json.  Every subsequent `orcanium gateway` then paid a 30s
    ...
    session_dir = get_orcanium_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: orcanium gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `orcanium gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  orcanium gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ orcanium Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  orcanium gateway")
        ...
        print()
        print("  Or install as a service: orcanium gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'orcanium whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``orcanium whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``orcanium.cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from orcanium.cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup

    return run_whatsapp_cloud_setup()


def cmd_setup(args):
    """Initialize orcanium directories, database, and seed default agent."""
    try:
        from orcanium.cli.setup import run_setup_wizard
        run_setup_wizard(args)
    except Exception as e:
        import traceback
        print(f"Setup error: {e}")
        traceback.print_exc()
        print("Try: python -m orcanium.app.app_cli setup")
        import sys
        sys.exit(1)
...
def cmd_postinstall(args):
    ...
    from orcanium.cli.config import stamp_install_method
    from orcanium.cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ orcanium post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from orcanium.cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``orcanium model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from orcanium.cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from orcanium.cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from orcanium.cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ORCANIUM_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from orcanium.cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'orcanium model' for "
                "available providers, or run 'orcanium doctor' to diagnose config "
                "issues."
            )
    ...
    from orcanium.cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        group_providers,
    ...
    # PROVIDER_GROUPS in orcanium.cli/models.py). A multi-member group shows one
    ...
    elif selected_provider == "orcanium":
        _model_flow_orcanium(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.orcanium/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.orcanium/.env if the active provider is not 'custom'.
    ...
    """
    from orcanium.cli.config import get_env_value, save_env_value, load_config
...
#
# orcanium uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `orcanium model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `orcanium model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`orcanium.cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from orcanium.cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from orcanium.cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  orcanium only falls back to a lightweight backend (OpenRouter,")
        print("  orcanium Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``orcanium model`` flow, then route aux tasks to them here.
    """
    from orcanium.cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from orcanium.cli.auth import _prompt_model_selection
    from orcanium.cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from orcanium.cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from orcanium.cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from orcanium.cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use orcanium URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from orcanium.cli.curses_ui import curses_radiolist
...
# downstream call sites read `orcanium.cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `orcanium` invocation — including
# fast paths like `orcanium --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from orcanium.cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``orcanium setup`` / ``orcanium model``.
    ...
    recover from a malformed paste without editing ``~/.orcanium/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from orcanium.cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from orcanium.cli.config import save_env_value
    from orcanium.cli.secret_prompt import masked_secret_prompt
    ...
    from orcanium.cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `orcanium setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from orcanium.channel.compat import display_orcanium_home as _dhh_fn

            print(
                f"    orcanium will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate orcanium CLI with a provider."""
    from orcanium.cli.auth import login_command
...
def cmd_logout(args):
    ...
    from orcanium.cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from orcanium.cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from orcanium.cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from orcanium.cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from orcanium.cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``orcanium slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: orcanium slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `orcanium slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from orcanium.cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from orcanium.cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from orcanium.cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from orcanium.cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `orcanium security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from orcanium.cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from orcanium.cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from orcanium.cli.debug import run_debug
...
def cmd_config(args):
    ...
    from orcanium.cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up orcanium home directory to a zip file."""
    if getattr(args, "quick", False):
        from orcanium.cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from orcanium.cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a orcanium backup from a zip file."""
    from orcanium.cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from orcanium.cli.banner import format_banner_version_label
    ...
    # Show update status (synchroorcanium — acceptable since user asked for version info)
    try:
        from orcanium.cli.banner import check_for_updates
        from orcanium.cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall orcanium Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from orcanium.cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from orcanium.cli.uninstall import run_gui_uninstall
    ...
    from orcanium.cli.uninstall import run_uninstall
...
# Critical files that every ``orcanium`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``orcanium update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "orcanium_cli/main.py",
    "orcanium_cli/config.py",
    "orcanium_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "orcanium_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``orcanium`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="orcanium-syntax-check-") as tmpdir:
...
def _rpc_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    """File-based IPC prompt for RPC mode.

    Writes a prompt marker file so the gateway can forward the question to the
    user, then polls for a response file.  Falls back to *default* on timeout.

    Used by ``orcanium update --rpc`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from orcanium.channel.compat import get_orcanium_home
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``orcanium.cli/web_dist/`` (per vite.config.ts
    outDir: "../orcanium.cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "orcanium_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``orcanium`` launcher present but ``orcanium.cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the orcanium venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — orcanium venv python3, no nix-shell overhead
    ...
    # a orcanium venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``orcanium update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``orcanium web``).
    ...
    # build path usable on a stock `py -m orcanium.cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (orcanium web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``orcanium update`` can unconditionally call ``orcanium desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``orcanium desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ORCANIUM_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ORCANIUM_HOME."""
    from orcanium.channel.compat import get_orcanium_home
    return get_orcanium_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``orcanium update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/orcanium.app/Contents/MacOS/orcanium"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "orcanium.exe",
            release_dir / "win-ia32-unpacked" / "orcanium.exe",
            release_dir / "win-arm64-unpacked" / "orcanium.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "orcanium",
            release_dir / "linux-unpacked" / "orcanium",
            release_dir / "linux-arm64-unpacked" / "orcanium",
            release_dir / "linux-arm64-unpacked" / "orcanium",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/orcanium's`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``orcanium.exe`` keeps an exclusive lock on
    ``release/win-unpacked/orcanium.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\orcanium.exe: Access is
    ...
    an unrelated "orcanium" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "orcanium is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../orcanium.app/Contents/MacOS/orcanium  ->  app bundle = .../orcanium.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ orcanium Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ orcanium Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from orcanium_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["ORCANIUM_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ORCANIUM_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "orcanium_root", None):
        env["ORCANIUM_DESKTOP_ORCANIUM_ROOT"] = str(Path(args.orcanium_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ORCANIUM_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  orcanium gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds orcanium.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `orcanium` rename: unpack-electron
                ...
                if purged:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose orcanium.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on orcanium.exe, close any")
                    print("  running orcanium desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> orcanium desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "orcanium is
    ...
    if source_mode:
        print("→ Launching orcanium Desktop from source build...")
    ...
    print(f"→ Launching packaged orcanium Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``orcanium dashboard`` processes other than ourselves.

    ``orcanium dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``orcanium update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the orcanium Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``orcanium dashboard`` as
    ...
    environment variable ``ORCANIUM_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "orcanium dashboard",
        "orcanium_cli.main dashboard",
        "orcanium_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "orcanium.*dashboard"`) keeps us consistent
            # with `orcanium.cli.channel._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `orcanium update`.
    ...
    print("  Preview now:  orcanium curator run --dry-run")
    print("  Pause it:     orcanium curator pause")
    print(
        "  Docs:         https://orcanium-agent.orcanium.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``orcanium update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``orcanium update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: orcanium curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``orcanium dashboard`` processes.

    Called at the end of ``orcanium update`` (default ``reason``) and also
    from ``orcanium dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the orcanium Desktop Electron app spawns this dashboard as a
    # backend child, it sets ORCANIUM_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ORCANIUM_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if sys.platform == "win32":
        ...
        while pending and _time.monotonic() < deadline:
            ...
            from channel.status import _pid_exists
            for pid in pending:
                if _pid_exists(pid):
    ...
    if killed:
        ...
        print("    orcanium dashboard --port <port>")
...
def _update_via_zip(args):
    """Update orcanium Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `orcanium update "
            f"--branch {branch}`, or update against main with `orcanium update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/Orcanium/orcanium-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="orcanium-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"orcanium-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for orcanium-agent itself
        ...
        # GitHub ZIPs extract to orcanium-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"orcanium-agent-{branch}")
    ...
    from orcanium.cli.managed_uv import ensure_uv, update_managed_uv
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "orcanium-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if orcanium behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes orcanium completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but orcanium couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but orcanium couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if orcanium behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but orcanium couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but orcanium couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `orcanium update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Orcanium/orcanium-agent.git",
    "git@github.com:Orcanium/orcanium-agent.git",
    "https://github.com/Orcanium/orcanium-agent",
    "git@github.com:Orcanium/orcanium-agent",
}
OFFICIAL_REPO_URL = "https://github.com/Orcanium/orcanium-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_orcanium_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_orcanium_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official orcanium repository.")
        print("  This means you may miss updates from Orcanium/orcanium-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Orcanium/orcanium-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Orcanium/orcanium-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``orcanium update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from orcanium.channel.compat import get_default_orcanium_root

    default_home = get_default_orcanium_root()
...
# Install-scoped breadcrumb dropped right before ``orcanium update`` mutates the
...
# survives and the next ``orcanium`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ORCANIUM_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``orcanium update`` left half-done.
    ...
    protocol stream (``orcanium acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `orcanium update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``orcanium update`` is still progressing even if pip/uv itself is silent.
    """
...
def _orcanium_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "orcanium.exe",
        scripts_dir / "orcanium-channel.exe",
    ]


def _detect_concurrent_orcanium_instances(
    ...
    The orcanium Desktop Electron app spawns ``orcanium.EXE`` as a backend child,
    so during ``orcanium update`` the user-invoked process and the desktop's
    ...
    shims (``orcanium.exe`` / ``orcanium-channel.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``orcanium update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``orcanium.exe``) is a separate process from the Python
    ...
    for shim in _orcanium_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated orcanium.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``orcanium update``
    ...
    #      orcanium.exe sitting *under* a non-orcanium parent (e.g. a orcanium
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "orcanium.exe"
    lines = ["✗ Another orcanium.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close orcanium Desktop, exit any open `orcanium` REPLs, and")
    lines.append("  stop the gateway (`orcanium gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `orcanium update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_orcanium_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``orcanium.exe``.
    ...
    when ``orcanium update`` runs, ``orcanium.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``orcanium.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next orcanium invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the orcanium Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       orcanium Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _orcanium_exe_shims(scripts_dir):
        ...
        print(
            "    Close orcanium Desktop, exit other `orcanium` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `orcanium update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_orcanium_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``orcanium.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``orcanium.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``orcanium`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_orcanium_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``orcanium.exe.old.*`` left by prior updates.

    Called early on every orcanium invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``orcanium update`` runs ``uv pip install -e .[all]`` which
    ...
    """
    try:
        from orcanium.app.tools import lazy_deps
    except Exception as exc:
    ...
    if failed:
        ...
        print("  `orcanium update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``orcanium.exe`` / ``orcanium-channel.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_orcanium_exe`` for the rationale.
    """
    ...
    # hours later inside ``orcanium update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``orcanium update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``orcanium.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``orcanium`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `orcanium update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `orcanium update --force` after closing other orcanium processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from orcanium.cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ORCANIUM_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from orcanium.cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `orcanium update`, so we install root-only first
    ...
    # Step 2: install only the workspaces update needs (orcanium/tui, web).
    # --workspace selects specific workspaces; the rest (desktop) are skipped.
    ws_args = [*extra_args, "--workspace", "orcanium-tui", "--workspace", "web"]
    ...
    if ws_result.returncode == 0:
        print("  ✓ repo root + orcanium/tui, web workspaces (desktop skipped)")
    else:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``orcanium update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.orcanium/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``orcanium update`` safe to
...
def _install_hangup_protection(rpc_mode: bool = False):
    """Protect ``cmd_update`` from SIGHUP and broken terminal pipes.

    Users commonly run ``orcanium update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.orcanium/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In RPC mode (``orcanium update --rpc``) the update is already
    ...
    try:
        ...
        # orcanium.cli.config.get_orcanium_home to simulate setup failure.
        from orcanium.cli.config import get_orcanium_home as _get_orcanium_home

        logs_dir = _get_orcanium_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``orcanium update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``orcanium update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from orcanium.cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from orcanium.cli.config import recommended_update_command
        from orcanium.cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``orcanium update`` without requiring a reinstall.  The
    ...
    ``orcanium`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/orcanium.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v orcanium'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/orcanium, code at /usr/local/lib/orcanium-agent).
    fhs_link = Path("/usr/local/bin/orcanium")
    ...
    path_comment = (
        "# orcanium Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ORCANIUM_HOME before running the update.
    ...
    ORCANIUM_HOME directories.  The ``--backup`` flag on ``orcanium update``
    ...
    try:
        from orcanium.cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_orcanium_home so the user sees ~/.orcanium/...
    try:
        from orcanium.channel.compat import get_orcanium_home, display_orcanium_home

        home = get_orcanium_home()
        try:
            display_path = f"{display_orcanium_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  orcanium import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``orcanium update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update orcanium Agent to the latest version.
    ...
    if is_managed():
        managed_error("update orcanium Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `orcanium update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
    ...
    rpc_mode = getattr(args, "rpc", False)

    # Protect against mid-update terminal disconnects (SIGHUP) and tolerate
    # writes to a closed stdout.  No-op in RPC mode.  See
    # _install_hangup_protection for rationale.
    _update_io_state = _install_hangup_protection(rpc_mode=rpc_mode)
    try:
        _cmd_update_impl(args, rpc_mode=rpc_mode)
    finally:
        _finalize_update_output(_update_io_state)


def _cmd_update_pip(args):
    """Update orcanium via pip (for PyPI installs)."""
    from orcanium.cli import __version__
    from orcanium.cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "orcanium-agent"]
        ...
        cmd = [pipx, "upgrade", "orcanium-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "orcanium-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "orcanium-agent"]
    ...
    print("✓ Update complete! Restart orcanium to use the new version.")


def _cmd_update_impl(args, rpc_mode: bool):
    """Body of ``cmd_update`` — kept separate so the wrapper can always
    restore stdio even on ``sys.exit``."""
    # In RPC mode, use file-based IPC for prompts instead of stdin
    gw_input_fn = (
        (lambda prompt, default="": _rpc_prompt(prompt, default))
        if rpc_mode
        else None
    )
    ...
    print("⚕ Updating orcanium Agent...")
    print()

    # On Windows, abort early if another orcanium.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_orcanium_instances(scripts_dir)
            if concurrent:
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://orcanium-agent.orcanium.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        prompt_for_restore = (
            ...
            and not assume_yes
            and (rpc_mode or (sys.stdin.isatty() and sys.stdout.isatty()))
        )
        ...
        # though `git pull` can't touch $ORCANIUM_HOME, this is cheap
        ...
        try:
            from orcanium.cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in orcanium.cli/config.py bricked
        # every user who ran ``orcanium update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``orcanium
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``orcanium update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_orcanium_home added to orcanium.channel.compat).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``orcanium`` launch finishes the
        ...
        # build.  ``orcanium desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``orcanium desktop`` shouldn't be forced into a full
        # Electron build by ``orcanium update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "orcanium_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `orcanium desktop` to retry)")
        ...
        # current — so copy it straight over ~/.orcanium/cache/model_catalog.json
        ...
        # modules in this process.  Reload orcanium.channel.compat so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_orcanium_home() added since the last release.
        try:
            import importlib
            import orcanium.channel.compat as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ORCANIUM_HOME so
        # it is not affected by sync_skills()'s module-level ORCANIUM_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ORCANIUM_HOME env var points at the default or a named profile.
        try:
            from orcanium.cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from orcanium.cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'orcanium config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                interactive_migration = not (
                    rpc_mode or assume_yes or response == "auto"
                )
                ...
                if (rpc_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: orcanium config migrate")
            else:
                print()
                print("Skipped. Run 'orcanium config migrate' later to configure.")
        else:
        ...
        try:
            from orcanium.cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `orcanium curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``orcanium update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from orcanium.cli.tools_config import install_cua_driver
        ...
        # When running as ``orcanium update --rpc`` (spawned by the channel's
        # /update command), this process lives inside the channel's systemd
        ...
        # exit-code marker file would never be created.  The new channel's
        ...
        if rpc_mode:
            _exit_code_path = get_orcanium_home() / ".update_exit_code"
            try:
        ...
        try:
            from orcanium.cli.channel import (
                is_macos,
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... orcanium-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... orcanium-gateway*)
            ...
            try:
                from orcanium.channel.compat import (
            ...
            # Discover all orcanium-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. orcanium-channel.service or orcanium-gateway-coder.service
                            ...
                            # The channel's SIGUSR1 handler calls
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `orcanium update` restart it automatically, allow\n"
                            ...
                            # path in `orcanium gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: orcanium gateway status"
                        )
            ...
            for pid, proc in profile_processes.items():
                ...
                # finish before the watcher respawns the channel.  If the
                ...
                # spawns the new channel.  Telegram holds the previous
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: orcanium gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: orcanium -p <profile> gateway run  for each profile)"
                        )
            ...
            try:
                ...
                if _stuck:
                    ...
                    from channel.status import terminate_pid as _terminate_pid
                    for pid in _stuck:
                        try:
        ...
        # Warn if legacy orcanium gateway unit files are still installed.
        # When both orcanium.service (from a pre-rename install) and the
        # current orcanium-channel.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `orcanium update` surfaces the issue until the user migrates.
        try:
            from orcanium.cli.channel import (
                has_legacy_orcanium_units,
                _find_legacy_orcanium_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_orcanium_units():
                print()
                print("⚠ Legacy orcanium gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_orcanium_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (orcanium.service) fight the current")
                print("  orcanium-channel.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    orcanium gateway migrate-legacy")
                print()
        ...
        print("  orcanium model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``orcanium -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from orcanium.channel.compat import display_orcanium_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `orcanium profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_orcanium_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → orcanium -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.orcanium)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  orcanium profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     orcanium -p {name} chat")
                else:
        ...
        from orcanium.cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from orcanium.channel.compat import get_orcanium_home as _hh
        ...
        from orcanium.cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `orcanium profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → orcanium -p {name}  ({wrapper})")
        print()
        ...
        from orcanium.cli.profiles import profile_exists
        ...
        from orcanium.cli.profiles import rename_profile
        ...
        from orcanium.cli.profiles import export_profile
        ...
        from orcanium.cli.profiles import import_profile
        ...
        import tempfile
        from orcanium.cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="orcanium_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  orcanium -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      orcanium -p {plan.manifest.name} chat")
        ...
        from orcanium.cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `orcanium profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"orcanium -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from orcanium.cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("orcanium_requires"):
            print(f"Requires:     orcanium {data['orcanium_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from orcanium.cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.orcanium_requires:
        print(f"  Requires: orcanium {mf.orcanium_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``orcanium dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``orcanium dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No orcanium dashboard processes running.")
        return 0

    print(f"{len(pids)} orcanium dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No orcanium dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `orcanium update`.
    ...
    # launches the dashboard (`worker dashboard` → ORCANIUM_HOME points into
    ...
    try:
        from orcanium.cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("ORCANIUM_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "orcanium_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        env = os.environ.copy()
        # Drop the profile ORCANIUM_HOME so the child binds the machine root.
        env.pop("ORCANIUM_HOME", None)
    ...
    # the same logs directory as every other orcanium surface.
    try:
        from orcanium_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "ORCANIUM_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ORCANIUM_WEB_DIST"])
            if "ORCANIUM_WEB_DIST" in os.environ
            else PROJECT_ROOT / "orcanium_cli" / "web_dist"
        )
    ...
    # (e.g. plugins/dashboard_auth/orcanium) registers BEFORE start_server's
    ...
    try:
        from orcanium.cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    # agents via rpc.server._make_agent.  That path only snapshots the
    # tool registry — it never starts MCP discovery (the stdio TUI does that in
    # rpc/entry.py, which the dashboard process doesn't run).  Without
    ...
    try:
        from orcanium.cli.mcp_startup import start_background_mcp_discovery
    ...
    from orcanium.cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from orcanium.cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from orcanium.cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter orcanium log files."""
    from orcanium.cli.logs import tail_log, list_logs
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        "acp", "admin", "auth", "backup", "bundles", "channel", "checkpoints", "claw", "completion",
        "computer-use",
        "config", "cron", "curator", "dashboard", "debug", "doctor",
        "dump", "fallback", "hooks", "import", "insights",
        "gui", "desktop", "kanban", "login", "logout", "logs", "lsp", "mcp", "memory", "migrate",
        "model", "pairing", "plugins", "postinstall", "profile", "proxy",
        "prompt-size",
        "run", "send", "sessions", "setup",
...
# so that in ``orcanium -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``orcanium.cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``orcanium -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``orcanium`` or only flags → defaults to ``chat``.
        return False
...
_AGENT_SUBCOMMANDS = {
    ...
    "channel": ("channel_command", {"run"}),
    "mcp": ("mcp_action", {"serve"}),
}


def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ORCANIUM_TUI") == "1")


def _command_has_dedicated_mcp_startup(args) -> bool:
    if args.command == "acp":
        return True
    if args.command == "channel" and getattr(args, "channel_command", None) == "run":
        return True
...
def _prepare_agent_startup(args) -> None:
    ...
    # Interactive chat is intentionally unavailable without a terminal. Do not
    # load plugins or connect MCP servers before returning that routing error.
    if args.command in {None, "chat"} and not (sys.stdin.isatty() and sys.stdout.isatty()):
        return
    ...
    if _run_inline_mcp_discovery:
        try:
            # MCP tool discovery remains synchroorcanium for entrypoints that do
            # not own a later bounded/executor startup path.
            from orcanium.app.tools.mcp_tool import discover_mcp_tools
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    # the TUI — explicit --tui/env or display.interface=tui.
    ...
    from orcanium.cli._parser import build_top_level_parser
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ORCANIUM_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ORCANIUM_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ORCANIUM_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `orcanium --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_orcanium_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_orcanium_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_orcanium_home()}/memories/\n")
    else:
        from orcanium.cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch orcanium Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from orcanium.cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from orcanium.cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from orcanium.cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from orcanium.cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from orcanium.cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from orcanium.cli.pairing import pairing_command

    pairing_command(args)


def cmd_knowledge(args):
    from orcanium.cli.subcommands.knowledge import cmd_knowledge as _kc

    _kc(args)


def cmd_plugins(args):
    from orcanium.cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from orcanium.cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from orcanium.cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for orcanium CLI."""
    # Cosmetic: make the process show up as 'orcanium' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from orcanium.cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``orcanium.exe.old.*`` quarantine files left by previous
    # ``orcanium update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_orcanium_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``orcanium update``
    ...
    # ``orcanium skills install update``) merely defers recovery one launch;
    # under-matching (missing ``orcanium -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in orcanium.cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from orcanium.cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://orcanium-agent.orcanium.com/docs/user-guide/features/fallback-providers"
        ),
    )
    fallback_subparsers = fallback_parser.add_subparsers(dest="fallback_command")
    fb_list = fallback_subparsers.add_parser(
        "list",
        aliases=["ls"],
        help="Show the current fallback chain (default when no subcommand)",
    )
    fb_list.add_argument("--agent", type=str, default=None, help="Show fallback chain for a specific agent.")
    fb_add = fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `orcanium model`) and append to the chain",
    )
    fb_add.add_argument("--agent", type=str, default=None, help="Add fallback for a specific agent.")
    fb_remove = fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
        help="Pick an entry to delete from the chain",
    )
    fb_remove.add_argument("--agent", type=str, default=None, help="Remove fallback from a specific agent.")
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.orcanium/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://orcanium-agent.orcanium.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from orcanium.cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from orcanium.cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in orcanium.cli/subcommands/channel.py)
    # =========================================================================
    build_channel_parser(
        subparsers,
        cmd_channel=cmd_channel_runtime,
        cmd_proxy=cmd_proxy,
    ...
    # =========================================================================
    # setup command  (parser built in orcanium.cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in orcanium.cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in orcanium.cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `orcanium whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in orcanium.cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from orcanium.cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in orcanium.cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in orcanium.cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in orcanium.cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in orcanium.cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in orcanium.cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in orcanium.cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from orcanium.cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in orcanium.cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in orcanium.cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in orcanium.cli/subcommands/security.py)
    # =========================================================================
    build_agent_parser(subparsers, cmd_agent=cmd_agent)
    build_tui_parser(subparsers, cmd_tui=cmd_tui)
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in orcanium.cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in orcanium.cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in orcanium.cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.orcanium/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo orcanium uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from orcanium.cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in orcanium.cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in orcanium.cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in orcanium.cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in orcanium.cli/subcommands/skills.py)
    # =========================================================================
    ...
    from orcanium.cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in orcanium.cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``orcanium --help``, ``orcanium version``, ``orcanium logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from orcanium.cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from orcanium.cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in orcanium.cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)
    build_knowledge_parser(subparsers, cmd_knowledge=cmd_knowledge)

    # =========================================================================
    # tools command  (parser built in orcanium.cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `orcanium computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `orcanium tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: orcanium computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: orcanium computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in orcanium.cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_orcanium_home() / "sessions"
            ...
            # Launch orcanium --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in orcanium.cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in orcanium.cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in orcanium.cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in orcanium.cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)
    build_run_parser(subparsers, cmd_run=cmd_run)
    build_admin_parser(subparsers, cmd_admin=cmd_admin)

    # =========================================================================
    # uninstall command  (parser built in orcanium.cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in orcanium.cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in orcanium.cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in orcanium.cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The orcanium-Setup.exe success screen tells users to
    # run `orcanium desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in orcanium.cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in orcanium.cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in orcanium.cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``orcanium -c Pokemon Agent Dev`` → ``orcanium -c 'Pokemon Agent Dev'``
    ...
    from orcanium.cli.config import get_container_exec_info
    ...
    # fails (e.g. 'orcanium -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (orcanium hooks list, cron
