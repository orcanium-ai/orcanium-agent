"""Orcanium webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    Orcanium webhook subscribe <name> [options]
    Orcanium webhook list
    Orcanium webhook remove <name>
    Orcanium webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.Orcanium/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _orcanium_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_orcanium_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     orcanium gateway setup
  ...
  Then start the gateway: orcanium gateway run
"""
...
def webhook_command(args):
    """Entry point for 'Orcanium webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: Orcanium webhook {subscribe|list|remove|test}")
        print("Run 'Orcanium webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (orcanium gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: Orcanium webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from Orcanium webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (orcanium gateway run)")
