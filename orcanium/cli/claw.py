"""Orcanium claw — OpenClaw migration commands.

Usage:
    Orcanium claw migrate              # Preview then migrate (always shows preview first)
    Orcanium claw migrate --dry-run    # Preview only, no changes
    Orcanium claw migrate --yes        # Skip confirmation prompt
    Orcanium claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    Orcanium claw migrate --no-backup  # Skip pre-migration snapshot
    Orcanium claw cleanup              # Archive leftover OpenClaw directories
    Orcanium claw cleanup --dry-run    # Preview what would be archived
"""
...
from orcanium.cli.config import get_orcanium_home, get_config_path, load_config, save_config
from orcanium.channel.compat import get_optional_skills_dir
    ...
    if sys.platform != "win32":
        try:
            result = subprocess.run(
                ["systemctl", "--user", "is-active", "openclaw-channel.service"],
                capture_output=True, text=True, timeout=5,
            )
            if result.stdout.strip() == "active":
                found.append("systemd service: openclaw-channel.service")
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Orcanium may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Orcanium gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Orcanium gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'orcanium gateway stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_orcanium.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_orcanium", script_path)
...
def claw_command(args):
    """Route Orcanium claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: Orcanium claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Orcanium")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'Orcanium claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Orcanium migration."""
    ...
    # migrate-Orcanium posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: Orcanium claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {orcanium_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.Orcanium/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: Orcanium claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Orcanium home ─────────
    # Delegates to orcanium.cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `orcanium import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from orcanium.cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(orcanium_home=orcanium_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: Orcanium import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Orcanium home."
            )
    ...
    # 'Orcanium claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        print_info("Stop OpenClaw first: systemctl --user stop openclaw-channel.service")
        print()
        if not auto_yes:
            if not sys.stdin.isatty():
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: Orcanium claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: Orcanium claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  Orcanium claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  Orcanium claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  orcanium config set OPENROUTER_API_KEY sk-or-v1-...")
