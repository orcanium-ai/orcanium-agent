#!/usr/bin/env python3
"""
SQLite State Store for Orcanium Agent.
...
DEFAULT_DB_PATH = get_orcanium_home() / "state.db"
...
# orcanium.cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``orcanium.cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see orcanium.cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple orcanium processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``orcanium_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `orcanium update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._orcanium_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._orcanium_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Orcanium) receive
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Orcanium surface and jump
    ...
    def _remove_session_files(sessions_dir: Optional[Path], session_id: str) -> None:
        ...
        ``request_dump_{session_id}_*.json`` files left by the channel.
    ...
    def delete_session_if_empty(
        ...
        and ``orcanium sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Orcanium, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Orcanium session.

        A Orcanium session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Orcanium session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ORCANIUM_DISABLE_FTS_TRIGRAM`` or not yet created), so
