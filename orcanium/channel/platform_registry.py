...
Allows platform adapters (built-in and plugin) to self-register so the channel
...
and are looked up first -- if nothing is found the channel falls through to
...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``orcanium setup`` display).
    ...
    # built-ins).  Used by ``orcanium channel setup`` to auto-enable the
    ...
    # ── Display ──
    # Emoji for CLI/channel display (e.g. "💬")
    emoji: str = "🔌"
    ...
    # ``channel status`` etc. can reflect env-only configuration without
    ...
    # YAML config translation instead of forcing core ``channel/config.py``
    ...
    # channel adapter.  Called by ``tools/send_message_tool._send_via_adapter``
    # when ``cron`` runs in a separate process from the channel and the
    ...
    # targets when the channel is not co-resident with the cron process.
