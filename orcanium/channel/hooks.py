...
Hooks are discovered from ~/.Orcanium/hooks/ directories, each containing:
  - HOOK.yaml  (metadata: name, description, events list)
  - handler.py (Python handler with async def handle(event_type, context))

Events:
  - channel:startup     -- Gateway process starts
...
Context dict passed to ``agent:start`` / ``agent:end`` handlers:
  ...
  session_id   -- Orcanium session id
...
HOOKS_DIR = get_orcanium_home() / "hooks"
...
class HookRegistry:
    ...
    def _register_builtin_hooks(self) -> None:
        ...
        point for future always-on channel hooks so they drop in without
    ...
    def discover_and_load(self) -> None:
        ...
        for hook_dir in sorted(HOOKS_DIR.iterdir()):
            ...
            try:
                ...
                module_name = f"orcanium_hook_{hook_name}"
