"""Periodic process memory usage logging for the channel.

Ported from cline/cline#10343 (src/standalone/memory-monitor.ts).

The channel is a long-lived process that accumulates memory as it caches
...
grep ``agent.log`` / ``channel.log`` for a time series of RSS + Python
...
Design notes (parity with the Cline port):
  ...
  * Uses ``resource`` (stdlib, Linux/macOS) first and falls back to
    ...
    the monitor rather than crashing the channel.

Config: ``logging.memory_monitor`` in ``config.yaml`` — see
``orcanium.cli/config.py`` for the defaults block.
"""
...
def _get_rss_mb() -> Optional[int]:
    ...
    falls back to ``psutil`` which is an optional orcanium-agent dep.
    """
...
def _monitor_loop(stop_event: threading.Event, interval: float) -> None:
    ...
    while not stop_event.wait(interval):
        try:
            ...
            # Never let the monitor crash the channel; just log and carry on.
