...
Extracted verbatim from ``channel/run.py`` (god-file decomposition Phase 3).
...
# "channel.run") so extracted log records keep their original logger name.
...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        Runs in the channel event loop; all SQLite work is pushed to a
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        """
        # Gate: only the dispatch-owning channel opens kanban DBs for notifier polling.
        # Non-dispatch gateways have no subscriptions to deliver — all kanban state lives
        # in the dispatch owner's per-board DBs. This prevents N-channel -shm contention.
        ...
        try:
            ...
            return
        env_override = os.environ.get("ORCANIUM_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban notifier: disabled via ORCANIUM_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        try:
            cfg = _load_config()
        ...
        if not kanban_cfg.get("dispatch_in_gateway", True):
            ...
            return
        from channel.config import Platform as _Platform
        try:
        ...
        # Initial delay so the channel can finish wiring adapters.
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # ORCANIUM_KANBAN_DB pins the board path; without this guard
                    # one channel could collect the same subscription/event
                    ...
                    for board_meta in boards:
                        ...
                        try:
                            ...
                            # — issue #21378) on every channel start against
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        When true, the channel hosts the single dispatcher for this profile:
        no separate `orcanium kanban daemon` process needed. When false, the
        ...
        Shutdown: the loop checks ``self._running`` between ticks; channel
        stop() flips it to False and cancels pending tasks, and the
        in-flight ``to_thread`` returns on its own after the current
        ``dispatch_once`` call finishes (typically <1ms on an idle board).
        """
        # Read config once at boot. If the user flips the flag later, they
        # restart the channel; same pattern as every other background
        # watcher here. Honours ORCANIUM_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        env_override = os.environ.get("ORCANIUM_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via ORCANIUM_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        ...
        # Initial delay so the channel finishes wiring adapters before the
        # dispatcher spawns workers (those workers may hit channel notify
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "the file changes, the channel restarts, or the "
                        "quarantine timer expires. Move or restore the file, "
                        "then run `orcanium kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Orcanium profile
            ...
            PATH, missing venv, credential loss for a real Orcanium profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
        ...
        def _auto_decompose_tick() -> int:
            ...
            """
            try:
                from orcanium.app.cli import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("ORCANIUM_KANBAN_BOARD")
                try:
                    os.environ["ORCANIUM_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("ORCANIUM_KANBAN_BOARD", None)
                    else:
                        os.environ["ORCANIUM_KANBAN_BOARD"] = prev_env
            return successes

        logger.info(
            "kanban dispatcher: embedded in channel (interval=%.1fs)", interval
        )
        while self._running:
            try:
            ...
            try:
                ...
                for slug, res in (results or []):
                    if res is not None and getattr(res, "spawned", None):
                        ...
                        # happened, so an idle channel stays silent.
                        logger.info(
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`orcanium kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
