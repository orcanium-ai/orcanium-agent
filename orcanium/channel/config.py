...
def _coerce_optional_positive_int(value: Any, key: str) -> Optional[int]:
    ...
    a warning so a typo never prevents the channel from starting.
    """
    if value is None:
        return None
...
class PlatformConfig:
    ...
    # Whether the channel is allowed to send "♻️ Gateway online" /
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_orcanium_home() / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        nested_gateway = data.get("channel") if isinstance(data.get("channel"), dict) else {}
        if "max_concurrent_sessions" in data:
            ...
            max_concurrent_key = "channel.max_concurrent_sessions"
...
def load_gateway_config() -> GatewayConfig:
    """
    Load channel configuration from multiple sources.

    Priority (highest to lowest):
    1. Environment variables
    2. ~/.Orcanium/config.yaml (primary user-facing config)
    3. ~/.Orcanium/channel.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_orcanium_home()
    gw_data: dict = {}

    # Legacy fallback: channel.json provides the base layer.
    # config.yaml keys always win when both specify the same setting.
    gateway_json_path = _home / "channel.json"
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # Each key overwrites whatever channel.json may have set.
            ...
            gateway_section = yaml_cfg.get("channel")
            ...
            if not isinstance(streaming_cfg, dict):
                # Fall back to nested channel.streaming written by
                # ``orcanium config set channel.streaming.*``
                streaming_cfg = yaml_cfg.get("channel", {}).get("streaming")
            ...
            # ``channel.platforms`` are loaded the same way as top-level
            # ``platforms``. Merge nested first so top-level config keeps
            # precedence, matching the existing channel.streaming fallback.
            gateway_cfg = yaml_cfg.get("channel")
            ...
            def _merge_platform_map(source_platforms: Any) -> None:
                ...
                for plat_name, plat_block in source_platforms.items():
                    ...
                    if not isinstance(existing, dict):
                        existing = {}
                    # Deep-merge extra dicts so channel.json defaults survive
            ...
            try:
                ...
                from channel.platform_registry import platform_registry as _pr
            except Exception as e:
            ...
            for plat in _shared_loop_targets:
                ...
                # ``channel.platforms`` so shared-key bridging (allow_from,
            ...
            if _pr is not None:
                for entry in _pr.all_entries():
                    ...
                    # Fall back to the platform's block under ``platforms`` /
                    # ``channel.platforms`` so adapter hooks still run when the
            ...
            if isinstance(telegram_cfg, dict):
                ...
                # channel.platforms.telegram.extra so the runtime config sees it.
        ...
        logger.warning(
            "Failed to process config.yaml — falling back to .env / channel.json values. "
        ...
        for platform, pconfig in config.platforms.items():
            ...
            if token and token.strip() and not has_usable_secret(token, min_length=4):
                logger.error(
                    ...
                    "Set a real bot token before starting the channel. "
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    if slack_token:
        ...
        # send Slack messages can use it without activating the channel adapter.
    ...
    # platforms the user never opted into, and the channel then tries to
