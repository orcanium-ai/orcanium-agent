...
The agent fires stream_delta_callback(text) synchroorcaniumly from its worker thread.
...
from orcanium.channel.platforms.base import BasePlatformAdapter as _BasePlatformAdapter
from orcanium.channel.platforms.base import _custom_unit_to_cp
from orcanium.channel.platforms.base import MEDIA_TAG_CLEANUP_RE
from orcanium.channel.config import (
    DEFAULT_STREAMING_EDIT_INTERVAL as _DEFAULT_STREAMING_EDIT_INTERVAL,
    DEFAULT_STREAMING_BUFFER_THRESHOLD as _DEFAULT_STREAMING_BUFFER_THRESHOLD,
    DEFAULT_STREAMING_CURSOR as _DEFAULT_STREAMING_CURSOR,
)

logger = logging.getLogger("channel.stream_consumer")
...
class StreamConsumerConfig:
    ...
    # behavior).  The channel enables this selectively per-platform.
    ...
    #   "off"   — handled by the channel before the consumer is even built.
    transport: str = "edit"
...
class GatewayStreamConsumer:
    ...
    def __init__(
        ...
        # fallback continuation). The channel uses this to linearize the
    ...
    def _reset_segment_state(self, *, preserve_no_edit: bool = False) -> None:
        ...
        # can't fool the channel. Safe: got_done returns before any reset, and
    ...
    def on_delta(self, text: str) -> None:
        ...
        appears below any tool-progress messages the channel sent in between.
        """
        if text:
    ...
    # machine; we do the same here so channel users never see raw
    ...
    def start(self):
        """Sync entry point: runs the async consumer in a daemon thread."""
        import asyncio
        self._thread = threading.Thread(
            target=lambda: asyncio.run(self.run()),
            daemon=True,
            name=f"stream-{self.chat_id[-8:]}",
        )
        self._thread.start()
        return self._thread
        ...
        try:
            while True:
                ...
                if got_done:
                    ...
                    # here instead of letting the base channel path send the
                    # full response again.
            ...
            # below suppress the channel's formatted re-send.
            ...
            # final_response_sent — which suppressed the channel's
    ...
    # MEDIA_TAG_CLEANUP_RE from channel/platforms/base.py — only tags whose
        ...
        for chunk in chunks:
            ...
            if not result or not result.success:
                if sent_any_chunk:
                    ...
                    # base channel final-send path should still deliver the
                    ...
                    self._fallback_prefix = ""
                    return
                # No fallback chunk reached the user — allow the normal channel
    ...
    def _resolve_draft_streaming(self) -> bool:
        ...
        if transport == "edit":
            return False
        # "off" is filtered upstream by the channel; treat as edit defensively.
        if transport == "off":
            return False
    ...
    async def _send_commentary(self, text: str) -> bool:
        ...
        try:
            ...
            # multiple tool calls. See: https://github.com/Orcanium/orcanium-agent/issues/10454
            if result.success:
    ...
    async def _try_fresh_final(self, text: str, *, is_turn_final: bool = True) -> bool:
        ...
        final-delivery flag is then left unset so the channel still delivers the
    ...
    async def _send_or_edit(
        ...
        # finishes (via _deliver_media_from_response in channel/run.py).
        text = self._clean_for_display(text)
        ...
        if (
            ...
            if ok:
                ...
                # _already_sent — that flag gates the channel's fallback
            ...
            if self._message_id is not None:
                if self._edit_supported:
                    ...
                    if result.success:
                        ...
                        if (
                            ...
                            # channel suppresses its normal full final send;
                ...
                if result.success:
                    ...
                    if not result.message_id:
                        ...
                        self._message_id = "__no_edit__"
                    # Notify the channel that a fresh content bubble was
