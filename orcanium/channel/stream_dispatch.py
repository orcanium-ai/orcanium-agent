...
events (channel/stream_events.py), and the *adapter* decides how each one is
...
progress queue the channel already drains, so the two no longer race through
independent code paths.

This module deliberately has no platform knowledge and no asyncio: it is a thin
synchroorcanium router callable from the agent's worker thread, exactly like the
...
from orcanium.channel.stream_events import (
    Commentary,
    GatewayNotice,
    LongToolHint,
    MessageChunk,
...
logger = logging.getLogger("channel.stream_events")
...
class GatewayEventDispatcher:
    ...
    enqueue_tool_line:
        Callback that places a rendered tool-progress line onto the channel's
    ...
    on_long_tool / on_notice:
        ...
        channel own the "should I surface this here?" decision.
    """
    ...
    def _dispatch(self, event: StreamEvent) -> None:
        ...
        if isinstance(event, ToolCallFinished):
            # Default: no chrome on completion (matches today — the channel only
