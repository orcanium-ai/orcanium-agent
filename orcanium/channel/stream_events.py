"""Structured streaming events — the agent→channel delivery contract.

Historically the agent drove channel delivery through a fan of loosely-typed
...
channel callback decided *both* what to render and how to send it.  That
...
channel knows what a given platform can render.

This module defines a small, typed event vocabulary that names *what happened*
without prescribing *how it is delivered*.  The channel's stream consumer
...
Design constraints (see orcanium-agent-dev skill — message-flow + cache
invariants):
  * Events describe *transport*, never *context*.  Nothing here is persisted to
    conversation history; what the channel chooses to "eat" (e.g. tool chrome on
  ...
  * Backward compatible by construction.  The channel adapts its existing
...
class ToolCallChunk:
    ...
    and the full ``args`` dict — and lets the *channel* decide presentation
    (emoji, truncation, verbose vs compact, or eat it entirely on platforms that
    don't show tool chrome).  Previously the agent's channel callback baked the
...
class ToolCallFinished:
    ...
    returned without raising.  The channel uses this to clear/settle a progress
...
class LongToolHint:
    ...
    The channel gates this on platform capability (the /verbose command must be
    usable) and on the user not having seen the hint before.  Modeled as an
    event so the *channel* owns the "should I surface this here?" decision rather
    than the agent.
    """
    tool_name: str = ""
    duration: float = 0.0
...
@dataclass(frozen=True)
class GatewayNotice:
    """A channel-originated control message (restart, online, long-run notice).
