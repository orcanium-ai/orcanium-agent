"""Shutdown forensics — capture context when the channel receives SIGTERM/SIGINT.

The channel's ``shutdown_signal_handler`` runs synchroorcaniumly inside the
asyncio event loop.  We can't safely block it for long, but we DO want a
durable record of who/what triggered the shutdown so that "the channel
...
the async helper, never in the synchroorcanium probe.
"""
...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    # Race-detection hint: did somebody recently start a sibling channel
    ...
    # Filenames mirror channel.status (._TAKEOVER_MARKER_FILENAME /
    # _PLANNED_STOP_MARKER_FILENAME); we use string literals here so the
    # signal-handler path stays import-light.
    try:
        orcanium_home_str = os.environ.get("ORCANIUM_HOME")
        if orcanium_home_str:
            takeover_path = Path(orcanium_home_str) / ".channel-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(orcanium_home_str) / ".channel-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    When the channel is run under a stale systemd unit file (e.g. the user
    upgraded orcanium-agent but never re-ran ``orcanium setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../orcanium-channel.service"
    ...
    # that's the common case for Orcanium.
    ...
    # constant in orcanium.cli/channel.py.
    headroom = 30.0
