"""
Email platform adapter for the Orcanium channel.

Allows users to interact with Orcanium by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            from orcanium.app.cli import __version__ as _orcanium_version
        except Exception:  # noqa: BLE001 — keep ID best-effort if import fails
            _orcanium_version = "0"
        imap.xatom(
            "ID",
            f'("name" "orcanium-agent" "version" "{_orcanium_version}" '
            '"vendor" "Orcanium" '
            '"support-email" "noreply@orcanium.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    """Email channel adapter using IMAP (receive) and SMTP (send)."""
    ...
    async def _dispatch_message(self, msg_data: Dict[str, Any]) -> None:
        ...
        # that the channel will never authorize.  Without this early guard,
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Orcanium Agent")
        ...
        msg_id = f"<orcanium-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
