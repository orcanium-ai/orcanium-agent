...
def _kill_stale_bridge_by_pidfile(session_path: Path) -> None:
    ...
    starts.  If the channel crashed without a clean shutdown the old bridge
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    # Default bridge location relative to the orcanium-agent install
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_orcanium_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
        ...
        # Without this, every graceful channel shutdown/restart would log
        ...
        # ``channel.platforms.whatsapp.extra.text_batch_delay_seconds`` /
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `orcanium channel`.",
                retryable=False,
            )
            return False
        ...
        # so every channel restart paid the 30s timeout + queued WhatsApp
        # for indefinite retries.  Mark non-retryable so the user gets a
        # clear "run Orcanium whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `orcanium whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `orcanium whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `orcanium update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".orcanium-pkg-hash"
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # bridge survives channel restarts AND
                                # `orcanium update`, so without this check it
            ...
            # Kill any orphaned bridge from a previous channel run
            ...
            # hardcodes ~/.Orcanium/{image,audio,document}_cache, which diverges
            # under ORCANIUM_HOME overrides, profiles, and the new cache/ layout.
            ...
            bridge_env["ORCANIUM_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["ORCANIUM_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["ORCANIUM_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: Orcanium whatsapp")
