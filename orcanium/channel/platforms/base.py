...
from orcanium.cli.compat import normalize_proxy_url

logger = logging.getLogger(__name__)

# Audio file extensions Orcanium recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Orcanium's DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Orcanium-created Telegram private-chat
...
from orcanium.channel.config import Platform, PlatformConfig
from orcanium.channel.session import SessionSource, build_session_key
from orcanium.channel.compat import get_default_orcanium_root, get_orcanium_dir, get_orcanium_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.Orcanium/.env manually."
)
...
async def _ssrf_redirect_guard(response):
    ...
    if response.is_redirect and response.next_request:
        ...
        from orcanium.channel.compat import is_safe_url
...
# Default location: {ORCANIUM_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_orcanium_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; OrcaniumAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_orcanium_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_orcanium_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_orcanium_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_orcanium_dir("cache/screenshots", "browser_screenshots")
_ORCANIUM_HOME = get_orcanium_home()
_ORCANIUM_ROOT = get_default_orcanium_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ORCANIUM_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ORCANIUM_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ORCANIUM_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ORCANIUM_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ORCANIUM_HOME / "image_cache",
    _ORCANIUM_HOME / "audio_cache",
    _ORCANIUM_HOME / "video_cache",
    _ORCANIUM_HOME / "document_cache",
    _ORCANIUM_HOME / "browser_screenshots",
    ...
    _ORCANIUM_HOME / "cache" / "images",
    _ORCANIUM_HOME / "cache" / "audio",
    _ORCANIUM_HOME / "cache" / "videos",
    _ORCANIUM_HOME / "cache" / "documents",
    _ORCANIUM_HOME / "cache" / "screenshots",
)
...
# should never be uploaded as a channel attachment, regardless of how new the
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Orcanium profile and shared Orcanium root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for orcanium_root in (_ORCANIUM_HOME, _ORCANIUM_ROOT):
        denied.append(orcanium_root / ".env")
        denied.append(orcanium_root / "auth.json")
        denied.append(orcanium_root / "credentials")
        denied.append(orcanium_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    denylist so that a non-root channel can't deliver another user's home, but
    on a root-run channel ``$HOME=/root`` and the operator's own deliverables
    (``/root/work/proposal.docx``) live directly under it. The credential
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Orcanium
    secrets (``~/.Orcanium/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Default mode (single-user / private channel): accept any existing regular
    ...
    Strict mode (opt-in via ``channel.strict`` in ``config.yaml`` or
    ``ORCANIUM_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Orcanium-managed cache, under an operator-allowlisted root
    (``ORCANIUM_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.Orcanium/.env,
    # ~/.Orcanium/auth.json, etc. — so the obvious prompt-injection sites
...
def cache_media_bytes(
    ...
    *,
    ...
    """
    from orcanium.channel.compat import to_agent_visible_cache_path
...
class MessageEvent:
    ...
    # new channel can advance the Telegram offset past it and avoid processing
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?channel[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?Orcanium\s+channel[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+Orcanium[.!?\s]*$", re.IGNORECASE),
)
...
def coerce_plaintext_gateway_command(event: "MessageEvent") -> None:
    ...
    This keeps high-impact operational phrases like ``restart channel`` out of
    the LLM/tool path, where they can trigger a self-restart from inside the
    currently running agent and leave the channel stuck in ``draining`` while it
...
class EphemeralReply(str):
    ...
    Slash-command handlers in ``channel/run.py`` can return this wrapper
...
class BasePlatformAdapter(ABC):
    ...
    # preview (see channel/run.py progress_callback).
    supports_code_blocks: bool = False

    # The command prefix users can always TYPE on this platform to reach
    # Orcanium commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # construction (channel/run.py). Default to "interrupt" so a stray
        # pre-sync read matches the single-knob default rather than silently
        # queueing.
        self._busy_text_mode: str = (
            os.environ.get("ORCANIUM_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ORCANIUM_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ORCANIUM_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
        self._text_debounce: dict[str, TextDebounceState] = {}
        # Background message-processing tasks spawned by handle_message().
        # Gateway shutdown cancels these so an old channel instance doesn't keep
    ...
    def enforces_own_access_policy(self) -> bool:
        ...
        reaches the channel unless it already passed that policy.

        The channel's env-based allowlist check runs *after* the adapter. When
        no env allowlist is configured, the channel consults this flag so it can
        ...
        sender, so the channel trusts the adapter only when its effective policy
        ...
        Adapters that delegate access control to the channel leave it ``False``
        (the default).
        """
        return False
    ...
    # streaming event (see channel/stream_events.py).  The default
    ...
    # channel has always produced.  Adapters override these to be more native
    ...
    def render_message_event(self, event: Any, sink: Any) -> None:
        ...
        """
        from channel.stream_events import MessageChunk, MessageStop, Commentary
        ...
        if isinstance(event, MessageChunk):
            ...
            # the terminal stop is signalled by the channel via finish(),
    ...
    def format_tool_event(self, event: Any, *, mode: str = "all",
        ...
        Reproduces the channel's historical tool-progress formatting: an emoji
        ...
        """
        from channel.stream_events import ToolCallChunk
        if not isinstance(event, ToolCallChunk):
            return None

        from orcanium.channel.compat import get_tool_emoji
        ...
        # "all" / "new": short preview, capped (default 40 to keep channel
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        from channel.status import acquire_scoped_lock
    ...
    def _release_platform_lock(self) -> None:
        ...
        if not identity:
            return
        from channel.status import release_scoped_lock
    ...
    async def create_handoff_thread(
        ...
        Used by the channel's handoff watcher when transferring a CLI
    ...
    def _schedule_ephemeral_delete(
        ...
        Best-effort — failures (channel restart, permission denied, message
    ...
    async def send_slash_confirm(
        ...
        Used by the channel's generic slash-confirm primitive (see
        ...
        routed back through the channel by calling
        ...
        through to the channel's text fallback (which sends ``message`` as
        plain text and intercepts the next ``/approve`` / ``/always`` /
        ``/cancel`` reply).

        ``confirm_id`` is a short string generated by the channel; the
    ...
    async def send_clarify(
        ...
        Two render modes:
          ...
          * **Open-ended** (``choices`` is None or empty) — render the
            ...
            session is captured by the channel's text-intercept and
        ...
        ("2") or with the literal choice text, and the channel intercepts
        and resolves.  For the text fallback path, the default calls
        ``mark_awaiting_text()`` so that the channel text-intercept
        ...
        if choices:
            ...
            text = "\n".join(lines)
            # Text fallback: enable text-capture so the channel intercept
            # picks up the user's typed reply (e.g. "2" or choice text).
            from orcanium.channel.compat import mark_awaiting_text
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.Orcanium/media/generated/stale.png"}
    ...
    def extract_local_files(content: str) -> Tuple[List[str], str]:
        ...
        partition lives in ``channel/run.py``.
        ...
        for match in path_re.finditer(content):
            ...
            if os.path.isfile(expanded):
                ...
                # channel.log rather than vanishing without a trace.
                logger.info(
    ...
    def register_post_delivery_callback(
        ...
        *,
        ...
        ``generation`` lets callers tie the callback to a specific channel run
    ...
    def _heal_stale_session_lock(self, session_key: str) -> bool:
        ...
        infinite "Interrupting current task..." until the channel is
        restarted.
        """
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            # dispatched directly to the channel runner.  Without this, they
            ...
            from orcanium.cli.commands import should_bypass_active_session
            ...
            if not cmd:
                try:
                    import orcanium.channel.compat as _clarify_mod
                    _has_text_clarify = (
        ...
        # pattern — set the guard synchroorcaniumly, not inside the task.)
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ORCANIUM_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ORCANIUM_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ORCANIUM_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ORCANIUM_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ORCANIUM_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ORCANIUM_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not media_files):
                    try:
                        from orcanium.channel.compat import text_to_speech_tool, check_tts_requirements
            ...
            # not at the top of this task.  _orcanium_run_generation is set on
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        Used during channel shutdown/replacement so active sessions from the old
