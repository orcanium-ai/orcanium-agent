...
from orcanium.channel.platforms.helpers import strip_markdown
...
from orcanium.channel.platforms.qqbot.constants import (
    API_BASE,
    TOKEN_URL,
    GATEWAY_URL_PATH,
    DEFAULT_API_TIMEOUT,
...
from orcanium.channel.platforms.qqbot.utils import (
    coerce_list as _coerce_list_impl,
    build_user_agent,
)
from orcanium.channel.platforms.qqbot.chunked_upload import (
    ChunkedUploader,
    UploadDailyLimitExceededError,
    UploadFileTooLargeError,
)
from orcanium.channel.platforms.qqbot.keyboards import (
    ApprovalRequest,
    InlineKeyboard,
    InteractionEvent,
...
class QQAdapter(BasePlatformAdapter):
    ...
    def _mark_transport_disconnected(self) -> None:
        ...
        short-lived channel or network failure.
        """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # ACKed it. Callers (channel wiring for approvals / update prompts)
        ...
        # to ~/.Orcanium/.update_response. Set here so the cross-adapter channel
    ...
    async def connect(self) -> bool:
        """Authenticate, obtain channel URL, and open the WebSocket."""
        ...
        try:
            ...
            from channel.platforms._http_client_limits import platform_httpx_limits
    ...
    async def _get_gateway_url(self) -> str:
        """Fetch the WebSocket channel URL from the REST API."""
        ...
        try:
            ...
            raise RuntimeError(f"Failed to get QQ Bot channel URL: {exc}") from exc

        url = data.get("url")
        if not url:
            raise RuntimeError(f"QQ Bot channel response missing url: {data}")
        return url
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        """Open a WebSocket connection to the QQ Bot channel."""
        ...
        # Honor WSL proxy env for QQ WebSocket. Orcanium upgrades overwrite this
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "orcanium-agent",
                    "$device": "orcanium-agent",
                },
            },
        }
        try:
    ...
    def _create_task(coro):
        ...
        ``_dispatch_payload`` synchroorcaniumly outside of ``asyncio.run()``.
        """
        try:
    ...
    def _dispatch_payload(self, payload: Dict[str, Any]) -> None:
        """Route inbound WebSocket payloads (dispatch synchroorcaniumly, spawn async handlers)."""
        op = payload.get("op")
        t = payload.get("t")
        s = payload.get("s")
        d = payload.get("d")
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.Orcanium/.update_response`` for the
          detached ``orcanium update --channel`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``orcanium update --channel`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `orcanium setup channel` / `orcanium channel`)
    ...
    async def send_approval_request(
        ...
        """
        from channel.platforms.qqbot.keyboards import build_approval_text
    ...
    # ------------------------------------------------------------------
    # Cross-adapter channel contract — send_exec_approval + send_update_prompt
    # ------------------------------------------------------------------
    #
    # These mirror the signatures that channel/run.py detects on the adapter
    ...
    async def send_exec_approval(
        ...
        Called by ``channel/run.py``'s ``_approval_notify_sync`` when the
    ...
    _APPROVAL_TIMEOUT_SECONDS = 300  # matches channel's default gateway_timeout
    ...
    async def send_update_prompt(
        ...
        ``channel/run.py``'s ``orcanium update --channel`` watcher. Button
        ...
        ``~/.Orcanium/.update_response`` so the detached update process
        can read it.
        """
