...
def _get_orcanium_version() -> str:
    """Return the orcanium-agent package version, or 'dev' if unavailable."""
    try:
        from importlib.metadata import version
        return version("orcanium-agent")
    except Exception:
        return "dev"
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Orcanium/<orcanium_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Orcanium/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    orcanium_version = _get_orcanium_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Orcanium/{orcanium_version})"
