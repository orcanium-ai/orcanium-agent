...
from orcanium.channel.platforms.whatsapp_common import WhatsAppBehaviorMixin
...
# Inbound media cache lives under the user's Orcanium dir so it survives
# restarts and channel reloads — same convention the Baileys bridge uses.
_INBOUND_MEDIA_CACHE = Path(get_orcanium_dir("platforms/whatsapp_cloud/media", "whatsapp_cloud/media"))
...
def check_whatsapp_cloud_requirements() -> bool:
    ...
    for Graph API calls (outbound). Both ship with orcanium-agent's default
...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # decide we need replay protection across channel restarts.
        ...
        # threading an extra kwarg through the channel's base contract.
        # In-memory only; on channel restart the next inbound message
        # repopulates it.
        ...
        # by the channel's resolver. See ``_handle_interactive_reply`` for
        ...
        # fallback path, same as after a channel restart).
    ...
    # look it up without needing the channel base contract to plumb
    ...
    async def send_clarify(
        ...
        - 4+ choices → ``interactive.type=list`` (tap-to-open sheet with
          ...
          text-capture mode handled by the channel's text intercept.
        - 0 choices (open-ended) → plain text question; the next message
          in the session is captured by the channel and resolves clarify.
        ...
        # Open-ended → just send the question, channel captures next msg.
        if not choices:
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        a generic audio attachment. Orcanium TTS produces MP3, so we try
    ...
    async def _dispatch_interactive_reply(
        ...
        if button_id.startswith("cl:"):
            ...
            if choice == "other":
                ...
                # into text-capture mode so the channel's text-intercept
                ...
                if not flipped:
                    ...
                    # (timeout, /new, channel restart). Drop the stale
    ...
    async def _build_message_event_from_cloud(
        ...
        """Convert a Cloud-API message object into a Orcanium MessageEvent.
        ...
        # channel resolver BEFORE falling through to text dispatch — the
