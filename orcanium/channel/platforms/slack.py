...
from orcanium.channel.platforms.helpers import MessageDeduplicator
...
def check_slack_requirements() -> bool:
    ...
    from orcanium.app.tools.lazy_deps import ensure_and_bind
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_orcanium_home() / "slack_tokens.json"
        ...
        try:
            ...
            # calling connect() a second time (e.g. during a channel restart or
            ...
            # Orcanium does not currently need to route them into the agent loop.
            # Ack the events explicitly so high-traffic channels do not fill
            # channel.error.log with Slack Bolt "Unhandled request" warnings.
            ...
            #
            # Every channel command from COMMAND_REGISTRY is a native Slack
            # slash, matching Discord and Telegram's model (e.g. /btw, /stop,
            # /model work directly without /Orcanium prefix). A single regex
            ...
            # manifest (see `orcanium slack manifest`). In Socket Mode, Slack
            ...
            from orcanium.cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/Orcanium$")

            @self._app.command(_slash_pattern)
            async def handle_orcanium_command(ack, command):
            ...
            for _action_id in (
                "orcanium_approve_once",
                "orcanium_approve_session",
                "orcanium_approve_always",
                "orcanium_deny",
            ):
            ...
            for _action_id in (
                "orcanium_confirm_once",
                "orcanium_confirm_always",
                "orcanium_confirm_cancel",
            ):
            ...
            # down the channel: any exception inside the plugin handler is
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Orcanium handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    async def send(
        ...
        try:
            ...
            # Controlled via platform config: channel.slack.reply_broadcast
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Orcanium session — matching the per-thread behavior channels already
    ...
    def _resolve_thread_ts(
        ...
        channel) over reply_to (which may be a child message's ts).
    ...
    async def send_multiple_images(
        ...
        try:
            ...
            from orcanium.app.tools.url_safety import is_safe_url as _is_safe_url
        except Exception:
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        # channel dispatcher) handles it like a normal slash command.  Only
        # rewrite when the first token resolves to a known channel command
        # so casual messages like "!nice work" pass through unchanged.
        if original_text.startswith("!"):
            try:
                from orcanium.cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@Orcanium`` still resolve.
        ...
        # Mirrors the Telegram/Discord implementations so that channel.run
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            # and the channel falls back to the plain-text prompt (no
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "orcanium_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "orcanium_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "orcanium_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "orcanium_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "orcanium_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "orcanium_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "orcanium_confirm_once": "once",
            "orcanium_confirm_always": "always",
            "orcanium_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        # bypass the normal message auth flow in channel/run.py, so we must
        # check here as well.
        allowed_csv = ""  # Interactive auth already ran above.
        if allowed_csv:
            allowed_ids = {uid.strip() for uid in allowed_csv.split(",") if uid.strip()}
        ...
        choice_map = {
            "orcanium_approve_once": "once",
            "orcanium_approve_session": "session",
            "orcanium_approve_always": "always",
            "orcanium_deny": "deny",
        }
        ...
        try:
            ...
            logger.error(
                "Failed to resolve channel approval from Slack button: %s", exc
            )
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        Every channel command in COMMAND_REGISTRY is registered as a native
        ...
        The legacy ``/Orcanium <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/Orcanium
        ...
        if slash_name in {"orcanium", ""}:
            # Legacy /Orcanium <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from orcanium.cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/Orcanium   ``).
            ...
            # channel command dispatcher by prepending the slash.
        ...
        # "Running /cmd…" ack shown by handle_orcanium_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/Orcanium <question>" must produce public replies so
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            for attempt in range(3):
                try:
                    ...
                    if audio:
                        from channel.platforms.base import cache_audio_from_bytes

                        return cache_audio_from_bytes(response.content, ext)
                    else:
                        from channel.platforms.base import cache_image_from_bytes
