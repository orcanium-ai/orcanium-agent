...
from orcanium.channel.platforms.base import (
    BasePlatformAdapter,
    MessageEvent,
    MessageType,
    ProcessingOutcome,
...
from orcanium.channel.platforms.telegram_network import (
    TelegramFallbackTransport,
    discover_fallback_ips,
    parse_fallback_ip_env,
)
from orcanium.channel.compat import atomic_replace
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("ORCANIUM_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "ORCANIUM_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "ORCANIUM_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Orcanium-created private-chat topic lanes are marked with
    ...
    async def _handle_polling_network_error(self, error: Exception) -> None:
        ...
        reconnect, etc.).  The channel process stays alive but the long-poll
        ...
        the supervisor restarts the channel process.
        """
        ...
        if attempt > MAX_NETWORK_RETRIES:
            message = (
                ...
                "Restarting channel." % MAX_NETWORK_RETRIES
            )
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        if self.has_fatal_error and self.fatal_error_code == "telegram_polling_conflict":
            return
        # Transient 409 Conflict errors arise when the previous channel process
        # has been killed (e.g. during `orcanium update` or `--replace` handoffs)
        ...
        # client disconnects, so a new channel starting immediately will receive
        ...
        if self._polling_conflict_count <= MAX_CONFLICT_RETRIES:
            ...
            try:
                ...
                # channel process is alive and reports "connected" but
        ...
        # Exhausted all retries — declare a fatal error so the channel
        ...
        message = (
            ...
            "The previous channel session is still held open on Telegram's servers, "
            "or another process is using the same bot token. "
            "To recover: ensure no other Orcanium or OpenClaw instance is running "
            "with this token, then restart the channel with 'orcanium channel restart'."
    ...
    def _persist_dm_topic_thread_id(
        ...
        try:
            ...
            config_path = get_orcanium_home() / "config.yaml"
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Orcanium process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("ORCANIUM_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("ORCANIUM_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("ORCANIUM_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("ORCANIUM_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("ORCANIUM_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("ORCANIUM_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            # channel command there automatically adds it to the Telegram menu.
            try:
                ...
                from orcanium.cli.commands import telegram_menu_commands
    ...
    async def _edit_overflow_split(
        ...
        *,
        ...
        and the channel has full visibility into every message id we put on
        screen.
    ...
    async def send_update_prompt(
        ...
        Used by the channel ``/update`` watcher when ``orcanium update --channel``
    ...
    async def send_clarify(
        ...
        the channel's text-intercept and resolves the clarify.
        """
        if not self._bot:
            return SendResult(success=False, error="Not connected")
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``orcanium model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from orcanium.cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from orcanium.cli.models import PROVIDER_GROUPS
        ...
        if data.startswith("ea:"):
            ...
            if len(parts) == 3:
                ...
                try:
                    ...
                    logger.error("Failed to resolve channel approval from Telegram button: %s", exc)
                    count = 0

                # Resume the typing indicator — paused when the approval was
                # sent (channel/run.py).  The text /approve and /deny paths
        ...
        if data.startswith("cl:"):
            ...
            if len(parts) == 3:
                ...
                if choice_token == "other":
                    ...
                    # their answer.  The channel's text-intercept will pick
                    ...
                    try:
                        from orcanium.app.tools.clarify_gateway import mark_awaiting_text
                ...
                try:
                    from orcanium.app.tools.clarify_gateway import _entries as _clarify_entries  # type: ignore
                ...
                try:
                    from orcanium.app.tools.clarify_gateway import resolve_gateway_clarify
    ...
    # Scripts live in ~/.Orcanium/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".Orcanium" / "scripts" / "gmail-triage" / script_name
    ...
    def _missing_media_path_error(self, label: str, path: str) -> str:
        """Build an actionable file-not-found error for channel MEDIA delivery.

        Paths like /workspace/... or /output/... often only exist inside the
        Docker sandbox, while the channel process runs on the host.
        """
        ...
        if path.startswith(("/workspace/", "/output/", "/outputs/")):
            error += (
                ...
                "path in MEDIA: for channel file delivery.)"
            )
        return error
    ...
    async def send_image(
        ...
        from orcanium.app.tools.url_safety import is_safe_url
    ...
    def _telegram_observe_allowed_chats(self) -> set[str]:
        ...
        ``group_allowed_chats`` is the channel authorization allowlist for
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@orcanium_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Orcanium bot profiles. A message like
    ...
    def _should_observe_unmentioned_group_message(self, message: Message) -> bool:
        ...
        # lets channel authorization pass even after the shared session source
    ...
    async def _cache_observed_media(self, msg: Message, event: MessageEvent) -> None:
        ...
        """
        from channel.platforms.base import cache_media_bytes
    ...
    def _effective_update_message(self, update: Update) -> Optional[Message]:
        ...
        consuming channel posts without ever building a channel event.
        """
    ...
    def _text_batch_key(self, event: MessageEvent) -> str:
        ...
        """
        from channel.session import build_session_key
    ...
    async def _queue_media_group_event(self, media_group_id: str, event: MessageEvent) -> None:
        ...
        If we forward each item immediately, the channel thinks the second image is a
    ...
    async def _handle_sticker(self, msg: Message, event: "MessageEvent") -> None:
        ...
        """
        from channel.sticker_cache import (
    ...
    def _reload_dm_topics_from_config(self) -> None:
        ...
        recognized without a channel restart.
        """
        try:
            from orcanium.channel.compat import get_orcanium_home
            config_path = get_orcanium_home() / "config.yaml"
    ...
    def _build_message_event(
        ...
        lets ``/restart`` record the triggering offset so the new channel
        ...
        # General-topic id so the channel routes back to the General topic
        ...
        from channel.platforms.base import resolve_channel_prompt
