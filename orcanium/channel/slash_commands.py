...
Extracted from ``channel/run.py`` (god-file decomposition Phase 3b). These are
the in-session slash commands (/model, /reset, /usage, /compress, ...) the
channel dispatches from ``_handle_message``. There are 42 of them (~3,200 LOC);
...
Module-level run.py helpers a handler needs (``_orcanium_home``,
``_load_gateway_config``, ``_resolve_gateway_model``, etc.) are imported lazily
inside the handler body — a deferred ``from channel.run import ...`` resolves at
call time (run.py fully loaded by then), avoiding an import cycle.
"""

from __future__ import annotations
import orcanium.cli.compat  # noqa: F401
...
from orcanium.channel.config import HomeChannel, Platform, PlatformConfig
from orcanium.channel.platforms.base import EphemeralReply, MessageEvent, MessageType
from orcanium.channel.session import SessionSource, build_session_key
from orcanium.cli.config import cfg_get
from orcanium.cli.compat import (
    atomic_json_write,
    atomic_yaml_write,
...
logger = logging.getLogger("channel.run")
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Orcanium commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            from orcanium.app.tools.env_passthrough import clear_env_passthrough
            clear_env_passthrough()
        except Exception:
            pass

        try:
            from orcanium.app.tools.credential_files import clear_credential_files
        ...
        if new_entry:
            header = self._telegram_topic_new_header(source) or t("channel.reset.header_default")
        else:
            # No existing session, just create one
            new_entry = self.session_store.get_or_create_session(source, force_new=True)
            header = self._telegram_topic_new_header(source) or t("channel.reset.header_new")
        ...
        if _title_arg and self._session_db and new_entry:
            ...
            try:
                ...
                sanitized = None
                _title_note = t("channel.reset.title_rejected", error=str(e))
            if sanitized:
                try:
                    self._session_db.set_session_title(new_entry.session_id, sanitized)
                    header = t("channel.reset.header_titled", title=sanitized)
                except ValueError as e:
                    _title_note = t("channel.reset.title_error_untitled", error=str(e))
                except Exception:
                    pass
            elif not _title_note:
                # sanitize_title returned empty (whitespace-only / unprintable)
                _title_note = t("channel.reset.title_empty_untitled")
        ...
        try:
            from orcanium.cli.tips import get_random_tip
            _tip_line = t("channel.reset.tip", tip=get_random_tip())
        except Exception:
            _tip_line = ""
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        display = display_orcanium_home()
        profile_name = get_active_profile_name()

        lines = [
            t("channel.profile.header", profile=profile_name),
            t("channel.profile.home", home=display),
        ]
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        channel event loop stays responsive.  Read operations (list,
        ...
        originating channel source (platform + chat + thread) to the new
        ...
        import re
        import shlex
        from orcanium.cli.kanban import run_slash
        ...
        try:
            ...
            return t("channel.kanban.error_prefix", error=exc)
            ...
            if m:
                ...
                try:
                    ...
                    if platform_str and chat_id:
                        def _sub():
                            from orcanium.app.cli import kanban_db as _kb
                        ...
                        output = (
                            output.rstrip()
                            + "\n"
                            + t("channel.kanban.subscribed_suffix", task_id=task_id)
                        )
        ...
        if len(output) > 3800:
            output = output[:3800] + "\n" + t("channel.kanban.truncated_suffix")
        return output or t("channel.kanban.no_output")
    ...
    async def _handle_status_command(self, event: MessageEvent) -> str:
        ...
        lines = [
            t("channel.status.header"),
            "",
            t("channel.status.session_id", session_id=session_entry.session_id),
        ]
        if title:
            lines.append(t("channel.status.title", title=title))
        lines.extend([
            t("channel.status.created", timestamp=session_entry.created_at.strftime('%Y-%m-%d %H:%M')),
            t("channel.status.last_activity", timestamp=session_entry.updated_at.strftime('%Y-%m-%d %H:%M')),
            t("channel.status.tokens", tokens=f"{db_total_tokens:,}"),
            t("channel.status.agent_running", state=t("channel.status.state_yes") if is_running else t("channel.status.state_no")),
        ])
        if queue_depth:
            lines.append(t("channel.status.queued", count=queue_depth))
        if source.platform == Platform.MATRIX:
            ...
            lines.extend([
                "",
                t("channel.status.matrix_scope_header"),
                t("channel.status.matrix_scope_room", room=source.chat_name or source.chat_id),
                t("channel.status.matrix_scope_room_id", room_id=source.chat_id),
                t("channel.status.matrix_scope_thread", thread_id=thread),
                t("channel.status.matrix_scope_mode", scope=scope),
                t(
                    "channel.status.matrix_scope_key",
                    session_key=self._redact_matrix_session_key(session_key),
                ),
            ])
        lines.extend([
            "",
            t("channel.status.platforms", platforms=', '.join(connected_platforms)),
        ])
    ...
    def _gateway_session_origin_for_id(self, session_id: str) -> Optional[SessionSource]:
        """Best-effort origin lookup for channel session IDs."""
    ...
    async def _handle_agents_command(self, event: MessageEvent) -> str:
        ...
        from channel.run import _AGENT_PENDING_SENTINEL
        from orcanium.app.tools.process_registry import format_uptime_short, process_registry
            ...
            agent_rows.append(
                {
                    "session_key": session_key,
                    "elapsed": elapsed,
                    "state": t("channel.agents.state_starting") if is_pending else t("channel.agents.state_running"),
        ...
        lines = [
            t("channel.agents.header"),
            "",
            t("channel.agents.active_agents", count=len(agent_rows)),
        ]

        if agent_rows:
            for idx, row in enumerate(agent_rows[:12], 1):
                current = t("channel.agents.this_chat") if row["session_key"] == current_session_key else ""
            ...
            if len(agent_rows) > 12:
                lines.append(t("channel.agents.more", count=len(agent_rows) - 12))

        lines.extend(
            [
                "",
                t("channel.agents.running_processes", count=len(running_processes)),
            ]
        )
        if running_processes:
            ...
            if len(running_processes) > 12:
                lines.append(t("channel.agents.more", count=len(running_processes) - 12))

        lines.extend(
            [
                "",
                t("channel.agents.async_jobs", count=len(background_tasks)),
            ]
        )

        if not agent_rows and not running_processes and not background_tasks:
            lines.append("")
            lines.append(t("channel.agents.none"))
    ...
    async def _handle_stop_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        """
        from channel.run import _AGENT_PENDING_SENTINEL, _INTERRUPT_REASON_STOP
        source = event.source
        session_entry = self.session_store.get_or_create_session(source)
        session_key = session_entry.session_key
        ...
        if agent is _AGENT_PENDING_SENTINEL:
            ...
            return EphemeralReply(t("channel.stop.stopped_pending"))
        if agent:
        ...
        return t("channel.stop.no_active")

    async def _handle_platform_command(self, event: MessageEvent) -> str:
        """Handle ``/platform list|pause|resume [name]`` — surface and
        manually control failed/paused channel adapters.
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`orcanium channel restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /restart command - drain active work, then restart the channel."""
        from channel.run import _orcanium_home
        # Defensive idempotency check: if the previous channel process
        ...
        # in channel.log).  Ignoring the stale redelivery prevents a
        # self-perpetuating restart loop where every fresh channel
        ...
        if self._is_stale_restart_redelivery(event):
            logger.info(
                ...
                "already processed by a previous channel instance.",
        ...
        if self._restart_requested or self._draining:
            ...
            if count:
                return t("channel.draining", count=count)
            return EphemeralReply(t("channel.restart.in_progress"))

        # Save the requester's routing info so the new channel process can
        ...
        try:
            ...
            atomic_json_write(
                _orcanium_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        # new channel sends the "channel restarted" notification), this
        # marker persists so the new channel can still detect a delayed
        ...
        try:
            ...
            atomic_json_write(
                _orcanium_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
        ...
        # exits when the channel dies, taking the detached helper with it).
        ...
        if active_agents:
            return t("channel.draining", count=active_agents)
        return EphemeralReply(t("channel.restart.restarting"))

    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Orcanium Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from channel.run import _telegramize_command_mentions
        from orcanium.cli.commands import channel_help_lines
        lines = [
            t("channel.help.header"),
            *gateway_help_lines(),
        ]
        try:
            ...
            if skill_cmds:
                lines.append(t("channel.help.skill_header", count=len(skill_cmds)))
                # Show first 10, then point to /commands for the rest
                sorted_cmds = sorted(skill_cmds)
                for cmd in sorted_cmds[:10]:
                    lines.append(f"`{cmd}` — {skill_cmds[cmd]['description']}")
                if len(sorted_cmds) > 10:
                    lines.append(t("channel.help.more_use_commands", count=len(sorted_cmds) - 10))
        except Exception:
            pass
    ...
    async def _handle_commands_command(self, event: MessageEvent) -> str:
        ...
        if raw_args:
            try:
                ...
                return t("channel.commands.usage")
        else:
            requested_page = 1
        ...
        try:
            ...
            if skill_cmds:
                entries.append("")
                entries.append(t("channel.commands.skill_header"))
                for cmd in sorted(skill_cmds):
                    desc = skill_cmds[cmd].get("description", "").strip() or t("channel.commands.default_desc")
                    entries.append(f"`{cmd}` — {desc}")
        except Exception:
            pass

        if not entries:
            return t("channel.commands.none")
        ...
        lines = [
            t("channel.commands.header", total=len(entries), page=page, total_pages=total_pages),
            "",
            *page_entries,
        ]
        if total_pages > 1:
            nav_parts = []
            if page > 1:
                nav_parts.append(t("channel.commands.nav_prev", page=page - 1))
            if page < total_pages:
                nav_parts.append(t("channel.commands.nav_next", page=page + 1))
            lines.extend(["", " | ".join(nav_parts)])
        if page != requested_page:
            lines.append(t("channel.commands.out_of_range", requested=requested_page, page=page))
        return _telegramize_command_mentions(
            "\n".join(lines),
            getattr(getattr(event, "source", None), "platform", None),
        )
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from channel.run import _orcanium_home, _load_gateway_config
        import yaml
        from orcanium.cli.model_switch import (
            switch_model as _switch_model, parse_model_flags,
            list_authenticated_providers,
            list_picker_providers,
        )
        from orcanium.cli.providers import get_label
        ...
        if not model_input and not explicit_provider:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        if not result.success:
                            return t("channel.model.error_prefix", error=result.error_message)
                        ...
                        lines = [t("channel.model.switched", model=result.new_model)]
                        lines.append(t("channel.model.provider_label", provider=plabel))
                        mi = result.model_info
                        from orcanium.cli.model_switch import resolve_display_context_length
                        _sw_config_ctx = None
                        try:
                        ...
                        if ctx:
                            lines.append(t("channel.model.context_label", tokens=f"{ctx:,}"))
                        if mi:
                            if mi.max_output:
                                lines.append(t("channel.model.max_output_label", tokens=f"{mi.max_output:,}"))
                            if mi.has_cost_data():
                                lines.append(t("channel.model.cost_label", cost=mi.format_cost()))
                            lines.append(t("channel.model.capabilities_label", capabilities=mi.format_capabilities()))
                        lines.append(t("channel.model.session_only_hint"))
                        return "\n".join(lines)
            ...
            lines = [t("channel.model.current_label", model=current_model or "unknown", provider=provider_label), ""]
            ...
            try:
                ...
                for p in providers:
                    tag = t("channel.model.current_tag") if p["is_current"] else ""
                    lines.append(f"**{p['name']}** `--provider {p['slug']}`{tag}:")
                    if p["models"]:
                        model_strs = ", ".join(f"`{m}`" for m in p["models"])
                        extra = t("channel.model.more_models_suffix", count=p["total_models"] - len(p["models"])) if p["total_models"] > len(p["models"]) else ""
            ...
            lines.append(t("channel.model.usage_switch_model"))
            lines.append(t("channel.model.usage_switch_provider"))
            lines.append(t("channel.model.usage_persist"))
            return "\n".join(lines)
        ...
        async def _finish_switch() -> str:
            ...
            lines = [t("channel.model.switched", model=result.new_model)]
            lines.append(t("channel.model.provider_label", provider=provider_label))

            # Context: always resolve via the provider-aware chain so Codex OAuth,
            # Copilot, and Orcanium-enforced caps win over the raw models.dev entry.
            mi = result.model_info
            from orcanium.cli.model_switch import resolve_display_context_length
            ...
            if cache_enabled:
                lines.append(t("channel.model.prompt_caching_enabled"))

            if result.warning_message:
                lines.append(t("channel.model.warning_prefix", warning=result.warning_message))

            if persist_global:
                lines.append(t("channel.model.saved_global"))
            else:
                lines.append(t("channel.model.session_only_hint"))
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        """Handle /codex-runtime command in the channel.

        Same surface as the CLI handler in cli.py:
            /codex-runtime                  — show current state
            /codex-runtime auto             — Orcanium default runtime
        ...
        from orcanium.app.cli import codex_runtime_switch as crs
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("channel.personality.none_configured", path=display_orcanium_home())

        if not args:
            lines = [t("channel.personality.header")]
            lines.append(t("channel.personality.none_option"))
            for name, prompt in personalities.items():
                if isinstance(prompt, dict):
                    preview = prompt.get("description") or prompt.get("system_prompt", "")[:50]
                else:
                    preview = prompt[:50] + "..." if len(prompt) > 50 else prompt
                lines.append(t("channel.personality.item", name=name, preview=preview))
            lines.append(t("channel.personality.usage"))
            return "\n".join(lines)
        ...
        if args in {"none", "default", "neutral"}:
            try:
                ...
                return t("channel.personality.save_failed", error=str(e))
            self._ephemeral_system_prompt = ""
            return t("channel.personality.cleared")
            ...
            return t("channel.personality.set_to", name=args)

        available = "`none`, " + ", ".join(f"`{n}`" for n in personalities)
        return t("channel.personality.unknown", name=args, available=available)
    ...
    async def _handle_retry_command(self, event: MessageEvent) -> str:
        ...
        if not last_user_msg:
            return t("channel.retry.no_previous")
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        """Handle /goal for channel platforms.
        ...
        if mgr is None:
            return t("channel.goal.unavailable")
        ...
        if lower == "pause":
            ...
            if state is None:
                return t("channel.goal.no_goal_set")
            try:
            ...
            return t("channel.goal.paused", goal=state.goal)

        if lower == "resume":
            state = mgr.resume()
            if state is None:
                return t("channel.goal.no_resume")
            return t("channel.goal.resumed", goal=state.goal)
        ...
        if lower in {"clear", "stop", "done"}:
            ...
            return t("channel.goal_cleared") if had else t("channel.no_active_goal")

        # Otherwise — treat the remaining text as the new goal.
        try:
            state = mgr.set(args)
        except ValueError as exc:
            return t("channel.goal.invalid", error=str(exc))
        ...
        return t("channel.goal.set", budget=state.max_turns, goal=state.goal)

    async def _handle_subgoal_command(self, event: "MessageEvent") -> str:
        """Handle /subgoal for channel platforms (mirror of CLI handler).
    ...
    async def _handle_undo_command(self, event: MessageEvent) -> str:
        ...
        (active-only) transcript — the channel's equivalent of the CLI's
        ...
        if raw_args:
            try:
                ...
                return t("channel.undo.invalid_count", arg=raw_args.split()[0])
            if n < 1:
                n = 1
    ...
    async def _handle_set_home_command(self, event: MessageEvent) -> str:
        ...
        from channel.run import _home_target_env_var, _home_thread_env_var
        source = event.source
        ...
        try:
            ...
            return t("channel.set_home.save_failed", error=e)

        # Keep the running channel config in sync too. The pre-restart
        ...
        return t("channel.set_home.success", name=chat_name, chat_id=chat_id)
    ...
    async def _handle_voice_command(self, event: MessageEvent) -> str:
        ...
        if args in {"on", "enable"}:
            ...
            return t("channel.voice.enabled_voice_only")
        elif args in {"off", "disable"}:
            self._voice_mode[voice_key] = "off"
            self._save_voice_modes()
            if adapter:
                self._set_adapter_auto_tts_disabled(adapter, chat_id, disabled=True)
            return t("channel.voice.disabled_text")
        elif args == "tts":
            self._voice_mode[voice_key] = "all"
            self._save_voice_modes()
            if adapter:
                self._set_adapter_auto_tts_enabled(adapter, chat_id, enabled=True)
            return t("channel.voice.tts_enabled")
            ...
            labels = {
                "off": t("channel.voice.label_off"),
                "voice_only": t("channel.voice.label_voice_only"),
                "all": t("channel.voice.label_all"),
            }
            ...
            if guild_id and hasattr(adapter, "get_voice_channel_info"):
                ...
                if info:
                    lines = [
                        t("channel.voice.status_mode", label=labels.get(mode, mode)),
                        t("channel.voice.status_channel", channel=info['channel_name']),
                        t("channel.voice.status_participants", count=info['member_count']),
                    ]
                    for m in info["members"]:
                        status = t("channel.voice.speaking") if m.get("is_speaking") else ""
                        lines.append(t("channel.voice.status_member", name=m['display_name'], status=status))
                    return "\n".join(lines)
            return t("channel.voice.status_mode", label=labels.get(mode, mode))
        else:
            ...
            if current == "off":
                ...
                if adapter:
                    self._set_adapter_auto_tts_enabled(adapter, chat_id, enabled=True)
                toggle_line = t("channel.voice.enabled_short")
            else:
                self._voice_mode[voice_key] = "off"
                self._save_voice_modes()
                if adapter:
                    self._set_adapter_auto_tts_disabled(adapter, chat_id, disabled=True)
                toggle_line = t("channel.voice.disabled_short")
            ...
            channels = (
                t("channel.voice.help_channels") if supports_voice_channels else ""
            )
            return t("channel.voice.help", toggle=toggle_line, channels=channels)

    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        """Handle /rollback command — list or restore filesystem checkpoints."""
        from channel.run import _orcanium_home
        from orcanium.app.tools.checkpoint_manager import CheckpointManager, format_checkpoint_list

        # Read checkpoint config from config.yaml
        cp_cfg = {}
        try:
            import yaml as _y
            _cfg_path = _orcanium_home / "config.yaml"
            if _cfg_path.exists():
        ...
        if not cp_cfg.get("enabled", False):
            return t("channel.rollback.not_enabled")
        ...
        checkpoints = mgr.list_checkpoints(cwd)
        if not checkpoints:
            return t("channel.rollback.none_found", cwd=cwd)
        ...
        try:
            ...
            if 0 <= idx < len(checkpoints):
                ...
                return t("channel.rollback.invalid_number", max=len(checkpoints))
        except ValueError:
            target_hash = arg
        ...
        if result["success"]:
            return t(
                "channel.rollback.restored",
                hash=result["restored_to"],
                reason=result["reason"],
            )
        return t("channel.rollback.restore_failed", error=result["error"])
    ...
    async def _handle_background_command(self, event: MessageEvent) -> str:
        ...
        if not prompt:
            return t("channel.background.usage")
        ...
        return t("channel.background.started", preview=preview, task_id=task_id)
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from channel.run import _orcanium_home, _platform_config_key
        import yaml
        ...
        if not raw_args:
            ...
            if rc is None:
                level = t("channel.reasoning.level_default")
            elif rc.get("enabled") is False:
                level = t("channel.reasoning.level_disabled")
            else:
                level = rc.get("effort", "medium")
            display_state = (
                t("channel.reasoning.display_on")
                if self._show_reasoning
                else t("channel.reasoning.display_off")
            )
            has_session_override = session_key in (getattr(self, "_session_reasoning_overrides", {}) or {})
            scope = (
                t("channel.reasoning.scope_session")
                if has_session_override
                else t("channel.reasoning.scope_global")
            )
            return t(
                "channel.reasoning.status",
                level=level,
                scope=scope,
        ...
        if args in {"show", "on"}:
            ...
            return t("channel.reasoning.display_set_on", platform=platform_key)

        if args in {"hide", "off"}:
            self._show_reasoning = False
            _save_config_key(f"display.platforms.{platform_key}.show_reasoning", False)
            return t("channel.reasoning.display_set_off", platform=platform_key)

        # Effort level change
        effort = args.strip()
        if effort == "reset":
            if persist_global:
                return t("channel.reasoning.reset_global_unsupported")
            self._set_session_reasoning_override(session_key, None)
            self._reasoning_config = self._load_reasoning_config()
            self._evict_cached_agent(session_key)
            return t("channel.reasoning.reset_done")
        if effort == "none":
        ...
        if persist_global:
            if _save_config_key("agent.reasoning_effort", effort):
                self._set_session_reasoning_override(session_key, None)
                self._evict_cached_agent(session_key)
                return t("channel.reasoning.set_global", effort=effort)
            self._set_session_reasoning_override(session_key, parsed)
            self._evict_cached_agent(session_key)
            return t("channel.reasoning.set_global_save_failed", effort=effort)

        self._set_session_reasoning_override(session_key, parsed)
        self._evict_cached_agent(session_key)
        return t("channel.reasoning.set_session", effort=effort)
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from channel.run import _orcanium_home
        from orcanium.cli.write_approval_commands import handle_pending_subcommand
        from orcanium.app.tools import write_approval as wa
        from orcanium.app.tools.memory_tool import MemoryStore
        ...
        # Apply approved writes against a fresh on-disk store (the channel has
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        """Handle /skills on the channel — pending skill-write review only.
        ...
        reject / diff / approval) so a skill staged from a channel session can
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + f"\n… (truncated — full diff: `/skills diff {pending_id}` "
                     f"on the CLI, or ~/.Orcanium/pending/skills/{pending_id}.json)")
        return out

    async def _handle_fast_command(self, event: MessageEvent) -> str:
        """Handle /fast — mirror the CLI Priority Processing toggle in channel chats."""
        from channel.run import _orcanium_home, _load_gateway_config, _resolve_gateway_model
        import yaml
        from orcanium.cli.models import model_supports_fast_mode
        ...
        if not model_supports_fast_mode(model):
            return t("channel.fast.not_supported")
        ...
        if not args or args == "status":
            status = t("channel.fast.status_fast") if self._service_tier == "priority" else t("channel.fast.status_normal")
            return t("channel.fast.status", mode=status)

        if args in {"fast", "on"}:
            self._service_tier = "priority"
            saved_value = "fast"
            label = t("channel.fast.label_fast")
        elif args in {"normal", "off"}:
            self._service_tier = None
            saved_value = "normal"
            label = t("channel.fast.label_normal")
        else:
            return t("channel.fast.unknown_arg", arg=args)

        if _save_config_key("agent.service_tier", saved_value):
            return t("channel.fast.saved", label=label)
        return t("channel.fast.session_only", label=label)
    ...
    async def _handle_yolo_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        if current:
            ...
            return EphemeralReply(t("channel.yolo.disabled"))
        else:
            enable_session_yolo(session_key)
            return EphemeralReply(t("channel.yolo.enabled"))
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from channel.run import _orcanium_home, _load_gateway_config, _platform_config_key
        ...
        if not gate_enabled:
            return t("channel.verbose.not_enabled")

        # --- cycle mode (per-platform) ----------------------------------------
        cycle = ["off", "new", "all", "verbose"]
        descriptions = {
            "off": t("channel.verbose.mode_off"),
            "new": t("channel.verbose.mode_new"),
            "all": t("channel.verbose.mode_all"),
            "verbose": t("channel.verbose.mode_verbose"),
        }
        ...
        try:
            ...
            return (
                ...
                + t("channel.verbose.saved_suffix", platform=platform_key)
            )
        except Exception as e:
            logger.warning("Failed to save tool_progress mode: %s", e)
            return f"{descriptions[new_mode]}\n" + t("channel.verbose.save_failed", error=e)
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from channel.run import _orcanium_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
        from channel.runtime_footer import resolve_footer_config
        ...
        try:
            ...
            return t("channel.config_read_failed", error=e)

        effective = resolve_footer_config(user_config, platform_key)

        if arg in {"status", "?"}:
            state = t("channel.footer.state_on") if effective["enabled"] else t("channel.footer.state_off")
            fields = ", ".join(effective.get("fields") or [])
            return t(
                "channel.footer.status",
                state=state,
                fields=fields,
        ...
        try:
            ...
            return t("channel.config_save_failed", error=e)

        state = t("channel.footer.state_on") if new_state else t("channel.footer.state_off")
        example = ""
        if new_state:
            # Show a preview using current agent state if available.
            from channel.runtime_footer import format_runtime_footer
            ...
            if preview:
                example = t("channel.footer.example_line", preview=preview)
        return t("channel.footer.saved", state=state, example=example)
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                return t("channel.compress.no_provider")
            ...
            try:
                ...
                if not compressor.has_content_to_compress(head):
                    return t("channel.compress.nothing_to_do")
            ...
            if focus_topic:
                lines.append(t("channel.compress.focus_line", topic=focus_topic))
            ...
            return t("channel.compress.failed", error=e)

    async def _handle_topic_command(self, event: MessageEvent, args: str = "") -> str:
        """Handle /topic for Telegram DM user-managed topic sessions."""
        source = event.source
        if source.platform != Platform.TELEGRAM or source.chat_type != "dm":
            return t("channel.topic.not_telegram_dm")
        if not self._session_db:
            from orcanium.channel.compat import format_session_db_unavailable
        ...
        if callable(auth_fn):
            try:
                if not auth_fn(source):
                    return t("channel.topic.unauthorized")
            except Exception:
        ...
        if args:
            if not source.thread_id:
                return t("channel.topic.restore_needs_topic")
        ...
        if capabilities.get("checked"):
            if capabilities.get("has_topics_enabled") is False:
                ...
                return t("channel.topic.topics_disabled")
            if capabilities.get("allows_users_to_create_topics") is False:
                if self._should_send_telegram_capability_hint(source):
                    await self._send_telegram_topic_setup_image(source)
                return t("channel.topic.topics_user_disallowed")
        ...
        try:
            ...
            return t("channel.topic.enable_failed", error=exc)
        ...
        if source.thread_id:
            ...
            if binding:
                ...
                try:
                    ...
                    title = None
                session_label = title or t("channel.topic.untitled_session")
                return t(
                    "channel.topic.bound_status",
                    label=session_label,
                    session_id=session_id,
                )
            return t("channel.topic.thread_ready")
    ...
    async def _handle_title_command(self, event: MessageEvent) -> str:
        ...
        if title_arg:
            ...
            try:
                ...
                return t("channel.shared.warn_passthrough", error=e)
            if not sanitized:
                return t("channel.title.empty_after_clean")
            # Set the title
            try:
                if self._session_db.set_session_title(session_id, sanitized):
                    return t("channel.title.set_to", title=sanitized)
                else:
                    return t("channel.title.not_found")
            except ValueError as e:
                return t("channel.shared.warn_passthrough", error=e)
        else:
            ...
            title = self._session_db.get_session_title(session_id)
            if title:
                return t("channel.title.current_with_title", session_id=session_id, title=title)
            else:
                return t("channel.title.current_no_title", session_id=session_id)
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            return t("channel.resume.parse_error", error=exc)
        ...
        if not name:
            ...
            try:
                ...
                if not titled:
                    if source.platform == Platform.MATRIX and not allow_all:
                        return t("channel.resume.matrix_no_named_sessions")
                    return t("channel.resume.no_named_sessions")
                lines = [t("channel.resume.list_header")]
                for idx, s in enumerate(titled[:10], start=1):
                    ...
                    preview_part = t("channel.resume.list_preview_suffix", preview=preview) if preview else ""
                    lines.append(t("channel.resume.list_item_numbered", index=idx, title=title, preview_part=preview_part))
                lines.append(t("channel.resume.list_footer_numbered"))
                return "\n".join(lines)
            except Exception as e:
                logger.debug("Failed to list titled sessions: %s", e)
                return t("channel.resume.list_failed", error=e)
        ...
        if not target_id:
            return t("channel.resume.not_found", name=name)
        # Compression creates child continuations that hold the live transcript.
        # Follow that chain so channel /resume matches CLI behavior (#15000).
        try:
        ...
        if source.platform == Platform.MATRIX:
            ...
            if not self._same_matrix_room(source, target_origin) and not allow_cross_room:
                if target_origin is None:
                    return t("channel.resume.matrix_blocked_no_origin", name=name)
                return t(
                    "channel.resume.matrix_blocked_other_room",
        ...
        if current_entry.session_id == target_id:
            return t("channel.resume.already_on", name=name)
        ...
        if not new_entry:
            return t("channel.resume.switch_failed")
        self._clear_session_boundary_security_state(session_key)
        ...
        if source.platform == Platform.MATRIX and allow_cross_room:
            return t(
                "channel.resume.matrix_cross_room_success",
                title=title,
                room=source.chat_name or source.chat_id,
                msg_part=msg_part,
            )
        if not msg_count:
            return t("channel.resume.resumed_no_count", title=title)
        if msg_count == 1:
            return t("channel.resume.resumed_one", title=title, count=msg_count)
        return t("channel.resume.resumed_many", title=title, count=msg_count)
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        if not history:
            return t("channel.branch.no_conversation")
        ...
        try:
            self._session_db.create_session(
                ...
                source=source.platform.value if source.platform else "channel",
            ...
            return t("channel.branch.create_failed", error=e)
        ...
        if not new_entry:
            return t("channel.branch.switch_failed")
        self._clear_session_boundary_security_state(session_key)
        ...
        key = "channel.branch.branched_one" if msg_count == 1 else "channel.branch.branched_many"
        return t(key, title=branch_title, count=msg_count, parent=parent_session_id, new=new_session_id)

    async def _handle_credits_command(self, event: MessageEvent) -> str:
        """Handle /credits -- show Orcanium credit balance and the top-up handoff.
        ...
        if view is None or not view.logged_in:
            return t("channel.credits.not_logged_in")

        lines: list[str] = ["💳 **Orcanium credits**"]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # block the channel. Failures are non-fatal -- account_lines stays [].
        ...
        # ── Orcanium credits magnitudes + monthly-grant % gauge ─────────────
        # Shared with the CLI / TUI /usage block via orcanium_credits_lines(): a single
        # auth-gate + portal-fetch + render path (which also honors the dev fixture).
        # Run off the event loop. The helper gates on "a Orcanium account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # Orcanium-credentialled user running inference elsewhere (or with none resident)
        ...
        try:
            from agent.account_usage import orcanium_credits_lines

            credits_lines = await asyncio.to_thread(orcanium_credits_lines, markdown=True)
        except Exception:
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            if rl_state and rl_state.has_data:
                ...
                lines.append(t("channel.usage.rate_limits", state=format_rate_limit_compact(rl_state)))
                lines.append("")
            ...
            lines.append(t("channel.usage.header_session"))
            lines.append(t("channel.usage.label_model", model=agent.model))
            lines.append(t("channel.usage.label_input_tokens", count=f"{input_tokens:,}"))
            if cache_read:
                lines.append(t("channel.usage.label_cache_read", count=f"{cache_read:,}"))
            if cache_write:
                lines.append(t("channel.usage.label_cache_write", count=f"{cache_write:,}"))
            lines.append(t("channel.usage.label_output_tokens", count=f"{output_tokens:,}"))
            lines.append(t("channel.usage.label_total", count=f"{agent.session_total_tokens:,}"))
            lines.append(t("channel.usage.label_api_calls", count=agent.session_api_calls))
            ...
            try:
                ...
                if cost_result.amount_usd is not None:
                    ...
                    lines.append(t("channel.usage.label_cost", prefix=prefix, amount=f"{float(cost_result.amount_usd):.4f}"))
                elif cost_result.status == "included":
                    lines.append(t("channel.usage.label_cost_included"))
            except Exception:
                pass
            ...
            if ctx.last_prompt_tokens:
                ...
                lines.append(t("channel.usage.label_context", used=f"{ctx.last_prompt_tokens:,}", total=f"{ctx.context_length:,}", pct=f"{pct:.0f}"))
            if ctx.compression_count:
                lines.append(t("channel.usage.label_compressions", count=ctx.compression_count))
        ...
        if history:
            ...
            lines = [
                t("channel.usage.header_session_info"),
                t("channel.usage.label_messages", count=len(msgs)),
                t("channel.usage.label_estimated_context", count=f"{approx:,}"),
                t("channel.usage.detailed_after_first"),
            ]
            if account_lines:
                lines.append("")
                lines.extend(account_lines)
        ...
        if account_lines or credits_lines:
            ...
            return "\n".join(parts)
        return t("channel.usage.no_data")
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
            ...
            while i < len(parts):
                if parts[i] == "--days" and i + 1 < len(parts):
                    try:
                        ...
                        return t("channel.insights.invalid_days", value=parts[i + 1])
                    i += 2
        ...
        try:
            ...
            return t("channel.insights.error", error=e)
    ...
    async def _handle_reload_mcp_command(self, event: MessageEvent) -> Optional[str]:
        ...
        # effect on the next invocation without restarting the channel.
        ...
        async def _on_confirm(choice: str) -> Optional[str]:
            if choice == "cancel":
                return t("channel.reload_mcp.cancelled")
            if choice == "always":
            ...
            if choice == "always":
                return f"{result}\n\n" + t("channel.reload_mcp.always_followup")
            return result

        prompt_message = t("channel.reload_mcp.confirm_prompt")
        return await self._request_slash_confirm(
            event=event,
            command="reload-mcp",
            title="/reload-mcp",
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        ``self._pending_skills_reload_notes[session_key]``. The channel
        ...
        try:
            ...
            lines = [t("channel.reload_skills.header")]
            if not added and not removed:
                lines.append(t("channel.reload_skills.no_new"))
                lines.append(t("channel.reload_skills.total", count=total))
                return "\n".join(lines)
            ...
            def _fmt_line(item: dict) -> str:
                ...
                if desc:
                    return t("channel.reload_skills.item_with_desc", name=nm, desc=desc)
                return t("channel.reload_skills.item_no_desc", name=nm)

            if added:
                lines.append(t("channel.reload_skills.added_header"))
                for item in added:
                    lines.append(_fmt_line(item))
            if removed:
                lines.append(t("channel.reload_skills.removed_header"))
                for item in removed:
                    lines.append(_fmt_line(item))
            lines.append(t("channel.reload_skills.total", count=total))
            ...
            return t("channel.reload_skills.failed", error=e)
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        message suitable for any channel adapter; bundles are loaded by
        ...
        if not bundles:
            return (
                ...
                "  `orcanium bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        flow as the CLI's synchroorcanium input() approval.
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                ...
                return t("channel.approval_expired")
            return t("channel.approve.no_pending")
        ...
        return t(f"channel.approve.{choice}_{plural}", count=count)
    ...
    async def _handle_deny_command(self, event: MessageEvent) -> str:
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                self._pending_approvals.pop(session_key)
                return t("channel.deny.stale")
            return t("channel.deny.no_pending")
        ...
        if count > 1:
            return t("channel.deny.denied_plural", count=count)
        return t("channel.deny.denied_singular")
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``orcanium debug share`` from the CLI.
        """
        import asyncio
        from orcanium.cli.debug import (
        ...
        def _collect_and_upload():
            ...
            try:
                ...
                return t("channel.debug.upload_failed", error=exc)

            # Schedule auto-deletion after 6 hours
            _schedule_auto_delete(list(urls.values()))

            lines = [_GATEWAY_PRIVACY_NOTICE, "", t("channel.debug.header"), ""]
            label_width = max(len(k) for k in urls)
            for label, url in urls.items():
                lines.append(f"`{label:<{label_width}}`  {url}")

            lines.append("")
            lines.append(t("channel.debug.auto_delete"))
            lines.append(t("channel.debug.full_logs_hint"))
            lines.append(t("channel.debug.share_hint"))
            return "\n".join(lines)

        return await loop.run_in_executor(None, _collect_and_upload)

    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Orcanium Agent to the latest version.

        Spawns ``orcanium update`` in a detached session (via ``setsid``) so it
        survives the channel restart that ``orcanium update`` may trigger. Marker
        files are written so either the current channel process or the next one
        can notify the user when the update finishes.
        """
        from channel.run import _orcanium_home, _resolve_orcanium_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from orcanium.cli.config import is_managed, format_managed_message
        ...
        if platform not in _allowed:
            try:
                ...
                if not entry or not entry.allow_update_command:
                    return t("channel.update.platform_not_messaging")
            except Exception:
                return t("channel.update.platform_not_messaging")

        if is_managed():
            return f"✗ {format_managed_message('update Orcanium Agent')}"

        project_root = Path(__file__).parent.parent.resolve()
        git_dir = project_root / '.git'

        if not git_dir.exists():
            return t("channel.update.not_git_repo")

        orcanium_cmd = _resolve_orcanium_bin()
        if not orcanium_cmd:
            return t("channel.update.orcanium_cmd_not_found")
        ...
        # Spawn `orcanium update --channel` detached so it survives channel restart.
        # --channel enables file-based IPC for interactive prompts (stash
        # restore, config migration) so the channel can forward them to the
        # user instead of silently skipping them.
        # Use setsid for portable session detach (works under system services
        # where systemd-run --user fails due to missing D-Bus session).
        # PYTHONUNBUFFERED ensures output is flushed line-by-line so the
        # channel can stream it to the messenger in near-real-time.
        # Spawn `orcanium update --channel` detached so it survives channel restart.
        # --channel enables file-based IPC for interactive prompts (stash
        # restore, config migration) so the channel can forward them to the
        # user instead of silently skipping them.
        ...
        # channel can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `orcanium update --channel`
        ...
        # we're already inside channel/run.py's update path which is async,
        ...
        try:
            if sys.platform == "win32":
                ...
                # orcanium_cmd is a list of argv parts we can pass directly
                ...
                orcanium_cmd_str = " ".join(shlex.quote(part) for part in orcanium_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {orcanium_cmd_str} update --channel"
            ...
            exit_code_path.unlink(missing_ok=True)
            return t("channel.update.start_failed", error=e)

        self._schedule_update_notification_watch()
        return t("channel.update.starting")
