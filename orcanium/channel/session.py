"""
Session management for the channel.
...
from .config import (
    Platform,
    GatewayConfig,
    SessionResetPolicy,  # noqa: F401 — re-exported via channel/__init__.py
    HomeChannel,
)
from .whatsapp_identity import (
    canonical_whatsapp_identifier,
    normalize_whatsapp_identifier,  # noqa: F401 - re-exported for channel.session callers
)
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `orcanium tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    # each user message by the channel.
    ...
    if context.source.platform == Platform.SLACK:
        ...
        lines.append(
            ...
            "Do not promise to perform these actions. The channel may inline the "
        ...
        # `discord` / `discord_admin` via `orcanium tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_orcanium_home()}/cron/output/)"
    )
...
class SessionEntry:
    ...
    # agent).  Persisted to sessions.json so the flag survives channel
    ...
    # When True the session was interrupted by a channel restart/shutdown
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        try:
            ...
            print(f"[channel] Warning: SQLite session store unavailable, falling back to JSONL: {e}")
    ...
    def _ensure_loaded_locked(self) -> None:
        ...
        if sessions_file.exists():
            try:
                ...
                print(f"[channel] Warning: Failed to load sessions: {e}")
    ...
    def get_or_create_session(
        ...
        if self._db and db_create_kwargs:
            try:
                ...
                print(f"[channel] Warning: Failed to create SQLite session: {e}")
    ...
    def suspend_session(self, session_key: str) -> bool:
        ...
        after a channel restart (#7536).  Returns True if the session
    ...
    def clear_resume_pending(self, session_key: str) -> bool:
        ...
        Called from the channel after ``run_conversation()`` returns a
    ...
    def suspend_recently_active(self, max_age_seconds: int = 120) -> int:
        ...
        Called on channel startup after a crash or fast restart to preserve
    ...
    def switch_session(self, session_key: str, target_session_id: str) -> Optional[SessionEntry]:
        ...
        previously ended, re-open it so channel resume semantics match the CLI.
        """
        db_end_session_id = None
        new_entry = None
