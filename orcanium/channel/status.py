...
Provides PID-file based detection of whether the channel daemon is running,
used by send_message's check_fn to gate availability in the CLI.

The PID file lives at ``{ORCANIUM_HOME}/channel.pid``.  ORCANIUM_HOME defaults to
``~/.Orcanium`` but can be overridden via the environment variable.  This means
separate ORCANIUM_HOME directories naturally get separate PID files — a property
...
from orcanium.channel.compat import get_orcanium_home as get_orcanium_ome
...
_GATEWAY_KIND = "orcanium-channel"
_RUNTIME_STATUS_FILE = "gateway_state.json"
_LOCKS_DIRNAME = "channel-locks"
_IS_WINDOWS = sys.platform == "win32"
_UNSET = object()
_GATEWAY_LOCK_FILENAME = "channel.lock"
...
def _get_pid_path() -> Path:
    """Return the path to the channel PID file, respecting ORCANIUM_HOME."""
    home = get_orcanium_ome()
    return home / "channel.pid"


def _get_gateway_lock_path(pid_path: Optional[Path] = None) -> Path:
    """Return the path to the runtime channel lock file."""
...
def _get_lock_dir() -> Path:
    """Return the machine-local directory for token-scoped channel locks."""
    override = os.getenv("ORCANIUM_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "orcanium" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Orcanium channel."""
    cmdline = _read_process_cmdline(pid)
    if not cmdline:
        return False

    patterns = (
        "orcanium_li.main channel",
        "orcanium_li/main.py channel",
        "orcanium channel",
        "orcanium-channel",
        "channel/run.py",
    )
...
def _record_looks_like_gateway(record: dict[str, Any]) -> bool:
    """Validate channel identity from PID-file metadata when cmdline is unavailable."""
...
def _cleanup_invalid_pid_path(pid_path: Path, *, cleanup_stale: bool) -> None:
    """Delete a stale channel PID file (and its sibling lock metadata).
...
def acquire_gateway_runtime_lock() -> bool:
    """Claim the cross-process runtime lock for the channel.
...
def release_gateway_runtime_lock() -> None:
    """Release the channel runtime lock when owned by this process."""
    global _gateway_lock_handle
...
def is_gateway_runtime_lock_active(lock_path: Optional[Path] = None) -> bool:
    """Return True when some process currently owns the channel runtime lock."""
    global _gateway_lock_handle
...
def write_pid_file() -> None:
    """Write the current process PID and metadata to the channel PID file.
    ...
    try:
        ...
        raise  # Let caller decide: another channel is racing us
    try:
...
def write_runtime_status(
    *,
    ...
    """Persist channel runtime health information for diagnostics/status."""
...
def read_runtime_status() -> Optional[dict[str, Any]]:
    """Read the persisted channel runtime health/status information."""
    return _read_json_file(_get_runtime_status_path())


def remove_pid_file() -> None:
    """Remove the channel PID file, but only if it belongs to this process.
    ...
    would delete the new process's record, leaving the channel running with no
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ORCANIUM_HOME dirs).
    """
    ...
    if existing:
        ...
        if not stale:
            if not _pid_exists(existing_pid):
                ...
                # record's own argv — the channel writes it at startup and
                # it's the only identity signal on platforms without ps.
                # Both oracles must indicate "not a channel" to mark stale.
                if (
                    not stale
...
def release_all_scoped_locks(
    *,
    ...
    channel processes that did not release their locks gracefully. When an
    ``owner_pid`` is provided, only lock records belonging to that channel
...
#
# When a new channel starts with ``--replace``, it SIGTERMs the existing
# channel so it can take over the bot token. PR #5646 made SIGTERM exit
# the channel with code 1 so ``Restart=on-failure`` can revive it after
...
# enabled in the user's systemd (e.g. ``Orcanium.service`` + ``orcanium-
# channel.service``).
#
...
_TAKEOVER_MARKER_FILENAME = ".channel-takeover.json"
_TAKEOVER_MARKER_TTL_S = 60  # Marker older than this is treated as stale
_PLANNED_STOP_MARKER_FILENAME = ".channel-planned-stop.json"
...
def _get_planned_stop_marker_path() -> Path:
    """Return the path to the intentional channel stop marker file."""
    home = get_orcanium_ome()
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``orcanium channel stop`` on Windows would be misclassified as an
...
def write_planned_stop_marker(target_pid: int) -> bool:
    ...
    The channel exits non-zero for unexpected SIGTERM so service managers can
...
def planned_stop_marker_targets_self() -> bool:
    ...
    (``channel/run.py:_run_planned_stop_watcher``) to decide whether to
    ...
    stale file left behind by a previous channel instance cannot wedge
    ...
    if _marker_is_stale(written_at, _PLANNED_STOP_MARKER_TTL_S):
        ...
        # clean it up so it cannot crash-loop a freshly booted channel.
        try:
            path.unlink(missing_ok=True)
        except OSError:
            pass
...
def get_running_pid(
    ...
    *,
    ...
    """Return the PID of a running channel instance, or ``None``.
...
def is_gateway_running(
    ...
    *,
    ...
    """Check if the channel daemon is currently running."""
