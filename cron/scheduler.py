...
Uses a file-based lock (~/.orcanium/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `orcanium update` reloads
# the module) fail with ModuleNotFoundError for orcanium_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from orcanium.orcanium_constants import get_orcanium_home
from orcanium.cli._subprocess_compat import windows_hide_flags
from orcanium.cli.config import load_config, _expand_env_vars
from orcanium.orcanium_time import now as _orcanium_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``orcanium tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from orcanium.cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_orcanium_home: Path | None = None


def _get_orcanium_home() -> Path:
    """Resolve Orcanium home dynamically while preserving test monkeypatch hooks."""
    return _orcanium_home or get_orcanium_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    orcanium_home = _get_orcanium_home()
    lock_dir = orcanium_home / "cron"
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if ":" in deliver_value:
        ...
        try:
            from channel.channel_directory import resolve_channel_name
...
# Media extension sets — audio routing is centralized in channel.platforms.base
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    from channel.config import load_gateway_config, Platform
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ORCANIUM_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ORCANIUM_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ORCANIUM_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ORCANIUM_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_orcanium_home() / "scripts"
    ...
    # escape — scripts MUST reside within ORCANIUM_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _orcanium_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_orcanium_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_orcanium_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["ORCANIUM_CRON_SESSION"] = "1"

    # Use ContextVars for per-job session/delivery state so parallel jobs
    # don't clobber each other's targets (os.environ is process-global).
    from channel.session_context import set_session_vars, clear_session_vars, _VAR_MAP

    # Cron execution is an internal scheduler context, not a live inbound
    # gateway message. Do not seed ORCANIUM_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ORCANIUM_SESSION_PLATFORM
    #     and ORCANIUM_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ORCANIUM_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ORCANIUM_SESSION_PLATFORM == "telegram".
    ...
    #     ORCANIUM_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read ORCANIUM_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ORCANIUM_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ORCANIUM_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "ORCANIUM_CRON_AUTO_DELIVER_PLATFORM",
        "ORCANIUM_CRON_AUTO_DELIVER_CHAT_ID",
        "ORCANIUM_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_orcanium_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_orcanium_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["ORCANIUM_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ORCANIUM_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ORCANIUM_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("ORCANIUM_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_orcanium_home() / "config.yaml")
        ...
        prefill_file = (
            os.getenv("ORCANIUM_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_orcanium_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject ORCANIUM_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # ORCANIUM_HOME. When a workdir is configured, also inject project
        ...
        # override via ORCANIUM_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ORCANIUM_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ORCANIUM_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_orcanium_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_orcanium_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    Args:
        ...
        adapters: Optional dict mapping Platform → live adapter (from channel)
        loop: Optional asyncio event loop (from channel) for live adapter sends
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _orcanium_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _orcanium_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set ORCANIUM_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ORCANIUM_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ORCANIUM_CRON_MAX_PARALLEL value; defaulting to unbounded")
