...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``orcanium://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``orcanium://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"orcanium://blueprint/{quote(blueprint.key)}{qs}"
